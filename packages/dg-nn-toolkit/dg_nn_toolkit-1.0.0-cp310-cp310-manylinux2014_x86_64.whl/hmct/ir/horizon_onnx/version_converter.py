...
from .onnx_cpp2py_export import version_converter
...
def convert_version(model: ModelProto, target_version: int) -> ModelProto:
    ...
    Args:
        model: model proto to convert
        target_version: target opset version

    Returns:
        converted model proto
    ...
    if not isinstance(model, ModelProto):
        raise ValueError(
            f"VersionConverter only accepts ModelProto as model, "
            f"incorrect type: {type(model)}"
        )
    if not isinstance(target_version, int):
        raise ValueError(
            f"VersionConverter only accepts int as target_version, "
            f"incorrect type: {type(target_version)}"
        )
    model_str = serialize_proto(model)
    converted_model_str = version_converter.convert_version(model_str,
                                                            target_version)
    return deserialize_proto(converted_model_str)


ConvertError = version_converter.ConvertError
