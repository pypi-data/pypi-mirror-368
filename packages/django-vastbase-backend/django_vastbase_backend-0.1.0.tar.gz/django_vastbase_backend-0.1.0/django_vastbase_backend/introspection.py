...
class DatabaseIntrospection(BaseDatabaseIntrospection):
    ...
    def get_table_list(self, cursor):
        ...
        cursor.execute("""
            SELECT c.relname,
            CASE WHEN c.parttype <> 'n' THEN 'p' WHEN c.relkind IN ('m', 'v') THEN 'v' ELSE 't' END
            FROM pg_catalog.pg_class c
            LEFT JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace
            WHERE (c.relkind IN ('f', 'm', 'r', 'v') OR c.parttype <> 'n')
    ...
    def get_constraints(self, cursor, table_name):
        ...
        cursor.execute("""
            ...
            FROM (
                SELECT
                    row_number() OVER () as arridex, c2.relname as indexname,
