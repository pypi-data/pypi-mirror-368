...
def test_extract_response_type_direct_class_missing_type_arg() -> None:
    with pytest.raises(
        RuntimeError,
        match="Expected type <class 'browser_use._response.AsyncAPIResponse'> to have a type argument at index 0 but it did not",
    ):
...
def test_response_parse_mismatched_basemodel(client: BrowserUse) -> None:
...
@pytest.mark.asyncio
async def test_async_response_parse_mismatched_basemodel(async_client: AsyncBrowserUse) -> None:
...
def test_response_parse_custom_stream(client: BrowserUse) -> None:
    response = APIResponse(
        raw=httpx.Response(200, content=b"foo"),
        client=client,
        stream=True,
...
@pytest.mark.asyncio
async def test_async_response_parse_custom_stream(async_client: AsyncBrowserUse) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=b"foo"),
        client=async_client,
        stream=True,
...
def test_response_parse_custom_model(client: BrowserUse) -> None:
    response = APIResponse(
...
@pytest.mark.asyncio
async def test_async_response_parse_custom_model(async_client: AsyncBrowserUse) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=json.dumps({"foo": "hello!", "bar": 2})),
        client=async_client,
        stream=False,
...
def test_response_parse_annotated_type(client: BrowserUse) -> None:
    response = APIResponse(
        raw=httpx.Response(200, content=json.dumps({"foo": "hello!", "bar": 2})),
        client=client,
        stream=False,
...
async def test_async_response_parse_annotated_type(async_client: AsyncBrowserUse) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=json.dumps({"foo": "hello!", "bar": 2})),
        client=async_client,
        stream=False,
...
def test_response_parse_bool(client: BrowserUse, content: str, expected: bool) -> None:
    response = APIResponse(
...
async def test_async_response_parse_bool(client: AsyncBrowserUse, content: str, expected: bool) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=content),
        client=client,
        stream=False,
...
def test_response_parse_expect_model_union_non_json_content(client: BrowserUse) -> None:
    response = APIResponse(
...
async def test_async_response_parse_expect_model_union_non_json_content(async_client: AsyncBrowserUse) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=b"foo", headers={"Content-Type": "application/text"}),
        client=async_client,
        stream=False,
