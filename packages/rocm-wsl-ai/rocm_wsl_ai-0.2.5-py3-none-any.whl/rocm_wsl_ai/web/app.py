...
async def api_models_presets():
    items = []
    for name, spec in models_dl.PRESETS.items():
        url = spec.get("url") or ""
        downloadable = bool(url) and ("example.invalid" not in url)
        items.append({
            "name": name,
...
def run(host: str = "0.0.0.0", port: int = 8000, *, log_level: str = "info", access_log: bool = False):
