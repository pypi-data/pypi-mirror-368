...
def monitor(object):
    try:
        ...
        if package_name == "langgraph":
            installed_version = importlib.metadata.version("langgraph")
            if version.parse(installed_version)  >= version.parse("0.6.0"):
                from langgraph.graph.state import CompiledStateGraph
                
                if type(object) is CompiledStateGraph:
                    graph = object
                    from langchain_core.runnables.config import ensure_config, merge_configs
                    import uuid

                    graph_id = str(uuid.uuid4())  
                    graph_manager.add_graph(graph_id, graph.get_graph())

                    graph.config = merge_configs(
                      ensure_config(ensure_config(graph.config)),
                      {
                        "metadata": {"_graph_id": graph_id} 
                      }
                    )
                else:
                    logger.warning("You can only use monitor on LangGraph compiled graph (`CompiledStateGraph`)")
 
            else:
                logger.warning("Please update langgraph to version >= 0.6.0 to use monitor your graph with Lunary`")


            
    except PackageNotFoundError:
        logger.warning("[LUNARY] You can only use monitor with `openai`, `langgraph` or `ibm-watsonx-ai`. More info: https://docs.lunary.ai")
        ...
        def on_chain_start(
            ...
            **kwargs: Any,
            ...
            try:
                ...
                graph = None
                if parent_run_id is None and metadata and isinstance(metadata.get('_graph_id'), str): #TODO: handle graphs that are not the root
                    type = "agent"
                    name = agent_name
                    graph_id = metadata['_graph_id'] 
                    graph = graph_manager.get_serialized_graph(graph_id) 
