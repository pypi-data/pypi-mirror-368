...
rho_p = 1200  # Assumed density of pollen in kg/m^3
radius_p_micrometers = 10  # Assumed radius of pollen in micrometers
pi = np.pi

def convert_units(polp, T, P, PB):
    # Calculate the numerator and denominator
    numerator = polp * ((1 / R) * ((T + 300) / (P + PB))) * 1e9
...
for file_path in directory.glob("*.nc"):
    ...
    # Convert units
    #polp_converted = convert_units(polp_data, T, P, PB)
    #polp_converted = polp_data * (1/287) * ((T + 300) / (P + PB)) * 1.e9 / ((4./3.) * np.pi * (10**3) * 1200.)
    polp_converted = polp_data * ( (1/287) * ( (P+PB) / (T+300) ) )  * 1.e9 / ( 4./3. * 3.14 * 10.**3 * 1200.)
    # Assuming you are working with a specific slice, like before
    polp_2d = polp_converted.isel(Time=0, bottom_top=0)
    ...
    ax.set_title("Surface Pollen Forecast", fontproperties=title_font, pad=25)
    ax.text(
        0.5,
        1.01,
        subtitle,
    ...
    save_filename = f"surface_pollen_forecast_{dynamic_filename_base}.png"
    plt.savefig(
