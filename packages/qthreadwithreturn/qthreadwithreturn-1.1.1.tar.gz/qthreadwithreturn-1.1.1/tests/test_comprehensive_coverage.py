...
class TestBoundaryConditions:
    ...
    @pytest.mark.parametrize("max_workers,expected_behavior", [
        (None, "default"),   # 边界：None使用默认值
        (1, "valid"),        # 边界：最小有效值
        (1000, "limited"),   # 边界：很大的值（但会被限制）
    ])
    def test_thread_pool_max_workers_boundaries(self, qapp, max_workers, expected_behavior):
    ...
    @pytest.mark.parametrize("timeout_ms,expected_behavior", [
        (-1, "no_timeout"),     # 边界：无超时
        (0, "immediate"),       # 边界：立即超时
...
class TestDataDrivenTests:
    ...
    @pytest.mark.parametrize("input_data,expected_output", [
        ...
        ("", ""),
        # 复杂数据结构
        ({"nested": {"deep": [1, 2, {"even": "deeper"}]}}, {"nested": {"deep": [1, 2, {"even": "deeper"}]}}),
    ])
    ...
    @pytest.mark.parametrize("num_threads,task_duration,expected_results", [
        (1, 0.01, ["result_0"]),
    ...
    def test_multiple_thread_scenarios(self, qapp, num_threads, task_duration, expected_results):
        """测试不同数量线程的场景"""
        def task(index):
    ...
    @pytest.mark.parametrize("callback_type,param_count", [
        ("no_param", 0),
        ("single_param", 1), 
        ("double_param", 2),
        ("triple_param", 3),
