...
class Milvuser:
    def __init__(self, url:str, get_model:Callable, default_col:str, default_vector_key:str, 
        ...
        self.aclient:AsyncMilvusClient=None
        self._lock = Lock()
        self.url = url
    ...
    def _getAClient(self)->AsyncMilvusClient:
        client = AsyncMilvusClient(self.url, self._user, self._pwd, self._db_name)
        return client
    
    def _getClient(self)->MilvusClient:
    ...
    async def asearch(self, *query:str|Tensor, min_similarity:float=0.5, kn:int=3, col:str=None, output_fields:list[str]=None, 
    ...
    async def aquery(self, limit:int=10, col:str=None, ids:list=None,
                     filter:str='', filter_params:dict=None, **kwargs)-> list[dict]:
        self.aclient = self.aclient or self._getAClient()
        res = await self.aclient.query(col or self.default_col, filter=filter, filter_params=filter_params, limit=limit, ids=ids, **kwargs)
        return res
    
    def query(self, limit:int=10, col:str=None, ids:list=None,
              filter:str='', filter_params:dict=None, **kwargs)-> list[dict]:
        self.client = self.client or self._getClient()
        res = self.client.query(col or self.default_col, filter=filter, filter_params=filter_params, limit=limit, ids=ids, **kwargs)
        return res

    async def adelete(self, filter:str, filter_params:dict=None, ids:list=None, col:str = None)-> list[dict]:
        self.aclient = self.aclient or self._getAClient()
        res = await self.aclient.delete(col or self.default_col, filter=filter, filter_params=filter_params, ids=ids)
        return res
    
    def delete(self, filter:str, filter_params:dict=None, ids:list=None, col:str = None)-> list[dict]:
        self.client = self.client or self._getClient()
        res = self.client.delete(col or self.default_col, filter=filter, filter_params=filter_params, ids=ids)
        return res
