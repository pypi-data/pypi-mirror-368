...
import cohesity_management_sdk.models_v2.recovery_task_info
import cohesity_management_sdk.models_v2.object_summary
import cohesity_management_sdk.models_v2.archival_target

class RecoverAcropolisSnapshotParams(object):

    """Implementation of the 'Recover Acropolis Snapshot Params.' model.

    Specifies the snapshot parameters to recover Acropolis VM.

    Attributes:
        bytes_restored (long|int):  Specify the total bytes restored.
        ...
        object_info (ObjectSummary): Specifies the information about the object
        ...
        recover_from_standby (bool): Specifies that user wants to perform standby restore if it is
          enabled for this object.
        datastore_migration_info (list of CommonFileAndFolderInfo): Specifies the info about datastore migration. This is only
            applicable for AcropolisVm.
        instant_recovery_info (string): Specifies the info about instant recovery. This is only applicable
            for AcropolisVm.
    ...
    def __init__(self,
                 ...
                 instant_recovery_info='kAcropolis'):
        """Constructor for the RecoverAcropolisSnapshotParams class"""
    ...
    def from_dictionary(cls,
        ...
        bytes_restored = dictionary.get('bytesRestored')
        ...
        object_info = cohesity_management_sdk.models_v2.object_summary.ObjectSummary.from_dictionary(
            dictionary.get('objectInfo')) if dictionary.get('objectInfo') else None
        snapshot_target_type = dictionary.get('snapshotTargetType')
        storage_domain_id = dictionary.get('storageDomainId')
        archival_target_info = cohesity_management_sdk.models_v2.archival_target.ArchivalTarget.from_dictionary(
            dictionary.get('archivalTargetInfo')) if dictionary.get('archivalTargetInfo') else None
        ...
        datastore_migration_info = cohesity_management_sdk.models_v2.recovery_task_info.RecoveryTaskInfo.from_dictionary(dictionary.get('datastoreMigrationInfo')) if dictionary.get('datastoreMigrationInfo') else None
        instant_recovery_info = cohesity_management_sdk.models_v2.recovery_task_info.RecoveryTaskInfo.from_dictionary(
            dictionary.get('instantRecoveryInfo')) if dictionary.get('instantRecoveryInfo') else None

        # Return an object of this model
        return cls(bytes_restored,
                   snapshot_id,
                   point_in_time_usecs,
                   protection_group_id,
