...
class ProtectionRunsController(BaseController):
    ...
    def get_protection_runs(self,
                            ...
                            filter_by_copy_task_end_time=None,
                            copy_run_types=None,
                            only_include_successful_copy_runs=None,
        ...
        Args:
            ...
            filter_by_end_time (bool, optional): If true, the runs with end
                time within the specified time range will be returned.
                Otherwise, the runs with start time in the time range are
                returned.
            filter_by_copy_task_end_time (bool, optional): If true, then the
                details of the backup runs along with CopyRuns will be returned
                where those backup run has atleast one CopyTask that is
                completed in the given Time Range. If this field is true, then
                other filters such as filterByEndTime should not be applied.
            copy_run_types (list of string, optional): Following field contains
                a list of valid CopyTarget i.e Replication, Archival
                representing the types of copy runs needed to be looked at the
                given time window, denoted via TimeRange.
                This input is only considered valid when
                filterByCopyTaskEndTime is set to true. Else it is ignored.
            only_include_successful_copy_runs (bool, optional): If marked
                false, all CopyTasks in any finished state like cancelled,
                failed in the given time window will be considered. Otherwise
                if kept empty or marked as true, only the copy_tasks with
                kSuccess status will be considered. This input is only
                considered valid when filterByCopyTaskEndTime is set to
                true. Else it is ignored.
            max_result_count (int, optional): Identifies the max number of items to be returned. This is specifically
                to be used with pagination.
            pagination_cookie (string, optional): Pagination cookie to fetch the next set of results.
        ...
        try:
            ...
            _query_parameters = {
                ...
                'filterByEndTime': filter_by_end_time,
                'filterByCopyTaskEndTime': filter_by_copy_task_end_time,
                'copyRunTypes': copy_run_types,
                'onlyIncludeSuccessfulCopyRuns': only_include_successful_copy_runs,
