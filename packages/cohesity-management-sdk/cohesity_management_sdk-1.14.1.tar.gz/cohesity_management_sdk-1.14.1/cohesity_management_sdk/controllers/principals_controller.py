...
class PrincipalsController(BaseController):
    ...
    def get_users(self,
                  ...
                  domain=None,
                  partial_match=None,
                  include_only_tenants_users=None):
        ...
        Args:
            ...
            include_only_tenants_users (bool, optional): Providing this option will only return tenant users and
        ...
        try:
            ...
            _query_parameters = {
                ...
                'includeOnlyTenantsUsers': include_only_tenants_users
            }
            _query_builder = APIHelper.append_url_with_query_parameters(
                _query_builder, _query_parameters,
                Configuration.array_serialization)
    ...
    def delete_user_api_key(self, sid, id):
        ...
        try:
            ...
            # Prepare headers
            self.logger.info('Preparing headers for delete_user_api_key.')
            _headers = {
                'accept'       : 'application/json' ,
                'content-type' : 'application/json; charset=utf-8'
            }
    ...
    def verify_otp_code(self, body):
        ...
        try:
            ...
            # Prepare query URL
            self.logger.info(
                'Preparing query URL for verify_otp_code.')
            ...
            # Prepare headers
            self.logger.info(
                'Preparing headers for verify_otp_code.')
            _headers = {
                'accept': 'application/json',
                'content-type': 'application/json; charset=utf-8'
            }
            ...
            AuthManager.apply(_request, self.config)
            _context = self.execute_request(
                _request, name='verify_otp_code')

            # Endpoint and global error handling using HTTP status codes.
            self.logger.info(
                'Validating response for verify_otp_code.')
            if _context.response.status_code == 0:
                raise RequestErrorErrorException('Error', _context)
            self.validate_response(_context)
