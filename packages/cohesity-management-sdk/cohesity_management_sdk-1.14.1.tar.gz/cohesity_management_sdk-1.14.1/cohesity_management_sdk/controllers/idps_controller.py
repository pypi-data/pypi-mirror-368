...
class IdpsController(BaseController):
    ...
    def add_idp_directory_principals(self, body=None):
        ...
        Args:
            body(IdpPrincipalsAddParameters): Request to add groups or users to
        ...
        """
        try:
            self.logger.info('add_idp_directory_principals called.')

            # Prepare query URL
            self.logger.info(
                'Preparing query URL for add_idp_directory_principals.')
            ...
            # Prepare headers
            self.logger.info(
                'Preparing headers for add_idp_directory_principals.')
            _headers = {'accept': 'application/json'}

            # Prepare and execute request
            self.logger.info(
                'Preparing and executing request for add_idp_directory_principals.'
            )
            ...
            _context = self.execute_request(_request,
                                            name='add_idp_directory_principals')

            # Endpoint and global error handling using HTTP status codes.
            self.logger.info(
                'Validating response for add_idp_directory_principals.')
    ...
    def get_idp_login(self, tenant_id=None):
        ...
        try:
            ...
            # Prepare headers
            self.logger.info('Preparing headers for get_idp_login.')
            _headers = { 'accept' : 'application/json' }
    ...
    def delete_idp(self, id):
        ...
        try:
            ...
            # Prepare headers
            self.logger.info('Preparing headers for delete_idp.')
            _headers = {
                'accept'       : 'application/json' ,
                'content-type' : 'application/json; charset=utf-8'
            }
