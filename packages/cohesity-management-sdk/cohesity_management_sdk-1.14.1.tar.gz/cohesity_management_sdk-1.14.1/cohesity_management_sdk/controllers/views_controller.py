...
class ViewsController(BaseController):
    ...
    def delete_view_alias(self, name):
        ...
        try:
            ...
            # Prepare headers
            self.logger.info('Preparing headers for delete_view_alias.')
            _headers = {
                'accept'       : 'application/json' ,
                'content-type' : 'application/json; charset=utf-8'
            }
    ...
    def get_view_dir_quota_info(self,
                            view_name,
                            output_format=None,
                            page_count=None,
                            cookie=None):
        ...
        Args:
            ...
            output_format (string): Specifies the format for the output such as
                ''csv'' or ''json''.
                If not specified, the json format is returned.
                If ''csv'' is specified, a comma-separated list with a heading
                row is returned
            page_count (long|int): Specifies the max entries that should be
                returned in the result.
            cookie (long|int): Cookie should be used from previous call to list
                user quota overrides. It resumes (or gives the next set of
                values) from the result of the previous call.
    ...
    def get_view_user_quotas(self,
        ...
        Args:
            ...
            output_format (string, optional): OutputFormat is the Output
                ...
                json.
            top_quotas (string, optional): TopQuotas is the quotas sorted by
                quota usage in descending order.
                This parameter defines number of results returned.
                No pagination cookie is returned if this is set to true.
    ...
    def delete_view_by_id(self, id):
        ...
        try:
            ...
            # Prepare headers
            self.logger.info('Preparing headers for delete_view_by_id.')
            _headers = {
                'accept'       : 'application/json' ,
                'content-type' : 'application/json; charset=utf-8'
            }
