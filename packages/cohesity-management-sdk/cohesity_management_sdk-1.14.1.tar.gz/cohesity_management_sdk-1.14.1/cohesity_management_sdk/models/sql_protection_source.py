...
import cohesity_management_sdk.models.aag_info
...
class SqlProtectionSource(object):
    ...
    Attributes:
        is_available_for_vss_backup (bool): Specifies whether the database is marked as
          available for backup according
        ...
        aag_info (AAGInfo): Specifies the information about the AAG source.
        ...
        created_timestamp (string): Specifies the time when the database was
            created. It is displayed in the timezone of the SQL server on which
            this database is running.
        ...
        db_compatibility_level (long|int): Specifies the versions of SQL server
            that the database is compatible with.
        db_file_groups (list of string): Specifies the information about the
            set of file groups for this db on the host. This is only set if the
            type is kDatabase.
        db_files (list of DbFileInfo): Specifies the last known information
            about the set of database files on the host. This field is set only
            for type 'kDatabase'.
        db_owner_username (string): Specifies the name of the database owner.
        default_database_location (string): Specifies the default path for data
            files for DBs in an instance
        default_log_location (string): Specifies the default path for log files
            for DBs in an instance
        id (SqlSourceId): Specifies an id that identifies an SQL Object.
        ...
        recovery_model (RecoveryModelEnum): Specifies the Recovery Model for
            ...
            'kDatabase' SQL Protection Source. Specifies the Recovery Model set
            for the Microsoft SQL Server. 'kSimpleRecoveryModel' indicates the
            Simple SQL Recovery Model which does not utilize log backups.
            ...
            'kBulkLoggedRecoveryModel' indicates the Bulk Logged SQL Recovery
            Model which requires log backups and allows high-performance bulk
            copy operations.
        sql_server_db_state (SqlServerDbStateEnum): The state of the database
            as returned by SQL Server. Indicates the state of the database. The
            values correspond to the 'state' field in the system table
            ...
            recovery state. 'kRecoveryPending' indicates that database recovery
            is in pending state. 'kSuspect' indicates that primary filegroup is
            suspect and may be damaged. 'kEmergency' indicates that manually
            forced emergency state. 'kOffline' indicates that database is in
            offline state. 'kCopying' indicates that database is in copying
            state. 'kOfflineSecondary' indicates that secondary database is in
            offline state.
        sql_server_instance_version (SQLServerInstanceVersion): The version of
            the SQL instance.
    ...
    def from_dictionary(cls,
        ...
        aag_info = cohesity_management_sdk.models.aag_info.AAGInfo.from_dictionary(
        ...
        db_files = None
        if dictionary.get("dbFiles") is not None:
            db_files = list()
