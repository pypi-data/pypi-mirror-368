...
class RestoreO365GroupsParams(object):
    ...
    Attributes:
        create_new_group (bool): Bool which specifies, if we have to create a new group if it doesn't
          exist.
        ms_groups_vec (list of RestoreO365GroupsParams_MSGroupInfo): List of
            groups getting restored.
        restore_original_owners_members (bool): Bool which specifies, if the original members/owners should be part of the
          newly created target group.
        restore_to_original (bool): Whether or not all groups are restored to
            original location.
        should_restore_lists (bool): Bool which specifies if SP lists needs to be restored.
        target_group (string): Target group in case restore_to_original is
            false.
        target_group_name (string): Target group name in case restore_to_original is false. This will be
          ignored if restoring to alternate existing group.
        target_group_owner (string): The string which contains the owner smtp address for the target
          group.
    ...
    def from_dictionary(cls,
        ...
        ms_groups_vec = None
        if dictionary.get("msGroupsVec") is not None:
            ms_groups_vec = list()
