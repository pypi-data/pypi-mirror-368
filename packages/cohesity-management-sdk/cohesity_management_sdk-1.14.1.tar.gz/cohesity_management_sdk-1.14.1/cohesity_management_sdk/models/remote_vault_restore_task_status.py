...
class RemoteVaultRestoreTaskStatus(object):
    ...
    Attributes:
        ...
        current_indexing_status (RemoteRestoreIndexingStatus): Specifies the
            status of an indexing task that builds an index from the Protection
            Job metadata retrieved from the remote Vault. The index contains
        ...
        remote_protection_job_information (RemoteProtectionJobInformation):
            Specifies the details about the original Protection Job that
            archived data to the remote Vault (External Target).
        search_job_uid (UniversalId): Specifies the unique id of the search Job
            that searched the remote Vault.
        vault_id (long|int): Vault Id  Specifies the Id of the vault from which
            the restore is going on.
        vault_name (string): Vault Name  Specifies the name of the vault from
            which the restore is going on.
    """
