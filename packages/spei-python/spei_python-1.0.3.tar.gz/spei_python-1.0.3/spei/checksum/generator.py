...
class ChecksumGenerator(object):
    ...
    def _fecha_to_bytearray(self, fecha_operacion: int) -> bytearray:
        fecha_operacion_dt = datetime.strptime(str(fecha_operacion), '%Y%m%d')

        day = int.to_bytes(int(fecha_operacion_dt.day), 1, 'big')
        month = int.to_bytes(int(fecha_operacion_dt.month), 1, 'big')
        year = int.to_bytes(int(fecha_operacion_dt.year), 2, 'big')
