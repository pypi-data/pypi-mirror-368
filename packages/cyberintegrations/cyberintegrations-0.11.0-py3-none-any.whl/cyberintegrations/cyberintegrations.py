...
class TIGeneratorInfo(GeneratorInfo):
    ...
    apply_has_exploit: Union[int, str] = None
    probable_corporate_access: Union[int, str] = None
    unique: Union[int, str] = None
    combolist: Union[int, str] = None
    ...
    def _validate_default_fields(self, collections_info=CollectionConsts.TI_COLLECTIONS_INFO):
        ...
        if self.apply_has_exploit is not None:
            try:
                ...
                logger.exception("Wrong apply_has_exploit input, it should be '0' or '1'.")
        ...
        if self.probable_corporate_access is not None:
            try:
                ...
                logger.exception("Wrong probable_corporate_access input, it should be '0' or '1'.")
        ...
        if self.unique is not None:
            try:
                ...
                logger.exception("Wrong unique input, it should be '0' or '1'.")
        ...
        if self.combolist is not None:
            try:
                ...
                logger.exception("Wrong combolist input, it should be '0' or '1'.")

@dataclass(order=True)
class DRPGeneratorInfo(GeneratorInfo):
    subtypes: Optional[List[int]] = None
    section: Optional[Union[List[int], List[str]]] = None
    brands: Optional[Union[List[int], List[str]]] = None
    use_typo_squatting: Optional[bool] = None
    approve_states: Optional[Union[List[int], List[str]]] = None
...
class DRPPoller(Poller):
    ...
    def change_status(
        ...
        if violation:
            if violation.get('status') == 'detected' and violation.get('approveState') == 'under_review':
                self._send_change_approve_request(feed_id=feed_id, status=status)
            else:
                logger.exception(AttributeError("Can not change the status of the selected issue"))

    def _send_change_approve_request(
        self, 
        feed_id, 
        status
    ):
        ...
        response = self.send_request(endpoint=collection_name, method="POST", data=json.dumps(json_data), headers={"Content-Type":"application/json", "accept":"application/json"})
    ...
    def get_subscriptions(
        ...
        Validator.validate_collection_name(collection_name)
        response = self.send_request(endpoint=collection_name, method="GET")
        subscriptions = ParserHelper.find_element_by_key(response, 'data.subscriptions')
    ...
    def create_update_generator(
        ...
        if date_from is None and date_to is None and sequpdate is None and use_typo_squatting:
            sequpdate = 1
...
class DRPUpdateFeedGenerator(FeedGenerator):
    ...
    def _get_params(self):
        ...
        if self.generator_info.subtypes:
            params.update(
                {'subtypes[]':self.generator_info.subtypes[0]}
            )
        return params
