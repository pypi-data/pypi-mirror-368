...
class Classifier(nn.Module):
    ...
    def forward(self, x):
        if self.update_grid:
            self.linear1[0].update_grid(x)
        x = self.linear1(x)
        if self.n_layers == 2:
            if self.update_grid:
                self.linear2[0].update_grid(x)
            x = self.linear2(x)
        return x
...
class Encoder3(nn.Module):
    def __init__(self, in_shape, layers: dict, dropout, device='cuda', update_grid=False):
        super().__init__()
        self.device = device
        self.update_grid = update_grid
...
class Decoder3(nn.Module):
    def __init__(self, in_shape, n_batches, layers: dict, dropout, device='cuda', update_grid=False):
        super().__init__()
        self.device = device
        self.n_batches = n_batches
        self.layer_names = list(layers.keys())
...
class KANAutoencoder3(nn.Module):
    def __init__(self, in_shape, n_batches, nb_classes, n_meta, n_emb, mapper, variational, layers: dict,
        ...
        self.dann_discriminator = Classifier2(layers[list(layers.keys())[-1]], 64, n_batches, device=device)
        self.classifier = Classifier(in_shape=layers[list(layers.keys())[-1]] + n_emb, out_shape=nb_classes,
