...
from dataset import MSCSV, MS2CSV, MSImages
...
def get_bacteria_images_ms2(path, args, seed=42):
    ...
    for group in ['train', 'valid', 'test']:
        if group == 'valid':
            ...
            if 'csv' in path:
                process = MS2CSV(path, args.scaler, new_size=args.new_size, test=False)
            else:
                process = MSImages(path, binarize=False, new_size=args.new_size, test=False)
            pool = multiprocessing.Pool(multiprocessing.cpu_count() - 1)
            data_images = pool.map(process.process, range(process.__len__()))
            images, labels, batches, plates, names = [x[0] for x in data_images], pd.Series(
                [x[1] for x in data_images]), pd.Series(
...
def get_dummy(args, seed=42):
    ...
    If args.groupkfold is True, uses StratifiedGroupKFold on batch groups so that
    no batch (group) is split across train / valid / test.
    """
    rng = np.random.RandomState(seed)

    n_features = getattr(args, 'dummy_features', 32)
    n_classes = getattr(args, 'dummy_classes', 3)
    n_batches = getattr(args, 'dummy_batches', 6)
    sizes = {
        'train': getattr(args, 'dummy_train', 60),
        'valid': getattr(args, 'dummy_valid', 20),
        'test': getattr(args, 'dummy_test', 20),
    }
    total_n = sizes['train'] + sizes['valid'] + sizes['test']
    ...
    # Generate full pool first
    y_idx = np.tile(np.arange(n_classes), int(np.ceil(total_n / n_classes)))[:total_n]
    rng.shuffle(y_idx)
    y = class_names[y_idx]

    # Batch group assignment (round-robin, then shuffle)
    batches = np.array([i % n_batches for i in range(total_n)], dtype=int)
    rng.shuffle(batches)

    # Features
    X = np.vstack([
        centers[y_i] + rng.normal(0, 1.0, size=n_features)
        for y_i in y_idx
    ])

    names = np.array([f'sample_{i:04d}' for i in range(total_n)])
    orders = np.arange(total_n, dtype=int)

    # If groupkfold requested, split by batches
    if getattr(args, 'groupkfold', False):
        from sklearn.model_selection import StratifiedGroupKFold
        unique_batch_ct = len(np.unique(batches))
        # Need at least 3 splits to carve out valid & test
        n_splits = min(5, max(3, unique_batch_ct)) if unique_batch_ct >= 3 else unique_batch_ct
        if n_splits >= 3:
            sgkf = StratifiedGroupKFold(n_splits=n_splits, shuffle=True, random_state=seed)
            splits_iter = sgkf.split(np.arange(total_n), y, batches)
            # First fold valid
            _, valid_idx = next(splits_iter)
            # Second fold test (if available)
            try:
                _, test_idx = next(splits_iter)
            except StopIteration:
                test_idx = valid_idx  # degenerate fallback
            used = set(valid_idx) | set(test_idx)
            train_idx = np.array([i for i in range(total_n) if i not in used])
        else:
            # Fallback: batch-level random partition
            ub = rng.permutation(np.unique(batches))
            if len(ub) >= 3:
                n_valid_batches = max(1, len(ub)//5)
                n_test_batches = max(1, len(ub)//5)
                valid_batches = ub[:n_valid_batches]
                test_batches = ub[n_valid_batches:n_valid_batches+n_test_batches]
                train_batches = ub[n_valid_batches+n_test_batches:]
            elif len(ub) == 2:
                valid_batches = ub[:1]; test_batches = ub[1:]; train_batches = ub[:0]
            else:
                valid_batches = ub; test_batches = ub[:0]; train_batches = ub[:0]
            valid_idx = np.argwhere(np.isin(batches, valid_batches)).ravel()
            test_idx = np.argwhere(np.isin(batches, test_batches)).ravel()
            train_idx = np.argwhere(np.isin(batches, train_batches)).ravel()
        # (Optional) size adjustment: keep requested maxima while preserving batches
        def trim(idx, target):
            if target <= 0 or len(idx) <= target:
                return idx
            return idx[:target]
        # Only trim down (never reassign across splits)
        train_idx = trim(train_idx, sizes['train'])
        valid_idx = trim(valid_idx, sizes['valid'])
        test_idx = trim(test_idx, sizes['test'])
    else:
        # Simple slice-based sizing (original behavior)
        train_idx = np.arange(0, sizes['train'])
        valid_idx = np.arange(sizes['train'], sizes['train'] + sizes['valid'])
        test_idx = np.arange(sizes['train'] + sizes['valid'], total_n)
    ...
    for group, idx in splits.items():
        idx = np.asarray(idx, dtype=int)
        inputs_df = pd.DataFrame(X[idx], columns=feature_names, index=names[idx])
        meta_df = pd.DataFrame(
            {
                'm1': rng.normal(0, 1, size=len(idx)),
                'm2': rng.uniform(0, 1, size=len(idx)),
            },
            index=names[idx],
        )
        data['inputs'][group] = inputs_df
        data['meta'][group] = meta_df
        data['names'][group] = names[idx]
        data['labels'][group] = y[idx]
        data['batches'][group] = batches[idx]
        data['orders'][group] = orders[idx]
        data['sets'][group] = np.array([group] * len(idx))

    # Concatenate all
    for key in list(data.keys()):
        if key in ['inputs', 'meta']:
            data[key]['all'] = pd.concat(
                (data[key]['train'], data[key]['valid'], data[key]['test']), axis=0
            )
        else:
            data[key]['all'] = np.concatenate(
                (data[key]['train'], data[key]['valid'], data[key]['test']), axis=0
    ...
    # Remap batches to contiguous ids across ALL (after split)
    unique_batches = np.unique(data['batches']['all'])
    batch_map = {b: i for i, b in enumerate(unique_batches)}
    for group in ['train', 'valid', 'test', 'all']:
        data['batches'][group] = np.array([batch_map[b] for b in data['batches'][group]])
