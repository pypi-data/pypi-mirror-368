...
class RealtimeStockData(Base):
    ...
    updated_at = Column(DateTime, nullable=False, default=datetime.utcnow,
                       onupdate=datetime.utcnow, comment="Record update time")
    
    # Cache metadata
    cache_ttl_minutes = Column(Integer, nullable=False, default=5,
                              comment="Cache TTL in minutes")
    ...
    @classmethod
    def from_akshare_data(cls, symbol: str, akshare_row: Dict[str, Any], 
                         asset_id: int = None) -> 'RealtimeStockData':
        """
        ...
        return cls(
            ...
            cache_ttl_minutes=5 if cls._is_trading_hours() else 60  # Longer cache outside trading hours
        )
...
class RealtimeDataCache:
    ...
    def cleanup_expired_cache(self) -> int:
        ...
        deleted_count = self.db.query(RealtimeStockData).filter(
            RealtimeStockData.timestamp < cutoff_time
        ).delete()
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        valid_records = self.db.query(RealtimeStockData).filter(
