...
class AsTypeTransformer(Transformer):
...
class ResizeTransformer(Transformer):
    ...
    def __init__(self,
        ...
        self.data_type = data_type
        if mode == 'opencv' and interpolation != "" and \
                interpolation not in INTERPOLATION_DICT.keys():
            raise ValueError(
                f" resize method is set to {mode}. It does not support "
                f"interpolation method {interpolation} at the moment.")
    ...
    def run_transform(self, data):
        if self.resize_mode == 'skimage':
            ...
            raise ValueError(f"unsupport resize mode:{self.resize_mode}."
                             "(skimage and opencv are supported)")

    def opencv_resize(self, data):
        target_h, target_w = self.target_size
        if self.interpolation \
                and self.interpolation in INTERPOLATION_DICT.keys():
            data = cv2.resize(
    ...
    def skimage_resize(self, image):
        ...
        if image.shape[-1] == 1 or image.shape[-1] == 3:
            ...
            if im_max > im_min:
                # skimage is fast but only understands {1,3}
                # channel images in [0, 1].
...
class ShortSideResizeTransformer(Transformer):
    def __init__(self, short_size, data_type='float', interpolation=""):
        ...
        self.data_type = data_type
        if interpolation != "" \
                and interpolation not in INTERPOLATION_DICT.keys():
            raise ValueError(
                "The transformer does not supported interpolation "
                f"method {interpolation} at the moment ")
        self.interpolation = interpolation
    ...
    def run_transform(self, image):
        ...
        if height < width:
            off = width / height
            if self.interpolation and \
                    self.interpolation in INTERPOLATION_DICT.keys():
                image = cv2.resize(
...
class PaddedCenterCropTransformer(Transformer):
    ...
    # 以原图短边的 ( image_size / (image_size + crop_pad) ) 为边长，
    # 从原图中 center_crop 出一个正方形
    ...
    def run_transform(self, image):
        ...
        padded_center_crop_size = int(
            (float(self.image_size) / (self.image_size + self.crop_pad)) *
...
class _ChannelSwapTransformer(Transformer):
    ...
    def run_transform(self, image):
        ...
        if self.channel_index == 0:
            ...
            raise ValueError(f"channel index: {self.channel_index} error in"
                             "_ChannelSwapTransformer")
        return image
...
class BGR2RGBTransformer(Transformer):
    def __init__(self, data_format="CHW"):
        if data_format == "CHW":
            ...
            raise ValueError(f"unsupported data_format: '{data_format}' in "
                             "BGR2RGBTransformer")
...
class _ColorConvertTransformer(Transformer):
    def __init__(self, source_type, target_type, data_format="CHW") -> None:
    ...
    def run_transform(self, image) -> np.ndarray:
