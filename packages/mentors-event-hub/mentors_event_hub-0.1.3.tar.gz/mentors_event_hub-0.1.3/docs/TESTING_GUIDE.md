...
```
mentors_event_hub/
├── mentors_event_hub/       # Código fonte
│   ├── __init__.py
│   ├── event_hub.py
│   ├── event_hub_client.py
│   ├── azure/
...
if python -m black --check mentors_event_hub/; then
    print_success "Black: Formatação correta"
else
    print_error "Black: Código precisa ser formatado"
    echo "💡 Execute: python -m black mentors_event_hub/"
    FAILURES=$((FAILURES + 1))
fi
echo ""
...
if python -m ruff check mentors_event_hub/; then
    print_success "Ruff: Qualidade do código OK"
else
    print_error "Ruff: Problemas de qualidade encontrados"
    echo "💡 Execute: python -m ruff check --fix mentors_event_hub/"
    FAILURES=$((FAILURES + 1))
fi
echo ""
...
if python -m mypy mentors_event_hub/ --ignore-missing-imports; then
...
if python -m pytest tests/ -v --cov=mentors_event_hub --cov-report=term --cov-report=html; then
...
if python -c "import mentors_event_hub; print('✅ Imports OK')"; then
...
```bash
python -m black --check mentors_event_hub/
```

**Formatar código:**
```bash
python -m black mentors_event_hub/
```
...
**Verificar qualidade:**
```bash
python -m ruff check mentors_event_hub/
```

**Corrigir automaticamente:**
```bash
python -m ruff check --fix mentors_event_hub/
```
...
**Verificar tipos:**
```bash
python -m mypy mentors_event_hub/ --ignore-missing-imports
```

**Verificação rigorosa:**
```bash
python -m mypy mentors_event_hub/ --strict
```
...
**Com cobertura:**
```bash
python -m pytest tests/ -v --cov=mentors_event_hub --cov-report=term
```

**Com relatório HTML:**
```bash
python -m pytest tests/ -v --cov=mentors_event_hub --cov-report=html
...
- name: 🧪 Executar testes
  run: |
    ...
    pytest tests/ -v --cov=mentors_event_hub --cov-report=xml
```
...
pytest --cov=mentors_event_hub --cov-report=html
...
- [ ] ✅ `pytest tests/` passou
- [ ] ✅ Cobertura ≥ 80%
- [ ] ✅ `python -c "import mentors_event_hub"` funciona
...
python -c "import mentors_event_hub; print('OK')"   # ✅ Imports
