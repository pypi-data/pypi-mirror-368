...
if python3 -m black --check mentors_event_hub/; then
    print_success "Black: Formatação correta"
else
    print_error "Black: Código precisa ser formatado"
    echo "💡 Execute: python3 -m black mentors_event_hub/"
    FAILURES=$((FAILURES + 1))
fi
echo ""
...
if python3 -m ruff check mentors_event_hub/; then
    print_success "Ruff: Qualidade do código OK"
else
    print_error "Ruff: Problemas de qualidade encontrados"
    echo "💡 Execute: python3 -m ruff check --fix mentors_event_hub/"
    FAILURES=$((FAILURES + 1))
fi
echo ""
...
if python3 -m mypy mentors_event_hub/ --ignore-missing-imports; then
...
if python3 -m pytest tests/ -v --cov=mentors_event_hub --cov-report=term --cov-report=html; then
...
if python3 -c "import mentors_event_hub; print('✅ Imports OK')"; then
