...
class SandboxContext(Loggable):
    ...
    def get_system_llm_resource(self, use_mock: bool | None = None) -> "BaseLLMResource | None":
        ...
        """
        # TODO: remove this once we have a proper system LLM resource
        if not self._system_llm_resource:
            from dana.core.resource.plugins.base_llm_resource import BaseLLMResource
            llm = BaseLLMResource(
                name='fallback_llm',
                model='openai:gpt-4o-mini',  # TODO: process "default" model properly
            )
            llm.initialize()
            assert llm.is_running()
            self.set_system_llm_resource(llm)
