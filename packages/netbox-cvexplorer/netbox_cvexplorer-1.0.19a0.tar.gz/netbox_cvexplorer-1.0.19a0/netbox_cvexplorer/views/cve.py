...
from netbox_cvexplorer.filtersets import CVEFilterSet
...
class CVEListView(generic.ObjectListView):
    ...
    action_buttons = ("add",)
    template_name = "generic/object_list.html"  # NetBox-Layout mit Filter-Tab

class CVEListViewOld(ListView):
    model = CVE
...
class CVEDeleteView(PermissionRequiredMixin, DeleteView):
    model = CVE
    template_name = 'netbox_cvexplorer/cve_confirm_delete.html'  # <- eigenes Template
