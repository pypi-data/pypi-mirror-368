...
class NoOpDTO(BaseModel):
    ...
    __properties = ["type", "node_id", "child_id", "label"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in ('no-op'):
            raise ValueError("must be one of enum values ('no-op')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NoOpDTO:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> NoOpDTO:
        """Create an instance of NoOpDTO from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return NoOpDTO.parse_obj(obj)

        _obj = NoOpDTO.parse_obj({
            "type": obj.get("type") if obj.get("type") is not None else 'no-op',
