...
from maleo_soma.types.literals.operation import (
...
class ResourceOperationActionSchema(BaseModel):
    ...
    def from_request(
        ...
        if from_state:
            ...
            if not isinstance(operation_action, cls):
                raise TypeError(
            ...
            if request.method == "POST":
                ...
                operation_type = ResourceOperationType.READ
                ...
                operation_type = ResourceOperationType.DELETE
            else:
    ...
    def as_dependency(cls, from_state: bool = True):
        """Create a FastAPI dependency for this resource operation action."""

        def dependency(request: Request) -> "Optional[ResourceOperationActionSchema]":
...
type AllResourceOperationAction = Union[
...
def generate_resource_operation_action(
    *,
    ...
    if type is ResourceOperationType.CREATE:
        ...
        return UpdateResourceOperationAction(
            update_type=update_type, status_update_type=status_update_type
        )
