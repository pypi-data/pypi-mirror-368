...
from ..compat_typing import TypedDict
...
def _parse_syntaxcheck_response(response_text: str) -> list[SyntaxCheckResult]:
...
def syntaxcheck(
    ...
    <chkrun:checkObject adtcore:uri="{source_uri}" chkrun:version="{version}">
        <chkrun:artifacts>
    ...
    messages = _parse_syntaxcheck_response(response.text)
    return messages
