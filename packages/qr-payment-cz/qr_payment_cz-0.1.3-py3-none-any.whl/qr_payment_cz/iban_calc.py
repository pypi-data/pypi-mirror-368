...
class IBANCalculator:
    ...
    def validate_account_number(
        cls, account_prefix: str, account_number: str, bank_id: str
    ) -> bool:
        if bank_id not in BANK_IDS:
            raise ParseException(f"Invalid bank ID '{bank_id}'")
        print(f"Bank {bank_id} => '{BANK_IDS[bank_id]}' detected.")

        account_no_2p = f"{account_prefix}{account_number}"
        if len(account_no_2p) != 16:
            raise ParseException(
                f"Invalid part of account number '{account_prefix}{account_number}' must be 16 characters long"
            )
        ...
        if sum_weight % 11 != 0:
            raise ParseException(
                "Invalid account number '{account_number}'. Validation by MOD 11 failed."
            )
