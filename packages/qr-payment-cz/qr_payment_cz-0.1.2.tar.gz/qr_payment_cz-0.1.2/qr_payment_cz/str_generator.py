...
from qr_payment_cz.iban_calc import IBANCalculator
from qr_payment_cz.exceptions import PaymentException
...
class Payment:
    ...
    def acc(self):
        ...
        acc = acc.replace(" ", "")
        if acc[:2] != "CZ":
            raise PaymentException(f"Invalid IBAN format '{acc}'")
        return f"*ACC:{acc}"
...
class StrGenerator:
    ...
    def generate_string(self) -> str:
        ...
        result = f"SPD*1.0{payment.acc}{payment.am}{payment.cc}{payment.dt}{payment.msg}{payment.rn}{payment.vs}{payment.ks}{payment.ss}"
        return result
