...
def resolve_with_ext(base: Path, is_allowed=None) -> Optional[Path]:
    ...
    for p in candidates:
        if p.is_file() and (REACT_ALLOWED(p) if REACT_ALLOWED else True):
            return p
    return None
...
def build_graph_all(src_root: Path,cfg=None) -> Dict:
    ...
    for f, info in analyses.items():
        ...
        for block in (info["reexports"], info["imports"]):
            for item in block:
                base = (fp.parent / item["spec"]).resolve()
                resolved = resolve_with_ext(base, is_allowed=REACT_ALLOWED)
                if resolved:
                    edges.append({"from": f, "to": str(resolved), "named": item["named"]})
                else:
                    # only record unresolved if the spec *would* be local but filtered out or missing
...
def start_analyzer(
    ...
    if dot:
        dot = to_dot(graph, src_root)
        Path(dot).write_text(dot, encoding="utf-8")
