...
def make_allowed_predicate(
    *,
    ...
    allowed_exts    = {e.lower() for e in (allowed_exts or DEFAULT_ALLOWED_EXTS)}
    unallowed_exts  = {e.lower() for e in (unallowed_exts or DEFAULT_UNALLOWED_EXTS)}
    exclude_dirs    = set(map(str.lower, (exclude_dirs or []))) | {d.lower() for d in DEFAULT_EXCLUDE_DIRS}
    exclude_patterns= set(map(str.lower, (exclude_patterns or []))) | {p.lower() for p in DEFAULT_EXCLUDE_PATTERNS}
