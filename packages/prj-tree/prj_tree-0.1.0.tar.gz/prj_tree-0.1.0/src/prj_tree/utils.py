...
def _read_text_internal(path: pathlib.Path, max_chars: int) -> Optional[str]:
...
def read_text_safely(path: pathlib.Path, max_chars: int) -> str:
...
def read_text_raw_safely(path: pathlib.Path, max_chars: int) -> str:
    content = _read_text_internal(path, max_chars)
    if content is None:
        return f"[Бинарный файл: {path.suffix or 'без расширения'}]"
    return content
