...
def _get_mlflow():
    try:
        # Import MLflow lazily so that importing PySpark doesn't automatically import
        # mlflow. This should improve Python REPL startup time and make it easier to identify
        # MLflow imports triggered by user code for usage analysis.
...
def _get_experiment_id():
    try:
        ...
        # mlflow. This should improve Python REPL startup time and make it easier to identify
        # MLflow imports triggered by user code for usage analysis.
        _get_experiment_id_fn = getattr(importlib.import_module('mlflow.tracking.fluent'),
                                        '_get_experiment_id')
        return _get_experiment_id_fn()
    except ImportError:
        return None
    ...
    if os.environ.get("PYSPARK_PIN_THREAD", "false").lower() != "true":
        try:
            ...
            warnings.warn("{} fit call failed but some spark jobs "
                          ...
                          "issue, you should enable pyspark pinned thread mode."
                          .format(uid))
            raise e
...
class _MLflowInstrumentation:
    ...
    _NO_MLFLOW_WARNING =\
        "Cannot find mlflow module. To enable MLflow logging, install mlflow from PyPI."
    ...
    def is_tracking_enabled(spark):
        ...
        """
        flag = spark.conf.get(_MLflowInstrumentation._MLFLOW_TRACKING_ENABLED_FLAG, default='false')
    ...
    def _log_generic_tuning(logging_func, tuning, metrics):
        ...
        if _mlflow is None:
            return
        _MLflowInstrumentation.mlflow_client = _get_mlflow_client()
    ...
    def _log_tuning_runs(parent_run_uuid, fit_uuid, est, epm, metric_names, metrics):
        ...
        for index in range(num_models):
            ...
            _MLflowInstrumentation._log_single_run(run_uuid,
                                                   ...
                                                   metric_names,
                                                   metrics[:, index])
            _MLflowInstrumentation.mlflow_client.set_terminated(run_uuid, status="FINISHED")
