...
def read_udfs(pickleSer, infile, eval_type):
    ...
    if eval_type in (PythonEvalType.SQL_SCALAR_PANDAS_UDF,
        ...
        pandas_zero_conf_conversion_groupby_apply_max_bytes = (
            int(pandas_zero_conf_conversion_groupby_apply_max_bytes)
        )
        should_use_edge = (
            pandas_zero_conf_conversion_groupby_apply_enabled
        ...
        if eval_type == PythonEvalType.SQL_COGROUPED_MAP_PANDAS_UDF and should_use_edge:
            ...
            ser = GroupUDFSerializer(timezone, safecheck, assign_cols_by_name,
                                     timely_flush_enabled=timely_flush_enabled,
                                     timely_flush_timeout_ms=timely_flush_timeout_ms,
                                     max_bytes=pandas_zero_conf_conversion_groupby_apply_max_bytes)
        elif eval_type == PythonEvalType.SQL_COGROUPED_MAP_PANDAS_UDF:
...
def main(infile, outfile):
    ...
    try:
        ...
        if os.environ.get("WSFS_ENABLE", "false") == "true":
            # Change the current directory to the directory of the notebook. Note that we are
            # doing this after the jvm makes the rpc call to workspace filesystem (wsfs) to send
            # the credential, otherwise changing directory will fail in the wsfs fuse daemon
            notebook_path = taskContext._localProperties.\
                get("spark.databricks.notebook.path", "")
            # currently we limit support to repos/projects, and we only show
            # /Projects|/Repos in the dataplane
            if notebook_path != "" and (notebook_path.startswith("/Repos/") or
                                        notebook_path.startswith("/Projects/")):
                prefix = "/Workspace"
                # to support local tests to read files in a temp folder
                test_prefix = os.environ.get("WSFS_TEST_DIR", "")
                if test_prefix != "":
                    prefix = test_prefix

                dir_path = prefix + os.path.dirname(notebook_path)

                # We want WSFS to fail closed so that commands that do not access WSFS can
                ...
                try:
                    ...
                    print("Failed to change to wsfs dir {} with error {}".format(dir_path, e),
                          file=sys.stderr)
