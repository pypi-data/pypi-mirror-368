...
class SparkContext(object):
    ...
    def _do_init(self, master, appName, sparkHome, pyFiles, environment, batchSize, serializer,
        ...
        self._encryption_enabled = self._jvm.PythonUtils.isEncryptionEnabled(self._jsc) and \
            not self._conf.get(
    ...
    def parallelize(self, c, numSlices=None):
        ...
        if self._conf.get(
            ...
            session = SparkSession.builder._sparkContext(self).getOrCreate()
    ...
    def textFile(self, name, minPartitions=None, use_unicode=True):
        ...
        return RDD(self._jsc.textFile(path, minPartitions), self,
    ...
    def addClusterWideLibraryToPath(self, libname):
        if libname[-4:].lower() in self.PACKAGE_EXTENSIONS:
            ...
            if path not in sys.path:    # if the path is already added, skip
                isolation_prefix = self._jvm.org.apache.spark \
                    .util.DatabricksUtils.ISOLATED_LIBRARY_PREFIX()
                virtualenv_root_dir = self._jvm.org.apache.spark \
                    .util.DatabricksUtils.PYTHON_VIRTUALENV_ROOTDIR_NAME()
    ...
    def addIsolatedLibraryPath(self, libname):
        """
    ...
    def runJob(self, rdd, partitionFunc, partitions=None, allowLocal=False):
        ...
        if self._conf.get(self._jvm.PythonSecurityUtils.USE_FILE_BASED_COLLECT()):
            ...
            with SCCallSiteSync(self) as css:
                import pwd  # Do this lazily since it's available on Linux only
                username = pwd.getpwuid(os.getuid()).pw_name
                filename = self._jvm.PythonRDD.runJobToPythonFile(self._jsc.sc(),
                                                                  mappedRDD._jrdd,
                                                                  partitions,
                                                                  username)
            try:
                deserializer = mappedRDD._jrdd_deserializer
