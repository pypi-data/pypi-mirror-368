...
from pyspark.sql.pandas.utils import estimate_pandas_size
...
class PandasConversionMixin(object):
    ...
    def toPandas(self):
        ...
        from pyspark.sql.pandas.utils import message_with_direct_cause,\
...
class SparkConversionMixin(object):
    ...
    def _get_pandas_num_slices(self, pdf):
        """
        Estimate the size of the pandas.DataFrame to be converted and use it to compute
        the number of slices needed for zero-conf conversion.
        :param pdf: The pandas.DataFrame to be converted
        :return: num_slices: The number of slices needed for zero-conf conversion, where
            if num_slices <= len(pdf):
                num_slices * pandas_max_bytes_per_slice >= estimated_data_size
                AND num_slices >= default_parallelism
                AND num_slices % default_parallelism == 0
            else:
                num_slices == len(pdf)
        """
        default_parallelism = self.sparkContext.defaultParallelism
        try:
            estimated_data_size = estimate_pandas_size(pdf)
            pandas_max_bytes_per_slice = self._wrapped._conf \
                .pandasZeroConfConversionMaxBytesPerSlice()
            # make slice size roughly `pandas_max_bytes_per_slice` but at the same time
            # ensure num_slices to be multiple of default_parallelism, so that when call
            # `sc.parallelize`, the partition will be more balanced.
            num_slices = math.ceil(
                estimated_data_size / pandas_max_bytes_per_slice / default_parallelism
            ) * default_parallelism
        except Exception:
            # estimate pandas dataframe size failed, fallback set num slices to be
            # default parallelism
            num_slices = default_parallelism

        length = len(pdf)
        # If num_slices > length, one row is estimated to be larger than one slice.
        # We DO NOT respect the maxBytes conf here. Instead, we still try to fit one
        # row into one slice.
        num_slices = min(num_slices, length)

        # If we estimate data size as 0 bytes, then will get num_slices 0.
        # increase it to be "default_parallelism" to keep consistency with OSS
        if num_slices < default_parallelism:
            num_slices = default_parallelism
    ...
    def _create_from_pandas_with_arrow(self, pdf, schema, timezone):
        ...
        from pyspark.sql.pandas.utils import require_minimum_pandas_version, \
            require_minimum_pyarrow_version, warn_inefficient_columns_for_conversion
        ...
        if isinstance(schema, (list, tuple)):
            ...
            for name, field in zip(schema, arrow_schema):
                try:
                    arrow_type = from_arrow_type(field.type)
                except TypeError as e:
                    raise TypeError("Unable to convert the field {}. If this column is not "
                                    "necessary, you may consider dropping it or converting "
                                    "to primitive type before the conversion.".format(name)) from e
        ...
        if self._wrapped._conf.pandasZeroConfConversionEnabled():
            num_slices = self._get_pandas_num_slices(pdf)
            length = len(pdf)
