...
class DataFrame(PandasMapOpsMixin, PandasConversionMixin):
    ...
    def _repr_html_(self):
        ...
        if self.sql_ctx._conf.isReplEagerEvalEnabled():
            ...
            port = self._jdf.getRowsToPython(
                max_num_rows, self.sql_ctx._conf.replEagerEvalTruncate())
            rows = list(_load_from_socket(port, BatchedSerializer(PickleSerializer())))
            head = rows[0]
            row_data = rows[1:]
    ...
    def toDF(self, *cols):
        ...
        for col in cols:
            if not isinstance(col, str):
                raise TypeError(
                    f"Argument `{cols}` should be a list[str], got {type(col).__name__}."
                )
