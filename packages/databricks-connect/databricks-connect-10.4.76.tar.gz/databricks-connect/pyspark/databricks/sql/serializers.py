...
from pyspark.sql.pandas.serializers import ArrowStreamPandasUDFSerializer, ArrowStreamSerializer, \
    CogroupUDFSerializer, read_int, write_int, SpecialLengths
...
def slice_pandas_frame(series, pandas_max_bytes_per_slice):
    for frame, t in series:
        ...
        estimated_data_size = estimate_pandas_size(frame)
...
class GroupUDFSerializer(ArrowStreamPandasUDFSerializer):
    def __init__(self, timezone, safecheck, assign_cols_by_name, df_for_struct=False,
                 max_prefetch=0, timely_flush_enabled=True, timely_flush_timeout_ms=100,
                 max_bytes=-1):
        super(GroupUDFSerializer, self) \
            .__init__(timezone, safecheck, assign_cols_by_name, df_for_struct,
                      max_prefetch, timely_flush_enabled, timely_flush_timeout_ms)
    ...
    def dump_stream(self, iterator, stream,
                    timely_flush_enabled=True, timely_flush_timeout_ms=100):
        """
        ...
        return ArrowStreamSerializer \
            .dump_stream(self, init_stream_yield_batches(), stream,
...
class DatabricksCogroupUDFSerializer(CogroupUDFSerializer):
    ...
    def __init__(self, timezone, safecheck, assign_cols_by_name, df_for_struct=False,
                 max_prefetch=0, timely_flush_enabled=True, timely_flush_timeout_ms=100,
                 max_bytes=-1):
        super(DatabricksCogroupUDFSerializer, self) \
            .__init__(timezone, safecheck, assign_cols_by_name, df_for_struct,
                      max_prefetch, timely_flush_enabled, timely_flush_timeout_ms)
        self.max_bytes = max_bytes
