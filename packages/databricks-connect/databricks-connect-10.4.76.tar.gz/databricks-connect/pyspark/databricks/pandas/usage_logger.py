...
class PandasOnSparkUsageLogger:
    def __init__(self):
        ...
        self.logger.recordEvent(
            self.logger.metricDefinitions().EVENT_PANDAS_ON_SPARK_IMPORTED(), {}, ""
        )

    def log_success(self, class_name, name, duration, signature=None):
        function_or_property = (
            self.logger.tagDefinitions().TAG_PANDAS_ON_SPARK_FUNCTION()
            if signature is not None
            else self.logger.tagDefinitions().TAG_PANDAS_ON_SPARK_PROPERTY()
        )
        self.logger.recordEvent(
            self.logger.metricDefinitions().EVENT_PANDAS_ON_SPARK_FUNCTION_CALLED(),
            {
    ...
    def log_missing(self, class_name, name, is_deprecated=False, signature=None):
        ...
        self.logger.recordEvent(
            self.logger.metricDefinitions().EVENT_PANDAS_ON_SPARK_FUNCTION_CALLED(),
            {
                self.logger.tagDefinitions().TAG_CLASS_NAME(): class_name,
                function_or_property: "{}{}".format(name, signature or ""),
                self.logger.tagDefinitions().TAG_STATUS(): (
