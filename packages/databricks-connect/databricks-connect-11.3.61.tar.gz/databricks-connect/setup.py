...
DBCONNECT_CONF_TARGET = os.path.join(TEMP_PATH, "conf")
...
try:
    ...
    if (in_spark):
        ...
        if _supports_symlinks():
            ...
            os.symlink(DBCONNECT_CONF_PATH, DBCONNECT_CONF_TARGET)
        else:
            ...
            copytree(DBCONNECT_CONF_PATH, DBCONNECT_CONF_TARGET)
    else:
    ...
    setup(
        ...
        install_requires=['py4j==0.10.9.5', 'six'],
        provides=['pyspark'],
        obsoletes=['pyspark'],
        extras_require={
            'ml': ['numpy>=1.15,<2.0.0'],
            'mllib': ['numpy>=1.15,<2.0.0'],
            'sql': [
