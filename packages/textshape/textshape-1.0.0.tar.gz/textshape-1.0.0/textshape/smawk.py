...
This file is licensed under the terms of the MIT License (http://opensource.org/licenses/MIT):
...
def ConcaveMinima(
    ...
    for r in row_indices:
        while len(stack) >= 1 and matrix(
            stack[-1], col_indices[len(stack) - 1]
        ) > matrix(r, col_indices[len(stack) - 1]):
            stack.pop()
        if len(stack) != len(col_indices):
            stack.append(r)
    row_indices = stack

    # Recursive call to search for every odd column
    minima = ConcaveMinima(
        row_indices, [col_indices[i] for i in range(1, len(col_indices), 2)], matrix
    )

    # Go back and fill in the even rows
    r = 0
    for c in range(0, len(col_indices), 2):
        col = col_indices[c]
        row = row_indices[r]
        if c == len(col_indices) - 1:
            last_row = row_indices[-1]
        else:
            last_row = minima[col_indices[c + 1]][1]
...
class OnlineConcaveMinima:
    ...
    _indices: List[int]
    _finished: int
    _matrix: Callable[[int, int], float]
    _base: int
    _tentative: int

    def __init__(self, matrix: Callable[[int, int], float], initial: float):
        ...
        self._indices = [0]  # ...and their indices
