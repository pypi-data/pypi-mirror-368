...
async def create_agent(request: AgentCreateRequest):
    ...
    try:
        ...
        # Store agent
        app_state["agents"][agent.agent_id] = agent  # type: ignore[attr-defined]

        # Return response
        return AgentResponse(
            agent_id=agent.agent_id,  # type: ignore[attr-defined]
            name=request.name,
...
async def get_agent(agent_id: str):
    ...
    return AgentResponse(
        ...
        model=getattr(agent.llm_provider, "model", "unknown"),
        spree_enabled=agent.config.spree_enabled,
        tools=getattr(agent.config, "tools", []),
        created_at=datetime.now(timezone.utc).isoformat(),
        metadata={},
    )
...
async def run_benchmark_task(
    ...
    try:
        ...
        for agent in agents:
            ...
            results[agent.name] = {  # type: ignore[attr-defined]
