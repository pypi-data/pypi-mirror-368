...
class TwampClient(Base):
    """A network stack element representing RFC 4656 TWAMP.
    The TwampClient class encapsulates a list of twampClient resources that are managed by the user.
    A list of resources can be retrieved from the server using the TwampClient.find() method.
    The list can be managed by using the TwampClient.add() and TwampClient.remove() methods.
    """

    __slots__ = ()
    _SDM_NAME = "twampClient"
    _SDM_ATT_MAP = {
        "Name": "name",
        "ObjectId": "objectId",
    }
    ...
    def __init__(self, parent, list_op=False):
        super(TwampClient, self).__init__(parent, list_op)
    ...
    def update(self, Name=None):
        # type: (str) -> TwampClient
        """Updates twampClient resource on the server.
    ...
    def add(self, Name=None):
        # type: (str) -> TwampClient
        """Adds a new twampClient resource on the server and adds it to the container.
        ...
        Returns
        -------
        - self: This instance with all currently retrieved twampClient resources using find and the newly added twampClient resources available through an iterator or index
    ...
    def remove(self):
        """Deletes all the contained twampClient resources in this instance from the server.
    ...
    def find(self, Name=None, ObjectId=None):
        # type: (str, str) -> TwampClient
        """Finds and retrieves twampClient resources from the server.

        All named parameters are evaluated on the server using regex. The named parameters can be used to selectively retrieve twampClient resources from the server.
        To retrieve an exact match ensure the parameter value starts with ^ and ends with $
        By default the find method takes no parameters and will retrieve all twampClient resources from the server.
        ...
        Returns
        -------
        - self: This instance with matching twampClient resources retrieved from the server available through an iterator or index
    ...
    def read(self, href):
        """Retrieves a single instance of twampClient data from the server.
        ...
        Returns
        -------
        - self: This instance with the twampClient resources from the server available through an iterator or index
    ...
    def TwampClearStats(self, *args, **kwargs):
        # type: (*Any, **Any) -> None
        """Executes the twampClearStats operation on the server.

        Clear TWAMP statistics for selected plugins

        twampClearStats(async_operation=bool)
        -------------------------------------
        - async_operation (bool=False): True to execute the operation asynchronously. Any subsequent rest api calls made through the Connection class will block until the operation is complete.
        ...
        for item in kwargs.items():
            payload[item[0]] = item[1]
        return self._execute("twampClearStats", payload=payload, response_object=None)
