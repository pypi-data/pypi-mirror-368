...
class Bfd(Base):
    ...
    def find(
        ...
        All named parameters are evaluated on the server using regex. The named parameters can be used to selectively retrieve bfd resources from the server.
        To retrieve an exact match ensure the parameter value starts with ^ and ends with $
        By default the find method takes no parameters and will retrieve all bfd resources from the server.
        ...
        - IntervalValue (number): Interval Value
        - PacketsPerInterval (number): Number of BFD control packets per interval.
        - RunningState (str(unknown | stopped | stopping | starting | started)): The current running state of the BFD protocol.

        Returns
        -------
        - self: This instance with matching bfd resources retrieved from the server available through an iterator or index
    ...
    def read(self, href):
        """Retrieves a single instance of bfd data from the server.
        ...
        Returns
        -------
        - self: This instance with the bfd resources from the server available through an iterator or index
