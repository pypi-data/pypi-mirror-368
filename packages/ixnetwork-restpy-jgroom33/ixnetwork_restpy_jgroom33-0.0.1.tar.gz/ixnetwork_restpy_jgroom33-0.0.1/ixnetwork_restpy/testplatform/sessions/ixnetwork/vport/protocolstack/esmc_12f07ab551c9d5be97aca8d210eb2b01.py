...
class Esmc(Base):
    ...
    def ESMCClearStats(self, *args, **kwargs):
        # type: (*Any, **Any) -> None
        """Executes the eSMCClearStats operation on the server.

        Clear ESMC statistics

        eSMCClearStats(async_operation=bool)
        ------------------------------------
        - async_operation (bool=False): True to execute the operation asynchronously. Any subsequent rest api calls made through the Connection class will block until the operation is complete.
        ...
        for item in kwargs.items():
            payload[item[0]] = item[1]
        return self._execute("eSMCClearStats", payload=payload, response_object=None)
