...
class PcsSyncErrors(Base):
    """
    The PcsSyncErrors class encapsulates a required pcsSyncErrors resource which will be retrieved from the server every time the property is accessed.
    """

    __slots__ = ()
    _SDM_NAME = "pcsSyncErrors"
    _SDM_ATT_MAP = {
        "Enable": "enable",
        "Name": "name",
        "Notes": "notes",
    ...
    def __init__(self, parent, list_op=False):
        super(PcsSyncErrors, self).__init__(parent, list_op)
    ...
    def update(self, Enable=None, Value=None):
        # type: (bool, int) -> PcsSyncErrors
        """Updates pcsSyncErrors resource on the server.
    ...
    def find(
        ...
        # type: (bool, str, str, str, str, int) -> PcsSyncErrors
        """Finds and retrieves pcsSyncErrors resources from the server.

        All named parameters are evaluated on the server using regex. The named parameters can be used to selectively retrieve pcsSyncErrors resources from the server.
        To retrieve an exact match ensure the parameter value starts with ^ and ends with $
        By default the find method takes no parameters and will retrieve all pcsSyncErrors resources from the server.
        ...
        Returns
        -------
        - self: This instance with matching pcsSyncErrors resources retrieved from the server available through an iterator or index
    ...
    def read(self, href):
        """Retrieves a single instance of pcsSyncErrors data from the server.
        ...
        Returns
        -------
        - self: This instance with the pcsSyncErrors resources from the server available through an iterator or index
