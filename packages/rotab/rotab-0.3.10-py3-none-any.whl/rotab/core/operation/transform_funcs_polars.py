...
# ---- numeric dtype helper (Polarsの古い版でも動くように自前実装) ----
NUMERIC_DTYPES = {
    pl.Int8,
    pl.Int16,
    pl.Int32,
...
def train_lgbm_with_optuna_multi_target(
    ...
    try:
        ...
        # 型指定を構築：日付＋One-Hot列はUtf8、それ以外のfeatures/targetsはFloat64
        schema_overrides: Dict[str, pl.DataType] = {}

        # 日付列は文字列で読む（後でフォーマットでパース）
        schema_overrides[split_by_column] = pl.Utf8

        # One-Hot対象は文字列で読む
        for c in one_hot_cols or []:
            schema_overrides[c] = pl.Utf8

        # それ以外のfeatures/targetsはFloat64（Boolも数値として扱う）
        for c in set(features + targets):
            if c not in schema_overrides:  # まだ型が決まってない列だけ
