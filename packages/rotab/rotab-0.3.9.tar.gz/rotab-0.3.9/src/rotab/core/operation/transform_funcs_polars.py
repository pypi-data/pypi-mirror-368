...
def month_window(
    ...
    """
    月単位の過去/未来ウィンドウで集約。出力型は入力に揃える（原則 df_base に合わせる）。
      - 両方Lazy -> Lazyで返す
      - 両方Eager -> DataFrameで返す
      - 混在     -> df_base の型に合わせる
    """
    ...
    if base_was_lazy and data_was_lazy:
        return_lazy = True
    elif (not base_was_lazy) and (not data_was_lazy):
        return_lazy = False
    else:
        return_lazy = base_was_lazy
    ...
    _keep_base = "__mw_base_date_keep__"
    ...
    # base: 日付パース＋キー生成＋コピー列保持＋row_id付与
    df_base_prepared = (
        ...
        .with_columns(pl.col(_base_date).alias(_keep_base))  # join後も使うため保持
        .with_row_index(_row_id)
    )
    df_base_sorted = df_base_prepared.sort([_join_key, _base_date])

    # data: 日付パース＋キー生成（パースは1回だけ）
    ...
    for m in months_list:
        ...
        df_joined = df_joined.with_columns(
            [
                pl.when(m < 0)
                .then(pl.col(_keep_base).dt.offset_by(f"{m}mo"))
                .otherwise(pl.col(_keep_base))
                .alias(_window_start),
                pl.when(m < 0)
                .then(pl.col(_keep_base))
                .otherwise(pl.col(_keep_base).dt.offset_by(f"{m}mo"))
                .alias(_window_end),
            ]
        )
    ...
    final_lf = df_base_prepared
    for res in results:
        final_lf = final_lf.join(res, on=keys + [_row_id], how="left")

    final_lf = final_lf.drop([_row_id, _base_date, _keep_base, _join_key])
...
# ---- numeric dtype helper (for Polars versions without pl.datatypes.is_numeric) ----
NUMERIC_DTYPES = {
    pl.Int8,
    pl.Int16,
    pl.Int32,
...
def train_lgbm_with_optuna_multi_target(
    ...
    log_period_optuna: int = 10,  # Optuna試行時の評価ログ間隔
    log_period_final: int = 10,  # 最終モデル学習時の評価ログ間隔
    run_id: Optional[str] = None,  # 実行識別子（未指定なら自動採番）
    one_hot_cols: Optional[List[str]] = None,  # 厳密運用: train+validate 由来カテゴリのみ使用
    one_hot_drop_first: bool = False,
    shap_sample_size: int = 10000,  # ← 追加: SHAP計算で使う最大行数
    ...
    if split_by == "timestamp":
        ...
        if n_train <= 0 or n_validate <= 0 or n_test <= 0:
            raise ValueError(
                f"Split sizes invalid (train={n_train}, validate={n_validate}, test={n_test}). "
                f"Adjust test_size/validate_size."
            )
        ...
        features_for_model = features[:]  # split_by_column を features に含めていない前提
        print("[INFO] Splitting data randomly (polars-based shuffle)")
        df_shuf = df.sample(fraction=1.0, with_replacement=False, shuffle=True, seed=42)
        n_total = len(df_shuf)
        n_test = int(n_total * test_size)
        n_validate = int(n_total * validate_size)
        n_train = n_total - n_test - n_validate
    ...
    # ---- Strict One-Hot (fit on train+validate, apply to each split, align columns) ----
    ...
    if oh_cols:
        print(
            f"[INFO] One-hot strict mode: fit on train+validate, apply aligned to all splits (drop_first={one_hot_drop_first})"
        )

        # Fit: use combined train+validate to determine required dummy columns (via to_dummies result)
        ...
        # Expand features_for_model by replacing original one-hot targets with their dummy columns
        expanded = []
    ...
    # ---- DType safety (features & targets) ----
    bad_cols: List[Tuple[str, str]] = []
    casts = []
    # (after one-hot, original oh_cols are removed; remaining features/targets must be numeric/bool)
    for c in features_for_model + targets:
        if c not in df.columns and c not in df_train.columns:
            # in case df had different schema vs splits (paranoia guard)
            raise ValueError(f"Column '{c}' not found in the split data.")
        dt = df_train.schema[c]  # check on train split
        if dt == pl.Boolean:
            casts.append(pl.col(c).cast(pl.Float64))
        elif _is_numeric_dtype(dt):
            casts.append(pl.col(c).cast(pl.Float64))  # normalize numeric to float
        else:
    ...
    for target in targets:
        ...
        trial_logs = []  # 集計して最後に一括CSV追記

        print(f"[INFO] Starting Optuna tuning (n_trials={n_trials})...")
        ...
        # ---- Write/append combined CSV for all targets ----
        trial_df = pl.DataFrame(trial_logs)
        if optuna_log_path.exists():
        ...
        print(f"[INFO] Best RMSE: {study.best_value:.5f}")
        ...
        print(f"[INFO] Training final model for {target} on train+validate data")
        best_model = lgb.train(
            final_params,
            lgb_train_full,
            valid_sets=[lgb_train_full],
            callbacks=[lgb.log_evaluation(period=log_period_final)],
        )
        ...
        # サンプリング（shap_sample_size より多い場合はランダム抽出）
