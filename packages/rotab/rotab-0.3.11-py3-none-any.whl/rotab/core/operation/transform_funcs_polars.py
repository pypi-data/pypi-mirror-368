...
# Polarsの数値dtype判定（古い版でも動くように自前実装）
NUMERIC_DTYPES = {
    pl.Int8,
    pl.Int16,
    pl.Int32,
...
# One-Hot適用＆列整列（足りない列は0埋め、余計な列は削除）
...
# ---- ブール推定（サンプルスキャンで true/false/1/0/yes/no のみならBool扱い）----
_BOOL_TOKENS_TRUE = {"true", "t", "yes", "y", "1"}
_BOOL_TOKENS_FALSE = {"false", "f", "no", "n", "0"}
...
def train_lgbm_with_optuna_multi_target(
    ...
    shap_sample_size: int = 10000,  # SHAP計算で使う最大行数
    ...
    try:
        ...
        # features/targets 残りは Bool or Float64
        for c in ft_cols:
    ...
    if oh_cols:
        ...
        # Fit: train+validate のみでカテゴリ集合を確定
