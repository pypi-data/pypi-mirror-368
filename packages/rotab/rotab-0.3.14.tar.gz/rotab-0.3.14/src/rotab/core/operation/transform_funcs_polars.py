...
def month_window(
    ...
    for m in months_list:
        ...
        # asof 用：past のとき厳密境界を避けるため微小ずらし（必要最小限）
        ...
        # asof join（右＝base）。右の列にだけ確実にサフィックスを付けないよう、衝突がある場合のみ "_r"
        ...
        # ★ row_id ではなく、keys + _keep_base でグループ化（asof後も確実に残る軸）
        group_keys = (keys + [_keep_base]) if keys else [_keep_base]
        df_agg = df_filtered.group_by(group_keys).agg(aggs)
        results.append(df_agg)

    # ベースに段階的に left join
    final_lf = df_base_prepared
    join_keys = (keys + [_keep_base]) if keys else [_keep_base]
    for res in results:
        final_lf = final_lf.join(res, on=join_keys, how="left")

    # 後片付け
    final_lf = final_lf.drop([_base_date, _join_key])
