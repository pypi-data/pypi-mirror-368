...
class SettingsWindow(QMainWindow):
    def __init__(self):
        ...
        # --- Ventana 3D con recuadro negro ---
        frame3d = QFrame()
        frame3d.setFrameShape(QFrame.Box)
        frame3d.setLineWidth(2)
        frame3d.setStyleSheet("QFrame { border: 2px solid black; }")
        frame3d_layout = QVBoxLayout(frame3d)
        self.plotter = QtInteractor(frame3d)
        frame3d_layout.addWidget(self.plotter)
        viewer_layout.addWidget(frame3d)

        # --- Ventana 2D con recuadro negro ---
        frame2d = QFrame()
        frame2d.setFrameShape(QFrame.Box)
        frame2d.setLineWidth(2)
        frame2d.setStyleSheet("QFrame { border: 2px solid black; }")
        frame2d_layout = QVBoxLayout(frame2d)
        self.fig = plt.figure(figsize=(4, 4))
        self.canvas = FigureCanvas(self.fig)
        frame2d_layout.addWidget(self.canvas)
        viewer_layout.addWidget(frame2d)
