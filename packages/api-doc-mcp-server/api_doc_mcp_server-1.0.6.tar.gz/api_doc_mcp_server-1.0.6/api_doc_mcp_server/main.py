...
def generate_api_documentation(batch_index: int = 0, workflow_id: str = None) -> List[types.TextContent]:
    ...
    result = {
        "message": f"请在当前项目根目录下为以下Rest API生成详细的api文档，要求:1.文档内容必须包含:(API地址,API所属类路径,API功能描述,API是否需要鉴权,API参数字段描述,API响应字段描述,API请求示例。其中参数和响应字段描述要求使用表格，表头为字段名,字段类型,是否必填,字段说明,备注说明)2.文档内容格式为markdown格式。3.文件名称为 ai_gen_api_doc_{timestamp}.md. (批次 {batch_index+1}/{total_batches})",
