...
indent_lvl = 0
target_debug = "something" and None     # Comment out the 'and None" to activate debugging logs
...
def log(method):
    ...
    def wrapper(self, *a,**kw):
        global indent_lvl
        indent_lvl += 1
        was_i, tok = self.i, repr(self.taste())
        if target_debug in self.content:
            print(f"{' ': >{indent_lvl}}{ method.__name__ }, {tok = }   i={ was_i }")
        out = method(self, *a,**kw)
        if target_debug in self.content:
            print(f"{' ': >{indent_lvl}}{ method.__name__ }, {tok = }   (from:{ was_i } -> to:{ self.i })")
        indent_lvl -= 1
        return out
    return wrapper
...
class JinjaCodeParser:
    ...
    Parse Jinja blocks or vars. Simplified grammar is like this:
        ...
        item_comma_sep ::= code ':' code [ ',' item_comma_sep ] ','? ]

        identifier ::= \w+              (broadly :p )

        extras ::= [ call | attr | brackets ]*

        id_stuff ::= identifier [ extras ]*

        attr ::= '.' id_stuff

        call ::= '(' comma_sep ')' [ extras ]
    ...
    * varargs syntax are forbidden in jinja, so no need to support them.
    ...
    def __init__(self,
        ...
        *args, **kwargs,
        ...
        to_pattern = lambda s: re.escape(s).replace('\\ ',r'\s*').replace('\\#','#')
        tokens = []
        ...
        closing_pattern = "|".join(self.matching_pairs.values())
        self.is_closing = lambda: bool( (tok := self.taste()) and re.fullmatch(closing_pattern, tok))

        # Add escaped versions of the quotes (separately because they are not "matching pairs"!):
        tokens.extend(("\\\\'", '\\\\"'))
    ...
    def eat(self,regex:str=None, msg:str=None) -> str :
        """
        Consume unconditionally a token.
        If there are no available tokens anymore, or if the @regex is given, the current
        token has to match the @regex, otherwise PmtParsingError is raised.
        Subsequent spaces after the eaten token are consumed automatically.
        """
        tok = self.taste()
        if tok is None or regex is not None and not re.fullmatch(regex, tok):
            tok = 'EOF' if tok is None else repr(tok)
            regex = regex or msg
            msg = 'Reached EOF' if not regex else f'Expected pattern was: {regex!r}, but found: {tok}'
            self.fatal(msg, tok)
        self.i += 1
        self.walk()
        return tok
    ...
    def walk(self):
        while (tok := self.taste()) and tok.isspace():
            self.i += 1
    ...
    is_id                = is_factory(r'\w+')
    is_opening           = is_factory(r'[\[{(]')
    is_closing           = lambda self:None         # Any kind of closing. Overridden on instance level.
    ...
    def string(self):
        """
        Cardinality: ONE OR MORE (string, not chars... :p )
    ...
    def paired_delimited_and_commas(self, allow_empty=True, allow_named_args=False):
        ...
        self.eat(target)        # est closing element
    ...
    def comma_sep(self, closing_target:str, is_dict:bool, allow_empty:bool, named_args:bool):
        ...
        Collection of `code` elements separated by commas and surrounded with a pair of
        matching tokens.
        - Collection may be empty, unless @allow_empty is False.
        - May allow matching named arguments if @named_args is True.
        - If `is_dict` is True, validate items as `code : code`
        """
        some = False
        while self.tokens_left() and not self.is_(closing_target):
            some = True
    ...
    def follow_up(self):
        ...
        - getitem:          `value[...]`    (non empty collections)
        - getattr:          `value.{id}`
        - function call:    `value(...)`    (may be empty collection)
        """
        while self.tokens_left() and self.is_(r'[(\[.]'):
...
class BaseMarkdownIndentParser(JinjaCodeParser):
    ...
    def start_parsing(self):
        self.indents = []       # Do not mutate! (because of the cache)
...
class IndentParserFeedbackManager(BaseMarkdownIndentParser):
    ...
    LOOK_AROUND: ClassVar[int] = 15
    LOOKING_FOR: str = "macro calls indentation data"



    def error_info(self, info:str, tok:str=...):
        info = info.rstrip('.')                 # In case I forget and put a dot... :p
        macro = ''
        ...
        if self.current:
            i,name = self.current
            macro = f"""
  Macro being parsed during the error (token index:{ i }/{ self.size }):  \033[31m>>{ name }<<\033[0m
"""
        gathered_data = self.gathered_data_as_str()
        ...
        source = ""
        if self.err_info_stack:
            i_src = self.err_info_stack[-1]
            source = f"""
  Currently parsed structure/element (token index: { i_src }/{ self.size }):
      { self._location_info(None, i_src) }
"""

        return f"""\
Parsing error while looking for { self.LOOKING_FOR }.\n
\033[34m  ERROR: { info } (in: { error_location })\033[0m\n
With the \033[31m>>tokens<<\033[0m of interest highlighted in red:{ macro }
{ source }
  Tokens where the error has been found (token index: { self.i }/{ self.size }):
      { self._location_info(tok) }
{ gathered_data }
