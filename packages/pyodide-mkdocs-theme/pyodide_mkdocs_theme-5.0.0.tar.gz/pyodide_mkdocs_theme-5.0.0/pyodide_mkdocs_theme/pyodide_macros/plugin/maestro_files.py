...
class PageGenerator:
    ...
    def generate_file(self, config:MkDocsConfig):
        ...
        in_nav    = InclusionLevel.NOT_IN_NAV if self.is_in_nav else InclusionLevel.INCLUDED
        self.file = File.generated(config, page_name, content=content, inclusion=in_nav)
        return self.file
    ...
    def _build_and_validate_generated_page_name(self, name:str, kind:str):
        ...
        if name != file_path.name:
            raise PmtIdesTestingError(
                f'The IDE { kind } page should be at the top level of the documentation '
                f'but was: { name }'
            )
        if file_path.exists():
            raise PmtIdesTestingError(
                f'Cannot create the IDE { kind } page: a file with the target name already '
                f'exists: { name }.'
            )
        return name
    ...
    def handle_nav(self, nav: Navigation):
        ...
        if self.is_in_nav:
            logger.info(f"Add IDEs { self.kind } page to navigation.")
            ...
            logger.info(f"Remove IDEs { self.kind } page from navigation")
