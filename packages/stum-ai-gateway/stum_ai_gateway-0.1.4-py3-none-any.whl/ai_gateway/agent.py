...
class LlmNatbusAgent:
    ...
    def __init__(
        ...
        # Track response subjects we have subscribed to, so we can add new ones dynamically
    ...
    async def start(self) -> None:
        ...
        # Subscribe to all known response subjects from registry, config extras, and abilities
        subjects = set(self.registry.all_response_subjects())
        subjects.update(self.cfg.extra_response_subjects or ())
        # Abilities may declare response subjects too
    ...
    async def _ensure_response_subscribed(self, subject: Optional[str]) -> None:
        if not subject:
            return
        if subject in self._resp_subscribed:
            return
        await self.bus.push_subscribe(
            subject,
    ...
    async def _on_human_command(self, rm: ReceivedMessage) -> None:
        ...
        try:
            plan = await self._llm_plan(cmd, args, mapping)
        except Exception as e:
            await self._reply_json(
        ...
        await self._publish_service_request(plan, correlation_id, cmd)
        ...
        if plan.await_response:
            ...
            if not resp_subject:
                ...
                # Dynamic subscription for newly introduced response subjects
    ...
    async def _on_service_response(self, rm: ReceivedMessage) -> None:
        ...
        # Build observation (auto-decompressed by ReceivedMessage)
        try:
        ...
        if ctx.iterative:
            try:
                ...
                # Fallback: return observation to human with error note
                await self._reply_json(
                    ctx.reply_subject,
                    {
            ...
            # send_request: publish and either continue waiting or acknowledge completion
            if isinstance(decision, LlmPlan) and decision.action == "send_request":
                ...
                if decision.await_response:
                    # Ensure we are subscribed to the next response subject if provided via abilities/plan
            ...
            # final_reply: send to human and end
            ...
            # Unknown decision type: pass through observation
            await self._reply_json(
                ctx.reply_subject,
        ...
        # Non-iterative path: pass observation directly
        await self._reply_json(
            ctx.reply_subject,
            {"correlation_id": correlation_id, "command": ctx.original_command, "data": observation},
            rm,
    ...
    async def _publish_service_request(self, plan: LlmPlan, correlation_id: str, origin_cmd: str) -> None:
        # Validate via ability schema if available
    ...
    async def _llm_plan(self, cmd: str, args: dict, mapping: CommandMapping) -> LlmPlan:
        ...
        # Mapping-level schema validation
        ...
        # Ability-level schema validation
    ...
    async def _llm_iterate(self, original_command: str, observation: dict):
        ...
        if action == "send_request":
            # Ability-level validation for iterative steps
            ability = self.abilities.get_by_subject(str(obj.get("subject", "")))
            if ability and ability.payload_schema:
                self._validate_payload(obj.get("payload", {}), ability.payload_schema)
            return LlmPlan.from_json(obj)
    ...
    async def _llm_json(
        ...
        *,
        ...
        for _ in range(max_retries + 1):
            ...
            try:
                raw = self._extract_first_json(text)
                obj = json.loads(raw)
