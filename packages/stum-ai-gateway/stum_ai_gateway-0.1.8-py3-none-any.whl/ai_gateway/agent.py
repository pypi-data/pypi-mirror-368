...
class LlmNatbusAgent:
    ...
    async def _on_human_command(self, rm: ReceivedMessage) -> None:
        ...
        # Schema-guided completion using LLM; fail fast if still missing
        ...
        missing = _missing(required, effective_args)
        if missing and self._canonicalizer is not None:
            try:
                ...
                if not getattr(self.cfg, "ack_on_canon_failure", True):
                    log.debug("ack_on_canon_failure=False requested; acknowledging anyway")
                await rm.ack()
                return
        ...
        if missing:
            await self._reply_json(
                ...
                {"error": "missing_required_args",
                 "detail": {"missing": missing, "example": mapping.example_payload or {}}},
                rm,
    ...
    async def _llm_json(
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                last_error = str(e)
                log.debug("llm_json_retry", extra={"attempt": attempt, "error": last_error})
                hint = (
