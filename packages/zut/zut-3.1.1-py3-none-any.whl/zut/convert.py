...
def parse_datetime(value: str|date, *, use_locale = False) -> datetime|None:
    if value is None or value == '':
        return None
...
def parse_date(value: str|datetime, *, use_locale = False) -> date|None:
    if value is None or value == '':
        return None
    elif isinstance(value, datetime):
        return value.date()
...
def parse_float(value: str|GoogleMoney, *, decimal_separator: Literal['.',',','detect'] = '.') -> float|None:
    if value is None or value == '':
        return None
...
def parse_decimal(value: str|float|GoogleMoney, *, max_decimals: int|None = None, lower_decimals:int|Sequence[int]|bool = False, decimal_separator: Literal['.',',','detect'] = '.') -> Decimal|None:
    """
    ...
    if lower_decimals is False or lower_decimals is None:
        lower_boundaries = []
    elif lower_decimals is True:
        lower_boundaries = [2, 5]
    elif isinstance(lower_decimals, int):
        lower_boundaries = [lower_decimals]
    elif isinstance(lower_decimals, Sequence):
        lower_boundaries = sorted(lower_decimals)
    else:
        raise TypeError(f"lower_decimals: {type(lower_decimals).__name__}")
...
def parse_list(value: str, *, separator='|') -> list[str]|None:
    if value is None:
...
def parse_dict(value: str, *, separator='|') -> dict[str,str|Literal[True]]|None:
    if value is None:
        return None
...
def parse_pg_array(value: str) -> list[str]|None:
...
def convert(value: Any, to: type[T]|Callable[[Any],T], *, use_locale = True, max_decimals: int|None = None, lower_decimals: int|Sequence[int]|bool = False, decimal_separator: Literal['.',',','detect'] = 'detect', list_separator = '|') -> T|None:
    if value is None:
        return None
    ...
    if isinstance(to, type):
        ...
        if issubclass(to, str):
            ...
            converted_value = parse_decimal(value, max_decimals=max_decimals, lower_decimals=lower_decimals, decimal_separator=decimal_separator)
            ...
            if converted_value is not None:
                ...
                if element_to: # type: ignore
                    converted_value = [convert(element, element_to, max_decimals=max_decimals, lower_decimals=lower_decimals, use_locale=use_locale, list_separator=list_separator) for element in converted_value]
