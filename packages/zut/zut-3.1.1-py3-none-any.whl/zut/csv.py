...
def dump_csv(data: Any, file: str|os.PathLike|IO[str], headers: Sequence[str|Column]|dict[str,Converter]|None = None, *, append = False, delay = False, check_newline_on_append: bool|None = None, fmt: CsvWriterFormat|str|None = None, tz: tzinfo|Literal['local','utc']|str|None = None, delimiter: str|None = None, encoding = 'utf-8-sig'):
    """
    :param tz: If set, aware datetimes are written to the CSV file as naive datetimes in the given timezone.
    """
    with CsvWriter(file, headers, append=append, delay=delay, check_newline_on_append=check_newline_on_append, fmt=fmt, tz=tz, delimiter=delimiter, encoding=encoding) as writer:
        for row in data:
            writer.writerow(row)
...
class CsvWriter:
    ...
    def __init__(self, file: str|os.PathLike|IO[str]|None = None, headers: Sequence[str|Column]|dict[str,Converter]|None = None, *, append = False, delay = False, check_newline_on_append: bool|None = None, fmt: CsvWriterFormat|str|None = None, tz: tzinfo|Literal['local','utc']|str|None = None, delimiter: str|None = None, encoding = 'utf-8-sig'):
        """        
        :param tz: If set, aware datetimes are written to the CSV file as naive datetimes in the given timezone.
        """
...
def format_csv_value(value, *, fmt: CsvWriterFormat|str|None = None, tz: tzinfo|Literal['local','utc']|None = None):
    ...
    def format_value(value, *, root):    
        ...
        if tz and isinstance(value, str):
...
class CsvReader:
    ...
    def _row_to_dict(self, row: Sequence[Any]) -> dict[str,Any]:
        if len(row) < len(self.headers):
            ...
            self._logger.warning(f"Missing column(s) in row {self.rowcount}: {', '.join(missing_columns)} (file: {self.name}). Row will contain null values for these columns.")

        elif len(row) > len(self.headers):
            ignore_columns = f"{len(row)+1}-{len(self.headers)}"
            self._logger.warning(f"Additional column(s) in row {self.rowcount}: {ignore_columns} (file: {self.name}). Row will not contain a value for these columns.")
            ...
            if value is not None and self._columns is not None and len(self._columns) > i:
                ...
                if column.converter is not None:
                    ...
                    if newvalue != value:
                        ...
                        if self.is_reading and self.tz and isinstance(value, datetime) and not value.tzinfo:
