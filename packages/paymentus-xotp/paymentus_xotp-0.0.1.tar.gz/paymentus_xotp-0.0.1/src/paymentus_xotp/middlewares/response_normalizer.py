...
class ResponseNormalizer:
    ...
    def pre_process_request(self, url: str, method: str, body: Optional[Any] = None) -> Tuple[str, str, Optional[Any]]:
        ...
        # Convert snake_case to kebab-case in the body
        if body is not None:
        ...
        # This normalizer only modifies POST requests for special transformations
        ...
        if isinstance(data, dict):
            ...
            if "credit-card-expiry-date" in data:
                ...
                if isinstance(expiry_date, dict):
                    # Convert month from string to int if needed
                    if "month" in expiry_date and isinstance(expiry_date["month"], str):
                        try:
                            expiry_date["month"] = int(expiry_date["month"])
