...
class BrillouinZone():
    def __init__(self, astruct, mesh, evals, fermi_energy):
        ...
        coords = numpy.array(grid, dtype=numpy.float64)/mesh
    ...
    def dump(self, path=None, npts=50, units='eV'):
        if path is None:
            ppath = BZPath(
        ...
        for b in toto.transpose():
            for x, y in zip(ppath.xaxis, b):
                print(x, y)
            print()

    def plot(self, path=None, npts=50, units='eV', ax=None):
        ...
        toto = self.interpolator(ppath.path)
        for b in toto.transpose():
            self.plot1 = plt.plot(ppath.xaxis, 
                                  [ib*self.conversion_factor(units) 
                                   for ib in b],
                                  'b-')
        ...
        if units == 'eV':
            self.ax1.set_ylabel('Energy [eV]', fontsize=18)
        else:
            self.ax1.set_ylabel('Energy [Ha]', fontsize=18)
