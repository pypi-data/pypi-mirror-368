...
def ase_cell_to_bigdft(cell):
    """
    Given an ASE unit cell, this transforms it into a BigDFT UnitCell.

    Args:
        cell (ase.Atoms.cell): the ASE unit cell.

    Returns:
        (BigDFT.UnitCells.UnitCell): the BigDFT Unit Cell.
    """
    from BigDFT.UnitCells import UnitCell

    if cell.orthorhombic:
        bcell = UnitCell([cell[0, 0], cell[1, 1], cell[2, 2]],
                         units="angstroem")
    else:
        bcell = UnitCell([cell[0, :].tolist(), 
                          cell[1, :].tolist(),
                          cell[2, :].tolist()], 
                         units="angstroem")
    return bcell
...
class BigASECalculator(Calculator):
    ...
    Args:
      ...
      name_gen (function): a function which takes in the ase atoms and
      label, and returns a name for a given BigDFT run.
    """
    def __init__(self, parameters, calc, restart=None,
                 ignore_bad_restart_file=False, directory=".", label=None,
                 atoms=None, name_gen=None, **kwargs):
        ...
        if calc.get_global_option("skip") and name_gen is None:
            raise ValueError("BigDFT Calculator Should Have Skip Off"
                             "or name_gen shoudl be set")
    ...
    def _run_calculation(self):
        if self.name_gen is not None:
            label = self.name_gen(self.atoms, self.label)
        else:
            label = self.label
        self.log = self.code.run(input=self.parameters,
                                 posinp=self._get_posinp(),
                                 name=label, run_dir=self.rundir)
