            ...
            for k1, v1 in inp.items():
                ...
                for k2, v2 in v1.items():
                    ...
                    if isinstance(v2, list):
                        ofile.write(":"+",".join([str(x) for x in v2]))
                    else:
                        ofile.write("="+str(v2))
                    ofile.write("\n")
                ofile.write("$end\n")
...
class XTBLogfile():
    ...
    def __init__(self, sys, logname, opt=False, md=False):
        ...
        self.scan_energy, _, self.scan_traj = \
            self._process_trajectory(fpath + ".xtbscan.log", False)

    def _process_trajectory(self, fname, has_gradient=True):
