...
def _get_datadir(log):
    ...
    Args:
      log (Logfile): logfile class instance from a BigDFT run.
    Returns:
    ...
    def fragment_population(self, sys, log, frag_indices=None, kxs=None):
        ...
        for fragid in sys:
            ...
            charges[fragid] = sum(x.nel for x in sys[fragid]) - \
                trace(smat.todense())
            charges[fragid] = charges[fragid].item()  # remove numpy
...
def _compute_purity_values(kxs, charges, frag_indices):
    ...
    for idx in frag_indices:
        indices = frag_indices[idx]
        smat = kxs[:, indices]
        smat = 0.5 * smat[indices, :]
        purity_values[idx] = 2.0 * trace(
            (smat.dot(smat) - smat).todense()) / charges[idx]
        purity_values[idx] = purity_values[idx].item()  # remove numpy
...
def units_quadratic_traces(unitlist1, unitlist2, frag_indices, mat1, mat2,
    ...
    interaction = {}
    for fid1 in unitlist1:
        interaction[fid1] = {}
        idx1 = frag_indices[fid1]
        lmat1 = mat1[:, idx1].todense()  # smat1
        rmat1 = mat2[idx1, :].todense()  # smat2.T

        for fid2 in unitlist2:
            idx2 = frag_indices[fid2]
            ...
            interaction[fid1][fid2] = a1*a2*einsum("ij,ji->", lmat2, rmat2)
            # remove numpy
            interaction[fid1][fid2] = interaction[fid1][fid2].item()
    return interaction


def superunits_quadratic_quantities(bo, superunits):
    """
    Compute quantities that transform like the bond orders of the superunits
...
def systems_heatmap(data, restrict_to=None, axs=None, columns=None, **kwargs):
    ...
    for i in range(mat.shape[0]-1):
        axs.axhline(i+0.49, color='k', linestyle='-', linewidth=0.5)
    for i in range(mat.shape[1]-1):
        axs.axvline(i+0.49, color='k', linestyle='-', linewidth=0.5)
