...
class DataLoader:
    ...
    def _write_levels(
        ...
        feature_description_map = (
            ...
            .explode("levels")
            .with_row_count("order")
            .rename({"levels": "level"})
            .select(["featureID", "level", pl.col("order").cast(pl.Int8)])
        )
    ...
    def _write_feature_data(
        ...
        db.handle_insertion_multi_line(
            ...
            mode=self.insertion_mode,
            returning_id=False,
            parallel_workers=1,  # anything not time-sku should use 1 worker
        )
