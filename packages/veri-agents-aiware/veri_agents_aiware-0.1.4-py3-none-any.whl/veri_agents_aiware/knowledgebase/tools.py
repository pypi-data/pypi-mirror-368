...
class AiwareKnowledgebaseListDocuments(BaseTool):
    ...
    def __init__(self, **kwargs):
        ...
        if self.name_suffix:
            self.name = self.name + self.name_suffix.replace("-", "_")
        self.description = (
            f"Lists the documents in the {self.knowledgebase.name} knowledgebase. Use this tool if you're interested in documents about {self.knowledgebase.description}"
        )
