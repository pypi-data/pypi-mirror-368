...
#   filename:  <stdin>
#   timestamp: 2025-08-01T19:36:05+00:00
...
class SearchScrollRequest(BaseModel):
    searchToken: str
...
class SearchAggregateResults(BaseModel):
    aggregate: Annotated[
        Optional[Dict[str, Any]], Field(description='Aggregate results.')
    ] = None


class TdoMetadata(BaseModel):
    absoluteStartTimeMs: Annotated[
        Optional[date], Field(examples=['2021-04-13T11:40:00.000Z'])
    ] = None
...
class TdoResult(BaseModel):
    recording: Optional[TdoMetadata] = None
    context: Optional[List[Dict[str, Any]]] = None
    hits: Optional[List[Dict[str, Any]]] = None


class TdoSliceResult(BaseModel):
    recording: Optional[TdoMetadata] = None
    context: Optional[List[Dict[str, Any]]] = None
    hits: Optional[List[Dict[str, Any]]] = None
...
class CountSearchResult(BaseModel):
    totalRecords: Optional[float] = None


class AutocompleteFieldResult(BaseModel):
    key: Annotated[
        Optional[str],
        Field(
...
class AuditLogResult(BaseModel):
    ...
    timestamp: Annotated[
        Optional[date], Field(examples=['2023-12-19T22:03:21.021Z'])
    ] = None
    userID: Annotated[
...
class VectorSearchRequestSemanticSearchVectorSimilarity(BaseModel):
    rawData: Optional[List[float]] = None


class VectorSearchRequestSemanticSearchTextSimilarity(BaseModel):
    rawData: Optional[str] = None


class VectorSearchRequestSemanticSearchImageSimilarity(BaseModel):
    rawData: Optional[str] = None


class VectorSearchRequestSemanticSearchAudioSimilarity(BaseModel):
    rawData: Optional[str] = None
...
class VectorSearchRequestSemanticSearchRescoringInput(BaseModel):
...
class VectorSearchRequestSemanticSearchOptimization(BaseModel):
    num_candidates: Optional[float] = None
    min_score: Optional[float] = None


class VectorSearchRequestSemanticSearch(BaseModel):
...
class VectorSearchResultsResultMetaData(BaseModel):
    recordingId: Optional[str] = None
    fileType: Optional[str] = None


class VectorSearchResultsResultSery(BaseModel):
...
class VectorSearchRequestFilter(BaseModel):
