...
class PySob:
    ...
    #   4. thus, a fallback to 'Type[PySub]' would be necessary
    def initialize(db_specs: tuple[type[StrEnum], int | str] |
                             tuple[type[StrEnum], int, bool],  # noqa
        ...
        if cls:
            ...
            attrs: list[StrEnum] = [attr.value for attr in db_specs[0]]
    ...
    def count(errors: list[str] = None,
        ...
        if not errors:
            ...
            tbl_name: str = sob_db_specs[cls_name][0]
            if table_alias:
                tbl_name = f"{table_alias}.{tbl_name}"
            if table_joins:
    ...
    def retrieve(errors: list[str] = None,
        ...
        if not errors:
            ...
            if table_joins:
                tbl_name += f" {table_joins}"
            sel_stmt: str = f"SELECT {', '.join(attrs)} FROM {tbl_name}"
