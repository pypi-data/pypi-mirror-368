...
def _find_bench_af_root() -> Path | None:
    ...
    for path in (Path.cwd() / path for path in PATHS_TO_SRC):
        if path.exists() and path.is_dir() and _is_bench_af_root(path):
            return path
    
    current_dir = Path.cwd()
    while not _is_bench_af_root(current_dir):
        if current_dir == Path("/"):
            return None
