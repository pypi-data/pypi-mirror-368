...
def _check_mnar_without_rule(missingness: MissingnessAnalysis, metadata: ColumnMetadata) -> Optional[ImputationException]:
    ...
    if (missingness.mechanism in [MissingnessMechanism.MNAR, MissingnessMechanism.UNKNOWN] 
...
def check_skip_column(data: pd.DataFrame, metadata: ColumnMetadata, missingness: MissingnessAnalysis, outlier_analysis: OutlierAnalysis, config: AnalysisConfig) -> Optional[ImputationException]:
    """Legacy function for skip column check."""
    if should_skip_column(metadata.column_name, config):
        return ImputationException(
            ImputationMethod.NO_ACTION_NEEDED,
            f"Column '{metadata.column_name}' is in skip list",
            1.0
        )
    return None

def check_nullable_violation(data: pd.DataFrame, metadata: ColumnMetadata, missingness: MissingnessAnalysis, outlier_analysis: OutlierAnalysis, config: AnalysisConfig) -> Optional[ImputationException]:
    """Legacy function for nullable violation check."""
    return _check_nullable_violation(missingness, metadata)

def check_metadata_validation_failure(data: pd.DataFrame, metadata: ColumnMetadata, missingness: MissingnessAnalysis, outlier_analysis: OutlierAnalysis, config: AnalysisConfig) -> Optional[ImputationException]:
    """Legacy function for metadata validation failure check."""
    return _check_metadata_validation(metadata, data)

def check_no_missing_values(data: pd.DataFrame, metadata: ColumnMetadata, missingness: MissingnessAnalysis, outlier_analysis: OutlierAnalysis, config: AnalysisConfig) -> Optional[ImputationException]:
    """Legacy function for no missing values check."""
    return _check_no_missing_values(missingness)

def check_unique_identifier(data: pd.DataFrame, metadata: ColumnMetadata, missingness: MissingnessAnalysis, outlier_analysis: OutlierAnalysis, config: AnalysisConfig) -> Optional[ImputationException]:
    """Legacy function for unique identifier check."""
    return _check_unique_identifier(metadata)

def check_all_values_missing(data: pd.DataFrame, metadata: ColumnMetadata, missingness: MissingnessAnalysis, outlier_analysis: OutlierAnalysis, config: AnalysisConfig) -> Optional[ImputationException]:
    """Legacy function for all values missing check."""
    return _check_all_missing(data, missingness)

def check_mnar_without_business_rule(data: pd.DataFrame, metadata: ColumnMetadata, missingness: MissingnessAnalysis, outlier_analysis: OutlierAnalysis, config: AnalysisConfig) -> Optional[ImputationException]:
    """Legacy function for MNAR without business rule check."""
    return _check_mnar_without_rule(missingness, metadata)

def check_allowed_values_violation(data: pd.Series, metadata: ColumnMetadata) -> Optional[ImputationException]:
    """Legacy function for allowed values violation check."""
    return _check_allowed_values(data, metadata)

def check_max_length_violation(data: pd.Series, metadata: ColumnMetadata) -> Optional[ImputationException]:
    """Legacy function for max length violation check."""
    return _check_max_length(data, metadata)