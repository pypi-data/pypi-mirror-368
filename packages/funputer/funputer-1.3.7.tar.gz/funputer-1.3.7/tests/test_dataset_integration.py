...
from funputer.metadata_fields import ALL_METADATA_FIELDS, INFERRABLE_FIELDS
...
class TestDatasetIntegration(GenericDatasetTestBase):
    ...
    def test_dataset_preflight_validation(self):
        ...
        assert report["file"]["compression"] == "none"
        assert report["structure"]["num_columns"] == self.dataset_info['column_count']
    ...
    def test_dataset_metadata_generation_cli(self):
        ...
        assert ("Enhanced metadata template created" in result.output or 
                "Auto-inferred metadata template created" in result.output)
        # Check for general success indicators rather than exact format
        assert ("columns with" in result.output or "Analyzed" in result.output)
        assert ("metadata fields" in result.output or "auto-inferred" in result.output)
        ...
        with open(output_path, 'r') as f:
            ...
            assert len(headers) in [expected_fields, inferrable_fields], f"Should have {expected_fields} or {inferrable_fields} columns, got {len(headers)}"
    ...
    def test_dataset_metadata_field_population(self):
        ...
        critical_checks = {
            ...
            'do_not_impute': lambda x: x.isin(['TRUE', 'FALSE']).all(),
            'policy_version': lambda x: x.eq('v1.0').all(),
        }
    ...
    def test_dataset_backward_compatibility(self):
        ...
        legacy_data = {
            ...
            'data_type': ['string', 'float', 'categorical'],
        ...
        assert result.exit_code == 0
        assert "Loading legacy metadata format" in result.output
    ...
    def test_dataset_error_handling(self):
        ...
        assert "Missing required columns" in result.output or "Failed to load metadata" in result.output
