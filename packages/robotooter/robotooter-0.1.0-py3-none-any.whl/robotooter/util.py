...
def import_list(names: list[str], expected_type: Type) -> dict[str, ModuleType]:
    klasses = {}
    for name in names:
        ...
        if not issubclass(type(klass), expected_type):
            raise TypeError(f"{filter} is not a {expected_type.__name__}")
