...
class FileFilter:
    ...
    def process_file(self, input_path_or_file: FileOrPath, output_path_or_file: FileOrPath) -> None:
        input_file, input_opened  = self._open(input_path_or_file)
        output_file, output_opened = self._open(output_path_or_file, 'w')
        [f.reset() for f in self.filters]

        try:
            for line in self._filter_lines(input_file):
    ...
    def _filter_lines(self, lines: Iterable[str]) -> Iterable[str]:
        for line in lines:
    ...
    def _open(self, file_or_path: FileOrPath, mode:str='r') -> tuple[TextIO, bool]:
