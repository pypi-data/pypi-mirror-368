...
public:
    ...
    virtual SearchResult search(const Index &index, const Query query,
                                std::optional<ComputeBuffer> buffer = std::nullopt) = 0;

    [[deprecated("Have to be removed")]] virtual SearchResult
    search(const Index &index, const Query query, bool ntt_out, std::optional<ComputeBuffer> buffer = std::nullopt) = 0;
};
...
public:
    ...
    [[deprecated("Have to be deleted on next update")]] SearchResult
    search(const Index &db, const Query query, bool ntt_out, std::optional<ComputeBuffer> buf = std::nullopt) override {
        throw NotSupportedError("Not supported IP search for QF on CPU");
    }

    SearchResult search(const Index &db, const Query query,
...
public:
    ...
    [[deprecated("Have to be deleted on next update")]] SearchResult
    search(const Index &db, const Query query, bool ntt_out, std::optional<ComputeBuffer> buff = std::nullopt) override;

    SearchResult search(const Index &db, const Query query, std::optional<ComputeBuffer> buff = std::nullopt) override;
