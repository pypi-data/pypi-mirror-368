...
def check_key_dir(key_path: str, key_id: str) -> bool:
    ...
    required_files = ["EvalKey.bin", "EncKey.bin", "SecKey.bin"]
...
def topk(vector: List[List[float]], k: int):
    ...
    for vec_idx, vec in enumerate(vector):
        for inner_idx, value in enumerate(vec):
            data_list.append(((vec_idx, inner_idx), value))

    topk_result = sorted(data_list, key=lambda x: x[1], reverse=True)[:k]

    return [Position(shard_idx=pos[0], row_idx=pos[1]) for pos, _ in topk_result]
