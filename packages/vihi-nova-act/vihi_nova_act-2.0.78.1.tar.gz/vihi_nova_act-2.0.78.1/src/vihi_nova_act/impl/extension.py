...
from vihi_nova_act.__version__ import VERSION as SDK_VERSION
from vihi_nova_act.impl.backend import BackendInfo
from vihi_nova_act.impl.common import get_extension_version
from vihi_nova_act.impl.custom_actuation.interface.browser import BrowserActuatorBase, BrowserObservation, JSONSerializable
...
from vihi_nova_act.impl.protocol import parse_errors
from vihi_nova_act.impl.window_messages import (
    CANCEL_PROMPT_TYPE,
    DISPATCH_PROMPT_TYPE,
    POST_MESSAGE_EXPRESSION,
    WAIT_FOR_PAGE_TO_SETTLE_PROMPT_TYPE,
)
from vihi_nova_act.types.act_errors import ActCanceledError, ActClientError, ActDispatchError, ActError
from vihi_nova_act.types.act_result import ActResult
from vihi_nova_act.types.errors import IAMAuthError
from vihi_nova_act.types.state.act import Act, ActCanceled, ActFailed, ActSucceeded
from vihi_nova_act.types.state.page import PageState
from vihi_nova_act.util.logging import (
    LoadScroller,
...
from vihi_nova_act.util.step_server_time_tracker import StepServerTimeTracker
            ...
            while time.time() < end_time:
                ...
                if len(act.steps) > num_steps_observed:
                    for step in act.steps[num_steps_observed:]:
                        ...
                        # adding stream support for model_response and tool_call
                        thinking_step = model_response.split(newline)[0]
                        tool_call = model_response.split(newline)[1]
                        if self._stream:
                            yield f"model_response: {thinking_step}"
                            yield f"tool_call: {tool_call}"
