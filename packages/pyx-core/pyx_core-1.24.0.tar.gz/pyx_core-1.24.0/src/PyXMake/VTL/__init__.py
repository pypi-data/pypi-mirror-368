...
def GetBuildCommand(make_opt=0, _format="fixed", _arch=Utility.GetArchitecture(), **kwargs):
    ...
    if parse(version) >= parse("2.0.0") or sys.version_info >= (3,12) or kwargs.get("backend",""): 
        command = " --backend=%s " % kwargs.get("backend","distutils" if sys.version_info < (3,12) else 'meson')
