...
class TestIntegratedGradients(unittest.TestCase):
    ...
    def test_integrated_gradients_output(self):
        integrated_gradients = IntegratedGradients(self.model)
        integrated_grad = integrated_gradients.generate(self.image, class_idx=0)
        self.assertEqual(integrated_grad.shape, (3, self.image.shape[2], self.image.shape[3]))
