...
impl<N, E: KlvmEncoder<Node = N>, const BYTE: u8> ToKlvm<E> for MatchByte<BYTE> {
...
impl<N, D: KlvmDecoder<Node = N>, const BYTE: u8> FromKlvm<D> for MatchByte<BYTE> {
...
mod tests {
    ...
    fn test_byte<const BYTE: u8>() {
        ...
        let encoded = match_byte.to_klvm(&mut allocator).unwrap();
        MatchByte::<BYTE>::from_klvm(&allocator, encoded).unwrap();
    }
