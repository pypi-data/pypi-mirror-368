...
# Import FX-based tracing functions
from .fx_trace import (
    capture_model_instance_fx,
    capture_model_class_fx,
    capture_model_instance_legacy_compat,
    capture_model_class_legacy_compat,
)

# Migration flag - set to True to use FX tracing, False for legacy profiler-only
USE_FX_TRACING = os.environ.get("KANDC_USE_FX_TRACING", "true").lower() == "true"
...
def capture_model_instance(
    ...
    **profiler_kwargs: Any,
    ...
    Now supports both FX-based computation graph tracing (default) and
    legacy profiler-only mode (via KANDC_USE_FX_TRACING=false).
    ...
    """
    if USE_FX_TRACING:
        # Use new FX-based tracing system
        return capture_model_instance_fx(
            model_instance=model_instance,
            model_name=model_name,
            record_shapes=record_shapes,
            profile_memory=profile_memory,
            trace_per_input_shape=True,  # Enable per-input-shape tracing
            **profiler_kwargs,
        )
    else:
        # Use legacy profiler-only system
        return _capture_model_instance_legacy(
            model_instance=model_instance,
            model_name=model_name,
            record_shapes=record_shapes,
            profile_memory=profile_memory,
...
def _capture_model_instance_legacy(
    model_instance,
    model_name: Optional[str] = None,
    record_shapes: bool = True,
    profile_memory: bool = True,
    **profiler_kwargs: Any,
    ...
    """Legacy implementation of capture_model_instance (original code)."""
    # Check if we're running on the Keys & Caches backend
    if os.environ.get(KANDC_BACKEND_RUN_ENV_KEY) != "1":
...
def capture_model_class(
    ...
    **profiler_kwargs: Any,
    ...
    if USE_FX_TRACING:
        ...
        # Use legacy profiler-only system
        return _capture_model_class_legacy(
            model_name=model_name,
            record_shapes=record_shapes,
            profile_memory=profile_memory,
            **profiler_kwargs,
...
def _capture_model_class_legacy(
    model_name: Optional[str] = None,
    record_shapes: bool = True,
    profile_memory: bool = True,
    **profiler_kwargs: Any,
):
    """Legacy implementation of capture_model_class (original code)."""
