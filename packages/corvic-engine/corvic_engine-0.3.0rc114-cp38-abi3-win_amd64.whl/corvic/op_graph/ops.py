...
def in_memory_input(
    input_name: str, *, schema: pa.Schema | None = None
) -> InMemoryInput:
    schema_bytes = schema.serialize().to_pybytes() if schema else None
    return from_proto(
        table_pb2.InMemoryInputOp(input_name=input_name, arrow_schema=schema_bytes),
        skip_validate=True,
    )
    ...
    def from_ops(  # noqa: C901
        ...
        match ops:
            case InMemoryInput():
                if ops.arrow_schema:
                    return Schema(
                        [Field.from_arrow(afield) for afield in ops.arrow_schema]
                    )
