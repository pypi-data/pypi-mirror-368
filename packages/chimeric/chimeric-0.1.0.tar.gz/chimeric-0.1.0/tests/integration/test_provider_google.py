...
@pytest.mark.google
def test_google_model_listing(api_keys):
    """Test Google model listing functionality."""
    if "google_api_key" not in api_keys:
        pytest.skip("Google API key not found")

    cassette_path = get_cassette_path("google", "test_model_listing")
    ...
    with get_vcr().use_cassette(cassette_path):
        ...
        assert any("gemini" in model_id for model_id in model_ids)

        print(f"Found {len(models)} Google models")


@pytest.mark.google
def test_google_sync_generation(api_keys):
    """Test Google synchronous generation functionality."""
    if "google_api_key" not in api_keys:
        pytest.skip("Google API key not found")

    chimeric = Chimeric(google_api_key=api_keys["google_api_key"])
    cassette_path = get_cassette_path("google", "test_sync_generation")
...
@pytest.mark.google
@pytest.mark.asyncio
async def test_google_async_generation(api_keys):
    """Test Google asynchronous generation functionality."""
    if "google_api_key" not in api_keys:
        pytest.skip("Google API key not found")

    chimeric = Chimeric(google_api_key=api_keys["google_api_key"])
    cassette_path = get_cassette_path("google", "test_async_generation")
...
@pytest.mark.google
def test_google_sync_tools_non_streaming(api_keys):
    """Test Google sync generation with tools (non-streaming)."""
    if "google_api_key" not in api_keys:
        pytest.skip("Google API key not found")

    chimeric = Chimeric(google_api_key=api_keys["google_api_key"])
    cassette_path = get_cassette_path("google", "test_sync_tools_non_streaming")
...
@pytest.mark.google
def test_google_sync_tools_streaming(api_keys):
    """Test Google sync generation with tools (streaming)."""
    if "google_api_key" not in api_keys:
        pytest.skip("Google API key not found")

    chimeric = Chimeric(google_api_key=api_keys["google_api_key"])
    cassette_path = get_cassette_path("google", "test_sync_tools_streaming")
...
@pytest.mark.google
@pytest.mark.asyncio
async def test_google_async_tools_streaming(api_keys):
    """Test Google async generation with tools (streaming)."""
    if "google_api_key" not in api_keys:
        pytest.skip("Google API key not found")

    chimeric = Chimeric(google_api_key=api_keys["google_api_key"])
    cassette_path = get_cassette_path("google", "test_async_tools_streaming")
...
@pytest.mark.google
@pytest.mark.asyncio
async def test_google_async_tools_non_streaming(api_keys):
    """Test Google async generation with tools (non-streaming)."""
    if "google_api_key" not in api_keys:
        pytest.skip("Google API key not found")

    chimeric = Chimeric(google_api_key=api_keys["google_api_key"])
    cassette_path = get_cassette_path("google", "test_async_tools_non_streaming")
...
@pytest.mark.google
def test_google_init_kwargs_propagation(api_keys):
    """Test Google kwargs propagation through the stack with fake cross-provider params."""
    if "google_api_key" not in api_keys:
        pytest.skip("Google API key not found")
...
@pytest.mark.google
def test_google_invalid_generate_kwargs_raises_provider_error(api_keys):
    ...
    with get_vcr().use_cassette(cassette_path):
        # Test with an invalid parameter that doesn't exist in Google API
        with pytest.raises(ProviderError) as exc_info:
            chimeric.generate(
                model="gemini-1.5-flash",
                messages=[{"role": "user", "content": "Hello"}],
                invalid_google_parameter="this_should_fail",
                stream=False,
            )

        # Verify the error contains provider information
        assert "Google" in str(exc_info.value) or "google" in str(exc_info.value).lower()
