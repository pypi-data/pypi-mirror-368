...
def alternative_extrapolation(
    ...
    """
    Calculation of new UFR extrapolation for EIOPA interest rate curve based on current information

    Args:
        zero_rates (np.array): initial zero rates to be used
        FSP (int): First Smoothing Point
        UFR (float): Ultimate Forward Rate
        LLFR (float): Last Liquid Forward Rate
        alfa (float): Mean Reversion parameter
        compounding (bool): apply compounding or not

    """
    # initialize zero, forward and discount curves
    zero = np.array([0.0] * len(zero_rates))
    forward = np.array([0.0] * len(zero_rates))
    discount = np.array([0.0] * len(zero_rates))

    # initial zero, forward and discount
    zero[0] = zero_rates[0]
    forward[0] = zero_rates[0]
    discount[0] = np.exp(-zero_rates[0])

    for i in range(1, FSP):
        zero[i] = zero_rates[i]
        forward[i] = (i + 1) * zero[i] - i * zero[i - 1]
        discount[i] = discount[i - 1] * np.exp(-forward[i])

    for i in range(FSP, len(zero_rates)):
        fwtemp = np.log(1 + UFR) + (LLFR - np.log(1 + UFR)) * (
            1 - np.exp(-alfa * (i + 1 - FSP))
        ) / alfa / (i + 1 - FSP)
        zero[i] = (FSP * zero[FSP - 1] + (i + 1 - FSP) * fwtemp) / (i + 1)
        discount[i] = np.exp(-(i + 1) * zero[i])
        forward[i] = np.log(discount[i - 1] / discount[i])

    if compounding:
        for i in range(len(zero_rates)):
            forward[i] = np.exp(forward[i]) - 1
            zero[i] = np.exp(zero[i]) - 1
