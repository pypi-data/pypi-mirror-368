    ...
    while True:
        ...
        if selected_index == 0:  # Select Provider
            ...
            if sel_provider_idx == 0: # Hackclub AI selected
                ...
                # Force tool strategy and set a dummy API key
                mode_cfg["tool_strategy"] = "xml"
                providers[new_provider_key]["api_key"] = "not_needed"
...
def _prompt_for_misc_settings(config_to_edit: dict):
    ...
    # Work on a copy of the settings. Only save back if requested.
    settings = config_to_edit.get("misc_settings", {}).copy()
    ...
    while True:
        ...
        config_view_content = (
            ...
            f"[bold cyan]Model Temperature:[/bold cyan] {temperature:.1f}"
        )
        ...
        # Menu indices: 0=Toggle, 1=Edit, 3=Save, 4=Discard

        if selected_index == 0:
            settings["show_reasoning"] = not settings.get("show_reasoning", False)
            continue
        
        elif selected_index == 1:
            new_temp_str = console.input(f"Enter new temperature (0.0-2.0) [current: {temperature:.1f}, press Enter to keep]: ").strip()
            if not new_temp_str:
                continue
            try:
                ...
                console.print("\n[bold red]Invalid input. Please enter a number between 0.0 and 2.0.[/bold red]")
                console.input("Press Enter to continue...")
            continue

        elif selected_index == 3:  # Save and Back
            config_to_edit["misc_settings"] = settings
            if not config_to_edit["misc_settings"]:
                 config_to_edit.pop("misc_settings", None)
            break
            
        elif selected_index is None or selected_index == 4: # Discard and Back
            # Do nothing, changes were on a copy.
            break
