...
def run_dashboard(env, solver_function=None):
    ...
    if st.button("Run Simulation", type="primary", key="run_sim"):
        ...
        try:
            ...
            nodes_df = pd.read_csv('robin_logistics/data/nodes.csv')
            edges_df = pd.read_csv('robin_logistics/data/edges.csv')
                        ...
                        if env.warehouses and env.orders:
                            ...
                            if solution and 'routes' in solution:
                                ...
                                for i, route in enumerate(solution['routes']):
                                    ...
                                    with st.expander(f"🚚 {vehicle_id} - Route {i+1} ({distance:.2f} km)", expanded=True):
