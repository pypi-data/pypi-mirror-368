...
def _validate_onnx_model_dir(onnx_dir: str) -> None:
    ...
    if len(bin_files) > 1:
        raise UserError(
            f"ONNX model directory '{onnx_dir}' can contain at most one .bin file."
        )
    has_bin = len(bin_files) == 1
    weight_files = [f for f in files if f.endswith(".data")]
    if len(weight_files) > 1:
        raise UserError(
            f"ONNX model directory '{onnx_dir}' can contain at most one .data file."
        )
    has_weight = len(weight_files) == 1
    if not has_bin and not has_weight:
        raise UserError(
            f"ONNX model directory '{onnx_dir}' must contain a .bin file and/or a .data file."
        )
    if len(files) != 1 + has_bin + has_weight:
        raise UserError(
            f"ONNX model directory '{onnx_dir}' can only contain .onnx, .data, and .bin files."
        )
        ...
        if filename is not None:
            ...
            if os.path.splitext(path)[1] == ".zip":
                with tempfile.TemporaryDirectory() as tmp_dir:
                    ...
                    onnx_file = os.path.join(model_dir, onnx_files[0])
                    self._model = onnx.load(onnx_file)
            else:
