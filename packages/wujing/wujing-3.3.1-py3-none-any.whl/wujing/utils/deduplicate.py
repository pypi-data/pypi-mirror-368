...
def __deduplicate_single(dataset: Dataset, key_path: str) -> Dataset:
    """
    对单个数据集进行去重
    ...
    return dataset.filter(_filter, desc="去重中")
...
def deduplicate(data: Dataset | DatasetDict, key_path: str) -> Dataset | DatasetDict:
    ...
    if isinstance(data, DatasetDict):
        ...
        for split_name, split_data in data.items():
            result[split_name] = __deduplicate_single(split_data, key_path)
