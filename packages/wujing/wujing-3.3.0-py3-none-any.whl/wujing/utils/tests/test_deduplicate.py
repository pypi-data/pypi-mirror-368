...
from wujing.utils.deduplicate import deduplicate
...
class TestDeduplicate:
    ...
    def test_deduplicate_invalid_key_path(self, capsys):
        """测试无效的key_path"""
        data = [
            {"id": 1, "text": "hello"},
            {"id": 2, "text": "world"},
        ...
        # 使用不存在的key_path
        result = deduplicate(dataset, "nonexistent")
    ...
    def test_deduplicate_invalid_nested_path(self, capsys):
        """测试无效的嵌套路径"""
        data = [
        ...
        # 使用不存在的嵌套路径
        result = deduplicate(dataset, "messages.[0].nonexistent")
