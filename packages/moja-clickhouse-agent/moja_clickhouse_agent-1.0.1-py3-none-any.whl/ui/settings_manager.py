...
class SettingsManager:
    ...
    def edit_ai_provider(self):
        ...
        console.print()
        if Confirm.ask("[bold bright_magenta]Change AI provider?[/bold bright_magenta]"):
            onboarding = OnboardingFlow()
            provider = onboarding.choose_ai_provider()
            
            # Remove old provider settings
            old_keys = ['openrouter_api_key', 'openrouter_model', 'openrouter_provider_only', 'openrouter_data_collection',
                       'ollama_base_url', 'ollama_model', 'lmstudio_base_url', 'openai_api_key', 'openai_model']
            for key in old_keys:
                config.pop(key, None)
            
            # Add new provider settings
            if provider == "openrouter":
                new_config = onboarding.setup_openrouter()
            elif provider == "ollama":
                new_config = onboarding.setup_ollama()
            elif provider == "lmstudio":
                new_config = onboarding.setup_lmstudio()
            elif provider == "openai":
                new_config = onboarding.setup_openai()
            
            config.update(new_config)
            self.save_config(config)
            ui.show_success("AI provider settings updated!")
    ...
    def run_settings_menu(self):
        ...
        while True:
            ...
            menu_table.add_row("1", "🤖 Change AI Provider")
