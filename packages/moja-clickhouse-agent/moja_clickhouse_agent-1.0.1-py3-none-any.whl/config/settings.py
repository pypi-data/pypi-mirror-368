...
class ClickHouseConfig(BaseModel):
    ...
    local_llm_base_url: str = Field(default="http://127.0.0.1:1234/v1", description="Local LLM OpenAI-compatible base URL")
    local_llm_model: str = Field(default="qwen3-1.7b", description="Local model alias/name to send in requests")
...
def load_config(
    ...
    # Only request OpenRouter API key if provider is explicitly set to openrouter
    if config.provider == "openrouter" and not config.openrouter_api_key:
        ...
        if Confirm.ask("Would you like to enter your OpenRouter API key now?"):
            ...
            console.print("[red]❌ OpenRouter API key is required for OpenRouter provider[/red]")
            raise typer.Exit(1)
