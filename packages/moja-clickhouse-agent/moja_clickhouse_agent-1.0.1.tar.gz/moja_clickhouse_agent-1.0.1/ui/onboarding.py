...
class OnboardingFlow:
    ...
    def show_welcome(self):
        ...
        subtitle = Text("Let's get you set up with your AI-powered ClickHouse assistant", style="italic bright_white")
    ...
    def choose_ai_provider(self) -> str:
        """Let user choose their AI provider"""
        provider_table = Table(show_header=False, box=ROUNDED, padding=(1, 2))
        provider_table.add_column("Option", style="bold bright_cyan")
        provider_table.add_column("Provider", style="bright_white")
        provider_table.add_column("Description", style="dim")
        
        provider_table.add_row("1", "🖥️  Local (llama.cpp)", "Uses Qwen3 1.7B via a local OpenAI-compatible server")
        provider_table.add_row("2", "🌐 OpenRouter", "Access to multiple models (GPT-4, Claude, etc.) - Requires API key")
        
        provider_panel = Panel(
            provider_table,
            title="[bold bright_magenta]Choose Your AI Provider[/bold bright_magenta]",
        ...
        while True:
            choice = Prompt.ask(
                "[bold bright_cyan]Select your preferred AI provider[/bold bright_cyan]",
                choices=["1", "2"],
                default="1"
            )
    ...
    def setup_local(self) -> Dict[str, Any]:
        """Setup local llama.cpp-compatible server configuration"""
        console.print()
        info_panel = Panel(
            Text.assemble(
                ("🖥️  Local Setup (llama.cpp)", "bold bright_green"), "\n\n",
                ("Runs Qwen3 1.7B locally with an OpenAI-compatible server.", "bright_white"), "\n",
                ("No API keys needed.", "green"), "\n\n",
                ("If you don't already run a server, we'll show a one-liner after setup.", "dim")
            ),
            border_style="bright_green",
            padding=(1, 2)
        )
        ...
        base_url = Prompt.ask("[bold bright_green]Local LLM base URL[/bold bright_green]", default="http://127.0.0.1:1234")
        model = Prompt.ask("[bold bright_green]Model alias/name[/bold bright_green]", default="qwen3-1.7b")
        
        return {
            "provider": "local",
            "local_llm_base_url": f"{base_url}/v1",
    ...
    def run_onboarding(self) -> Dict[str, Any]:
        ...
        # Choose AI provider
        provider = self.choose_ai_provider()
        
        # Setup provider-specific config
        if provider == "openrouter":
            ai_config = self.setup_openrouter()
        elif provider == "local":
