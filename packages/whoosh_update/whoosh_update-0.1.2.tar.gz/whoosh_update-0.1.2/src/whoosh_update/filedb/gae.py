...
To create a new index::
    ...
    from whoosh_update.filedb.gae import DatastoreStorage
...
from whoosh_update.compat import BytesIO
from whoosh_update.index import TOC, FileIndex, _DEF_INDEX_NAME
from whoosh_update.filedb.filestore import ReadOnlyError, Storage
...
class MemcacheLock(object):
    ...
    def acquire(self, blocking=False):
        val = memcache.add(self.name, "L", 360, namespace="whoosh_updatelocks")
        ...
        if blocking and not val:
            ...
            while not val:
                time.sleep(0.1)
                val = memcache.add(self.name, "", 360, namespace="whoosh_updatelocks")

        return val

    def release(self):
        memcache.delete(self.name, namespace="whoosh_updatelocks")


class DatastoreStorage(Storage):
    """An implementation of :class:`whoosh_update.store.Storage` that stores files in
