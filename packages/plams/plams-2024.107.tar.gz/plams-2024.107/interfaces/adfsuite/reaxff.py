...
class ReaxFFJob(SingleJob):
    ...
    def _get_ready(self):
        ...
        self._write_geofile(
            ...
            filename=opj(self.path, "geo"),
            settings=self.settings.input.geo,
        ...
        *ffield* should be a string with a path to some external file or with a filename present in ``$AMSHOME/atomicdata/ForceFields/ReaxFF``. The location of this search folder is defined by ``ffield_path`` class attribute).
        ...
        if os.path.isfile(ffield):
            ...
            if os.path.isfile(path):
                ...
                raise FileError(
                    "settings.input.ffield={} is neither a path to a file nor an existing force field from {}".format(
                        ffield, self.ffield_path
                    )
                )
        ...
        if isinstance(settings, str) and os.path.isfile(settings):
            ...
            if lattice is True:
                if molecule.align_lattice(convention="reax"):
                    log(
                        "The lattice supplied for job {} did not follow the convention required by ReaxFF. I rotated the whole system for you. You're welcome".format(
                            self._full_name()
                        ),
                        3,
                    )
                ...
                header.append(
                    "CRYSTX  {:10.5f} {:10.5f} {:10.5f} {:10.5f} {:10.5f} {:10.5f}\n".format(
                        *self._convert_lattice(molecule.lattice)
                    )
                )
            ...
            for i, at in enumerate(molecule):
                newline = (
                    "HETATM {:>5d} {:<2}               {: >10.5f}{: >10.5f}{: >10.5f} {:<2}     1 1  0.0\n".format(
                        i + 1, at.symbol, *at.coords, at.symbol
                    )
                )
...
def load_reaxff_control(filename, keep_order=True):
    ...
    for line in lines:
        ...
        if len(tmp) > 1:
            ...
            if len(key) > 6:
                log(
                    "load_reaxff_control: I am confused by this line:\n{}\nAre you sure {} is correct?".format(
                        line, key
                    ),
                    3,
                )
...
def reaxff_control_to_settings(fpath: str) -> Settings:
    ...
    if s.input.ams.task == "moleculardynamics":
        if "nmdit" in d:
            s.input.ams.moleculardynamics.nsteps = d["nmdit"]
        if "tstep" in d:
            s.input.ams.moleculardynamics.timestep = d["tstep"]
        if "mdtemp" in d:
            s.input.ams.moleculardynamics.initialvelocities.temperature = d["mdtemp"]
        if "iout2" in d:
            s.input.ams.moleculardynamics.samplingfreq = d["iout2"]
        if "imdmet" in d:
