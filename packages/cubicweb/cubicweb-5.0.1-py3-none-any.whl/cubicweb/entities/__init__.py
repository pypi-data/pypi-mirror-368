...
class AnyEntity(Entity):
    ...
    def cw_create_url(cls, req, **kwargs):
        ...
        return req.build_url(f"add/{cls.__regid__}", **kwargs)
    ...
    def cw_fti_index_rql_limit(cls, req, limit=1000):
        ...
        """
        restrictions = [f"X is {cls.__regid__}"]
        selected = ["X"]
        start = 0
        for attrschema in sorted(cls.e_schema.indexable_attributes()):
            varname = attrschema.type.upper()
            restrictions.append(f"X {attrschema} {varname}")
            selected.append(varname)
        while True:
            q_restrictions = restrictions + [f"X eid > {start}"]
            rset = req.execute(
                "Any %s ORDERBY X LIMIT %s WHERE %s"
                % (", ".join(selected), limit, ", ".join(q_restrictions))
            )
            if rset:
                start = rset[-1][0]
                yield rset
