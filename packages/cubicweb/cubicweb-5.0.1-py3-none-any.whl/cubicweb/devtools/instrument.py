...
class PropagationAnalyzer:
    ...
    def should_include(self, eschema):
        ...
        if self.prop_rel in eschema.subject_relations or self.is_root(eschema):
            return True
        return False
    ...
    def prop_edges(self, s_rels, o_rels, eschemas):
        ...
        for rtype in s_rels:
            found = False
            for subj, obj in schema.relation_schema_for(rtype).relation_definitions:
    ...
    def detect_problems(self, eschemas, edges):
        ...
        if not_problematic:
            info(
                f"nothing problematic in: {', '.join(e.type for e in not_problematic)}"
            )
        return problematic
    ...
    def has_problem(self, eschema, edges):
        ...
        has_prop_rel = self.prop_rel in eschema.subject_relations
        ...
        if not has_prop_rel and any(edge for edge in edges if edge[1] == eschema):
            ...
            rdef = eschema.relation_definition(self.prop_rel, take_first=True)
