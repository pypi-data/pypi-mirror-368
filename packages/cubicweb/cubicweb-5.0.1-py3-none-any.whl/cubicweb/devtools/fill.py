...
class _ValueGenerator:
    ...
    def __generate_value(self, entity, attrname, index, **kwargs):
        ...
        gen_func = getattr(
            self,
            f"generate_{eschema}_{attrname}",
            getattr(self, f"generate_Any_{attrname}", None),
        )
        if gen_func is not None:
            return gen_func(entity, index, **kwargs)
        # If no specific values domain, then generate a dummy value
        gen_func = getattr(self, f"generate_{attrtype}")
    ...
    def generate_string(self, entity, attrname, index, format=None):
        ...
        if num_len >= attrlength:
            ascii = getattr(
                self.eschema.relation_definition(attrname), "internationalizable", False
            )
...
""" % (
    ...
    def generate_bytes(self, entity, attrname, index, format=None):
        fakefile = Binary(f"{attrname}{index}".encode("ascii"))
        fakefile.filename = f"file_{attrname}"
        return fakefile
...
def insert_entity_queries(
    ...
    for index in range(entity_num):
        ...
        for attrname, value in make_entity(
            ...
            restrictions.append(f"X {attrname} %({attrname})s")
            args[attrname] = value
        if restrictions:
            queries.append((f"INSERT {etype} X: {', '.join(restrictions)}", args))
            assert "eid" not in args, args
        else:
            queries.append((f"INSERT {etype} X", {}))
    return queries
...
def select(constraints, cnx, selectvar="O", objtype=None):
    ...
    """
    try:
        rql = f"Any {selectvar} WHERE {constraints}"
        if objtype:
            rql += f", {selectvar} is {objtype}"
        rset = cnx.execute(rql)
    except Exception:
        print(f"could restrict eid_list with given constraints ({constraints!r})")
        return []
...
def check_card_satisfied(card, remaining, subj, rschema, obj):
    if card in "1+" and remaining:
        raise Exception(
            f"can't satisfy cardinality {card} for relation {subj} {rschema} {obj}"
        )
