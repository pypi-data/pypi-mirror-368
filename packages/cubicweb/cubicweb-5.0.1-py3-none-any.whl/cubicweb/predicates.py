...
class EClassPredicate(Predicate):
    ...
    * elif `rset` is specified return score for each entity class found in the
    ...
    def __init__(self, accept_none=True, mode="all"):
        assert mode in ("any", "all"), f"bad mode {mode}"
...
class rql_condition(EntityPredicate):
    ...
    def __str__(self):
        return f"{self.__class__.__name__}({self.rql!r})"
...
class is_in_state(score_entity):
    ...
    * you must use the latest tr info thru the workflow adapter for repository
    ...
    def _validate(self, adapted):
        ...
        if unknown:
            raise ValueError(f"{wf.name}: unknown state(s): {','.join(unknown)}")

    def __str__(self):
        return f"{self.__class__.__name__}({','.join(str(s) for s in self.expected)})"
...
def match_context_prop(cls, req, context=None, **kwargs):
    ...
    * the appobject's context property value is None
    ...
    if context is None:
        return 1
    propval = req.property_value(f"{cls.__registry__}.{cls.__regid__}.context")
...
class match_edited_type(ExpectedValuePredicate):
    ...
    def _values_set(self, cls, req, **kwargs):
        try:
            return frozenset((req.form[f"__type:{req.form['__maineid']}"],))
        except KeyError:
            return frozenset()
