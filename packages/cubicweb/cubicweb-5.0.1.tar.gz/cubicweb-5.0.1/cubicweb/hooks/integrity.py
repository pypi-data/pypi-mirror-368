    ...
    def __call__(self):
        ...
        for rschema, targetschemas, role in eschema.relation_definitions():
            ...
            if rschema.type in DONT_CHECK_RTYPES_ON_ADD:
                continue
            rdef = rschema.role_relation_definition(eschema, targetschemas[0], role)
...
class CheckAttributeConstraintHook(IntegrityHook):
    ...
    def __call__(self):
        ...
        for attr in self.entity.cw_edited:
            if eschema.subject_relations[attr].final:
                constraints = [
                    c
                    for c in eschema.relation_definition(attr).constraints
...
class DeleteCompositeOrphanHook(hook.Hook):
    ...
    def __call__(self):
        ...
        for rdef, role in self.entity.e_schema.composite_rdef_roles:
            rtype = rdef.relation_type.type
            target = getattr(rdef, neg_role(role))
            expr = f"C {rtype} X" if role == "subject" else (f"X {rtype} C")
