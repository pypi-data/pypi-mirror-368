...
def add_inline_relation_column(cnx, etype, rtype):
    ...
    try:
        cnx.system_sql(
            str(f"ALTER TABLE {table} ADD {column} integer REFERENCES entities (eid)"),
...
class DropTable(hook.Operation):
    ...
    def precommit_event(self):
        ...
        self.cnx.system_sql(f"DROP TABLE {self.table}")
...
class DropRelationTable(DropTable):
    def __init__(self, cnx, rtype):
        super(DropRelationTable, self).__init__(cnx, table=f"{rtype}_relation")
...
class CWETypeAddOp(MemSchemaOperation):
    ...
    * add <meta rtype> relation by creating the necessary CWRelation entity
    ...
    def revertprecommit_event(self):
        ...
        self.cnx.system_sql(f"DROP TABLE {SQL_PREFIX}{self.entity.name}")
...
class CWETypeRenameOp(MemSchemaOperation):
    ...
    def rename(self, oldname, newname):
        ...
        for attrs in eschema._unique_together or ():
            columns = [f"{SQL_PREFIX}{attr}" for attr in attrs]
        ...
        if not inlined:
            ...
            for etype in rschema.subjects():
                ...
                if args:
                    column = SQL_PREFIX + rtype
                    cursor.executemany(
                        f"UPDATE {table} SET {column}=%(val)s WHERE {eidcolumn}=%(x)s",
                        args,
                    )
                # drop existant table
...
class CWAttributeAddOp(MemSchemaOperation):
    ...
    * register an operation to add the relation definition to the
    ...
    def precommit_event(self):
        ...
        try:
            cnx.system_sql(
                str(f"ALTER TABLE {table} ADD {column} {attrtype}"),
                rollback_on_failure=False,
            )
        ...
        if default is not None:
            ...
            cnx.system_sql(
                f"UPDATE {table} SET {column}=%(default)s", {"default": default}
            )
...
class CWUniqueTogetherConstraintAddOp(MemSchemaOperation):
    ...
    def precommit_event(self):
        ...
        prefix = SQL_PREFIX
        entity = self.entity
        table = f"{prefix}{entity.constraint_of[0].name}"
        cols = [f"{prefix}{r.name}" for r in entity.relations]
...
class CWUniqueTogetherConstraintDelOp(MemSchemaOperation):
    ...
    def precommit_event(self):
        cnx = self.cnx
        prefix = SQL_PREFIX
        table = f"{prefix}{self.entity.type}"
        dbhelper = cnx.repo.system_source.dbhelper
        cols = [f"{prefix}{c}" for c in self.cols]
...
class UpdateFTIndexOp(hook.DataOperationMixIn, hook.SingleLastOperation):
    ...
    def postcommit_event(self):
        ...
        for etype in to_reindex:
            rset = cnx.execute(f"Any X WHERE X is {etype}")
            self.info(
