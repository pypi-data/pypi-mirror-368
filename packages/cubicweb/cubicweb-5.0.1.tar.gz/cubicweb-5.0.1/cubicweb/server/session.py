...
def _open_only(func):
    ...
    def check_open(cnx, *args, **kwargs):
        if not cnx._open:
            raise ProgrammingError(f"Closed Connection: {cnx}")
...
class Connection(RequestSessionAndConnectionBase):
    ...
    def set_entity_cache(self, entity):
        ...
        if entity.cw_etype != "CWUser" and not self._open:
            raise ProgrammingError(f"Closed Connection: {self}")
    ...
    def rollback(self):
        ...
        try:
            ...
            with self.security_enabled(False, False):
                ...
                cnxset.rollback()
                if debug:
                    print(f"rollback for transaction {self} done")
        finally:
            self.clear()
        ...
        try:
            ...
            with self.security_enabled(False, False):
                ...
                try:
                    ...
                    if debug:
                        print(f"precommit transaction {self} done")
                except BaseException:
                ...
                with self.running_hooks_ops():
                    ...
                    if self.repo.config.mode == "test" and error:
                        raise error
                if debug:
                    print(f"postcommit transaction {self} done")
