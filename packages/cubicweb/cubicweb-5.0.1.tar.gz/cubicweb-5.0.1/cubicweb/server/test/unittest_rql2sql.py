...
""",
            ...
            # Temporary skip the FTIHL test, as the tests are not escaping
            # variables as expected.
            # see https://forge.extranet.logilab.fr/cubicweb/cubicweb/-/issues/1108
            # (NB: can't use pytest.skipTest easily with subTest, hence the
            # comments.
            #             (
            #                 'Any X, FTIHL(N, "toto") WHERE X is Tag, X name N, X name ILIKE "%toto%"',
            #                 """
            # SELECT _X.cw_eid, ts_headline(_X.cw_name, plainto_tsquery('toto'))
            # FROM cw_Tag AS _X
            # WHERE _X.cw_name ILIKE %toto%""",
            #             ),
            #             (
            #                 'Any X, FTIHL(N, "toto", "StartSel=<hl>,StopSel=</hl>,MaxFragments=2") WHERE X is Tag, X name N, X name ILIKE "%toto%"',
            #                 """
            # SELECT _X.cw_eid, ts_headline(_X.cw_name, plainto_tsquery('toto'), 'StartSel=<hl>,StopSel=</hl>,MaxFragments=2')
            # FROM cw_Tag AS _X
            # WHERE _X.cw_name ILIKE %toto%""",
            #             ),
        ]:
