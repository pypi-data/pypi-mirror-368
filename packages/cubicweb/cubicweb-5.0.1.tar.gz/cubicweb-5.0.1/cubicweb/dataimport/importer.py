...
from cubicweb.toolsutils import Spinner
    ...
    * ``values``, a dictionary whose keys are attribute or relation names from the schema (eg.
    ...
    def prepare(self, schema):
        ...
        * remove non inlined relations and return them as a [(e1key, relation, e2key)] list
        ...
        """
        assert self._schema is None, f"prepare() has already been called for {self}"
        self._schema = schema
        ...
        for key, rtype, role in self.iter_rdefs():
            ...
            if rschema.final or (rschema.inlined and role == "subject"):
                ...
                ), f"more than one value for {rtype}: {entity_dict[key]} ({self.extid})"
                if entity_dict[key]:
    ...
    def is_ready(self, extid2eid):
        ...
        currently existing.
        """
        assert self._schema, f"prepare() method should be called first on {self}"
...
class ExtEntitiesImporter:
    ...
    def _import_entities(self, ext_entities, queue):
        ...
        spinner = Spinner(display=sys.stdout.isatty())
    ...
    def prepare_insert_deferred_relations(self, deferred):
        ...
        for rtype, relations in deferred.items():
            self.import_log.record_debug(
                f"importing {len(relations)} {rtype} relations"
            )
    ...
    def _warn_about_missing_work(self, queue, missing_relations):
        ...
        if queue:
            ...
            for ext_entities in queue.values():
                for ext_entity in ext_entities:
                    msg = f"{ext_entity}: {ext_entity.why_not_ready(self.extid2eid)}"
                    msgs.append(msg)
            map(error, msgs)
...
class SimpleImportLog:
    ...
    def _log(self, severity, msg, path, line):
        encodedmsg = f"{severity}\t{self.filename}\t{line or ''}\t{msg}"
