...
import { renderContextTagsAsStyled, getTagTypeFromCssClass } from '../utils/contextTagUtils';
...
export class ChatMessages {
  ...
  private subscribeToContextChanges(): void {
    this.contextService.subscribe((newContexts) => {
      ...
      const hasContextTags = this.messageHistory.some(message => 
        message.role === 'user' && 
        typeof message.content === 'string' &&
      ...
      if (hasContextTags) {
        console.log('[ChatMessages] Context items updated, refreshing message displays');
  ...
  private refreshMessageDisplays(): void {
    ...
    const userMessages = this.container.querySelectorAll('.sage-ai-user-message .sage-ai-message-content');
    ...
    userMessages.forEach((messageContent) => {
      ...
      if (currentHTML.includes('sage-ai-mention')) {
        ...
        const originalMessage = this.extractOriginalMessageFromHTML(currentHTML);
  ...
  private processUserMessageContent(message: string): string {
    ...
    const mentionSpanRegex = /<span class="sage-ai-mention[^"]*"[^>]*>[^<]*<\/span>/g;
  ...
  public addContinueButton(): void {
    ...
    defaultButton.addEventListener('click', () => {
      ...
      console.log('[ChatMessages] Default prompt button clicked:', currentPrompt);
  ...
  private updatePromptButtons(recommendedPrompts: string[]): void {
    ...
    const buttonsContainer = this.waitingReplyBox?.querySelector('.sage-ai-prompt-buttons-container');
    if (!buttonsContainer) {
  ...
  private shouldShowContinueButton(): boolean {
    ...
    const recentMessages = this.messageHistory.slice(-10); // Check last 10 messages
