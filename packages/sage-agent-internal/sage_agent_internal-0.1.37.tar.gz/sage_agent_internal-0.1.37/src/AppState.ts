...
export const AppStateService = {
  ...
  updateSnippet: async (snippetId: string, updates: Partial<ISnippet>) => {
    ...
    const currentSnippets = AppStateService.getState().snippets;
    console.log('[AppStateService] Current snippets count:', currentSnippets.length);
    
    const updatedSnippets = currentSnippets.map(snippet => {
      if (snippet.id === snippetId) {
        console.log('[AppStateService] Found snippet to update:', snippet.title);
        return { ...snippet, ...updates };
      }
      return snippet;
    });
    
    console.log('[AppStateService] Updated snippets count:', updatedSnippets.length);
    ...
    try {
      ...
      console.log('[AppStateService] Successfully persisted updated snippets to StateDB');
    } catch (error) {
      console.error(
        '[AppStateService] Failed to persist snippets to StateDB:',
        error
  ...
  loadSnippets: async (): Promise<void> => {
    try {
      ...
      const migratedSnippets = await AppStateService.migrateSnippetsForUniqueIds(snippets);
   ...
   * Migrate snippets to ensure all have unique IDs
   */
  migrateSnippetsForUniqueIds: async (snippets: ISnippet[]): Promise<ISnippet[]> => {
    let needsUpdate = false;
    const usedIds = new Set<string>();
    
    console.log('[AppStateService] Checking', snippets.length, 'snippets for unique IDs');
    
    const migratedSnippets = snippets.map((snippet, index) => {
      // Check if snippet has no ID, invalid ID, or duplicate ID
      const hasValidId = snippet.id && typeof snippet.id === 'string' && snippet.id.trim().length > 0;
    ...
    if (needsUpdate) {
      try {
        ...
        console.log('[AppStateService] Successfully migrated snippet IDs to StateDB');
      } catch (error) {
        console.error('[AppStateService] Failed to save migrated snippets:', error);
      }
    } else {
      console.log('[AppStateService] All snippets already have valid unique IDs');
    }
  ...
  loadInsertedSnippets: async (): Promise<void> => {
    try {
      const insertedSnippets = await StateDBCachingService.getObjectValue<string[]>(
  ...
  saveInsertedSnippets: async (): Promise<void> => {
    try {
      ...
      await StateDBCachingService.setObjectValue(
        STATE_DB_KEYS.INSERTED_SNIPPETS,
        currentState.insertedSnippets
      );
      console.log(
        '[AppStateService] Saved',
        currentState.insertedSnippets.length,
  ...
  addInsertedSnippet: async (snippetId: string): Promise<void> => {
    const currentState = AppStateService.getState();
    if (!currentState.insertedSnippets.includes(snippetId)) {
      const newInsertedSnippets = [...currentState.insertedSnippets, snippetId];
      AppStateService.setState({ 
      ...
      try {
        ...
        console.log('[AppStateService] Added inserted snippet to StateDB:', snippetId);
      } catch (error) {
        console.error('[AppStateService] Failed to save inserted snippet to StateDB:', error);
      }
    }
  },
  ...
  removeInsertedSnippet: async (snippetId: string): Promise<void> => {
    const currentState = AppStateService.getState();
    const newInsertedSnippets = currentState.insertedSnippets.filter(id => id !== snippetId);
    ...
    try {
      ...
      console.log('[AppStateService] Removed inserted snippet from StateDB:', snippetId);
    } catch (error) {
      console.error('[AppStateService] Failed to remove inserted snippet from StateDB:', error);
    }
  },
  ...
  getInsertedSnippets: (): ISnippet[] => {
    ...
    return currentState.snippets.filter(snippet => 
      currentState.insertedSnippets.includes(snippet.id)
    );
  },
  ...
  isSnippetInserted: (snippetId: string): boolean => {
    const currentState = AppStateService.getState();
    return currentState.insertedSnippets.includes(snippetId);
  },
  ...
  clearInsertedSnippets: async (): Promise<void> => {
    ...
    try {
      ...
      console.log('[AppStateService] Cleared all inserted snippets from StateDB');
    } catch (error) {
      console.error('[AppStateService] Failed to clear inserted snippets from StateDB:', error);
    }
  },
