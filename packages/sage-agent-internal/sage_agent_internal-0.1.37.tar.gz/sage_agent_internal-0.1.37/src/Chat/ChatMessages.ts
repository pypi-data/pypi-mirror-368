...
export class ChatMessages {
  ...
  private ensureWaitingReplyBoxIsLast(): void {
    if (this.waitingReplyBox && this.waitingReplyBox.parentNode === this.container) {
   ...
   */
  public showWaitingReplyBox(): void {
    ...
    if (this.waitingReplyBox) {
      ...
      // When explicitly called (from wait_user_reply tool), always show the continue button
  ...
  /**
   * Check if we should show the continue button based on thread state and message history
   * This version is only used when wait_user_reply tool is called
   */
  private checkAndShowContinueButton(): void {
    console.log('[ChatMessages] checkAndShowContinueButton() called');
    ...
    // When called from wait_user_reply tool, always show the continue button
    console.log('[ChatMessages] wait_user_reply tool called, showing continue button');
  ...
  private checkAndShowContinueButtonOnStartup(): void {
    ...
    if (shouldShow) {
      console.log('[ChatMessages] Startup conditions met, showing continue button');
      this.showContinueButton();
