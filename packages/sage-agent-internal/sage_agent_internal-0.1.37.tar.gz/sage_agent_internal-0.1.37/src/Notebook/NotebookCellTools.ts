...
export class NotebookCellTools {
  ...
  edit_plan(options: {
    ...
    // Handle smart scrolling after plan update
    // this.handleSmartScroll(notebook);
  ...
  private checkUserScrolledAway(
    ...
    if (notebook.widgets.length === 0) return;
    ...
    const isLastCellVisible = lastCellInputBottom <= containerBottom + 50; // 50px buffer
  ...
  static preventScrolling(notebook: Notebook): () => void {
    ...
    notebook.scrollToCell = () =>
      Promise.resolve(
        console.warn('Scroll to cell prevented with intentional override')
      );
    notebook.scrollToItem = () =>
      Promise.resolve(
        console.warn('Scroll to item prevented with intentional override')
      );
