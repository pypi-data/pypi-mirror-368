...
class MarginPresenter(BasePresenter):
    ...
    def _calculate_spine_width(self) -> None:
        ...
        # Protect against division by zero (though unlikely with current constants)
        if SPINE_WIDTH_DIVISOR == 0:
            logger.error("SPINE_WIDTH_DIVISOR is zero, cannot calculate spine width")
            self._model.spine_width = SPINE_WIDTH_OFFSET * MM_TO_POINTS
            return
