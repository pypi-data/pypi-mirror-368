...
class BaseStrategy(ABC):
    ...
    def update_scene_rect(self) -> None:
        ...
        """
        # CRITICAL: Save view centers before changing scene rect
        # This prevents jumping when scene rect changes
        view_centers = []
        for view in self.graphics_scene.views():
            if hasattr(view, "mapToScene") and hasattr(view, "viewport"):
                center = view.mapToScene(view.viewport().rect().center())
                view_centers.append((view, center))
        ...
        logger.debug(
            f"Scene rect expanded from {content_bounds} to {expanded_bounds} "
            f"(added {padding}px padding for zoom)"
        )

        # CRITICAL: Restore view centers after changing scene rect
        # This prevents the view from jumping
        for view, center in view_centers:
