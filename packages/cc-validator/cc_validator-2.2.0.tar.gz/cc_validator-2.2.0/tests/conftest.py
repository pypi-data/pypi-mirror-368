...
def clean_tdd_context():
    ...
    # More aggressive cleanup for parallel test execution
    max_retries = 3
    for attempt in range(max_retries):
        try:
            if tdd_data_dir.exists():
                shutil.rmtree(tdd_data_dir)
            # Ensure directory is recreated fresh
            tdd_data_dir.mkdir(parents=True, exist_ok=True)
            # Clear it again to ensure it's empty
            shutil.rmtree(tdd_data_dir)
            break
            ...
            if attempt < max_retries - 1:
                ...
                # If we can't remove the directory, try to remove individual files
                try:
    ...
    # Longer delay to ensure cleanup is complete before test starts
    time.sleep(0.2)
    
    yield
    
    # Also clean up after the test to prevent interference with subsequent tests
    try:
        if tdd_data_dir.exists():
            shutil.rmtree(tdd_data_dir)
    except:
