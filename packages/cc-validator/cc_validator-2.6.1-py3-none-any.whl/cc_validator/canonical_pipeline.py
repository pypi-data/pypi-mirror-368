        ...
        if combined_results:
            ...
            if combined_data:
                ...
                if not security_approved and not tdd_approved:
                    ...
                    # Check if already prefixed to avoid duplication
                    if not sec_reason.startswith(
                        "Security:"
                    ) and not sec_reason.startswith("Template Security:"):
                        sec_reason = f"Security: {sec_reason}"
                    reason = sec_reason
                ...
                if result_part:
                    ...
                    if not security_approved_fallback and not tdd_approved_fallback:
                        ...
                        if not tdd_reason.startswith("TDD:"):
                            tdd_reason = f"TDD: {tdd_reason}"
                        fallback_reason = f"{sec_reason} | {tdd_reason}"
...
async def early_exit_aggregator(
    ...
    if not overall_approved:
        ...
        if not security_approved:
            ...
            # Check if already prefixed to avoid duplication
            if not security_reason.startswith(
                "Security:"
            ) and not security_reason.startswith("Template Security:"):
                security_reason = f"Security: {security_reason}"
...
async def both_failed_aggregator(
    ...
    tdd_reason = tdd_result.get("reason", "TDD validation failed")
    # Check if already prefixed to avoid duplication
    if not security_reason.startswith("Security:") and not security_reason.startswith(
        "Template Security:"
    ):
        security_reason = f"Security: {security_reason}"
    if not tdd_reason.startswith("TDD:"):
        tdd_reason = f"TDD: {tdd_reason}"
    primary_reason = f"{security_reason} | {tdd_reason}"
...
async def security_failed_aggregator(
    ...
    sec_reason = security_result.get("reason", "Security validation failed")
    # Check if already prefixed to avoid duplication
