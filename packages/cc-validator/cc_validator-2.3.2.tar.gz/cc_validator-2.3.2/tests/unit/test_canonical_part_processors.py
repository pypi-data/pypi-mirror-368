...
# These processors are now part of unified_processor
from cc_validator.canonical_pipeline import (
    create_canonical_validation_pipeline,
)
from cc_validator.unified_processor import (
...
class TestUnifiedSecurityTDDProcessor:
    ...
    async def test_unified_processor_with_write_operation(self):
        ...
        with (
            patch(
                "cc_validator.unified_processor.validate_security_pure"
            ) as mock_security,
            patch("cc_validator.unified_processor.validate_tdd_pure") as mock_tdd,
        ):
