...
from .pure_validation import PureValidationPipeline
...
def validate_hook_input() -> None:
    ...
    # Initialize pure validation pipeline with config
    config = ProcessorConfig(api_key=api_key)
    validator = PureValidationPipeline(config)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Claude Code Pure Processor Security + TDD Validation Hooks",
...
Examples:
  ...
  cargo test --message-format json | uvx cc-validator --capture-test-results rust
    ...
    if args.version:
        ...
        print("Pure processor security + TDD validation for Claude Code tool execution")
