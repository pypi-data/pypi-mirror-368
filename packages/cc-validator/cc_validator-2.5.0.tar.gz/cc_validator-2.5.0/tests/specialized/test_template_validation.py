...
<body>
    ...
    assert (
        "AWS Access secret key detected" in result["reason"]
        or "Stripe API secret key detected" in result["reason"]
    )
...
def test_template_xss_detection_with_strict_enabled() -> None:
    ...
    with patch("cc_validator.config.STRICT_TEMPLATE_VALIDATION", True):
        api_key = os.environ.get("GEMINI_API_KEY")
        file_storage = FileStorage()
        config = ProcessorConfig(api_key=api_key) if api_key else None
        validator = CanonicalPipelineValidator(file_storage, config)
