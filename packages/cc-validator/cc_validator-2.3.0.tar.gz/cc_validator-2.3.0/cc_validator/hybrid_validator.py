...
class HybridValidator:
    ...
    def validate_tool_use(
        ...
        # Execute both validations sequentially using sync wrappers
        security_result = self._validate_security_sync(tool_name, tool_input, context)
        tdd_result = self._validate_tdd_sync(tool_name, tool_input, context)

        # Stage 3: Aggregate results
        return self.aggregate_validation_results(security_result, tdd_result, tool_name)

    def _validate_security_sync(
        self, tool_name: str, tool_input: Dict[str, Any], context: str
    ) -> Dict[str, Any]:
        """Synchronous wrapper for security validation"""
        try:
            return self.security_validator.validate(tool_name, tool_input, context)
    ...
    def _validate_tdd_sync(
        self, tool_name: str, tool_input: Dict[str, Any], context: str
    ) -> Dict[str, Any]:
        """Synchronous wrapper for TDD validation"""
        try:
            return self.tdd_pipeline.validate(tool_name, tool_input, context)
        except Exception as e:
            return {
                "approved": False,
                "reason": f"TDD validation error: {str(e)}",
                "suggestions": ["Check TDD validation configuration"],
                "tdd_phase": "unknown",
            }
    ...
    def before_tool_callback(self, hook_data: Dict[str, Any]) -> int:
        ...
        try:
            ...
            # Perform hybrid validation (now synchronous)
