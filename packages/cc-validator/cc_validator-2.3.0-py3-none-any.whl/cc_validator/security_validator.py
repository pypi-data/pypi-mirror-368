...
class SecurityValidator:
    ...
    def validate(
        ...
        try:
            # Simple synchronous security analysis without LLM processors
            # For now, just return the quick result since we already have comprehensive rule-based validation
            return quick_result
    ...
    def validate_file_operation(self, tool_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if requires_strict_security:
            ...
            return self._validate_production_secrets(content)
        elif is_test_file:
            # Lenient validation for test files - allow test fixtures
            return self._validate_test_secrets(content, file_path)
    ...
    def _validate_production_secrets(self, content: str) -> Dict[str, Any]:
        ...
        if potential_secrets and self.client:
            llm_result = self._llm_validate_secrets(
    ...
    def _validate_test_secrets(self, content: str, file_path: str) -> Dict[str, Any]:
        ...
        potential_secrets = self._extract_potential_secrets(content)
        if potential_secrets and self.client:
            return self._llm_validate_secrets(
...
Be context-aware: {file_context} files should be more lenient. Only block if confident it's a real production secret."""
            ...
            # Process through streaming processor using pure processor execution
            result = {}
            processed_parts = processor.apply_sync(self.processor, [request_part])
            for response_part in processed_parts:
...
Provide structured assessment and actionable recommendations."""
            ...
            # Process file analysis synchronously using genai-processors
            try:
                # Create the processor with the right config
                genai_processor = self.processor._get_processor(
                    FILE_ANALYSIS_THINKING_BUDGET, FileAnalysisResponse
                )

                if not genai_processor:
                    result = {"error": "No API key configured"}
                else:
                    # Create input parts
                    from genai_processors import content_api

                    input_parts = [content_api.ProcessorPart(prompt)]
                    if uploaded_file:
                        input_parts.append(uploaded_file)

                    # Process synchronously
                    response_text = ""
                    processed_parts = processor.apply_sync(genai_processor, input_parts)
                ...
                result = {"error": f"File analysis error: {str(e)}"}
