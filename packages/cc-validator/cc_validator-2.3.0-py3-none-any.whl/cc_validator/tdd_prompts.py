...
- Implementation code contains comments (not applicable to test creation)
...
- Self-evident code (no comments in implementation files)
- Test files: focus on single test, clean test code
...
VALIDATE:
- TEST FILE: Only ONE new test allowed (TDD rule), no comments in test file itself
- IMPL FILE: Must have failing test first, no comments allowed, SOLID compliance
- Zen of Python required
- Minimal implementation only

DECISION: 
- TEST FILES: Allow if single test with no comments in the test file itself
- IMPL FILES: Block if no test exists, comments present, or rules violated
"""
