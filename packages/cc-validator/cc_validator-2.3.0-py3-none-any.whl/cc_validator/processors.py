...
class ParallelValidationProcessor:
    ...
    def validate_parallel(
        ...
        """Run security and TDD validation (sequentially since asyncio removed)"""

        # Run validations sequentially since asyncio removed
        try:
            # Note: These methods are still async, so we'd need to handle that
            # but since this file is not used, keeping as placeholder
            security_result = {
                "approved": False,
                "reason": "Legacy processor file - not implemented without asyncio",
            }
