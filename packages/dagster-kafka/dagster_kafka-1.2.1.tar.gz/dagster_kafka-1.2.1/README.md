...
Transform your JSON data pipeline with automatic validation, ensuring data quality and catching errors before they impact downstream systems.
...
Strict vs Lenient Validation: Choose between failing fast or warning on validation errors
Schema Evolution Tracking: Monitor schema changes over time
File-based Schemas: Load schemas from external JSON files
DLQ Integration: Invalid messages automatically routed to Dead Letter Queue
Circuit Breaker Support: Prevent cascade failures with smart error handling

JSON Schema Usage Examples
python# Strict validation (fail on invalid data)
strict_manager = create_json_schema_kafka_io_manager(
    kafka_resource=kafka_resource,
    schema_file="schemas/strict_user_events.json",
    strict_validation=True,
    enable_dlq=True
)

# Lenient validation (warn but continue processing)
lenient_manager = create_json_schema_kafka_io_manager(
    kafka_resource=kafka_resource,
...
YAML-based configuration for teams without Python expertise!
Simple YAML Configuration
Transform complex Python setup into simple YAML configuration:
yaml# Configure Kafka assets with just a few lines of YAML
...
yaml# Production-ready configuration with security and JSON Schema validation
type: dagster_kafka.KafkaComponent
attributes:
  kafka_config:
  ...
  topics:
    - name: "critical-events"
      format: "json_schema"
      schema_file: "schemas/critical_events.json"
      strict_validation: true
    - name: "transaction-data"
      format: "protobuf"
      schema_registry_url: "{{ env('SCHEMA_REGISTRY_URL') }}"
Components vs Python API
ApproachLines of CodePython Knowledge RequiredTeam AccessibilityPython API30-50 linesAdvancedDevelopers onlyYAML Components5-10 linesNoneEveryone
...
@asset
def quality_controlled_events():
    """Consume events with automatic data quality validation."""
    pass

defs = Definitions(
    assets=[quality_controlled_events],
    resources={
        "kafka": KafkaResource(bootstrap_servers="localhost:9092"),
...
pythonfrom dagster import asset, Config
...
pythonfrom dagster_kafka import DLQConfiguration, DLQStrategy
...
Feature Comparison
FeatureJSONJSON SchemaAvroProtobufSecurityDLQSchema EvolutionBasicValidationAdvancedAdvancedN/AError RoutingPerformanceGoodGoodBetterBestOverheadMinimalSchema RegistryNoNoYesYesHTTPSTopic-basedBackward CompatibilityManualValidationAutomaticAutomaticMaintainedPreservedBinary FormatNoNoYesYesEncryptedJSONHuman ReadableYesYesNoNoNoYesCross-LanguageYesYesYesYesYesYesData ValidationNoYesSchemaSchemaFullSecuredError HandlingDLQDLQ+ValidationDLQDLQMonitoredCore Feature
Development & Testing
Comprehensive Test Coverage
bash# Run all validation tests (11 phases)
...
python -m pytest tests/test_json_schema_io_manager.py -v   # JSON Schema tests (NEW)
...
JSON Schema Support - Data validation with evolution checking (NEW) ✅
...
JSON Schema testing - Help test validation scenarios (NEW)
...
Version 1.2.0 - JSON Schema Validation Release
