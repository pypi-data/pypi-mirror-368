#include "yirage/yica/yica_kernel_generator.h"
#include <algorithm>
#include <numeric>
#include <sstream>
#include <iomanip>
...
YICAKernelGenerationResult YICAKernelGenerator::generate_kernel(
    ...
    try {
        ...
        // 应用优化
        if (kernel_config.enable_loop_unrolling) {
            result.yis_code = apply_loop_optimizations(result.yis_code, kernel_config);
            result.applied_optimizations.push_back("Applied loop unrolling optimization");
        }
        
        if (kernel_config.enable_instruction_fusion) {
            result.yis_code = apply_instruction_fusion(result.yis_code);
            result.applied_optimizations.push_back("Applied instruction fusion optimization");
        }
        
        if (kernel_config.enable_register_tiling) {
            result.yis_code = apply_register_tiling(result.yis_code, kernel_config);
            result.applied_optimizations.push_back("Applied register tiling optimization");
        }
        ...
        // 资源分析
        result.resource_usage = analyze_resource_usage(result.yis_code, kernel_config);
...
YICAKernelGenerationResult YICAKernelGenerator::generate_cim_matmul_kernel(
    ...
    // 生成 CIM 矩阵乘法代码
    yis_code << cim_code_gen_->generate_cim_code("matmul", 0, {"input_a", "input_b"}) << "\n";
    ...
    // 添加优化指标
    result.optimization_metrics["matrix_a_rows"] = static_cast<float>(a_shape[0]);
    result.optimization_metrics["matrix_a_cols"] = static_cast<float>(a_shape[1]);
    result.optimization_metrics["matrix_b_rows"] = static_cast<float>(b_shape[0]);
    result.optimization_metrics["matrix_b_cols"] = static_cast<float>(b_shape[1]);
    result.optimization_metrics["cim_arrays_used"] = static_cast<float>(config.cim_config.num_arrays);
...
YICAKernelGenerationResult YICAKernelGenerator::generate_cim_attention_kernel(
    ...
    // 添加优化指标
    result.optimization_metrics["batch_size"] = static_cast<float>(batch_size);
    result.optimization_metrics["seq_length"] = static_cast<float>(seq_length);
    result.optimization_metrics["num_heads"] = static_cast<float>(num_heads);
    result.optimization_metrics["head_dim"] = static_cast<float>(head_dim);
...
YICAKernelGenerationResult YICAKernelGenerator::generate_fused_mlp_kernel(
    ...
    result.optimization_metrics["kernel_type"] = 4.0f; // fused_mlp indicator
    result.optimization_metrics["fusion_pattern"] = 1.0f; // linear_activation_linear indicator
...
std::string YICAKernelGenerator::generate_spm_setup_code(const YICAKernelConfig::SPMConfig& spm_config) {
    ...
    code << "  spm_cache_line_size " << spm_config.cache_line_size << "\n";
    return code.str();
}
...
std::string CIMArrayCodeGenerator::generate_cim_code(const std::string& operation, 
    ...
    // Default matrix dimensions for demo
...
int CIMArrayCodeGenerator::calculate_optimal_array_count(const std::vector<int>& shape) {
    ...
    int optimal_count = std::min(static_cast<int>(std::sqrt(total_ops / 1000)), static_cast<int>(config_.num_cim_arrays));
...
namespace kernel_utils {
    ...
    YICAKernelConfig recommend_kernel_config(const kernel::KNOperator* op,
        ...
        // CIM 配置
        config.cim_config.num_arrays = std::min(16u, hardware_config.num_cim_arrays);
