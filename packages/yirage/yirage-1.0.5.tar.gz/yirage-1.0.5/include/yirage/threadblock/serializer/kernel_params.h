...
struct alignas(16) NewKernelParams {
  ...
  yirage::type::TBOperatorType operator_types[MAX_NUM_OPERATORS];
...
struct alignas(16) KernelParams {
  ...
  yirage::type::TBOperatorType operator_types[MAX_NUM_OPERATORS];
  yirage::threadblock::STensor smem_inputs[MAX_TOTAL_SMEM_INPUTS];
  yirage::threadblock::STensor smem_outputs[MAX_TOTAL_SMEM_OUTPUTS];
  // input dtensors in device memory
  int num_dmem_inputs, num_dmem_outputs;
  yirage::kernel::DTensor dmem_inputs[MAX_NUM_DMEM_INPUTS];
  yirage::kernel::DTensor dmem_outputs[MAX_NUM_DMEM_INPUTS];
