...
def start(args: argparse.Namespace, docker: DockerManager):
    ...
    if container.status != 'stopped':
        log.warning(
            'Container is not stopped; doing nothing..',
            extra={
    ...
    try:
        ...
        log.error('error starting container', extra={'container': container.name, 'error': str(e)})
        raise e

    log.info('Successfully started container', extra={'container': container.name})
