...
│   └── resources/                           # 资源文件目录
│       ├── data/                            # 游戏数据文件
│       │   ├── characters.csv               # 干员数据（CSV格式）
│       │   ├── career.json                  # 职业映射数据
│       │   ├── camp.json                    # 势力映射数据
│       │   ├── pool_settings.json           # 题库设置数据
│       │   ├── mode_settings.json           # 模式设置数据
│       │   └── continuous_settings.json     # 连战模式设置数据
│       ├── images/                          # 图片资源目录
│       │   ├── illustrations.zip            # 干员立绘压缩包
│       │   └── xlpj/                       # 特殊图片资源
│       └── templates/                       # HTML模板目录
│           ├── guess.html                   # 猜测结果模板
│           ├── correct.html                 # 正确答案模板
...
├── README.md                                # 项目说明文档
├── CHANGELOG.md                             # 更新日志
├── DEPENDENCIES.md                          # 依赖兼容性说明
├── DEPENDENCY_UPDATE_SUMMARY.md             # 依赖更新总结
├── build.py                                 # 构建和发布脚本
├── .gitignore                               # Git忽略文件配置
└── MANIFEST.in                              # 分发包文件清单
```
