...
from tabeval.utils.serialization import save as serialize
from tabeval.version import MAJOR_VERSION
...
class Serializable:
    ...
    def load_dict(representation: dict) -> Any:
        ...
        if representation["version"] != Serializable.version():
            raise RuntimeError(
                f"Invalid tabeval API version. Current version is {Serializable.version()}, but the object was serialized using version {representation['version']}"
            )
        ...
        if representation["module_relative_path"] is not None:
            ...
            if not isinstance(spec.loader, Loader):
                raise RuntimeError("invalid tabeval object type")
