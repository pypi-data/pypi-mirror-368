...
from svc_order_zxw.apis.func_创建订单 import 创建新订单_并创建支付单,PYD_OrderResponse,PYD_PaymentResponse
...
async def func1_创建新Order和Payment(
        db: AsyncSession,
        user_id: int,
        product_id: int,
        payment_price: float,
    ...
    return await 创建新订单_并创建支付单(
        ...
        apple_auto_renew_status=auto_renew_status,
        apple_offer_identifier=None,
        apple_offer_type=None,
        apple_environment=apple_environment,
    )

async def func2_更新订单(
        db: AsyncSession,
        order_number: str,
        payment_price: float,
        quantity: int,
