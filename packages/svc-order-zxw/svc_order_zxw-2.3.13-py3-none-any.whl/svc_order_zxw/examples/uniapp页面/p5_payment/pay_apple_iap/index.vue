	...
	<view class="apple-pay-container">
		...
		<view class="offers-section" v-if="selectedProduct?.discount && selectedProduct.discount.length > 0">
			...
			<view v-for="discount in selectedProduct.discount" :key="discount.code" class="offer-item">
				<view class="offer-info">
					...
					<text
						class="offer-discount">¥{{ discount.price }}/{{ discount.units }}{{ discount.periodUnit === 'month' ? '月' : discount.periodUnit }}</text>
				</view>
		...
		<view class="actions">
			<button @click="startPurchase" :disabled="loading || !productInfo || !selectedProduct" class="purchase-btn">
				{{ loading ? '处理中...' : selectedProduct ? `立即购买 ¥${currentPrice}` : '商品验证中...' }}
			</button>
...
<script setup>
	...
	const selectProduct = (product) => {
		...
		addLog(`选择产品: ${product.productid} - ¥${product.price}`)
	}
	...
	const startPurchase = async () => {
		...
		// 0. 向后端创建订单，获取订单号
		const orderResult = await createOrderWithBackend()
        ...
        switch (state) {
            case IapTransactionState.purchased: {
                if (purchaseResult.transactionIdentifier && purchaseResult.transactionReceipt) {
                    ...
                    const orderResult = await createOrderWithBackend(transaction)
	...
	// 调用后端API创建订单
	async function createOrderWithBackend(transaction=null) {
		try {
			addLog('创建后端订单...')
			...
			const orderResult = await applePayApi.createOrder(orderRequest)
