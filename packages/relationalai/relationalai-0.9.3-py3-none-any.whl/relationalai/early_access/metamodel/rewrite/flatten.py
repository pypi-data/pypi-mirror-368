...
from relationalai.early_access.metamodel import dependency
from relationalai.early_access.metamodel.util import FrozenOrderedSet, filter_by_type
from relationalai.early_access.metamodel.typer.typer import to_type, is_primitive
...
class Flatten(Pass):
    ...
    def create_task_name(self, task: ir.Task, prefix: Optional[str]=None) -> str:
        ...
        return self.name_cache.get_name(task.id, prefix)
    ...
    def handle_logical(self, task: ir.Logical, ctx: Context):
        ...
        if groups["outputs"] and self._handle_outputs:
    ...
    def adjust_outputs(self, task: ir.Logical, body: OrderedSet[ir.Task], groups: dict[str, OrderedSet[ir.Task]], ctx: Context):
        ...
        # for GNF outputs we need to generate a rule for each "column" in the output
            ...
            for output in filter_by_type(body, ir.Output):
                ...
                # add an explicit check for null values in the last argument of the output
                # null values might be introduced there if the column value is also a nullable
                # key, but we don't want to explicitly represent them in the column values
                assert isinstance(output, ir.Output)
                output_keys_anno = next(filter(lambda anno: anno.relation == builtins.output_keys, output.annotations), None)
                if output_keys_anno:
                    last_arg = output.aliases[-1]
                    if last_arg and isinstance(last_arg[1], ir.Var) and last_arg[1] in output_keys_anno.args:
                        deps.add(f.lookup(builtins.neq, [last_arg[1], None]))
    ...
    def _generate_output_column(self, body: OrderedSet[ir.Task], output: ir.Output, idx: int, alias: tuple[str, ir.Value], is_export: bool):
        ...
        if is_export and isinstance(alias[1], ir.Var) and not is_primitive(alias[1].type):
