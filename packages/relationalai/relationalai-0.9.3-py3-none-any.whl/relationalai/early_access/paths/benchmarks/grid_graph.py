...
import time
from relationalai.early_access.builder import Integer, define
...
def create_labeled_grid(model, side_length, directed = True, label = "A"):
    ...
    """
    Node = model.Concept(f"GridNode_{time.time_ns()}")
    Edge = model.Relationship(f"{{src:{Node}}} has an edge to {{dst:{Node}}} with label {{String}} {time.time_ns()}")
