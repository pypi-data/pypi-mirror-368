...
from relationalai.early_access.builder import define, where, Integer
...
def find_paths_via_automaton(graph, automaton, Source, Target,
    ...
    FinalState = model.Relationship("{state:Integer} is a final state of the automaton")
    ...
    ProductGraphSource = model.Relationship(f"{{node:{ProductGraphNode}}} is a product graph source node")
    ProductGraphTarget = model.Relationship(f"{{node:{ProductGraphNode}}} is a product graph target node")
