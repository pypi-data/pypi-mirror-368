...
from relationalai.early_access.builder import define, not_, count

from relationalai.early_access.paths.graph import Graph
from relationalai.early_access.paths.utilities.iterators import setup_iteration
...
def two_balls_upto(g:Graph, Source, Target, max_length=None):
    ...
    source_ball = g.model.Relationship(f"At hop {{Integer}} the source ball has node {{{Node}}}")
    target_ball = g.model.Relationship(f"At hop {{Integer}} the target ball has node {{{Node}}}")
    source_condition = g.model.Relationship("The iteration from the source can continue from {Integer}")
    target_condition = g.model.Relationship("The iteration from the target can continue from {Integer}")
    source_surface = g.model.Relationship("The size of the source sphere is {Integer}")
    target_surface = g.model.Relationship("The size of the target sphere is {Integer}")
