...
from relationalai.early_access.metamodel.util import OrderedSet, Printer as BasePrinter, ordered_set
import json
...
class Printer(BasePrinter):
    ...
    def print_node(self, node, indent=0) -> None:
        ...
        elif isinstance(node, PyDecimal):
            # Use parse_decimal128 so we don't lose any precision since just printing the value
            # will be parsed as a float in Rel.
            self._indent_print(indent, f"::std::common::parse_decimal[#128, #{rel_utils.DECIMAL128_SCALE}, ")
            self._print_value(str(node))
            self._print("]")
