...
def _translate_descending_rank(ctx: TranslationCtx, rank: ir.Rank, body: lqp.Formula, abstr_args) -> lqp.Formula:
    ...
    if rank.limit != 0:
        limit_term, _ = _translate_term(ctx, rank.limit)
...
def _translate_aggregate(ctx: TranslationCtx, aggr: ir.Aggregate, body: lqp.Formula) -> Union[lqp.Reduce, lqp.Formula]:
    ...
    if aggr_name == "avg":
        ...
        result = mk_exists(
            [
                (sum_result, sum_type),
                (count_result, mk_type(lqp.TypeName.INT))
            ],
            conjunction,
        )
...
def _translate_relterm(ctx: TranslationCtx, term: ir.Value) -> Tuple[lqp.RelTerm, lqp.Type]:
    if isinstance(term, ir.Literal) and term.type == types.Symbol:
        if isinstance(term.value, str):
            ...
            value = lqp.Value(value=term.value, cast_type=None, meta=None)
            return lqp.SpecializedValue(value=value, meta=None), meta_type_to_lqp(types.Int)
        else:
...
def _translate_term(ctx: TranslationCtx, term: ir.Value) -> Tuple[lqp.Term, lqp.Type]:
    if isinstance(term, ir.Var):
        ...
        assert isinstance(term.value, lqp_types.PrimitiveValue), \
            f"expected primitive value, got {type(term.value)}: {term.value}"
        value = lqp.Value(value=term.value, cast_type=None, meta=None)
        ...
        value = lqp.Value(value=term, cast_type=None, meta=None)
