...
import time
from relationalai.early_access.builder import Model, Integer, select, String, define

from relationalai.early_access.paths.graph import Graph

# from path_to_automaton import path, star
from relationalai.early_access.paths.api import node, path, match
...
if __name__ == "__main__":
    ...
    model = Model(f"paths_{time.time_ns()}", use_lqp=True) # , use_lqp=True)  # strict=True  # dry_run=False  # use_direct_access=True
    ...
    Source = model.Relationship(f"{{node:{Node}}} is a source node")
    Target = model.Relationship(f"{{node:{Node}}} is a target node")
