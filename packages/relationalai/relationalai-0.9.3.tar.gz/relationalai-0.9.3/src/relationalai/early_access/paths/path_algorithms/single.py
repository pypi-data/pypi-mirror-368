...
from relationalai.early_access.builder import where, define, select, min
...
def single_path(g: Graph, ball, dst, radius):
    ...
    path = g.model.Relationship(f"Path at position {{Integer}} has node {{v:{Node}}}")
...
def single_walk(g: Graph, Source, Target, radius):
    ...
    walk = g.model.Relationship(f"Walk at position {{Integer}} has node {{v:{Node}}}")
...
def single_shortest_path(g: Graph, Source, Target):
    ...
    path = g.model.Relationship(f"Path at position {{Integer}} has node {{v:{Node}}}")
    cand = g.model.Relationship(f"At position {{Integer}} there is target node {{v:{Node}}}")
