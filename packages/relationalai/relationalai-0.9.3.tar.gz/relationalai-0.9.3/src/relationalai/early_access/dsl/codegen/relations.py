...
def _get_map_madlib_and_name(name_prefix: str, value_concept: qb.Concept) -> tuple[str, str]:
    ...
    madlib = f'{name} {{_row_ix:Integer}} {{_val:{value_concept}}}'
    return madlib, name
...
class AbstractMap(GeneratedRelation):
    ...
    @staticmethod
    def _construct_filtering_atoms(conditions, row, column_ref=None):
...
class ValueMap(RoleMap):
    ...
    def _generate_body(self):
        row = qb.Integer.ref()
        orig_val = self._binding.column.type().ref()

        filter_atoms = self._construct_filtering_atoms(self._binding.filter_by, row, orig_val)
        head_atoms, body_atoms = self._construct_body_atoms(row, orig_val, filter_atoms)

        define(*head_atoms).where(*body_atoms)

    def _construct_body_atoms(self, row, orig_val, filter_atoms):
        result_type = self.value_player()
        has_transforms = self._value_converter or self._binding.transform_with
        if has_transforms:
            converted = result_type.ref()
            body = self._generate_transform_body(orig_val, converted)
            return [self(row, result_type(converted))], [
                where(
                    self._binding.column(row, orig_val),
                    body,
                    filter_atoms
                )]
        else:
            return [self(row, result_type(orig_val))], [
                self._binding.column(row, orig_val),
                filter_atoms
            ]

    def _generate_transform_body(self, val, converted):
        ...
        if converter and not transformations:
            return converter(val, converted)
        elif transformations and not converter:
            return self._chain_transformations(val, converted, transformations)
        elif converter and transformations:
            val_converted = converter.result_type().ref()
            chain = self._chain_transformations(val_converted, converted, transformations)
            return where(
                converter(val, val_converted),
                chain
            )
        raise ValueError("Incorrect state, cannot apply binding transformations with empty value converter(s)")

    def _chain_transformations(self, input_var, output_var, transformations):
        if len(transformations) == 1:
            return self._apply_transformations(input_var, output_var, transformations[0])
        intermediate_vars = [self._get_transform_output_type(tr) for tr in transformations[:-1]]
        all_vars = [input_var, *intermediate_vars, output_var]
        return where(
...
class EntitySubtypeMap(AbstractMap):
    ...
    def _handle_params(binding: Union[IdentifierConceptBinding, SubtypeConceptBinding, ReferentConceptBinding],
        ...
        madlib = f'{name} {{_row_ix:Integer}} {{_val:{result_concept}}}'
        return madlib, name
    ...
    def _generate_body_formula(self, row, parent_entity):
        filtering_atom = self._construct_filtering_atoms(self._binding.filter_by, row)
        return where(
