...
def webAPI_request_forecast(fcdate,origin,grid,variable,data_format,webapi_param,leadtime_hour,leveltype,filename,plevs):
    ...
    os.system(f'grib_set -s type=pf -w type=cf {filename}_control {filename}_control2')
    # merge both control and perturbed forecast
    os.system(f'cdo merge {filename}_control2 {filename}_perturbed {filename}_allens')
    # convert to netcdf if data formatted request, else keep as grib
    if data_format == 'netcdf':
        os.system(f'grib_to_netcdf -o {filename}.nc {filename}_allens')
    else:
        os.system(f'mv {filename}_allens {filename}.grb')
    # remove all unnecessary files    
    os.system(f'rm {filename}_control {filename}_control2 {filename}_perturbed {filename}_allens')

def webAPI_request_forecast_lag_ensemble(fcdate,origin,grid,variable,data_format,webapi_param,leadtime_hour,leveltype,filename,plevs,fc_enslags):
    from ecmwfapi import ECMWFDataServer
    server = ECMWFDataServer()
    ...
    for lag in fc_enslags:
        ...
        print (lag)
        print (leadtimes)
        print (leadtime_hour_copy.shape[0])
    ...
    if lag != [0]:
        all_fcs = xr.open_mfdataset(f'{filename}_allens_*',engine='cfgrib',combine='nested',concat_dim='step')
        all_fcs = all_fcs.drop_vars(['step','time'])
        all_fcs = all_fcs.rename({'valid_time':'time'})

        if 'time' not in all_fcs.dims:
            all_fcs = all_fcs.swap_dims({'step':'time'})
        ...
        combined.to_netcdf(f'{filename}.nc')

    # once downloading all requested lag. combine all
    #os.system(f'cdo merge {filename}_allens_* {filename}_allens')
    # convert to netcdf if data formatted request, else keep as grib
    #if data_format == 'netcdf':
    #    os.system(f'grib_to_netcdf -o {filename}.nc {filename}_allens')
    #else:
...
def download_forecast(variable,model,fcdate,local_destination=None,filename=None,area=[90,-180,-90,180],data_format='netcdf',grid='1.5/1.5',plevs=None,leadtime_hour=None,fc_enslags=None):
    ...
    webAPI_request_forecast_lag_ensemble(fcdate,origin_id,grid,variable,data_format,webapi_param,leadtime_hour,leveltype,filename,plevs,fc_enslags)
