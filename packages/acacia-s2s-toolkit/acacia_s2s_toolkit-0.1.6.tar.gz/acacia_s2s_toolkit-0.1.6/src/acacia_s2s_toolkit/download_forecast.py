...
def webAPI_request_forecast(fcdate,origin,grid,variable,data_format,webapi_param,leadtime_hour,leveltype,filename,plevs):
    ...
    # set forecast type in control to pf (perturbed forecast). Also set parameter number
    os.system(f'grib_set -s type=pf,parameterNumber={webapi_param} -w type=cf {filename}_control {filename}_control2')
    os.system(f'grib_set -s parameterNumber={webapi_param} {filename}_perturbed {filename}_perturbed2') # need to set parameternumber in perturbed download, so they are the same in control and perturbed
    # merge both control and perturbed forecast
    os.system(f'cdo merge {filename}_control2 {filename}_perturbed2 {filename}_allens')
    ...
    os.system(f'rm {filename}_control {filename}_control2 {filename}_perturbed {filename}_perturbed2 {filename}_allens')
...
def download_forecast(variable,model,fcdate,local_destination=None,filename=None,area=[90,-180,-90,180],data_format='netcdf',grid='1.5/1.5',plevs=None,leadtime_hour=None):
    ...
    webAPI_request_forecast(fcdate,origin_id,grid,variable,data_format,webapi_param,leadtime_hour,leveltype,filename,plevs)
