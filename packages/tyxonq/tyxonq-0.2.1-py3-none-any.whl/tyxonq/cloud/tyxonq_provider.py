...
from .config import tyxonq_base_url, tyxonq_api_version
...
def tyxonq_headers(token: Optional[str] = None) -> Dict[str, str]:
    if token is None:
        token = "ANY;0"
...
def list_devices(token: Optional[str] = None, **kws: Any) -> List[Device]:
    ...
    r = api_post("devices/list", json, token)
    r = error_handling(r)
    ds = r["devices"]

    rs = []
    for d in ds:
        rs.append(Device.from_name("tyxonq" + sep + d["id"]))
    return rs


def list_properties(device: Device, token: Optional[str] = None) -> Dict[str, Any]:
    json = {"id": device.name}
    r = api_post("device/detail", json, token)
    r = error_handling(r)
    if "device" in r:
        r = r["device"]
        if "links" in r:
...
def submit_task(
    ...
    **kws: Any
    ...
    """
    Submit task via tyxonq provider, we suggest to enable one of the compiling functionality:
    ...
    :param circuit: tyxonq or qiskit circuit object, defaults to None
    ...
    r = api_post("tasks/submit_task", json, token)
    r = error_handling(r)
    if 'id' not in r:
        raise ValueError(f"failed to submit task: {r}")
    
    rtn = []

   
    if "err" in r or "id" not in r:
        if "err" in r:
            logger.warning(t["err"])
        else:
            logger.warning("unsuccessful submission of the task:\n" + dumps(r))
    else:
        ti = Task(id_=r["id"], device=device)
        ti.add_details(**r)
        rtn.append(ti)
...
def resubmit_task(task: Task, token: str) -> Task:
    ...
    json = {"id": task.id_}
    r = api_post("task/start", json, token)
    r = error_handling(r)
    try:
...
def remove_task(task: Task, token: str) -> Any:
    ...
    json = {"id": task.id_}
    r = api_post("task/remove", json, token)
    r = error_handling(r)
    return r
...
def list_tasks(device: Device, token: str, **filter_kws: Any) -> List[Task]:
    ...
    json['task_type'] = 'quantum_api'
    r = api_post("tasks/api_key/list", json, token)
    r = error_handling(r)

    try:
        rtn = []
        for t in r["tasks"]:
            device_name = t['device']
            rtn.append(
                Task(
                    id_=t["job_name"],
                    
                    device=Device.from_name("tyxonq" + sep + device_name),
                )
            )
        return rtn
    except KeyError:
...
def get_task_details(
    ...
    r = api_post("tasks/detail", json, token)
    r = error_handling(r)

    if 'task' not in r:
        raise ValueError(f"failed to get task details: {r}")
    
    try:
        if "result" in r["task"] and r["task"]["result"] is not None:
