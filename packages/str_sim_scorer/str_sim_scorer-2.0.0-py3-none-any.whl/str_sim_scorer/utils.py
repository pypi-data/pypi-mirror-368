...
def count_matching_alleles(
    ...
    :param sample_ids: optional list of sample IDs to order the array by (instead of the
    default Python string sorting of the sample IDs)
    ...
    if sample_ids is not None:
        # ensure df is pivoted using the provided order of sample IDs
        df["id"] = pd.Categorical(df["id"], categories=sample_ids, ordered=True)
...
def scores_array_to_df(
    ...
    id1 = pd.Categorical.from_codes(i_upper, categories=sample_ids)
    id2 = pd.Categorical.from_codes(j_upper, categories=sample_ids)
    ...
    df = pd.DataFrame(
        ...
        {
            "n_common_loci": "uint8",
            "n_matching_alleles": "uint8",
            "n_total_alleles": "uint8",
