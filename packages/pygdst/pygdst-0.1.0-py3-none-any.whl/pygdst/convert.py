...
from .gdst import  bheader_decode,fheader_decode
...
def bheader2list(header, start=0, interval=60, 
                 ) -> list[dict]:

    '''
    把block_header 中的data中的有效信息转换出来,默认输出一个list 
    ...
    for i in range(start, ns, interval):
        inf_dict =  bheader_decode(header[i,:])
        inf_list.append(inf_dict)

    return inf_list

def bheader_list2csv(inf_dicts:list[dict], file_name:str,M=',',REMOVE_ZERO=False, date=None):
    '''
    ...
    lines = []
    if len(inf_dicts)==0:
        print(f'no inf_dicts, {file_name} is not created')
        return 0
    
    keys = list(inf_dicts[0].keys())
    lines.append( f'{M}'.join([str(item) for item in keys])+'\n')
    for inf_i in inf_dicts:
        ...
        if date is not None:
            date_UTC = inf_i['date_UTC']
            if date_UTC!=date:
                continue
        saved_items = [inf_i[key] for key in keys]
        line_i = f'{M}'.join([str(i) for i in saved_items])
...
def bheader2csv(header, file_name, start=0, interval=60, REMOVE_ZERO=False,keys=None):
    '''
    ...
    file_name: csv 文件名
    '''
    inf_dicts = bheader2list(header, start=start, interval=interval)
    return bheader_list2csv(inf_dicts, file_name,M=',',REMOVE_ZERO=REMOVE_ZERO)
...
def bins2sac(bins_file:list,sac_file:str,dt:float,name:str,
    ...
    for i, file in enumerate(bins_file):
        ...
        # print(t_s,data.shape)
        f_inf_dict = fheader_decode(H)
        if 1/f_inf_dict['sampling_rate']!=dt:
            print(f'{file} dt {1/f_inf_dict['sampling_rate']} != targeted dt={dt}, skip this file')
            continue
        # print(data.max())
        #头文件
        inf_list = bheader2list(hs[0,:,:], start=0, interval=int(60/dt/DATA_SIZE))
        header_list+=inf_list

        #健康度
        health_i = np.ones([1,60,1])
        health_i[0,:,0] = [i['date_UTC']==t_s[:6] for i in inf_list]
...
def bins2h5_day(ROOT_folder, h5_name,date, header_file_pattern='./{date}_{name}.csv',
    ...
    for i, sn_i in enumerate(sns):
        ...
        health_i = np.zeros(len(hours)*60, dtype='float32')
        ...
        for j, hour in enumerate(hours):
            ...
            # print(file_ij,data_ij.shape)

            f_inf_dict = fheader_decode(H)
            if 1/f_inf_dict['sampling_rate']!=dt:
                print(f'{file_ij} dt {1/f_inf_dict['sampling_rate']} != targeted dt={dt}, skip this file')
                inf_list = bheader2list(hs[0,:,:], start=0, interval=int(60/dt/DATA_SIZE))
                header_list+=inf_list
                continue
            ...
            # 为分钟值的健康度赋值
            health_i[j*60:(j+1)*60] = [i['date_UTC']==date for i in inf_list]
        ...
        # 写头文件
        if len(header_list)>=1 and header_file_pattern is not None:
            file_name = header_file_pattern.format(date=date, name=name_i)
            print(f' save header to {file_name}')
            bheader_list2csv(header_list,file_name, REMOVE_ZERO=False, date=date)
