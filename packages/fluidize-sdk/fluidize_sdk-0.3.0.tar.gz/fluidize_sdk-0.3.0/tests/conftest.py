...
from fluidize_sdk import FluidizeSDK, AsyncFluidizeSDK, DefaultAioHttpClient
from fluidize_sdk._utils import is_dict
...
logging.getLogger("fluidize_sdk").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[FluidizeSDK]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with FluidizeSDK(
        base_url=base_url, api_token=api_token, firebase_token=firebase_token, _strict_response_validation=strict
    ) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncFluidizeSDK]:
