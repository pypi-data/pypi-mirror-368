...
class TestParagraphProcessor:
    ...
    def test_decompile_padding(self):
        xml = '''
        <w:p>
            <w:pPr>
                <w:ind w:start="21pt"/>
        ...
        assert 'padding-left:21pt' in result
        xml = '''
        <w:p>
            <w:pPr>
                <w:ind w:end="21pt"/>
        ...
        '''
        result = ParagraphProcessor.decompile(xml)
        assert 'padding-right:21pt' in result
        xml = '''
        <w:p>
            <w:pPr>
        ...
        result = ParagraphProcessor.decompile(xml)
        assert 'padding-left:21pt' in result
        assert 'padding-right:22pt' in result

    def test_compile_padding(self):
        xml = '<xl-p style="padding-left:21pt">This is a paragraph with left indentation of 21pt.</xl-p>'
        result = ParagraphProcessor.compile(xml)
        assert '<w:ind w:start="21pt"/>' in result
        xml = '<xl-p style="padding-right:21pt">This is a paragraph with right indentation of 21pt.</xl-p>'
        result = ParagraphProcessor.compile(xml)
        assert '<w:ind w:end="21pt"/>' in result
        xml = '<xl-p style="padding-left:21pt;padding-right:22pt">This is a paragraph with left and right indentation of 21pt and 22pt.</xl-p>'
        result = ParagraphProcessor.compile(xml)
    ...
    def test_compile_paragraph_with_style(self):
        """测试编译带样式的段落"""
        xml = '<xl-p style="align:center;padding-top:10px;font-size:14px">content</xl-p>'
        result = ParagraphProcessor.compile(xml)
    ...
    def test_compile_paragraph_complex_style(self):
        """测试编译复杂样式的段落"""
        xml = '''<xl-p style="align:right;padding-top:20px;padding-bottom:10px;english:Times New Roman;chinese:宋体;font-size:16px;font-weight:bold">content</xl-p>'''
        result = ParagraphProcessor.compile(xml)
    ...
    def test_decompile_paragraph_with_empty_runs(self):
        ...
        result = ParagraphProcessor.decompile(xml)
        assert '<xl-p><xl-span style=""></xl-span></xl-p>' in result
