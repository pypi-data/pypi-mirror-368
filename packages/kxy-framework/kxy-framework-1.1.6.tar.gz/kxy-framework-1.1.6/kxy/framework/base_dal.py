...
class BaseDal(Generic[T]):
    ...
    def _add_extend_filter(self, filters):
        ...
        if tenant_id!='*' and hasattr(self.modal, 'tenant_field'):
            tenant_field_name = self.modal.tenant_field
            if hasattr(self.modal, tenant_field_name):
                tenant_id = current_tenant_id.get()
                if tenant_id:
                    tenant_filter = getattr(self.modal, tenant_field_name)==tenant_id
                    filters_copy.append(tenant_filter)
        currentDepId = dep_id.get()
        if currentDepId!='*' and hasattr(self.modal, 'dep_field'):
            dep_field_name = self.modal.dep_field
            if hasattr(self.modal, tenant_field_name):
                if currentDepId:
                    dep_filter = getattr(self.modal, dep_field_name)==currentDepId
                    filters_copy.append(dep_filter)
        return filters_copy
    ...
    async def Insert(self, entity):
        ...
        if hasattr(self.modal, 'tenant_field'):
            tenId = getattr(entity, self.modal.tenant_field)
            if not tenId:
                tenId = current_tenant_id.get()
                if not tenId:
                    raise FriendlyException('添加数据时未指定租户')
                setattr(entity, self.modal.tenant_field, tenId)
