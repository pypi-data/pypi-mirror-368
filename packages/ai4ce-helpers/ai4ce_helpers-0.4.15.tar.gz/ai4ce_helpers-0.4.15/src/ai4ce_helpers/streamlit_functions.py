...
def st_load_components(uid: list[str] = None) -> None:
    ...
    if uid is None:
        if 'generated_design_comp_list_uids' in st.session_state:
            for uid in st.session_state['generated_design_comp_list_uids']:
                status_code, component = _backend_GET(f"/components/{uid}")
                if status_code == 200:
                    st.session_state['db']['components'][uid] = component
                else:
                    st.error(f"Error loading component {uid}: {component}")

        else:
            st.error("No UIDs provided and no generated design selected.")
            return

    # Load each component by UID
    for component_uid in uid:
        status_code, component = _backend_GET(f"/components/{component_uid}")
        if status_code == 200:
