...
def _detect_existing_project(current_dir: str) -> dict:
    ...
    for indicator_file, proj_type in project_indicators.items():
        if os.path.exists(os.path.join(current_dir, indicator_file)):
    ...
    has_src_structure = any(os.path.isdir(os.path.join(current_dir, d)) for d in common_dirs)
    ...
    is_project = bool(existing_files) or has_src_structure or is_git_repo
    should_continue = is_project and not is_metis_project  # Continue in existing non-metis projects
...
def _get_project_location(project_name: str, auto: bool = False) -> str:
    ...
    if project_info['should_continue']:
        if auto:
            return current_dir
        ...
        if RICH_AVAILABLE:
            ...
            click.echo(click.style(f"\n[EXISTING PROJECT] {project_info['project_type'] or 'Project'} detected", fg="yellow", bold=True))
            click.echo(click.style(f"Working in: {current_dir}", fg="white"))
        
        while True:
            choice = click.prompt(
                click.style("Continue in current project directory?", fg="yellow") + " " +
                click.style("(y)es", fg="green") + " / " +
                click.style("(n)ew project", fg="blue"),
                type=str,
                default="y"
            ).lower().strip()
            ...
            if choice in ['y', 'yes', '']:
                ...
                click.echo(click.style("Please enter 'y' for yes or 'n' for new project", fg="red"))
