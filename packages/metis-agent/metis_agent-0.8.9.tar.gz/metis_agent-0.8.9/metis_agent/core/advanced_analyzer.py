...
{{
  ...
  "reasoning": "why you chose this classification"
    ...
    def _select_best_tools(self, capable_tools: List[str], query: str, tools_registry: Dict) -> List[str]:
        ...
        # Select tools with score > 0.6, maximum 3 tools
        selected_tools = []
        for tool_name, score in tool_scores:
            if score > 0.6 and len(selected_tools) < 3:
                selected_tools.append(tool_name)
        
        # Always include the highest scoring tool if it's above 0.4
        if not selected_tools and tool_scores and tool_scores[0][1] > 0.4:
