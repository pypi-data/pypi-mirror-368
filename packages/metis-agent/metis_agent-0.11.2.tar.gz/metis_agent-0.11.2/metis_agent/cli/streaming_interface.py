...
Return ONLY the complete file content, nothing else."""
    ...
    def _handle_project_analysis(self, analysis_type: str, session_id: Optional[str] = None) -> Generator[str, None, None]:
        ...
        click.echo(click.style("\n🔍 [PROJECT ANALYSIS] Analyzing project...", fg="cyan", bold=True))
        click.echo(click.style("├─ Scanning code files and structure", fg="blue"))
        ...
        try:
            ...
            if analysis_result['success']:
                ...
                click.echo(click.style("└─ Generating intelligent summary...", fg="blue"))
                
                # Stream the analysis summary
                self._stream_project_analysis_results(analysis_result)
                
            else:
                click.echo(click.style(f"└─ Analysis failed: {analysis_result.get('error', 'Unknown error')}", fg="red"))
                
        except Exception as e:
            click.echo(click.style(f"└─ Analysis error: {str(e)}", fg="red"))
    
    def _stream_project_analysis_results(self, analysis_result: Dict[str, Any]):
        ...
        if data.get('code_files'):
            click.echo(click.style("\n📁 Key Code Files:", fg="cyan", bold=True))
            ...
            for file_path, file_info in list(data['code_files'].items())[:5]:
                ...
                if file_info.get('functions'):
                    ...
                    click.echo(f"  Functions: {func_list}")
        ...
        if data.get('technologies'):
            ...
            click.echo(click.style(f"⚡ Technologies: {tech_list}", fg="yellow"))
        
        if data.get('main_files'):
            main_list = ', '.join(data['main_files'])
            click.echo(click.style(f"🚀 Entry Points: {main_list}", fg="blue"))
        ...
        if data.get('dependencies'):
            ...
            click.echo(click.style(f"📦 Dependencies: {dep_list}", fg="magenta"))
        
        click.echo(click.style("\n[COMPLETE] Project analysis finished", fg="green", bold=True))
