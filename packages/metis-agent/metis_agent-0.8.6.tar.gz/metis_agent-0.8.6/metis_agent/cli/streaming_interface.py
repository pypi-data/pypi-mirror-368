...
class GeminiStreamingInterface:
    ...
    def _get_write_permission(self, filename: str, content: str) -> bool:
        ...
        if RICH_AVAILABLE:
            ...
            try:
                syntax = Syntax(preview_content, file_ext, theme="monokai", line_numbers=False, background_color="default")
                content_panel = Panel(syntax, title="📄 Content Preview", border_style="dim")
            except:
                # Fallback without syntax highlighting
                content_panel = Panel(preview_content, title="📄 Content Preview", border_style="dim")
            
            # Main confirmation panel
            main_panel = Panel(
                file_table,
                title="✏️ [bold yellow]File Write Confirmation[/bold yellow]",
                title_align="left",
                border_style="yellow",
                padding=(0, 1)
            )
        ...
        if RICH_AVAILABLE:
            ...
            if os.path.exists(filepath):
                try:
                    ...
                    try:
                        syntax = Syntax(preview_content, file_ext, theme="monokai", line_numbers=False, background_color="default")
                        current_content_panel = Panel(syntax, title="📄 Current Content", border_style="dim")
                    except:
                        current_content_panel = Panel(preview_content, title="📄 Current Content", border_style="dim")
                    
                except:
                    edit_table.add_row("Current Size:", Text("Unable to read", style="red"))
                    current_content_panel = Panel("Unable to read current content", title="📄 Current Content", border_style="red")
            else:
                edit_table.add_row("Status:", Text("File does not exist", style="yellow"))
                current_content_panel = Panel("File will be created", title="📄 New File", border_style="green")
            
            # Show edit instructions
            instruction_text = instructions if len(instructions) <= 200 else instructions[:200] + "..."
            instruction_panel = Panel(instruction_text, title="📝 Edit Instructions", border_style="cyan")
            
            # Main confirmation panel
            main_panel = Panel(
                edit_table,
                title="✏️ [bold blue]File Edit Confirmation[/bold blue]",
                title_align="left",
                border_style="blue",
                padding=(0, 1)
            )
