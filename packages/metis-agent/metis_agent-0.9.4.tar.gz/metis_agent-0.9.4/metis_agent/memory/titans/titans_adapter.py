...
class TitansMemoryAdapter:
    ...
    def _get_sqlite_conversation_history(self, query: str, session_id: Optional[str] = None, max_results: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Add session-specific inputs first
            for row in session_inputs:
                content = row[0]
                if content and content.strip():
                    all_contents.append((content, "user_input", 1.0))  # Higher relevance for session data
            ...
            # Add general inputs if needed (prioritize recent over semantic similarity)
            for i, row in enumerate(general_inputs):
                content = row[0]
                if content and content.strip():
                    # For conversation continuity, prioritize recent messages over semantic similarity
                    # Recent messages get higher relevance scores (1.0 for most recent, decreasing)
                    recency_score = max(0.3, 1.0 - (i * 0.1))  # Start at 1.0, decrease by 0.1 per position
                    all_contents.append((content, "user_input", recency_score))
    ...
    def _build_memory_context(self, relevant_memories: List[Dict[str, Any]], max_tokens: int = 1000) -> str:
        ...
        for i, memory in enumerate(relevant_memories, 1):
            ...
            # Format memory entry
            entry = f"{i}. [{memory_type}] {content} (relevance: {relevance:.2f})"
