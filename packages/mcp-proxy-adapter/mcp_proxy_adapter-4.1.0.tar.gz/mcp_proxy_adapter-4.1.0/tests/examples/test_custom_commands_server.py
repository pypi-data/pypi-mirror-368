...
class TestRegisterCustomCommands:
    ...
    def test_register_custom_commands_all_enabled(self, mock_get_logger, mock_get_setting, mock_registry):
        ...
        # EchoCommand is always registered (1 call)
        # Custom commands: help and health (2 calls)
        # Conditional commands: data_transform, intercept, manual_echo (3 calls)
        assert mock_registry.register_custom_command.call_count == 2  # help and health
        assert mock_registry.register.call_count == 4  # echo + data_transform + intercept + manual_echo
        assert mock_logger.info.call_count >= 7  # Multiple log messages
    ...
    def test_register_custom_commands_all_disabled(self, mock_get_logger, mock_get_setting, mock_registry):
        ...
        # EchoCommand is always registered (1 call) regardless of settings
        # Custom commands (help, health) are disabled (0 calls)
        # Conditional commands are disabled (0 calls)
        assert mock_registry.register_custom_command.call_count == 0
        assert mock_registry.register.call_count == 1  # Only echo command
        assert mock_logger.info.call_count >= 3  # At least echo and total commands logs
