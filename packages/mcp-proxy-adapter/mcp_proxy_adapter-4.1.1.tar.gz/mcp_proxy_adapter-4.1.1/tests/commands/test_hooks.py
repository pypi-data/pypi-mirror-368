...
class TestHooks:
    ...
    def test_unregister_nonexistent_command_hooks(self, caplog):
        """Test unregistering hooks for commands that don't exist (covers lines 104-105, 122-123)."""
        def test_hook(context: HookContext):
            pass
        
        def other_hook(context: HookContext):
            pass
        
        # Register a hook for one command
        hooks.register_before_hook("existing", test_hook)
        hooks.register_after_hook("existing", test_hook)
        
        # Try to unregister a different hook from the same command
        hooks.unregister_before_hook("existing", other_hook)
        hooks.unregister_after_hook("existing", other_hook)
        
        # Verify warnings were logged
        assert "Hook not found for command: existing" in caplog.text

    def test_unregister_nonexistent_global_hooks(self, caplog):
        """Test unregistering global hooks that don't exist (covers lines 135-136, 148-149)."""
        def test_hook(context: HookContext):
            pass
        
        # Try to unregister global hooks that were never registered
        hooks.unregister_global_before_hook(test_hook)
        hooks.unregister_global_after_hook(test_hook)
        
        # Verify warnings were logged
        assert "Global before hook not found" in caplog.text
        assert "Global after hook not found" in caplog.text

    def test_unregister_hook_removes_empty_command(self):
        """Test that unregistering the last hook removes the command entry (covers lines 104-105, 122-123)."""
        def test_hook(context: HookContext):
            pass
        ...
        # Verify command is removed from hooks
        hook_info = hooks.get_hook_info()
        assert "test" not in hook_info["before_hooks"]
        assert "test" not in hook_info["after_hooks"]

    def test_global_hook_error_handling(self, caplog):
        """Test global hook error handling (covers lines 172-173, 209-210, 216-217)."""
        def error_global_hook(context: HookContext):
            raise Exception("Global hook error")
        
        # Register error hooks
        hooks.register_global_before_hook(error_global_hook)
        hooks.register_global_after_hook(error_global_hook)
        
        # Execute hooks - should not raise exception
        hooks.execute_before_hooks("test", {})
        hooks.execute_after_hooks("test", {}, SuccessResult(data={"test": "result"}))
        
        # Verify errors were logged
        assert "Error in global before hook for command test: Global hook error" in caplog.text
        assert "Error in global after hook for command test: Global hook error" in caplog.text

    def test_after_hook_error_handling(self, caplog):
        """Test after hook error handling (covers lines 209-210)."""
        def error_after_hook(context: HookContext):
            raise Exception("After hook error")
        
        hooks.register_after_hook("test", error_after_hook)
        
        # Execute hooks - should not raise exception
        hooks.execute_after_hooks("test", {}, SuccessResult(data={"test": "result"}))
        
        # Verify error was logged
        assert "Error in after hook for command test: After hook error" in caplog.text

    def test_clear_nonexistent_command_hooks(self):
        """Test clearing hooks for a command that doesn't exist (covers lines 216-217)."""
        # Clear hooks for nonexistent command - should not raise exception
        hooks.clear_hooks("nonexistent")
