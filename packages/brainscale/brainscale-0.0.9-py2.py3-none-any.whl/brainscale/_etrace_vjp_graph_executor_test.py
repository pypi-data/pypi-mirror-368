...
class TestETraceVjpGraphExecutor(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        executor = brainscale.ETraceVjpGraphExecutor(self.model, vjp_method='multi-step')
        self.assertEqual(executor.vjp_method, 'multi-step')

    def test_invalid_vjp_method(self):
        with self.assertRaises(AssertionError):
            brainscale.ETraceVjpGraphExecutor(self.model, vjp_method='invalid')
    ...
    def test_single_step_vs_multi_step(self):
        single_step_executor = brainscale.ETraceVjpGraphExecutor(self.model, vjp_method='single-step')
        multi_step_executor = brainscale.ETraceVjpGraphExecutor(self.model, vjp_method='multi-step')
