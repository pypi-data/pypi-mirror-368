...
def test_die_rolls():
    ...
    assert three_d6.die_rolls(a_rng) == [1, 2, 3]

    assert nd(0, 6).die_rolls(rng) == [0]
    assert minus(2).die_rolls(rng) == [2]


def test_to_plus():
    minus_dice = Dice(3, 4, Op.MINUS)
    plus_dice = minus_dice.to_plus()
    assert plus_dice.count == 3
