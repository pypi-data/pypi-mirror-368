...
class SatelliteMapGenerator:
    ...
    def generate_indicator_map(self):
        ...
        for indicator in self.indicator_columns:
            ...
            # 根据是否支持国标分级和是否有实测数据决定生成哪些类型的图
            if supports_grading and has_measured_data:
                map_types = [
                    "distribution",
                    "interpolation",
                ...
                logger.info(f"{indicator} 支持国标分级且有实测数据，将生成完整的图表集")
                ...
                # 无实测数据时，只生成散点图和插值图，不生成等级图
                map_types = ["distribution", "interpolation", "clean_interpolation_svg"]
                logger.info(
                    f"{indicator} 无实测数据，跳过等级图生成，colorbar将显示相对高低"
                )
...
def generate_distribution_indicator_map(
    ...
    Args:
        has_measured_data: 是否有实测数据，影响colorbar显示方式
    """
    logger.info(f"散点图 {indicator} - has_measured_data参数: {has_measured_data}")
    # 解包卫星数据信息
    img_width, img_height, img_obj = satellite_info
    ...
    # 根据是否有实测数据决定colorbar显示方式
    logger.info(
        f"散点图 {indicator} - colorbar设置: has_measured_data={has_measured_data}"
    )
    if not has_measured_data:
        # 无实测数据时，只显示"低"和"高"
