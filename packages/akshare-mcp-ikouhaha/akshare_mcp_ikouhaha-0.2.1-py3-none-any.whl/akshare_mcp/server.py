...
async def list_tools() -> List[Tool]:
    ...
    return [
        ...
        Tool(
            name="get_gold_data",
            description="获取黄金数据，支持期货和现货。Get gold data for futures and spot (XAUUSD).",
            inputSchema={
                "type": "object",
                "properties": {
                    "data_type": {
                        "type": "string",
                        "enum": ["futures", "spot", "spot_xauusd", "futures_5min"],
                        "default": "futures",
                        "description": "数据类型：futures中国期货，spot国际现货(XAUUSD)，spot_xauusd国际现货，futures_5min期货5分钟数据 (Data type)"
                    },
                    "period": {
                        "type": "string",
                        "enum": ["5min", "15min", "30min", "60min", "daily"],
...
async def get_gold_data(data_type: str = "futures", period: str = "daily") -> Dict[str, Any]:
    ...
    try:
        if data_type == "spot" or data_type == "spot_xauusd":
            # 嘗試獲取國際黃金現貨價格（XAUUSD）
            try:
                # 方法1：嘗試使用 akshare 的宏觀數據
                import akshare as ak
                
                # 嘗試獲取貴金屬相關數據
                try:
                    # 查看是否有能源貴金屬數據
                    spot_gold = ak.energy_oil_hist()  # 這是測試，看看是否有相關接口
                    result["data"] = {
                        "data_type": "spot",
                        "description": "國際黃金現貨價格（XAUUSD）",
                        "note": "暫時使用備用數據源，正在集成更準確的現貨黃金接口",
                        "price": "數據獲取中...",
                        "currency": "USD",
                        "unit": "盎司"
                    }
                    ...
                    result["data"] = {
                        "data_type": "spot",
                        "description": "國際黃金現貨價格（XAUUSD）",
                        "note": "現貨黃金數據接口開發中，目前建議使用期貨數據作為參考",
                        "alternative": "建議使用 data_type='futures' 獲取黃金期貨數據",
                        "currency": "USD", 
                        "unit": "盎司",
                ...
                result["data"] = {
                    "data_type": "spot",
                    "description": "國際黃金現貨價格（XAUUSD）",
                    "error": f"現貨黃金數據暫時無法獲取: {str(e)}",
                    "alternative": "建議使用 data_type='futures' 獲取黃金期貨數據"
                }
                
        elif data_type == "futures" or data_type == "futures_5min":
            # 黃金期貨數據
            ...
            result = gold_data
            result["data"]["description"] = f"黃金期貨{period}數據（上海黃金交易所）"
            result["data"]["note"] = "這是中國黃金期貨數據，以人民幣/克計價"
            
        else:
            result["data"] = {
                "description": "黃金數據類型說明",
                "available_types": {
                    "futures": "黃金期貨數據（AU0主力合約）",
                    "futures_5min": "黃金期貨5分鐘數據",
                    "spot": "國際黃金現貨（開發中）"
                },
                "note": "目前推薦使用 futures 或 futures_5min 獲取黃金交易數據"
            }
