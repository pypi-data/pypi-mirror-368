...
def plot(df: pd.DataFrame, metrics_to_plot: list, group_by=['dataset', 'language']):
    """
    Plot evaluation metrics for any DataFrame grouped by dataset and language.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame containing at least: dataset, language, and metric columns.
    metrics_to_plot : list
        List of metric column names to plot (e.g., ['accuracy', 'f1']).
    group_by : list
        Columns to group by before plotting (default: ['dataset', 'language']).
    """
    ...
    # Group by dataset_language and compute mean for metrics
