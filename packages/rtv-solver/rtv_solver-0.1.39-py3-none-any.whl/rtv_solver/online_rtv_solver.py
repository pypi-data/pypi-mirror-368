...
class OnlineRTVSolver:
    ...
    def __init__(self,server_url,SHAREABLE_COST_FACTOR=10,RTV_TIMEOUT=20, LARGEST_TSP = 8, MAX_CARDINALITY = 8, ):
    ...
    def solve_pdptw_rtv(self, payload):
        ...
        try:
            ...
            error_message = "Error in TripHandler: " + str(e)
    ...
    def simulate_manifest(self, current_time, driver_runs, intermediate_location=True):
        ...
        for driver_run in driver_runs:
            ...
            state[PayloadParser.DRIVER_STATE_LOC] = copy.deepcopy(next_immediate_loc)
    ...
    def solve_pdptw(self, payload, skip_swapping=True, skip_rtv=False):
        if not skip_rtv:
            remaining_requests = []
            for driver_run in payload["driver_runs"]:
        ...
        if len(unserved_requests) > 0:
            ...
            new_request = payload["requests"][0]
        ...
        swap_handler = SwapHandler(self.server_url,updated_driver_runs,payload["depot"],self.DWELL_PICKUP, self.DWELL_ALIGHT, self.MAX_THREAD_CNT, new_request=new_request)
        swaped_driver_runs, reduced_cost, no_of_swaps = swap_handler.run_swap()
        while reduced_cost != 0 and time.time() - start_time < self.RTV_TIMEOUT:
    ...
    def evaluate_insertion(args):
        i, j, remaining_stops, pickup_stop, dropoff_stop, start_time, start_node, load, state, objective, depot, end_time, dwell_pickup, dwell_alight, tt_matrix = args
        ...
        for stop in new_manifest:
            ...
            travel_time = NetworkHandler.travel_time_from_matrix(tt_matrix, current_node, next_node)
            cost += travel_time
        ...
        if current_time+NetworkHandler.travel_time_from_matrix(tt_matrix, current_node,depot) > end_time:
            return float("inf"), None
