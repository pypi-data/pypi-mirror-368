...
class gasket_stress_XY(Operator):
    r"""Read/compute elemental gasket stress XY shear component (01 component)
    ...
    >>> op = dpf.operators.result.gasket_stress_XY()
    ...
    def __init__(
        ...
        super().__init__(name="GKSXY", config=config, server=server)
        self._inputs = InputsGasketStressXy(self)
        self._outputs = OutputsGasketStressXy(self)
    ...
    def _spec() -> Specification:
        description = r"""Read/compute elemental gasket stress XY shear component (01 component)
by calling the readers defined by the datasources. Regarding the
requested location and the input mesh scoping, the result location can
be Nodal/ElementalNodal/Elemental.
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="GKSXY", server=server)

    @property
    def inputs(self) -> InputsGasketStressXy:
...
class InputsGasketStressXy(_Inputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(gasket_stress_XY._spec().inputs, op)
        self._time_scoping = Input(gasket_stress_XY._spec().input_pin(0), 0, op, -1)
        self._inputs.append(self._time_scoping)
        self._mesh_scoping = Input(gasket_stress_XY._spec().input_pin(1), 1, op, -1)
        self._inputs.append(self._mesh_scoping)
        self._fields_container = Input(gasket_stress_XY._spec().input_pin(2), 2, op, -1)
        self._inputs.append(self._fields_container)
        self._streams_container = Input(
            gasket_stress_XY._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources = Input(gasket_stress_XY._spec().input_pin(4), 4, op, -1)
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global = Input(
            gasket_stress_XY._spec().input_pin(5), 5, op, -1
        )
        self._inputs.append(self._bool_rotate_to_global)
        self._mesh = Input(gasket_stress_XY._spec().input_pin(7), 7, op, -1)
        self._inputs.append(self._mesh)
        self._requested_location = Input(
            gasket_stress_XY._spec().input_pin(9), 9, op, -1
        )
        self._inputs.append(self._requested_location)
        self._read_cyclic = Input(gasket_stress_XY._spec().input_pin(14), 14, op, -1)
        self._inputs.append(self._read_cyclic)
        self._read_beams = Input(gasket_stress_XY._spec().input_pin(22), 22, op, -1)
...
class OutputsGasketStressXy(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(gasket_stress_XY._spec().outputs, op)
        self._fields_container = Output(gasket_stress_XY._spec().output_pin(0), 0, op)
