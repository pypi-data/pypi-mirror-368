...
class gasket_thermal_closure(Operator):
    ...
    def __init__(
        ...
        super().__init__(name="GKTH", config=config, server=server)
        self._inputs = InputsGasketThermalClosure(self)
        self._outputs = OutputsGasketThermalClosure(self)
...
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="GKTH", server=server)

    @property
    def inputs(self) -> InputsGasketThermalClosure:
...
class InputsGasketThermalClosure(_Inputs):
    ...
    gasket_thermal_closure operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(gasket_thermal_closure._spec().inputs, op)
        self._time_scoping = Input(
            gasket_thermal_closure._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping = Input(
            gasket_thermal_closure._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container = Input(
            gasket_thermal_closure._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container = Input(
            gasket_thermal_closure._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources = Input(
            gasket_thermal_closure._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global = Input(
            gasket_thermal_closure._spec().input_pin(5), 5, op, -1
        )
        self._inputs.append(self._bool_rotate_to_global)
        self._mesh = Input(gasket_thermal_closure._spec().input_pin(7), 7, op, -1)
        ...
        self._split_shells = Input(
            gasket_thermal_closure._spec().input_pin(26), 26, op, -1
        )
...
class OutputsGasketThermalClosure(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(gasket_thermal_closure._spec().outputs, op)
        self._fields_container = Output(
            gasket_thermal_closure._spec().output_pin(0), 0, op
        )
