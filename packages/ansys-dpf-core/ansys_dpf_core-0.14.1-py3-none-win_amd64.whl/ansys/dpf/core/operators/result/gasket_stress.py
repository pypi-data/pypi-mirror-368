...
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="GKS", server=server)

    @property
    def inputs(self) -> InputsGasketStress:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsGasketStress.
        """
        return super().inputs

    @property
    def outputs(self) -> OutputsGasketStress:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsGasketStress.
        """
        return super().outputs


class InputsGasketStress(_Inputs):
    """Intermediate class used to connect user inputs to
    gasket_stress operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(gasket_stress._spec().inputs, op)
        self._time_scoping = Input(gasket_stress._spec().input_pin(0), 0, op, -1)
        self._inputs.append(self._time_scoping)
        self._mesh_scoping = Input(gasket_stress._spec().input_pin(1), 1, op, -1)
        self._inputs.append(self._mesh_scoping)
        self._fields_container = Input(gasket_stress._spec().input_pin(2), 2, op, -1)
        self._inputs.append(self._fields_container)
        self._streams_container = Input(gasket_stress._spec().input_pin(3), 3, op, -1)
        self._inputs.append(self._streams_container)
        self._data_sources = Input(gasket_stress._spec().input_pin(4), 4, op, -1)
        self._inputs.append(self._data_sources)
        self._bool_rotate_to_global = Input(
            gasket_stress._spec().input_pin(5), 5, op, -1
        )
        self._inputs.append(self._bool_rotate_to_global)
        self._mesh = Input(gasket_stress._spec().input_pin(7), 7, op, -1)
...
class OutputsGasketStress(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(gasket_stress._spec().outputs, op)
        self._fields_container = Output(gasket_stress._spec().output_pin(0), 0, op)
