...
from globalgenie.api.schemas.evals import EvalType
from globalgenie.eval.utils import async_log_eval_run, log_eval_run, store_result_in_file
from globalgenie.exceptions import EvalError
from globalgenie.models.base import Model
from globalgenie.team.team import Team
from globalgenie.utils.log import logger, set_log_level_to_debug, set_log_level_to_info
...
class AccuracyEval:
    ...
    # Enable debug logs
    debug_mode: bool = getenv("GLOBALGENIE_DEBUG", "false").lower() == "true"
    # Log the results to the GlobalGenie platform. On by default.
    monitoring: bool = getenv("GLOBALGENIE_MONITOR", "true").lower() == "true"
    ...
    def get_evaluator_agent(self) -> Agent:
        ...
        if model is None:
            try:
                ...
                logger.exception(e)
                raise EvalError(
                    "GlobalGenie uses `openai` as the default model provider. Please run `pip install openai` to use the default evaluator."
                )
...
""",
    ...
    def run(
        ...
        *,
        ...
        # Log results to the GlobalGenie platform if requested
        if self.agent is not None:
