"""Async router handling exposing an GlobalGenie Agent or Team in an AG-UI compatible format."""
...
from globalgenie.app.agui.utils import (
    async_stream_globalgenie_response_as_agui_events,
    convert_agui_messages_to_globalgenie_messages,
)
...
async def run_agent(agent: Agent, run_input: RunAgentInput) -> AsyncIterator[BaseEvent]:
    """Run the contextual Agent, mapping AG-UI input messages to GlobalGenie format, and streaming the response in AG-UI format."""
    run_id = run_input.run_id or str(uuid.uuid4())

    try:
        # Preparing the input for the Agent and emitting the run started event
        messages = convert_agui_messages_to_globalgenie_messages(run_input.messages or [])
        ...
        async for event in async_stream_globalgenie_response_as_agui_events(
...
async def run_team(team: Team, input: RunAgentInput) -> AsyncIterator[BaseEvent]:
    """Run the contextual Team, mapping AG-UI input messages to GlobalGenie format, and streaming the response in AG-UI format."""
    run_id = input.run_id or str(uuid.uuid4())
    try:
        # Extract the last user message for team execution
        messages = convert_agui_messages_to_globalgenie_messages(input.messages or [])
