...
from globalgenie.api.schemas.playground import PlaygroundEndpointCreate
...
def deploy_playground_archive(name: str, tar_path: Path) -> bool:
    ...
    if token := read_auth_token():
        headers[globalgenie_cli_settings.auth_token_header] = token
    if globalgenie_api_key := getenv(GLOBALGENIE_API_KEY_ENV_VAR):
        headers["Authorization"] = f"Bearer {globalgenie_api_key}"

    try:
        with (
            HttpxClient(base_url=globalgenie_cli_settings.api_url, headers=headers) as api_client,
