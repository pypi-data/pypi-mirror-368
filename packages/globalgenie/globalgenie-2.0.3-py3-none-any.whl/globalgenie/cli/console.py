...
def log_config_not_available_msg() -> None:
    logger.error("GlobalGenie config not found, please run `gg init` and try again")


def log_active_workspace_not_available() -> None:
    logger.error("Could not find an active workspace. You can:")
    logger.error("- Run `gg ws setup` to setup a workspace at the current path")
    logger.error("- Run `gg ws create` to create a new workspace")
