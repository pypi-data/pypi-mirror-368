...
from globalgenie.constants import GLOBALGENIE_API_KEY_ENV_VAR
...
class Api:
    def __init__(self):
        self.headers: Dict[str, str] = {
            "user-agent": f"{globalgenie_cli_settings.app_name}/{globalgenie_cli_settings.app_version}",
    ...
    def authenticated_headers(self) -> Dict[str, str]:
        if self._authenticated_headers is None:
            ...
            if token is not None:
                self._authenticated_headers[globalgenie_cli_settings.auth_token_header] = token
            globalgenie_api_key = getenv(GLOBALGENIE_API_KEY_ENV_VAR)
            if globalgenie_api_key is not None:
                self._authenticated_headers["Authorization"] = f"Bearer {globalgenie_api_key}"
        return self._authenticated_headers

    def Client(self) -> HttpxClient:
        return HttpxClient(
            base_url=globalgenie_cli_settings.api_url,
