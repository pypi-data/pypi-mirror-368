...
class GlobalGenieLogger(logging.Logger):
...
def build_logger(logger_name: str, source_type: Optional[str] = None) -> Any:
    ...
    logging.setLoggerClass(GlobalGenieLogger)
    ...
    rich_handler = ColoredRichHandler(
        ...
        show_path=True if getenv("GLOBALGENIE_API_RUNTIME") == "dev" else False,
...
agent_logger: GlobalGenieLogger = build_logger(LOGGER_NAME, source_type="agent")
team_logger: GlobalGenieLogger = build_logger(TEAM_LOGGER_NAME, source_type="team")
workflow_logger: GlobalGenieLogger = build_logger(WORKFLOW_LOGGER_NAME, source_type="workflow")

# Set the default logger to the agent logger
logger: GlobalGenieLogger = agent_logger
