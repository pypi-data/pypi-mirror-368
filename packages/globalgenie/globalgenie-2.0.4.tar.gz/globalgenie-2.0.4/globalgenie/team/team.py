...
from globalgenie.agent.metrics import SessionMetrics
from globalgenie.exceptions import ModelProviderError, RunCancelledException
from globalgenie.knowledge.agent import AgentKnowledge
from globalgenie.media import Audio, AudioArtifact, AudioResponse, File, Image, ImageArtifact, Video, VideoArtifact
from globalgenie.memory.agent import AgentMemory
from globalgenie.memory.team import TeamMemory, TeamRun
from globalgenie.memory.v2.memory import Memory, SessionSummary
...
from globalgenie.reasoning.step import NextAction, ReasoningStep, ReasoningSteps
from globalgenie.run.base import RunResponseExtraData, RunStatus
from globalgenie.run.messages import RunMessages
from globalgenie.run.response import RunEvent, RunResponse, RunResponseEvent
from globalgenie.run.team import TeamRunEvent, TeamRunResponse, TeamRunResponseEvent, ToolCallCompletedEvent
...
from globalgenie.utils.events import (
...
from globalgenie.utils.log import (
    log_debug,
    log_error,
    log_exception,
    log_info,
...
from globalgenie.utils.message import get_text_from_message
from globalgenie.utils.response import (
...
from globalgenie.utils.safe_formatter import SafeFormatter
from globalgenie.utils.string import is_valid_uuid, parse_response_model_str, url_safe_string
...
class Team:
    ...
    # If `response_model` is set, sets the response mode of the model,
    # i.e. if the model should explicitly respond with a JSON object instead of a Pydantic model
    ...
    # monitoring=True logs Team information to globalgenie.com for monitoring
    ...
    def _set_monitoring(self) -> None:
        ...
        telemetry_env = getenv("GLOBALGENIE_TELEMETRY")
    ...
    def _set_default_model(self) -> None:
        ...
        if self.model is None:
            try:
                ...
                log_exception(e)
                log_error(
                    "GlobalGenie agents use `openai` as the default model provider. "
    ...
    def _make_memories_and_summaries(
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            if self.enable_session_summaries:
                futures.append(
                    executor.submit(self.memory.create_session_summary, session_id=session_id,
    ...
    def _print_response(
        ...
        **kwargs: Any,
        ...
        from globalgenie.utils.response import format_tool_calls
                ...
                if isinstance(resp, tuple(get_args(TeamRunResponseEvent))):
                    if resp.event == TeamRunEvent.run_response_content:
                        if isinstance(resp.content, str):
                            ...
                            try:
                                _response_content = JSON(resp.content.model_dump_json(exclude_none=True),
    ...
    def _reason(
        ...
        if reasoning_model_provided:
            from globalgenie.reasoning.azure_ai_foundry import is_ai_foundry_reasoning_model
            from globalgenie.reasoning.deepseek import is_deepseek_reasoning_model
            from globalgenie.reasoning.groq import is_groq_reasoning_model
            from globalgenie.reasoning.helpers import get_reasoning_agent
            from globalgenie.reasoning.ollama import is_ollama_reasoning_model
            from globalgenie.reasoning.openai import is_openai_reasoning_model
            ...
            if is_deepseek or is_groq or is_openai or is_ollama or is_ai_foundry:
                ...
                if is_deepseek:
                    from globalgenie.reasoning.deepseek import get_deepseek_reasoning
                    ...
                    from globalgenie.reasoning.groq import get_groq_reasoning
                    ...
                    from globalgenie.reasoning.openai import get_openai_reasoning
                    ...
                    from globalgenie.reasoning.ollama import get_ollama_reasoning
                    ...
                    from globalgenie.reasoning.azure_ai_foundry import get_ai_foundry_reasoning
                ...
                log_warning(
                    ...
                    f"defaulting to manual Chain-of-Thought reasoning"
                )
        ...
        if use_default_reasoning:
            from globalgenie.reasoning.default import get_default_reasoning_agent
            from globalgenie.reasoning.helpers import get_next_action, update_messages_with_reasoning
    ...
    async def _areason(
        ...
        if reasoning_model_provided:
            ...
            if is_deepseek or is_groq or is_openai or is_ollama or is_ai_foundry:
                reasoning_message: Optional[Message] = None
                if is_deepseek:
                    from globalgenie.reasoning.deepseek import aget_deepseek_reasoning
                    ...
                    from globalgenie.reasoning.groq import aget_groq_reasoning
                    ...
                    from globalgenie.reasoning.openai import aget_openai_reasoning
    ...
    def get_system_message(
        ...
        if self.add_location_to_instructions:
            from globalgenie.utils.location import get_location
        ...
        if self.knowledge is not None and self.enable_agentic_knowledge_filters:
            ...
            if valid_filters:
                ...
                additional_information.append(
                    ...
                    Examples:
                    1. If the user asks about a specific person like "Jordan Mitchell",
                        you MUST use the search_knowledge_base tool with the filters parameter set to {{'<valid key like user_id>': '<valid value based on the user query>'}}.
                    2. If the user asks about a specific document type like "contracts",
                        you MUST use the search_knowledge_base tool with the filters parameter set to {{'document_type': 'contract'}}.
                    4. If the user asks about a specific location like "documents from New York",
                        you MUST use the search_knowledge_base tool with the filters parameter set to {{'<valid key like location>': 'New York'}}.
                    General Guidelines:
                    - Always analyze the user query to identify relevant metadata.
                    - Use the most specific filter(s) possible to narrow down results.
                    - If multiple filters are relevant, combine them in the filters parameter (e.g.,
                        {{'name': 'Jordan Mitchell', 'document_type': 'contract'}}).
        ...
        if self.get_member_information_tool:
            system_message_content += "If you need to get information about your team members,
                you can use the `get_member_information` tool at any time.\n"
        system_message_content += "</team_members>\n"
        ...
        if self.mode == "coordinate":
            system_message_content += (
                ...
                "  - member_id (str): The ID of the member to transfer the task to. Use only the ID of the member,
                    not the ID of the team followed by the ID of the member.\n"
                "  - task_description (str): A clear description of the task.\n"
                "  - expected_output (str): The expected output.\n"
                "- You can transfer tasks to multiple members at once.\n"
                "- You must always analyze the responses from members before responding to the user.\n"
                "- After analyzing the responses from the members, if you feel the task has been completed,
                    you can stop and respond to the user.\n"
            ...
            system_message_content += (
                ...
                "  - member_id (str): The ID of the member to forward the task to. Use only the ID of the member,
                    not the ID of the team followed by the ID of the member.\n"
                "  - expected_output (str): The expected output.\n"
            ...
            system_message_content += (
                ...
                "- To run the members in your team,
                    call `run_member_agents` ONLY once. This will run all members in your team.\n"
        ...
        if self.memory:
            if isinstance(self.memory, Memory) and self.add_memory_references:
                ...
                if self.enable_agentic_memory:
                    system_message_content += (
                        ...
                        "You can use the `update_user_memory` tool to add new memories, update existing memories,
                            delete memories, or clear all memories.\n"
            ...
            if isinstance(self.memory, Memory) and self.add_session_summary_references:
                if not user_id:
                    user_id = "default"
                session_summary: SessionSummary = self.memory.summaries.get(user_id, {}).get(session_id,
                    None)  # type: ignore
                    ...
                    try:
                        if member_agent_run_response.content is None and (
                            ...
                            if len(member_agent_run_response.content.strip()) > 0:
                                ...
                                yield f"Agent {member_agent.name}: {',
                                    '.join([tool.result for tool in member_agent_run_response.tools])}"  # type: ignore
                ...
                try:
                    if member_agent_run_response.content is None and (
                        ...
                        # If the content is empty but we have tool calls
                            ...
                            yield ",
                                ".join([tool.result for tool in member_agent_run_response.tools if tool.result])  # type: ignore
                    elif issubclass(type(member_agent_run_response.content), BaseModel):
                        yield member_agent_run_response.content.model_dump_json(indent=2)  # type: ignore
                    else:
                        import json
    ...
    def get_forward_task_function(
        ...
        def forward_task_to_member(
            ...
            # Since we return the response directly from the member agent,
                # we need to set the response model from the team down.
        ...
        if session.session_data is not None:
            ...
            if "session_state" in session.session_data:
                ...
                if (
                    ...
                    # If the session_state is already set,
                        # merge the session_state from the database with the current session_state
            ...
            if "team_session_state" in session.session_data:
                ...
                if (
                    ...
                    # If the team_session_state is already set,
                        # merge the team_session_state from the database with the current team_session_state
            ...
            if "workflow_session_state" in session.session_data:
                ...
                if (
                    ...
                    # If the workflow_session_state is already set,
                        # merge the workflow_session_state from the database with the current workflow_session_state
            ...
            if isinstance(self.memory, TeamMemory):
                try:
                    ...
                    if "memories" in session.memory:
                        from globalgenie.memory.memory import Memory as UserMemoryV1
                ...
                if "team_context" in session.memory:
                    from globalgenie.memory.v2.memory import TeamContext
                ...
                if "memories" in session.memory:
                    if self.memory.memories is not None:
                        ...
                        from globalgenie.memory.v2.memory import UserMemory as UserMemoryV2
                ...
                if "summaries" in session.memory:
                    if self.memory.summaries is not None:
                        ...
                        from globalgenie.memory.v2.memory import SessionSummary as SessionSummaryV2
    ...
    def _add_reasoning_step_to_extra_data(self, run_response: TeamRunResponse, reasoning_step: ReasoningStep) -> None:
        if run_response.extra_data is None:
            from globalgenie.run.response import RunResponseExtraData
    ...
    def _log_team_run(self, session_id: str, user_id: Optional[str] = None) -> None:
        ...
        from globalgenie.api.team import TeamRunCreate, create_team_run
    ...
    async def _alog_team_run(self, session_id: str, user_id: Optional[str] = None) -> None:
        ...
        from globalgenie.api.team import TeamRunCreate, acreate_team_run
    ...
    def _log_team_session(self, session_id: str, user_id: Optional[str] = None):
        ...
        from globalgenie.api.team import TeamSessionCreate, upsert_team_session
    ...
    def _get_agentic_or_user_search_filters(
        ...
        # If both agentic filters exist and manual filters (passed by user) exist,
            use manual filters (give priority to user and override)
