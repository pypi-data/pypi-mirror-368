...
from globalgenie.api.schemas.user import TeamSchema, UserSchema
from globalgenie.api.schemas.workspace import WorkspaceSchema
...
class GlobalGenieCliConfig:
    """The GlobalGenieCliConfig class manages user data for the globalgenie cli"""
    ...
    def clear_user_cache(self) -> None:
        ...
        globalgenie_cli_settings.ai_conversations_path.unlink(missing_ok=True)
        logger.info("Workspaces cleared. If applicable, please setup your workspace using `gg ws setup`")
    ...
    def _add_or_update_ws_config(
        ...
        if existing_ws_config is None:
            logger.error(f"Could not find workspace at: {ws_root_str}, please run `gg ws setup`")
            return None
    ...
    def add_new_ws_to_config(
        ...
        """Adds a newly created workspace to the GlobalGenieCliConfig"""
    ...
    def delete_ws(self, ws_root_path: Path) -> None:
        """Handles Deleting a workspace from the GlobalGenieCliConfig and api"""
        ...
        print_info("Note: this does not delete any data locally or from globalgenie.com, please delete them manually\n")
    ...
    def save_config(self):
        ...
        write_json_file(file_path=globalgenie_cli_settings.config_file_path, data=config_data)

    @classmethod
    def from_saved_config(cls) -> Optional["GlobalGenieCliConfig"]:
        try:
            config_data = read_json_file(file_path=globalgenie_cli_settings.config_file_path)
            ...
            logger.warning(e)
            logger.warning("Please setup the workspace using `gg ws setup`")
            return None

    #
    #  Print GlobalGenieCliConfig
    #
    ...
    def print_to_cli(self, show_all: bool = False):
        ...
        if self.active_ws_dir:
            ...
            print_info(
                "Please create a workspace using `gg ws create` or setup an existing workspace using `gg ws setup`"
            )
