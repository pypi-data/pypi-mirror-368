"""GlobalGenie Workspace Cli

This is the entrypoint for the `globalgenie ws` application.
"""
...
import typer
from globalgenie.cli.console import (
    log_active_workspace_not_available,
    log_config_not_available_msg,
    print_available_workspaces,
    print_info,
)
from globalgenie.utils.log import logger, set_log_level_to_debug

ws_cli = typer.Typer(
    name="ws",
    short_help="Manage workspaces",
    help="""\b
Use `gg ws [COMMAND]` to create, setup, start or stop your workspace.
Run `gg ws [COMMAND] --help` for more info.
""",
...
def create(
    ...
    \b
    Examples:
    > gg ws create -t ai-app                -> Create an `ai-app` in the current directory
    > gg ws create -t ai-app -n my-ai-app   -> Create an `ai-app` named `my-ai-app` in the current directory
    """
    if print_debug_log:
        set_log_level_to_debug()

    from globalgenie.workspace.operator import create_workspace
...
def setup(
    ...
    \b
    Examples:
    > `gg ws setup`           -> Setup the current directory as a workspace
    > `gg ws setup ai-app`    -> Setup the `ai-app` folder as a workspace
    """
    if print_debug_log:
        set_log_level_to_debug()

    from globalgenie.workspace.operator import setup_workspace
...
def up(
    ...
    \b
    Examples:
    > `gg ws up`            -> Deploy all resources
    > `gg ws up dev`        -> Deploy all dev resources
    > `gg ws up prd`        -> Deploy all prd resources
    > `gg ws up prd:aws`    -> Deploy all prd aws resources
    > `gg ws up prd:::s3`   -> Deploy prd resources matching name s3
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from globalgenie.utils.resource_filter import parse_resource_filter
    from globalgenie.workspace.config import WorkspaceConfig
    from globalgenie.workspace.helpers import get_workspace_dir_path
    from globalgenie.workspace.operator import setup_workspace, start_workspace
    ...
    ws_at_current_path: Optional[WorkspaceConfig] = globalgenie_config.get_ws_config_by_path(current_path)
    if ws_at_current_path is not None:
        logger.debug(f"Found workspace at: {ws_at_current_path.ws_root_path}")
        if str(ws_at_current_path.ws_root_path) != globalgenie_config.active_ws_dir:
            logger.debug(f"Updating active workspace to {ws_at_current_path.ws_root_path}")
            globalgenie_config.set_active_ws_dir(ws_at_current_path.ws_root_path)
    ...
    if ws_to_start is None:
        active_ws_config: Optional[WorkspaceConfig] = globalgenie_config.get_active_ws_config()
    ...
    if ws_to_start is None:
        log_active_workspace_not_available()
        avl_ws = globalgenie_config.available_ws
        if avl_ws:
...
def down(
    ...
    \b
    Examples:
    > `gg ws down`            -> Delete all resources
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from globalgenie.workspace.config import WorkspaceConfig
    from globalgenie.workspace.helpers import get_workspace_dir_path
    from globalgenie.workspace.operator import setup_workspace, stop_workspace
...
def patch(
    ...
    Examples:
    \b
    > `gg ws patch`           -> Patch all resources
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from globalgenie.workspace.config import WorkspaceConfig
    from globalgenie.workspace.helpers import get_workspace_dir_path
    from globalgenie.workspace.operator import setup_workspace, update_workspace
...
def restart(
    ...
    """\b
    Restarts the active workspace. i.e. runs `gg ws down` and then `gg ws up`.
...
def config(
    ...
    \b
    Examples:
    $ `gg ws config`         -> Print the active workspace config
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from globalgenie.cli.config import GlobalGenieCliConfig
    from globalgenie.cli.operator import initialize_globalgenie
    from globalgenie.utils.load_env import load_env
    from globalgenie.workspace.config import WorkspaceConfig
...
def delete(
    ...
    all_workspaces: bool = typer.Option(
        ...
        "-a",
        "--all",
        help="Delete all workspaces from GlobalGenie",
    ),
    print_debug_log: bool = typer.Option(
        False,
        "-d",
    ...
    """\b
    Deletes the workspace record from globalgenie.
    NOTE: Does not delete any physical files.

    \b
    Examples:
    $ `gg ws delete`         -> Delete the active workspace from GlobalGenie
    $ `gg ws delete -a`      -> Delete all workspaces from GlobalGenie
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from globalgenie.cli.config import GlobalGenieCliConfig
    from globalgenie.cli.operator import initialize_globalgenie
    from globalgenie.workspace.operator import delete_workspace
    ...
    if ws_name is not None:
        ws_config = globalgenie_config.get_ws_config_by_dir_name(ws_name)
        ...
        if all_workspaces:
            ws_to_delete = [ws.ws_root_path for ws in globalgenie_config.available_ws if ws.ws_root_path is not None]
        else:
            # By default, we assume this command is run for the active workspace
            if globalgenie_config.active_ws_dir is not None:
                ws_to_delete.append(Path(globalgenie_config.active_ws_dir))

    delete_workspace(globalgenie_config, ws_to_delete)
