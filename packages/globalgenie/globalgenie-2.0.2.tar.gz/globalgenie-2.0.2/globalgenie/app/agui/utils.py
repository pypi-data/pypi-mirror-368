...
from globalgenie.run.response import RunEvent, RunResponseContentEvent, RunResponseEvent, RunResponsePausedEvent
from globalgenie.run.team import RunResponseContentEvent as TeamRunResponseContentEvent
from globalgenie.run.team import TeamRunEvent, TeamRunResponseEvent


@dataclass
class EventBuffer:
    """Buffer to manage event ordering constraints, relevant when mapping GlobalGenie responses to AG-UI events."""
...
def convert_agui_messages_to_globalgenie_messages(messages: List[AGUIMessage]) -> List[Message]:
    """Convert AG-UI messages to GlobalGenie messages."""
    result = []
    for msg in messages:
        if msg.role == "tool":
...
def stream_globalgenie_response_as_agui_events(
    response_stream: Iterator[Union[RunResponseEvent, TeamRunResponseEvent]], thread_id: str, run_id: str
) -> Iterator[BaseEvent]:
    """Map the GlobalGenie response stream to AG-UI format, handling event ordering constraints."""
    message_id = str(uuid.uuid4())
...
async def async_stream_globalgenie_response_as_agui_events(
