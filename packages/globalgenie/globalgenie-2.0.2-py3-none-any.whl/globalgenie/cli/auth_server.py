...
class CliAuthRequestHandler(BaseHTTPRequestHandler):
    ...
    TODO:
        * Fix the header and limit to only localhost or globalgenie.com
    """
    ...
    def _redirect_with_status(self, theme: str, redirect_uri, result: str, error_type: str = ""):
        ...
        if theme == "dark":
            ...
            loader_color = "#FAFAFA"
            auth_svg_link = "https://globalgenie-public.s3.us-east-1.amazonaws.com/assets/Auth-darkmode.svg"
        else:
            background_color = "#FFFFFF"
            text_color_large = "#18181B"
            text_color_small = "rgba(113, 113, 122, 1)"
            loader_color = "#18181B"
            auth_svg_link = "https://globalgenie-public.s3.us-east-1.amazonaws.com/assets/Auth-lightmode.svg"

        html = f"""
        <html>
        <head>
            <title>GlobalGenie Workspace</title>
    ...
    def _store_token(self, auth_token: str):
        ...
        globalgenie_cli_settings.tmp_token_path.parent.mkdir(parents=True, exist_ok=True)
        globalgenie_cli_settings.tmp_token_path.touch(exist_ok=True)
        globalgenie_cli_settings.tmp_token_path.write_text(json.dumps({"AuthToken": auth_token}))
    ...
    def do_POST(self):
        ...
        globalgenie_cli_settings.tmp_token_path.parent.mkdir(parents=True, exist_ok=True)
        globalgenie_cli_settings.tmp_token_path.touch(exist_ok=True)
        globalgenie_cli_settings.tmp_token_path.write_text(decoded_post_data)
...
def get_auth_token_from_web_flow(port: int) -> Optional[str]:
    ...
    if globalgenie_cli_settings.tmp_token_path.exists() and globalgenie_cli_settings.tmp_token_path.is_file():
        auth_token_str = globalgenie_cli_settings.tmp_token_path.read_text()
        auth_token_json = json.loads(auth_token_str)
        globalgenie_cli_settings.tmp_token_path.unlink()
