...
from globalgenie.cli.console import print_heading, print_info
from globalgenie.cli.settings import GLOBALGENIE_CLI_CONFIG_DIR, globalgenie_cli_settings
...
def delete_globalgenie_config() -> None:
    from globalgenie.utils.filesystem import delete_from_fs

    logger.debug("Removing existing GlobalGenie configuration")
    delete_from_fs(GLOBALGENIE_CLI_CONFIG_DIR)


def authenticate_user() -> None:
    """Authenticate the user using credentials from globalgenie.com
    Steps:
    1. Authenticate the user by opening the globalgenie sign-in url.
        Once authenticated, globalgenie.com will post an auth token to a
        mini http server running on the auth_server_port.
    2. Using the auth_token, authenticate the user with the api.
    3. After the user is authenticated update the GlobalGenieCliConfig.
    4. Save the auth_token locally for future use.
    """
    from globalgenie.api.schemas.user import UserSchema
    from globalgenie.api.user import authenticate_and_get_user
    from globalgenie.cli.auth_server import get_auth_token_from_web_flow, get_port_for_auth_server
    from globalgenie.cli.credentials import save_auth_token

    print_heading("Authenticating with globalgenie.com")
    ...
    auth_url = "{}?source=cli&action=signin&redirection_supported=true&redirecturi={}".format(
        globalgenie_cli_settings.cli_auth_url, redirect_uri
    )
    ...
    if auth_token is None:
        logger.error("Could not authenticate, please set GLOBALGENIE_API_KEY or try again")
        return

    globalgenie_config: Optional[GlobalGenieCliConfig] = GlobalGenieCliConfig.from_saved_config()
    existing_user: Optional[UserSchema] = globalgenie_config.user if globalgenie_config is not None else None
    ...
    if globalgenie_config is None:
        globalgenie_config = GlobalGenieCliConfig(user)
        globalgenie_config.save_config()
    else:
        globalgenie_config.user = user

    print_info("Welcome {}".format(user.email))


def initialize_globalgenie(reset: bool = False, login: bool = False) -> Optional[GlobalGenieCliConfig]:
    """Initialize GlobalGenie on the users machine.

    Steps:
    1. Check if GLOBALGENIE_CLI_CONFIG_DIR exists, if not, create it. If reset == True, recreate GLOBALGENIE_CLI_CONFIG_DIR.
    2. Authenticates the user if login == True.
    3. If GlobalGenieCliConfig exists and auth is valid, returns GlobalGenieCliConfig.
    """
    from globalgenie.api.user import create_anon_user
    from globalgenie.utils.filesystem import delete_from_fs

    print_heading("Welcome to GlobalGenie!")
    if reset:
        delete_globalgenie_config()

    logger.debug("Initializing GlobalGenie")

    # Check if ~/.config/gg exists, if it is not a dir - delete it and create the directory
    if GLOBALGENIE_CLI_CONFIG_DIR.exists():
        logger.debug(f"{GLOBALGENIE_CLI_CONFIG_DIR} exists")
        if not GLOBALGENIE_CLI_CONFIG_DIR.is_dir():
            try:
                delete_from_fs(GLOBALGENIE_CLI_CONFIG_DIR)
            except Exception as e:
                logger.exception(e)
                raise Exception(f"Something went wrong, please delete {GLOBALGENIE_CLI_CONFIG_DIR} and run again")
            GLOBALGENIE_CLI_CONFIG_DIR.mkdir(parents=True, exist_ok=True)
    else:
        GLOBALGENIE_CLI_CONFIG_DIR.mkdir(parents=True)
        logger.debug(f"Created {GLOBALGENIE_CLI_CONFIG_DIR}")

    # Confirm GLOBALGENIE_CLI_CONFIG_DIR exists otherwise we should return
    if GLOBALGENIE_CLI_CONFIG_DIR.exists():
        logger.debug(f"GlobalGenie config location: {GLOBALGENIE_CLI_CONFIG_DIR}")
    else:
        raise Exception("Something went wrong, please try again")

    globalgenie_config: Optional[GlobalGenieCliConfig] = GlobalGenieCliConfig.from_saved_config()
    if globalgenie_config is None:
        logger.debug("Creating new GlobalGenieCliConfig")
    ...
    if login:
        ...
        if anon_user is not None and globalgenie_config is not None:
            globalgenie_config.user = anon_user

    logger.debug("GlobalGenie initialized")
    return globalgenie_config
