...
from globalgenie.api.schemas.user import TeamIdentifier, TeamSchema
from globalgenie.api.schemas.workspace import WorkspaceCreate, WorkspaceEvent, WorkspaceSchema, WorkspaceUpdate
from globalgenie.api.workspace import log_workspace_event
from globalgenie.cli.config import GlobalGenieCliConfig
from globalgenie.cli.console import console, log_config_not_available_msg, print_heading, print_info, print_subheading
from globalgenie.infra.resources import InfraResources
from globalgenie.utils.common import str_to_int
from globalgenie.utils.log import logger
from globalgenie.workspace.config import WorkspaceConfig
from globalgenie.workspace.enums import WorkspaceStarterTemplate
...
TEMPLATE_TO_REPO_MAP: Dict[WorkspaceStarterTemplate, str] = {
    WorkspaceStarterTemplate.agent_app: "https://github.com/globalgenie-agi/agent-app-aws",
    WorkspaceStarterTemplate.agent_api: "https://github.com/globalgenie-agi/agent-api-aws",
}
...
def create_workspace(
    ...
    from globalgenie.utils.filesystem import rmdir_recursive
    from globalgenie.utils.git import GitCloneProgress
    from globalgenie.workspace.helpers import get_workspace_dir_path

    current_dir: Path = Path(".").resolve()

    # Initialize GlobalGenie before creating a workspace
    ...
    if not globalgenie_config:
        ...
        if not globalgenie_config:
            ...
            return None
    globalgenie_config = cast(GlobalGenieCliConfig, globalgenie_config)
    ...
    globalgenie_config.add_new_ws_to_config(ws_root_path=ws_root_path)
...
def setup_workspace(ws_root_path: Path) -> Optional[WorkspaceConfig]:
    """Setup an GlobalGenie workspace at `ws_root_path` and return the WorkspaceConfig

    1. Pre-requisites
    1.1 Check ws_root_path exists and is a directory
    1.2 Create GlobalGenieCliConfig if needed
    ...
    2.1 Check if a ws_schema exists for this workspace, meaning this workspace has a record in globalgenie-api
    2.2 Create WorkspaceSchema if it doesn't exist
    2.3 Update WorkspaceSchema if git_url is updated
    """
    from globalgenie.cli.operator import initialize_globalgenie
    from globalgenie.utils.git import get_remote_origin_for_dir
    from globalgenie.workspace.helpers import get_workspace_dir_path
    ...
    ws_config: Optional[WorkspaceConfig] = globalgenie_config.get_ws_config_by_path(ws_root_path)
    if ws_config is None:
        # There's no record of this workspace, reasons:
        # - The user is setting up a new workspace
        # - The user ran `gg init -r` which erased existing workspaces
        ...
        ws_config = globalgenie_config.create_or_update_ws_config(ws_root_path=ws_root_path, set_as_active=True)
        if ws_config is None:
    ...
    if globalgenie_config.user is None:
        from globalgenie.api.user import create_anon_user
        ...
        if anon_user is not None:
            globalgenie_config.user = anon_user
    ...
    if globalgenie_config.user is not None:
        # 2.2 Create WorkspaceSchema if it doesn't exist
        if ws_schema is None or ws_schema.id_workspace is None:
            from globalgenie.api.workspace import create_workspace_for_user, get_teams_for_user
            ...
            logger.debug(f"Getting teams for user: {globalgenie_config.user.email}")
            ...
            with Live(transient=True) as live_log:
                ...
                live_log.update(status)
                teams = get_teams_for_user(globalgenie_config.user)
                status.stop()
            ...
            ws_config = globalgenie_config.create_or_update_ws_config(
        ...
        if git_remote_origin_url is not None and ws_schema is not None and ws_schema.git_url != git_remote_origin_url:
            from globalgenie.api.workspace import update_workspace_for_team, update_workspace_for_user
    ...
    if ws_config is not None:
        ...
        print_info("\tgg ws down")

        if ws_config.ws_schema is not None and globalgenie_config.user is not None:
            log_workspace_event(
                user=globalgenie_config.user,
...
def start_workspace(
    globalgenie_config: GlobalGenieCliConfig,
    ...
    """Start an GlobalGenie Workspace. This is called from `gg ws up`"""
    if ws_config is None:
    ...
    if (
        globalgenie_config.user is not None
...
def stop_workspace(
    ...
    """Stop an GlobalGenie Workspace. This is called from `gg ws down`"""
    if ws_config is None:
        logger.error("WorkspaceConfig invalid")
        return
...
def update_workspace(
    ...
    """Update an GlobalGenie Workspace. This is called from `gg ws patch`"""
    if ws_config is None:
        logger.error("WorkspaceConfig invalid")
        return
...
def delete_workspace(globalgenie_config: GlobalGenieCliConfig, ws_to_delete: Optional[List[Path]]) -> None:
    if ws_to_delete is None or len(ws_to_delete) == 0:
        print_heading("No workspaces to delete")
        return

    for ws_root in ws_to_delete:
        globalgenie_config.delete_ws(ws_root_path=ws_root)
...
def set_workspace_as_active(ws_dir_name: Optional[str]) -> None:
    ...
    #
    #
    # 1.1 Check GlobalGenieCliConfig is valid
    #
    globalgenie_config: Optional[GlobalGenieCliConfig] = GlobalGenieCliConfig.from_saved_config()
    if not globalgenie_config:
        globalgenie_config = initialize_globalgenie()
    ...
    if ws_dir_name is None:
        # If the user does not provide a ws_name, that implies `gg set` is ran from
        # the workspace directory.
        ws_root_path = Path(".").resolve()
    else:
        # If the user provides a workspace name manually, we find the dir for that ws
        ws_config: Optional[WorkspaceConfig] = globalgenie_config.get_ws_config_by_dir_name(ws_dir_name)
        if ws_config is None:
    ...
    active_ws_config: Optional[WorkspaceConfig] = globalgenie_config.get_ws_config_by_path(ws_root_path)
    if active_ws_config is None:
        ...
        print_info("If this workspace has not been setup, please run `gg ws setup` from the workspace directory")
        return
    ...
    globalgenie_config.set_active_ws_dir(active_ws_config.ws_root_path)
