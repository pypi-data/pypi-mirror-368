...
class AdvancedOptions(dict):  # type: ignore[type-arg]
    ...
    response_cap : Optional[bool] or Optional[float in [0.5, 1)]
    ...
    scaleout_modeling_mode : Optional[str]
    ...
    allowed_pairwise_interaction_groups : Optional[List[Tuple[str, ...]]]
    ...
    default_monotonic_decreasing_featurelist_id : Optional[str]
        Returned from server on Project GET request - not able to be updated by user
    ...
    chunk_definition_id : Optional[str]
    ...
    def __init__(
        ...
        if model_group_id:
            deprecation_warning(
                subject="Parameter `model_group_id` in Advanced Options is deprecated",
                deprecated_since_version="3.6",
                will_remove_version="3.7",
                message="Please use parameter `series_id` instead.",
            )
            if not self.series_id:
                self.series_id = model_group_id

        if model_regime_id:
            deprecation_warning(
                subject="Parameter `model_regime_id` in Advanced Options is deprecated",
                deprecated_since_version="3.6",
                will_remove_version="3.7",
                message="Please use parameter `forecast_distance` instead.",
            )
            if not self.forecast_distance:
                self.forecast_distance = model_regime_id

        if model_baselines:
            deprecation_warning(
                subject="Parameter `model_baselines` in Advanced Options is deprecated",
                deprecated_since_version="3.6",
                will_remove_version="3.7",
                message="Please us parameter `forecast_offsets` instead.",
            )
            if not self.forecast_offsets:
                self.forecast_offsets = model_baselines
...
class ClassMappingAggregationSettings:
    ...
    Attributes
    ----------
    max_unaggregated_class_values : Optional[int]
        Maximum amount of unique values allowed before aggregation kicks in.
    min_class_support : Optional[int]
    ...
    excluded_from_aggregation : Optional[List]
        List of target values that should be guaranteed to kept as is,
        regardless of other settings.
    aggregation_class_name : Optional[str]
