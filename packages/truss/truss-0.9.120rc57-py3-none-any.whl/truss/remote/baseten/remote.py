...
class BasetenRemote(TrussRemote):
    ...
    def create_build_time_config(self, context_path_str: Path, truss_config: TrussConfig) -> None:
        """Create a build time config for the truss, excludes run-time only attributes."""
        tc = TrussConfig.from_yaml(
            context_path_str / "config.yaml"
        )
        if tc.docker_server:
            truss_config.environment_variables["BT_DOCKER_SERVER_START_COMMAND"] = truss_config.docker_server.start_command
