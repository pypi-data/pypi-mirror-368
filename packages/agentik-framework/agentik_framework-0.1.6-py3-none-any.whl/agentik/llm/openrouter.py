...
class OpenRouterClient:
    ...
    def _post_with_tool_fallback(self, payload: Dict[str, Any], stream: bool = False):
        """
        POST to /chat/completions. If 404 with 'support tool use' and payload
        contains tool params, retry once with tools removed and tool_choice='none'.
        Returns httpx.Response for non-streaming, or yields bytes for streaming.
        """
        url = f"{self.base_url}/chat/completions"
        headers = self._headers()

        def _has_tool_params(p: Dict[str, Any]) -> bool:
            if "tools" in p and p["tools"]:
                return True
            tc = p.get("tool_choice")
            return tc is not None and tc != "none"

        def _strip_tools(p: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if stream:
                with self._client.stream("POST", url, headers=headers, json=payload) as r:
                    try:
                        ...
                        # tool fallback for streaming
                        text = (e.response.text or "").lower()
                        if (
                            e.response.status_code == 404
                            and "support tool use" in text
                            and _has_tool_params(payload)
                        ):
                            payload2 = _strip_tools(payload)
    ...
    def chat(self, messages: List[Dict[str, str]], model: str, **params: Any) -> Dict[str, Any]:
        ...
        r = self._post_with_tool_fallback(payload, stream=False)
        data = r.json()
    ...
    def stream_chat(
        ...
        for line in self._post_with_tool_fallback(payload, stream=True):
