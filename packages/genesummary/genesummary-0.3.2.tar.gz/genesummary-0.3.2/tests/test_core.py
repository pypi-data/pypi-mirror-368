...
class TestGeneInfo:
    ...
    def test_get_gene_info_structure(
        ...
        mock_mygene_instance.get_gene_info.return_value = MOCK_GENE_DATA[
            "TP53"
        ]["basic_info"]
        ...
        assert any(
            i.get("source_database") == "STRING-db" for i in interactions
        )
    ...
    @patch("geneinfo.core.BioGRIDFetcher")
    @patch("geneinfo.core.StringDBFetcher")
    def test_protein_interactions_biogrid_only(
        self, mock_stringdb, mock_biogrid
    ):
        ...
        mock_stringdb_instance = Mock()
        mock_stringdb_instance.get_protein_interactions.side_effect = Exception(
            "Network error"
        )
        mock_stringdb.return_value = mock_stringdb_instance
        ...
        with (
            patch.object(gene_info, "ensembl_fetcher") as mock_ensembl,
            ...
            mock_ensembl.get_homologs.return_value = {
                "orthologs": [],
                "paralogs": [],
            }
    ...
    @patch("geneinfo.core.BioGRIDFetcher")
    @patch("geneinfo.core.StringDBFetcher")
    def test_protein_interactions_stringdb_only(
        self, mock_stringdb, mock_biogrid
    ):
