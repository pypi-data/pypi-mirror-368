...
BEGIN
    ...
    v_object_key := v_extension || '/' || NEW.document_id::text || '/images/' || v_filename;
    ...
    INSERT INTO document_images (document_id, source_line_id, object_key, filename, image_hash, status) -- <== ДОБАВИЛИ status
    VALUES (NEW.document_id, NEW.id, v_object_key, v_filename, v_image_hash, 'pending') -- <== ДОБАВИЛИ 'pending'
