...
[dim]Available Models:[/dim]
  /config set llm.model gpt-5             [dim]# Default: Most capable model[/dim]
  /config set llm.model gpt-5-mini        [dim]# Faster, more affordable option[/dim]
...
[dim]Reset configuration:[/dim]
  ...
  /config reset llm.model         [dim]# Reset model to auto-detect[/dim]
...
def handle_config_command(args: str, config: Config) -> None:
    ...
    if subcommand == "show":
        ...
        if not config_data:
            ...
            # Check root level first, then synthesis for backward compatibility
        ...
        if len(args_parts) < 2:
            ...
            console.print("  /config set llm.provider openai         [dim]# Uses GPT-5 automatically[/dim]")
            console.print("\n[cyan]Available OpenAI Models:[/cyan]")
            console.print("  /config set llm.model gpt-5             [dim]# Default: Most capable model[/dim]")
            console.print("  /config set llm.model gpt-5-mini        [dim]# Faster, more affordable option[/dim]")
            console.print("\n[dim]Before using, set your API key:[/dim]")
            console.print("  [dim]export OPENAI_API_KEY=sk-...    # Get from platform.openai.com/api-keys[/dim]")
            console.print("\n[bold]Other Settings:[/bold]")
            console.print("  /config set editor.vi_mode true         [dim]# Enable vi keybindings[/dim]")
            console.print("  /config set editor.vi_mode false        [dim]# Use default keybindings[/dim]")
            console.print("\n[bold]Tool Approval Configuration:[/bold]")
            console.print("  /config set tool_approval true   [dim]# Enable tool approval prompts (all modes)[/dim]")
            console.print("  /config set tool_approval false  [dim]# Auto-approve all tools (all modes)[/dim]")
        ...
        if not (
            ...
            or key == "tool_approval"
        ):
            console.print(f"[red]Invalid configuration key: {key}[/red]")
            console.print(
                "Supported keys: llm.provider, llm.model, llm.api_key_env, editor.vi_mode, display.list_columns, tool_approval, synthesis.tool_approval (deprecated)",
            )
            return
