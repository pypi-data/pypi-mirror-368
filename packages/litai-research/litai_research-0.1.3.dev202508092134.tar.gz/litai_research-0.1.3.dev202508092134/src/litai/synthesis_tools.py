...
SYNTHESIS_TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Load/extract content from selected papers and show preview. This is for DATA LOADING only - you only see truncated previews. To actually analyze or answer questions, use synthesize tool afterward.",
            "parameters": {
                "type": "object",
                "properties": {
                    "depth": {
                        ...
                        "description": "Level of detail to extract: abstracts (quick), key_points (medium), notes (user notes), agent_notes (AI insights), sections (specific), full_text (complete)",
                    },
                    "sections": {
                        "type": "array",
                        "items": {"type": "string"},
    ...
    async def extract_context(
        ...
        for paper in papers:
            ...
            if context_type == "abstracts":
                ...
                content = agent_notes if agent_notes else "No agent notes available"
...
conversationally without using tools. Be concise and helpful."""
    ...
    async def _extract_context(self, depth: str, sections: list[str] | None = None) -> dict:
        ...
        for paper_id, context in self.orchestrator.current_context.items():
            ...
            if paper:
                ...
                if depth in ("notes", "agent_notes"):
                    # For notes, show full content since they're usually short
                    if context.content not in ("No notes available", "No agent notes available"):
                        ...
                        note_type = "notes" if depth == "notes" else "agent notes"
                        context_previews.append(f"• {title}: No {note_type} found")
                elif depth == "abstracts":
                    # For abstracts, show more content since they're key information
                    preview = context.content[:400] + "..." if len(context.content) > 400 else context.content
                    context_previews.append(f"• {title}: {preview}")
                elif depth in ("full_text", "sections"):
                    # For full text/sections, show brief preview since content is large
                    preview = context.content[:200] + "..." if len(context.content) > 200 else context.content
                    context_previews.append(f"• {title}: {preview}")
                else:
                    # Default fallback
                    preview = context.content[:150] + "..." if len(context.content) > 150 else context.content
                    context_previews.append(f"• {title}: {preview}")
    ...
    def _get_tool_description(self, tool_name: str, arguments: dict) -> str:
        ...
        if tool_name == "extract_context":
            ...
            desc = f"Extract {depth} from selected papers"
            if sections:
