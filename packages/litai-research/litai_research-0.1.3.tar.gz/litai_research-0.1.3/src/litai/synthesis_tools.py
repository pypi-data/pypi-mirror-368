...
from litai.prompts.synthesis_prompts import (
    get_comparative_prompt,
    get_generic_prompt,
    get_quick_prompt,
    get_thorough_prompt,
)
from litai.prompts.synthesis_system import get_synthesis_system_prompt
...
class ContextExtractor:
    ...
    async def extract_context(
        self,
        papers: list[Paper],
        context_types: list[str] | str | None = None,
    ...
    async def answer(
        ...
        for i, (paper_id, ctx) in enumerate(context.items(), 1):
            ...
            if paper:
                ...
                # Allow more context for thorough analysis
                context_limit = 3000 if depth == "thorough" else 1500
                context_parts.append(ctx.content[:context_limit])
                if len(ctx.content) > context_limit:
        ...
        if depth == "quick":
            prompt = get_quick_prompt(question, context_str)
        elif depth == "thorough":
            prompt = get_thorough_prompt(question, context_str)
        elif depth == "comparative":
            prompt = get_comparative_prompt(question, context_str)
        else:
            prompt = get_generic_prompt(question, context_str)
            
        response = await self.llm.complete(
            prompt, 
            max_tokens=3000 if depth == "thorough" else 800,
        )
...
class SynthesisConversation:
    ...
    def _build_system_prompt(self) -> str:
        ...
        if paper_count > 0:
            paper_list = "\n".join([
                f"- {p.title} ({p.year}) [ID: {p.paper_id}]"
                for p in self.session.selected_papers[:5]
            ])
            if paper_count > 5:
                paper_list += f"\n... and {paper_count - 5} more"
        
        # Get user research context if available
        user_prompt = ""
        ...
        return get_synthesis_system_prompt(
            paper_count=paper_count,
            paper_list=paper_list,
            context_type=self.session.context_type,
            synthesis_history_count=len(self.session.synthesis_history),
    ...
    async def _synthesize(self, question: str, mode: str = "quick") -> dict:
        ...
        # Store in history
        from litai.models import RelevantPaper, SynthesisResult
        relevant_papers = [
            RelevantPaper(
                paper=p,
    ...
    def _get_tool_description(self, tool_name: str, arguments: dict) -> str:
        ...
        if tool_name == "select_papers":
            ...
            # Look up paper titles to show them in the description
            paper_titles = []
            for paper_id in paper_ids[:3]:  # Limit to first 3 to avoid overly long descriptions
                paper = self.db.get_paper(paper_id)
                if paper:
                    title = paper.title[:50] + "..." if len(paper.title) > 50 else paper.title
                    paper_titles.append(f'"{title}"')
                else:
                    paper_titles.append(f"ID:{paper_id[:8]}...")
            
            titles_display = ", ".join(paper_titles)
            if len(paper_ids) > 3:
                titles_display += f" and {len(paper_ids) - 3} more"
            
            if operation == "add":
                return f"Add {count} paper(s) to selection: {titles_display}"
            if operation == "remove":
                return f"Remove {count} paper(s) from selection: {titles_display}"
            return f"Set selection to {count} paper(s): {titles_display}"
