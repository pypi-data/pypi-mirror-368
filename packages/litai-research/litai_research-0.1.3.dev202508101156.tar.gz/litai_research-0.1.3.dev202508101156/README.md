...
Install globally to access `litai` from anywhere:

```bash
# Using uv tool (recommended - isolated dependencies)
uv tool install litai-research

# Using pipx (alternative global install)
pipx install litai-research

# Pre-release (latest features, may have bugs)
uv tool install --prerelease=allow litai-research
pipx install --prerelease litai-research
```

### Local Installation (Alternative)
Install in a virtual environment for development or testing:

```bash
# Using uv with pip (fast)
...
# Pre-release
uv pip install --pre litai-research
pip install --pre litai-research
```
...
uv tool install --prerelease=allow --force litai-research
