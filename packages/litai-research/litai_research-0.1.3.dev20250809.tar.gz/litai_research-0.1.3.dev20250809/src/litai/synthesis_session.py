...
class SynthesisSession:
    ...
    async def change_context_depth(
        ...
        Returns:
            The new context type being used
        """
        old_type = self.context_type
        self.context_type = context_type
        
        # Store section preferences if provided
        if context_type == "sections" and sections:
            self._preferred_sections = sections
        
        if old_type != context_type:
            self._context_modified = True
            logger.info(f"Changed context depth from {old_type} to {context_type}")
