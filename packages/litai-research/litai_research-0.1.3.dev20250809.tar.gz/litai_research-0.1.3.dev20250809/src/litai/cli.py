...
def enter_synthesis_mode(db: Database, config: Config) -> None:
    ...
    try:
        ...
        conversation = SynthesisConversation(db, llm_client, orchestrator)
                ...
                if user_input.startswith("/"):
                    ...
                    if user_input.startswith("/context"):
                        ...
                        if len(parts) > 1:
                            ...
                            if context_type in ["abstracts", "notes", "key_points", "full_text"]:
                                ...
                                console.print("[red]Invalid context type. Use: abstracts, notes, key_points, or full_text[/red]")
                        else:
                            console.print("[yellow]Current context: " + conversation.session.context_type + "[/yellow]")
                        continue
                        
                    if user_input.startswith("/clear"):
                        logger.info("Slash command: /clear - clearing session")
                        # Clear session
                        conversation.session.clear()
                        conversation.message_history.clear()
                        console.print("[green]Session cleared.[/green]")
                        continue
                    console.print("[yellow]Unknown command. Use /papers, /selected, /context, /clear, or /exit[/yellow]")
                    continue
    ...
    def get_completions(self, document: Any, complete_event: Any) -> Any:
        ...
        if text.startswith("/"):
            ...
            commands = {
                ...
                "/context": "Change context depth (abstracts, notes, key_points, full_text)",
                "/add": "Add papers to session by number",
