...
This module provides the main FastAPI application for the SAGE Frontend server.
"""
...
# 创建 FastAPI 应用
app = FastAPI(
    title="SAGE Frontend Server",
    description="Web前端和仪表板界面组件，提供可视化的数据处理管道管理和监控功能",
    version="0.1.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
async def root():
    ...
    <html>
    <head>
        <title>SAGE Frontend Server</title>
        <style>
            body {
...
async def api_info():
    """API 信息端点"""
    return {
        "name": "SAGE Frontend API",
        "version": "0.1.0",
        "description": "Web前端和仪表板界面API",
...
def start_server(host: str = "127.0.0.1", port: int = 8080, reload: bool = False):
    """启动服务器"""
    print(f"🚀 启动 SAGE Frontend Server...")
