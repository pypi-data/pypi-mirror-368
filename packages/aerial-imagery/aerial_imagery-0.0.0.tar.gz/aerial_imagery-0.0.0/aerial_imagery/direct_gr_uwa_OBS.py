...
class DGR(object):
    ...
    # UAS frame - IMU - to END frame. from imu/data then rotated to END
    _yaw_UAS   = np.nan #rad
    ...
    def __init__(self, camera='ZED2_C2022', camera_options={}, **kwargs):
        ...
        self.R_C_to_G = np.array([[0, 0, 1],[-1, 0, 0],[0, 1, 0]])  # Corriea et al (2022)
        self.R_C_to_G = np.array([[1, 0, 0],[0, 1, 0],[0, 0, 1]])   # Leave unchanged
        self.R_C_to_G = np.array([[0, -1, 0],[-1, 0, 0],[0, 0, 1]]) # Swap X and Y and reverse
        ...
        self.R_END_to_ENU = np.array([[1, 0, 0],[0, 1, 0],[0, 0, -1]])
    ...
    def update_IMU_G(self, yaw, pitch, roll, frame='UAS'):
        ...
        if frame.upper() == 'UAS':
            ...
            """
            This gives you your frame straight into END as we believe the P4RTK does. 
            """
                
            inv = np.linalg.inv

            # R_G_to_ENU   = self.R_END_to_ENU @ self.R_UAS_to_END @ self.R_G_to_UAS
            # R_G_to_ENU   = R_yaw_pitch_roll(self._yaw_G, self._pitch_G, self._roll_G)
            R_G_to_ENU   = R_yaw_pitch_roll(yaw, pitch, roll)
    ...
    def __call__(self, P_u, P_v, target_z = 0, fulloutput=False, halfoutput=False,
        ...
        if reverse:
            ...
            R_C_to_G     = self.R_C_to_G
            R_C_to_UAS   = self.R_G_to_UAS@self.R_C_to_G
            R_C_to_END   = self.R_UAS_to_END@self.R_G_to_UAS@self.R_C_to_G
            R_C_to_ENU   = self.R_END_to_ENU@self.R_UAS_to_END@self.R_G_to_UAS@self.R_C_to_G

            R_END_to_ENU = self.R_END_to_ENU
            R_UAS_to_ENU = self.R_END_to_ENU@self.R_UAS_to_END 

            T_END_to_ENU =  self.UAS_UTM_xyz
            T_UAS_to_ENU =  T_END_to_ENU + self.R_END_to_ENU@self.T_UAS_to_END 
            T_G_to_ENU   =  T_UAS_to_ENU + self.R_END_to_ENU@self.R_UAS_to_END@self.T_G_to_UAS 
            T_C_to_ENU   =  T_G_to_ENU   + self.R_END_to_ENU@self.R_UAS_to_END@self.R_G_to_UAS@self.T_C_to_G

            testing = True
            if testing:
                OENU = np.array([0., 0., 0.])[:, None]
                # cam_pos = np.linalg.inv(R_C_to_ENU) * OENU
                print(f'Camera translation in the reverse code: {T_C_to_ENU.T}')
                print(f'Gimbal translation in the reverse code: {T_G_to_ENU.T}')
                print(f'UAS    translation in the reverse code: {T_UAS_to_ENU.T}')
            ...
            # return M
            return P_i, self.K, R_C_to_ENU, T_C_to_ENU
            ...
            # R_UAS_to_END = R_yaw_pitch_roll(self._yaw_UAS, self._pitch_UAS, self._roll_UAS)

            P_END_prime = self.R_UAS_to_END @ P_UAS_prime + self.T_UAS_to_END

            if fulloutput:
                print("\n P_END_prime:")
                print(P_END_prime)

            P_ENU_prime = self.R_END_to_ENU @ P_END_prime + T_END_to_ENU

            if fulloutput:
                print("\n P_END_prime:")
                print(P_END_prime)

            # print(3*(R_END_to_ENU @ P_END_prime) + T_END_to_ENU)
            ...
            T += self.R_END_to_ENU@self.R_UAS_to_END@self.R_G_to_UAS@self.T_C_to_G
    ...
    def get_component_positions(self):
        ...
        print(f'Gimbal position is {OG.T}')
        print(f'UAS position is {OUAS.T}')
    ...
    def plot_scene(self, bbox_alt=0, targets=[], uas_view=False, 
        ...
        OC, OG, OUAS = self.get_component_positions()
