...
class DGR(object):
    ...
    def __init__(self, camera='workswell_wirspro', camera_options={}, **kwargs):
        ...
        self.R_C_to_G = np.array([[0,  -1, 0],[1, 0, 0],[0, 0, 1]]) # Swap X and Y and reverse Y

        self.T_C_to_G = np.array([[0],[0],[0]])

        # UAS frame to NED frame. This was NED in C2022.
        ...
        # NED frame to ENU frame. This was NED in C2022.
    ...
    def update_IMU_G(self, yaw, pitch, roll, frame='UAS'):
        ...
        if frame.upper() == 'UAS':
            ...
            # R_G_to_ENU   = self.R_NED_to_ENU @ self.R_UAS_to_NED @ self.R_G_to_UAS
            # R_G_to_ENU   = R_yaw_pitch_roll(self._yaw_G, self._pitch_G, self._roll_G)
            R_G_to_ENU   = R_yaw_pitch_roll(yaw, pitch, roll)
            self.R_G_to_UAS = inv(self.R_UAS_to_NED) @ inv(self.R_NED_to_ENU) @ R_G_to_ENU
    ...
    def __call__(self, P_u, P_v, target_z = 0, fulloutput=False, halfoutput=False,
        ...
        if reverse:
            ...
            R_C_to_NED   = self.R_UAS_to_NED@self.R_G_to_UAS@self.R_C_to_G
            R_C_to_ENU   = self.R_NED_to_ENU@self.R_UAS_to_NED@self.R_G_to_UAS@self.R_C_to_G

            R_NED_to_ENU = self.R_NED_to_ENU
            R_UAS_to_ENU = self.R_NED_to_ENU@self.R_UAS_to_NED 

            T_NED_to_ENU =  self.UAS_UTM_xyz
            T_UAS_to_ENU =  T_NED_to_ENU + self.R_NED_to_ENU@self.T_UAS_to_NED 
            T_G_to_ENU   =  T_UAS_to_ENU + self.R_NED_to_ENU@self.R_UAS_to_NED@self.T_G_to_UAS 
            T_C_to_ENU   =  T_G_to_ENU   + self.R_NED_to_ENU@self.R_UAS_to_NED@self.R_G_to_UAS@self.T_C_to_G
    ...
    def plot_scene(self, bbox_alt=0, targets=[], uas_view=False, 
        ...
        if plot_img_axis:
            ...
            ax.quiver(ul[0], ul[1], ul[2], img_y[0]-ul[0], img_y[1]-ul[1], 0, color='k')
            ax.quiver(ul[0], ul[1], ul[2], img_x[0]-ul[0], img_x[1]-ul[1], 0, color='k')
