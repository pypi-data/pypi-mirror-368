...
def R_kappa_phi_omega(kappa, phi, omega):
    ...
    return R_omega @ R_phi @ R_kappa  # NED → Camera frame
...
class DGR(object):
    ...
    def __init__(self, ext_imu, camera='ZED2_C2022', camera_options={}, **kwargs):
        """
        ...
        self.R_C_to_Ref_deterministic = kappa_rot_mat@R_C0_to_Ref # This is actually C to Ref when "aligned". 
    ...
    def update_translations(self, offsets):
        ...
        self.R_C_to_Ref = R_C_to_Ref_mounting@self.R_C_to_Ref_deterministic
            ...
            # self.R_C_to_Ref = self.R_C_to_G @  self.R_G_to_Ref # Camera to Reference frame is the deterministic rotation
    ...
    def update_HPR_Ref(self, heading, pitch, roll, units='rad'):
        ...
        # self.HPR_R_Ref_to_NED   = HPR_R_Ref_to_NED  # Inverse of the rotation matrix
        self.HPR_R_Ref_to_ENU   = self.R_NED_to_ENU @ self.HPR_R_Ref_to_NED
        
        # HPR_R_NED_to_Ref = self.HPR_R_Ref_to_NED.T  # Inverse of the rotation matrix
    ...
    def check_IMU_consistency(self):
        ...
        print(f'R_C_to_NED calculated from Kappa Phi Omega is:\n {self.KPO_R_C_to_NED}')
        print()

        HPR_R_NED_to_Ref = self.HPR_R_Ref_to_NED.T
        HPR_R_Ref_to_C   = self.R_C_to_Ref.T

        HPR_R_NED_to_C   = HPR_R_Ref_to_C @ HPR_R_NED_to_Ref
        print(f'R_NED_to_C calculated from heading, pitch, roll, camera-body to camera, and kappa cardinal rot is:\n {HPR_R_NED_to_C}')
        print()

        print('Now calculating forward-backward reprojection error angle.')
        compute_reprojection_error_angle(HPR_R_NED_to_C, self.KPO_R_C_to_NED)


    def calculate_reference_frames(self, method='HPR'):
        """
        ...
        if method.upper() == 'HPR':
            ...
            HPR_R_C_to_ENU   = self.R_NED_to_ENU @ self.HPR_R_Ref_to_NED @ self.R_C_to_Ref

            # R_NED_to_ENU = self.R_NED_to_ENU
            HPR_R_Ref_to_ENU = self.R_NED_to_ENU @ self.HPR_R_Ref_to_NED 
            HPR_R_Ref_to_NED = self.HPR_R_Ref_to_NED

            # self.T_NED_to_ENU =  self.Ref_UTM_xyz
            HPR_T_Ref_to_ENU      =  self.T_NED_to_ENU + self.R_NED_to_ENU @ self.T_Ref_to_NED 

            HPR_T_C_to_ENU        =  HPR_T_Ref_to_ENU + self.R_NED_to_ENU @ HPR_R_Ref_to_NED @ self.T_C_to_Ref # This applies the translation in the Reference Frame - not what we want.
            ...
            return HPR_R_C_to_ENU, HPR_R_Ref_to_ENU, HPR_R_Ref_to_NED, HPR_T_C_to_ENU

        elif method.upper() == 'KPO':

            if not self.has_KappaPhiOmega:
                raise ValueError('KPO angles of the Sensor frame are not set. Use update_KappaPhiOmega() first.')

            KPO_R_C_to_NED = self.KPO_R_C_to_NED    
            ...
            KPO_R_C_to_ENU   = R_NED_to_ENU @ KPO_R_C_to_NED

            KPO_ENU_to_C     = KPO_R_C_to_ENU.T
            
            KPO_ENU_to_Ref   = self.R_C_to_Ref @ KPO_ENU_to_C
            KPO_R_Ref_to_ENU = KPO_ENU_to_Ref.T


            KPO_R_Ref_to_NED = R_ENU_to_NED @ KPO_R_Ref_to_ENU 

            KPO_T_Ref_to_ENU =  self.T_NED_to_ENU + self.R_NED_to_ENU @ self.T_Ref_to_NED 
            KPO_T_C_to_ENU   =       KPO_T_Ref_to_ENU + self.R_NED_to_ENU @ KPO_R_Ref_to_NED @ self.T_C_to_Ref

            self.KPO_R_C_to_ENU   = KPO_R_C_to_ENU
            self.KPO_R_Ref_to_ENU = KPO_R_Ref_to_ENU
            self.KPO_R_Ref_to_NED = KPO_R_Ref_to_NED

            self.KPO_T_Ref_to_ENU = KPO_T_Ref_to_ENU
            self.KPO_T_C_to_ENU   = KPO_T_C_to_ENU


            print('Calculated KPO')

            return KPO_R_C_to_ENU, KPO_R_Ref_to_ENU, KPO_R_Ref_to_NED, KPO_T_C_to_ENU
        
        else:
            raise ValueError(f'Method {method} not recognised. Use "HPR" or "KPO"')
    ...
    def __call__(self, P_u, P_v, target_z = 0, fulloutput=False, halfoutput=False,
        ...
        if type(P_u) in [int, float, list]: # Scalar
            ...
            mesh = self(P_u_, P_v_, target_z, reverse=reverse, method=method)
            # print(mesh)
            # print(mesh.shape)
        ...
        R_C_to_ENU, R_Ref_to_ENU, R_Ref_to_NED, T_C_to_ENU = self.calculate_reference_frames(method=method)
        ...
        if reverse:
            ...
            P_NED_prime = R_Ref_to_NED @ P_Ref_prime + self.T_Ref_to_NED
            ...
            if z_ENU is None:
                ...
                T += self.R_NED_to_ENU @ R_Ref_to_NED @ self.T_C_to_Ref
    ...
    def get_cam_ref_lines(self, method, ax=None, cols=['r', 'g', 'b']):
        """
        Get coordinates of lines connecting camera to reference frame in ENU coordinates.
        This is used to plot the camera reference lines in the ENU frame.

        THIS IS A HORRIBLE WAY T DO THIS 
        
        """

        R_C_to_ENU, R_Ref_to_ENU, R_Ref_to_NED, T_C_to_ENU = self.calculate_reference_frames(method=method)

        T_C_to_Ref  = self.T_C_to_Ref 
        T_NED_to_ENU =  self.Ref_UTM_xyz
        T_Ref_to_ENU =  T_NED_to_ENU + self.R_NED_to_ENU@self.T_Ref_to_NED 
        ...
        if method.upper() in ['HPR', 'KPO']:
            pass
        else:
            raise ValueError(f'Method {method} not recognised. Use "HPR".')
        ...
        for i in np.arange(0, 3):
            ...
            T_C_to_Ref_here = T_C_to_Ref.copy()
            # T_C_to_Ref_here[2-i::] = 0
            T_C_to_Ref_here[i+1:3] = 0
            print(T_C_to_Ref_here)
            print()
            # print(T_C_to_Ref_here)

            
            T_C_to_ENU   =  T_Ref_to_ENU + self.R_NED_to_ENU@R_Ref_to_NED@ T_C_to_Ref_here
        ...
        if ax is None:
            ...
            for i in np.arange(3):
                ...
                x, y, z = zip(s, e)
                ax.plot(x, y, z, '--', color=cols[i])
                print('Plot', x, y, z, cols[i])

        return ORef, OCs[1::]

    def get_component_positions(self, method):
        """
        ...
        if method.upper() in ['HPR', 'KPO']:
            ...
            # R_C_to_Ref   = self.R_C_to_Ref
            # R_C_to_NED   = self.HPR_R_Ref_to_NED@self.R_C_to_Ref
            # R_C_to_ENU   = self.R_NED_to_ENU@R_Ref_to_NED@self.R_C_to_Ref

            # R_NED_to_ENU = self.R_NED_to_ENU
            # R_Ref_to_ENU = self.R_NED_to_ENU@R_Ref_to_NED 

            T_NED_to_ENU =  self.Ref_UTM_xyz
            T_Ref_to_ENU =  T_NED_to_ENU + self.R_NED_to_ENU@self.T_Ref_to_NED 

            T_C_to_ENU   =  T_Ref_to_ENU + self.R_NED_to_ENU@R_Ref_to_NED@self.T_C_to_Ref
    ...
    def make_georef_grid(self, grid_n=None, alt=0, method='HPR'):
        """
        ...
        meshx, meshy, meshz = self(xpixg, ypixg, alt, method=method)
