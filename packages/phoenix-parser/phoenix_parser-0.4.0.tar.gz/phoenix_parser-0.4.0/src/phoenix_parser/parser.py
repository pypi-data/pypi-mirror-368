...
class AdaptiveSemanticParser:
    ...
    def parse(self, raw_llm_output: str, expected_schema: Type[BaseModel]) -> Dict[str, Any]:
        """The main method based on the original, robust cascade logic, v4.0"""
        if not raw_llm_output or not raw_llm_output.strip():
            raise ParsingError("Input text is empty.")
        text_to_process = raw_llm_output
        # Layer 1: Prioritize the content of a Markdown block
        ...
        try:
            ...
            healed_data = self._parse_semantic_fallback(
                raw_llm_output, expected_schema)
            # We run the healed data through validation again, just in case
            return self._heal_and_validate(healed_data, expected_schema)
