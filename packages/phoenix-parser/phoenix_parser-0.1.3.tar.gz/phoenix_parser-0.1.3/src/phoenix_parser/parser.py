...
class AdaptiveSemanticParser:
    ...
    def parse(self, raw_llm_output: str, expected_schema: Type[BaseModel]) -> Dict[str, Any]:
        """The main method that goes through a cascade of parsers, v1.3"""
        ...
        if match:
            ...
            try:
                return self._clean_and_load(json_str_from_block, expected_schema)
        ...
        try:
            ...
            if start != -1 and end != -1:
                json_str_from_text = raw_llm_output[start:end]
                return self._clean_and_load(json_str_from_text, expected_schema)
        except (json.JSONDecodeError, ValidationError):
        ...
        try:
            ...
            raise ParsingError(
                "Failed after all layers of parsing and semantic recovery.", 
