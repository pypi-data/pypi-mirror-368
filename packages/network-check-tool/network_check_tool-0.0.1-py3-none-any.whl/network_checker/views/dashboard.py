...
def get_ping_data():
    ...
    results = db.session.query(PingResult).filter(
        PingResult.target_host == host,
    ...
    return jsonify({
        ...
        'packet_loss_rate': (sum(packet_losses) / len(packet_losses)) * 100 if packet_losses else 0
    })
...
def get_ping_chart():
    ...
    if failed_times:
        fig.add_trace(go.Scatter(
            x=failed_times,
            y=[max(successful_response_times) * 1.1] * len(failed_times) if successful_response_times else [100] * len(failed_times),
            mode='markers',
            name='Packet Loss',
...
def get_stats():
    ...
    successful = [r for r in results if not r.packet_loss and r.response_time is not None]
    ...
    return jsonify({
        ...
        'min_response_time': min(successful_response_times) if successful_response_times else 0,
        'max_response_time': max(successful_response_times) if successful_response_times else 0
    })