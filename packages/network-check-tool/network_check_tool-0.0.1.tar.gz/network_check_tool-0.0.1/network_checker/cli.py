...
def status(host, limit):
    ...
    with app.app_context():
        ...
        for result in results:
            ...
            response_str = f"{result.response_time:.2f}ms" if result.response_time else "timeout"
            time_str = result.timestamp.strftime("%Y-%m-%d %H:%M:%S")
            
            click.echo(f"{result.target_host:<20} {time_str:<20} {response_str:<12} {status_str}")
