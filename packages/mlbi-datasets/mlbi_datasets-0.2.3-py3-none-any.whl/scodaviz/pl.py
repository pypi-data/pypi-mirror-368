...
import importlib.metadata  # Python 3.8 이상 (아래 참고)

SEABORN = True
SEABORN_VERSION_OLDER = True
try:
    import seaborn as sns

    current_version = importlib.metadata.version("seaborn")
    target_version = "0.12.3"
    
    if version.parse(current_version) <= version.parse(target_version):
        print('Your version of seaborn is %s.' % current_version)
        SEABORN_VERSION_OLDER = True
    else:
        SEABORN_VERSION_OLDER = False
        # print(f"현재 버전 {current_version}은 기준과 같음")
...
STATANNOT = True
STATANNOT_VERSION_OLDER = False
try:
    if SEABORN_VERSION_OLDER:
        from statannot import add_stat_annotation
        STATANNOT_VERSION_OLDER = True
    else:
        from statannotations.Annotator import Annotator
        
except ImportError:
    if SEABORN_VERSION_OLDER:
        print('WARNING: You need to install statannot. ')
    else:
        print('WARNING: You need to install statannotations. ')
    STATANNOT = False
...
def plot_gsa_result( df_res, terms_sel, items_to_plot, lims = None, dpi = 100,
    ...
    with warnings.catch_warnings():
        ...
        plt.rcParams['figure.figsize'] = (4,(len(terms_sel)*1.1 + 8)/9)      
        plt.rcParams['figure.dpi'] = dpi             
