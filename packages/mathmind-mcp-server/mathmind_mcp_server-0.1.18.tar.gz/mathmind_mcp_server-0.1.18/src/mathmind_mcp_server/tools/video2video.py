...
class Video2Video:
    ...
    def __call__(self, *args, **kwds):
        ...
        voiceVolume = kwds.get("voiceVolume",100)  if kwds.get("voiceVolume",100)  else 100
        bgmVolume = kwds.get("bgmVolume",100)  if kwds.get("bgmVolume",100)  else 100
        headerVideoUrl = kwds.get("headerVideoUrl")
        footerVideoUrl = kwds.get("footerVideoUrl")
        coverImageUrl = kwds.get("coverImageUrl")
        coverImageDuration = kwds.get("coverImageDuration",0.1) if kwds.get("coverImageDuration",0.1)  else 0.1
        ...
        # print(body)
        response = requests.post(self.url, json=body, headers=headers,timeout=600)
