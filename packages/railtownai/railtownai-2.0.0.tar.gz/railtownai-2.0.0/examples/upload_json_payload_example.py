...
def upload_json_payload_example():
    ...
    # Get the RailtownHandler instance from the root logger
    root_logger = logging.getLogger()
    railtown_handler = None

    for handler in root_logger.handlers:
        if isinstance(handler, RailtownHandler):
            railtown_handler = handler
            break

    if not railtown_handler:
        print("Error: RailtownHandler not found. Make sure railtownai.init() was called.")
        return
    ...
    success = railtown_handler.upload_agent_run(test_data)
    ...
    success = railtown_handler.upload_agent_run(test_data_array)
