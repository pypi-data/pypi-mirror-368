...
def download_single_iso(url, index, res_msg, username, password, client_id, client_secret) -> str:
    """下载单个ISO文件"""
    token = None
    ...
    try:
        ...
        fp = download_iso_with_sha256sum_check(url, root_path=DOWNLOAD_ROOT_PATH, logger_=logger, token=token)
        if fp:
            return fp
        else:
