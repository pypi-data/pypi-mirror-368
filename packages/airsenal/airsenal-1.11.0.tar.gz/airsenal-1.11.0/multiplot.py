...
if __name__ == "__main__":
    df = pd.read_csv("airsenalRanks.csv")
    df["Relative rank (%)"] = 100*df["rank"]/df["total_players"]
    df.drop(columns=["rank","total_players"], inplace=True)
    ax = plot_multi(df, x="Season", figsize=(12,6))
    plt.show()
