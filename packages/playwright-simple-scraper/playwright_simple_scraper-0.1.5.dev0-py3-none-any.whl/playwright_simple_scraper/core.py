...
def scrape_context(
    ...
    Returns:
      ...
      * if the selector matches an element with no text content,
        the text will be an empty string.
    """
...
def scrape_href(
    ...
    Returns:
      ScrapeResult
        * containing the texts, URL, and other metadata.
        * if the selector matches an element with no href attribute,
          the href will be an empty string.
    """
    _validate_inputs(url, selector)
    # _respect_robots(url, respect_robots, user_agent) # Not implemented yet
