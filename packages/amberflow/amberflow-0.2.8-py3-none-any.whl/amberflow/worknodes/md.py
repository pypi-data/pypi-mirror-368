...
class BaseMDWorkNode(BaseSingleWorkNode):
    ...
    def _fill_output_artifacts(
        self, output_dir: dirpath_t, *args, prefix: str, tags: tuple[str, ...], sysname: str, **kwargs
    ) -> ArtifactContainer:
    ...
    def _run_md(
        ...
        *,
        ...
        self.copy_input_files_to_work_dir(self.work_dir, c, p)
        ...
        if ref is not None:
            cmd_line.append(f"-ref {ref.name}")
...
class MDRun(BaseMDWorkNode):
    ...
    def _run(
        ...
        **kwargs,
        ...
        sbatch_params = {"ntasks-per-node": self.cores} if sbatch_params is None else sbatch_params | {"ntasks-per-node": self.cores}
