...
from ....utils.utils_lm import softmax_one


class MultiHeadedAttention(nn.Module):
    def __init__(self, hidden_size, heads_num, attention_head_size, dropout, has_bias=True, with_scale=True, offbyone=False):
        super(MultiHeadedAttention, self).__init__()
        self.heads_num = heads_num
        self.offbyone = offbyone
    ...
    def forward(self, key, value, query, mask, position_bias=None, has_residual_attention=False, prev_attn=None):
        ...
        if self.offbyone:
            probs = softmax_one(scores, dim=-1)
        else:
