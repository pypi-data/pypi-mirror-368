...
class LMTrainer(BaseTrainer):
    def __init__(self, config, logger, start_step, start_epoch, global_step):
        super(LMTrainer, self).__init__(config, logger, start_step, start_epoch, global_step)
    ...
    def report_and_reset_stats(self):
        ...
        training_info = "| {:8d}/{:8d} steps"\
            ...
            loss_mlm,
            acc_mlm)
        if self.logger is not None: self.logger.info(training_info)
        else: print(training_info, flush=True)

        if WANDB_ and self.wandb: wandb.log({'token per secs': token_per_secs, 'loss': loss_mlm, 'lm_acc': acc_mlm}, step=self.current_step)
