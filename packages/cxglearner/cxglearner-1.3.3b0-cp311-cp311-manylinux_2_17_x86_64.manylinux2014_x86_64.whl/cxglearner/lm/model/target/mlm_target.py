...
class MlmTarget(nn.Module):
    ...
    def __init__(self, config, vocab_size):
        ...
        self.hidden_size = config.lm.hidden_size
        self.emb_size = config.lm.emb_size
        self.factorized_embedding_parameterization = config.lm.factorized_embedding_parameterization
        self.act = activate_fns[config.lm.hidden_act]

        if self.factorized_embedding_parameterization:
            self.mlm_linear_1 = nn.Linear(config.lm.hidden_size, config.lm.emb_size)
            self.layer_norm = LayerNorm(config.lm.emb_size)
            self.mlm_linear_2 = nn.Linear(config.lm.emb_size, self.vocab_size)
        else:
            self.mlm_linear_1 = nn.Linear(config.lm.hidden_size, config.lm.hidden_size)
            self.layer_norm = LayerNorm(config.lm.hidden_size)
            self.mlm_linear_2 = nn.Linear(config.lm.hidden_size, self.vocab_size)
