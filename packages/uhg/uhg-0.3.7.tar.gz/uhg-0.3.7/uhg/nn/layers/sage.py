...
class ProjectiveSAGEConv(UHGLayer):
    ...
    def aggregate_neighbors(
        ...
        if self.aggregator == 'mean':
            ...
            for i in range(len(row)):
                ...
                weight = self.compute_cross_ratio_weight(x[src % num_nodes], x[dst])
                weights.append(weight)
            weights = torch.as_tensor(weights, device=x.device, dtype=neigh_features.dtype)
    ...
    def forward(
        ...
        points = torch.stack([self_trans[:out_size], neigh_trans[:out_size]], dim=-2)  # [N, 2, D]
