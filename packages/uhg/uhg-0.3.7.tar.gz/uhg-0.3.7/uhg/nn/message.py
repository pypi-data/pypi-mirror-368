...
class HyperbolicMessagePassing(nn.Module):
    ...
    def forward(
        ...
        if self.edge_features and (edge_attr is None):
            edge_attr = torch.zeros(E, self.edge_features, device=x.device, dtype=x.dtype)
        if edge_attr is None:
            msg_input = torch.cat([x[col], x[row]], dim=-1) if self.in_features * 2 == self.message_mlp[0].in_features else x[col]
        else:
            msg_input = torch.cat([x[col], edge_attr], dim=-1)
        if E == 0:
            messages = torch.zeros(0, self.out_features, device=x.device, dtype=x.dtype)
        else:
