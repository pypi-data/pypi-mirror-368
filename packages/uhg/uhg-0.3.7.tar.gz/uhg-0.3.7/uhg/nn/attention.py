...
class HyperbolicAttention(nn.Module):
    ...
    def forward(
        ...
        # No cross-ratio bookkeeping inside attention
        
        # Ensure homogeneous coordinate present, but do not normalize full x here
        if x.size(-1) == self.in_features:
            x = torch.cat([x, torch.ones_like(x[..., :1])], dim=-1)
        x_proj = x
        
        # Linear projections (per head) on Euclidean feature part
        feats = x_proj[..., :-1]
        queries = self.query(feats).view(N, self.num_heads, self.out_features)
        keys = self.key(feats).view(N, self.num_heads, self.out_features)
        values = self.value(feats).view(N, self.num_heads, self.out_features)
        ...
        # Masking
        if mask is not None:
            # Convert node-pair mask [N,N] to edge mask [E]
            if mask.dim() == 2 and mask.size(0) == mask.size(1) and mask.size(0) == N:
                row, col = edge_index
            ...
            # Expand to heads
            edge_mask = edge_mask.unsqueeze(-1).expand(-1, self.num_heads)  # [E, num_heads]
    ...
    def _compute_attention_scores(
        ...
        # Lift to homogeneous by appending ones (no global normalization to avoid rejection)
        q_h = torch.cat([q, torch.ones_like(q[..., :1])], dim=-1)
        k_h = torch.cat([k, torch.ones_like(k[..., :1])], dim=-1)
        # Compute hyperbolic inner product
        scores = self.uhg.inner_product(q_h, k_h)
