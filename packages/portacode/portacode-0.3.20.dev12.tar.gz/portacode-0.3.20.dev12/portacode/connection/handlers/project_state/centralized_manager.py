...
class CentralizedProjectStateManager:
    ...
    async def initialize_project_state(self, client_session_id: str, 
        ...
        # Write debug state after initialization
    ...
    async def refresh_git_state(self, client_session_id: str) -> bool:
        ...
        if changed:
            ...
            # Write debug state on git changes
            self._write_debug_state()
    ...
    async def refresh_file_system_state(self, client_session_id: str) -> bool:
        ...
        if changed:
            # Notify client of changes  
            await self._notification_manager.notify_if_changed(state)
            # Write debug state on file system changes
            self._write_debug_state()
    ...
    def set_debug_mode(self, enabled: bool, debug_file_path: Optional[str] = None):
        ...
        if enabled:
            logger.info("Centralized project state debug mode enabled, output to: %s", debug_file_path)
    ...
    def _write_debug_state(self):
        ...
        try:
            # Generate debug data for this manager instance
            debug_data = {
                "_instance_info": {
                    "pid": os.getpid(),
                    "timestamp": __import__('time').time(),
                    "project_count": len(self._project_states)
                }
            }
            
            # Add state for each project
            for client_session_id, state in self._project_states.items():
                debug_data[f"specific..inmemory!{client_session_id}"] = {
                        ...
                        "staged_changes": [
                            {
                                ...
                                "diff_details": {
                                    "diffs": f.diff_details.get("diffs", []) if f.diff_details else [],
                                    "stats": {
                                        "char_additions": f.diff_details.get("stats", {}).get("char_additions", 0) if f.diff_details else 0,
                                        "char_deletions": f.diff_details.get("stats", {}).get("char_deletions", 0) if f.diff_details else 0,
                                        "char_unchanged": f.diff_details.get("stats", {}).get("char_unchanged", 0) if f.diff_details else 0,
                                        "total_changes": f.diff_details.get("stats", {}).get("total_changes", 0) if f.diff_details else 0
                                    },
                                    "algorithm": f.diff_details.get("algorithm", "diff-match-patch") if f.diff_details else "diff-match-patch"
                                }
            ...
            logger.error("Error writing debug state file: %s", e)
...
def get_or_create_centralized_manager(context: Dict, 
    ...
    if _global_manager is None:
        ...
        if context.get("debug", False):
            ...
            logger.info("Setting up centralized debug mode with file: %s", debug_file_path)
            _global_manager.set_debug_mode(True, debug_file_path)
        else:
