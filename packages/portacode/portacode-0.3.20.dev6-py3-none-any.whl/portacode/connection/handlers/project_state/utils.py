...
def generate_tab_id(file_path: str, tab_type: str = "file", **kwargs) -> str:
    ...
    if tab_type == 'file':
        ...
        # For untitled tabs, use a UUID as the ID
        return f"untitled:{str(uuid.uuid4())}"
    else:
        # For other tab types, use file_path if available, otherwise UUID
        return file_path if file_path else f"{tab_type}:{str(uuid.uuid4())}"
