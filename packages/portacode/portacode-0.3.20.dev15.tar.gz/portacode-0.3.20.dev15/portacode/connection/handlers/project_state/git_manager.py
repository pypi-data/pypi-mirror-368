...
class GitManager:
    ...
    def _create_status_snapshot(self) -> Optional[str]:
        """Create a snapshot of current git status for change detection."""
        if not self.is_git_repo or not self.repo:
            return None
        
        try:
            # Create a compact snapshot of git state
            branch = self.get_branch_name()
            head_hash = self.get_head_commit_hash()
            
            # Get porcelain status output (stable format)
            status_output = self.repo.git.status(porcelain=True).strip()
            
            # Combine into a snapshot string
            snapshot = f"branch:{branch}|head:{head_hash}|status:{status_output}"
            return snapshot
        except Exception as e:
            logger.debug("Error creating status snapshot: %s", e)
            return None
    ...
    def has_status_changed(self) -> bool:
        ...
        current_snapshot = self._create_status_snapshot()
        if current_snapshot is None:
            return False
        
        # Check if this is the first check or if status changed
        if self._last_status_snapshot is None or self._last_status_snapshot != current_snapshot:
