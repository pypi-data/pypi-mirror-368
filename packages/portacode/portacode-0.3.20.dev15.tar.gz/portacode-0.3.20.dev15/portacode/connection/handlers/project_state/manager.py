...
class ProjectStateManager:
    ...
    def __init__(self, control_channel, context: Dict[str, Any]):
        ...
        # Periodic git status checking
        self._git_status_checker_task: Optional[asyncio.Task] = None
        self._start_git_status_checker()
    ...
    def cleanup_all_projects(self):
        ...
        # Stop git status checker
        if self._git_status_checker_task and not self._git_status_checker_task.done():
            self._git_status_checker_task.cancel()
    ...
    def _start_git_status_checker(self):
        """Start the periodic git status checker."""
        async def git_status_checker():
            """Check git status every second for all git repositories."""
            while True:
                try:
                    await asyncio.sleep(1.0)  # Check every second
                    
                    # Check all active git repositories
                    for client_session_id, git_manager in self.git_managers.items():
                        if git_manager.is_git_repo and git_manager.has_status_changed():
                            logger.debug("Git status changed detected for session: %s", client_session_id)
                            await self._refresh_project_state(client_session_id)
                            
                except asyncio.CancelledError:
                    logger.info("Git status checker cancelled")
                    break
                except Exception as e:
                    logger.error("Error in git status checker: %s", e)
                    # Continue running even if there's an error
        
        # Start the background task
        try:
            self._git_status_checker_task = asyncio.create_task(git_status_checker())
            logger.info("Started periodic git status checker")
        except Exception as e:
            logger.error("Failed to start git status checker: %s", e)
