...
class TabularConverterFactory(
    ...
    ego_vehicle_id: Optional[int] = None
    ...
    def build_planning_problem_set_creator(self) -> PlanningProblemCreator:
        if self.ego_vehicle_id is not None:
            return FixedEgoPlanningProblemCreator(
                self.ego_vehicle_id,
                self.keep_ego,
    ...
    def build_job_filter(self) -> Callable[[TabularJob], bool]:
        ...
        def has_planning_problems(job):
            """Check if the job has planning problems."""
            return len(job.planning_problem_set.planning_problem_dict) > 0

        if self.num_time_steps is not None:
            filter_fn = has_planning_problems
        else:
