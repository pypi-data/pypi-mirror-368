...
if python -m mypy mentorstec/ --ignore-missing-imports; then
...
if python -m pytest tests/ -v --cov=mentorstec --cov-report=term --cov-report=html; then
...
if python -c "import mentorstec; print('✅ Imports OK')"; then
...
```bash
python -m mypy mentorstec/ --strict
```
...
**Com cobertura:**
```bash
python -m pytest tests/ -v --cov=mentorstec --cov-report=term
```

**Com relatório HTML:**
```bash
python -m pytest tests/ -v --cov=mentorstec --cov-report=html
...
pytest --cov=mentorstec --cov-report=html
...
- [ ] ✅ `pytest tests/` passou
- [ ] ✅ Cobertura ≥ 80%
- [ ] ✅ `python -c "import mentorstec"` funciona
...
python -c "import mentorstec; print('OK')"   # ✅ Imports
