...
DECLSPEC u32x crc32_g (GLOBAL_AS const u32x *w, const u32 pw_len, const u32 iv)
{
  u32x a = ~iv;
...
KERNEL_FQ KERNEL_FA void m11500_mxx (KERN_ATTR_BASIC ())
{
  ...
  u32x a_ref = crc32_g (pws[gid].i, pws[gid].pw_len, iv);
  ...
  {
    u32x a = crc32_g (combs_buf[il_pos].i, combs_buf[il_pos].pw_len, a_ref);
...
KERNEL_FQ KERNEL_FA void m11500_sxx (KERN_ATTR_BASIC ())
{
  ...
  const u32x a_ref = crc32_g (pws[gid].i, pws[gid].pw_len, iv);
