...
def task_download_line_coordinates(scan_id, target, ctx):
    ctx.verbose_log(f'Downloading info with line coordinates for scan ID = {scan_id}')
    try:
        data = ctx.provider.get_processed_scan_details(scan_id)
    except ProviderHTTPClientException as err:
        click.echo(str(err))
        return
    else:
        file_name = f'lines_coordinates_{data["id"]}.json'
        info_to_download = deepcopy(data['features']['lines'])
        info_to_download['image_size'] = data['image_size']

        with open(os.path.join(target, file_name), 'w') as f:
            json.dump(info_to_download, f, indent=4)
            ctx.verbose_log(f'{file_name} successfully saved')
