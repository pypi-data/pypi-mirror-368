...
class ShorturlEntity:
    ...
    def validate(self) -> None:
        ...
        if not self.attributeName or len(self.attributeName) < 3:
            raise ShorturlValueError(field="attributeName", detail="El attributeName debe tener al menos 3 caracteres")

        if self.attributeEmail and len(self.attributeEmail) > 500:
            raise ShorturlValueError(field="attributeEmail", detail="El attributeEmail no puede superar los 500 caracteres")
    ...
    def update(self, data:dict, addMode:bool = False) -> None:
        ...
        :raises ShorturlValueError: Si hay un error de estructura en los datos.
        """
        ...
        for key, value in data.items():
            if hasattr(self, key) or addMode:
                try:
                    ...
                    raise ShorturlValueError(field=key, detail=f"Error de estructura en los datos: {str(e)}") from e
    ...
    @staticmethod
    def from_dict(data: dict) -> "ShorturlEntity":
        """
        ...
        try:
            entity = ShorturlEntity(**data)
        except TypeError as e:
            raise ShorturlValueError("Error al construir la entidad:", str(e)) from e
