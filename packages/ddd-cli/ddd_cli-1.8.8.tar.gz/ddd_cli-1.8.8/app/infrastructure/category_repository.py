...
class CategoryRepository:
    """
    Repositorio para manejar la persistencia de datos de la entidad: category.
    ...
    @staticmethod
    def get_all(filters: Optional[dict] = None) -> List[ CategoryEntity ]:
        """
        ...
        returns: 
            List[ CategoryEntity ]: Lista de entidades recuperadas.
        raises:
            CategoryValueError:  Si el valor de entrada no es válido.
        ...
        try:
            instance_list = Category.objects.all()    

            # Aplicar filtros si se proporcionan
            if filters is not None:
                if not isinstance(filters, dict):
                    raise CategoryValueError(field="filters", detail="filters debe ser un diccionario o None")
            ...
            return [Mapper.model_to_entity(instance, CategoryEntity) for instance in instance_list]        
    ...
    @staticmethod
    def get_by_id(id) -> Optional[ CategoryEntity ]:
        """
        ...
        raises:
            CategoryValueError: Si el valor de entrada no es válido.        
            CategoryNotFoundError: Si no existe el registro con el ID dado.
        ...
        if not is_integer(id):
            raise CategoryValueError(field="id", detail="El ID debe ser un entero.")

        try:
            instance = Category.objects.get(id=id)
            return Mapper.model_to_entity(instance, CategoryEntity)

        except Category.DoesNotExist as e:
            raise CategoryNotFoundError(id=id) from e
        except DatabaseError as e:
            raise ConnectionDataBaseError("Error al acceder a la base de datos") from e    
        except Exception as e:
    ...
    def exists_by_field(field_name, value) -> bool:
        ...
        if field_name not in ALLOWED_FIELDS:
            raise CategoryValueError(field=field_name, detail=f"El campo '{field_name}' no es válido para verificar existencia.")

        try:
            return Category.objects.filter(**{field_name: value}).exists()
    ...
    @staticmethod
    def create(entity: CategoryEntity, external_id: Optional[int], externals: Optional[List[int]], adicionalData=None) -> CategoryEntity:
        """
        ...
        raises:
            CategoryValueError:  Si el valor de entrada no es válido.
            CategoryValidationError: Si los datos no son válidos.
            CategoryAlreadyExistsError: Si ya existe un registro con el mismo nombre.
            ConnectionDataBaseError: Si ocurre un error al acceder a la base de datos.   
            RepositoryError: Si ocurre un error inesperado (interno del sistema).     
        """
        ...
        if not entity or not hasattr(entity, "to_dict"):
            raise CategoryValueError(field="Category", detail="Entidad nula o no tiene el método 'to_dict'")
        ...
        try:
            ...
            raise CategoryValueError(field="data", detail=f"Error de estructura en los datos: {str(e)}") from e
        except ValidationError as e:
            raise CategoryValidationError(f"Error de validación: {e.message_dict}") from e
        except IntegrityError as e:
            if 'duplicate' in str(e).lower() or 'unique constraint' in str(e).lower():
                raise CategoryAlreadyExistsError('attributeName', instance.attributeName)  # Ajusta según el campo único
            # Otro error de integridad → regla de negocio?
            raise CategoryValidationError({"integridad": "Datos duplicados o inconsistentes"})            
        except DatabaseError as e:
            raise ConnectionDataBaseError("Error al acceder a la base de datos") from e
        except Exception as e:
    ...
    @staticmethod
    def update(entity: CategoryEntity, external_id: Optional[int], externals: Optional[List[int]], adicionalData=None) -> CategoryEntity:
        """
        ...
        try:
            ...
            instance = Category.objects.get(id=entity.id)
            ...
            raise CategoryNotFoundError(id=entity.id) from e
        except (TypeError, ValueError) as e:
            raise CategoryValueError(field="data", detail=f"Error de estructura en los datos: {str(e)}") from e
        except ValidationError as e:
            raise CategoryValidationError(f"Error de validación: {e.message_dict}") from e
    ...
    def delete(id) -> bool:
        ...
        try:
            ...
            raise CategoryValidationError(f"Validation error occurred: {e.message_dict}") from e
        except DatabaseError as e:
            raise ConnectionDataBaseError("Error al acceder a la base de datos") from e            
        except Exception as e:
...
#### 1. 🗣️ **Métodos específicos orientados al dominio**
    En lugar de exponer solo filtros genéricos por `nombre`, puedes agregar métodos que expresen reglas de negocio:
        @staticmethod
        def get_activos():
            return Category.objects.filter(estado='activo')

        @staticmethod
        def find_by_slug(slug: str) -> OptionalCategoryEntity:
            try:
...
#### 2. 🔍 **QuerySets y Managers personalizados**
    ...
    Luego, en el repositorio:
        ...
        def get_all(filters=None):
            instance_list = Category.objects.activos()  # Usa tu Manager
            if filters and "nombre" in filters:
                instance_list = instance_list.filter(nombre__icontains=filters["nombre"])
            return [Mapper.model_to_entity(inst, CategoryEntity) for inst in instance_list]

        @staticmethod
        def get_all_with_relations():
            instance_list = Category.objects.activos().con_relacion() # Usa tu Manager
            if filters:
                instance_list = instance_list.filter(nombre__icontains=filters["nombre"])
            return [Mapper.model_to_entity(inst, CategoryEntity) for inst in instance_list]
...
#### 5. 🧠 **Consultas complejas bien encapsuladas**
    Cuando necesites agregaciones o filtros avanzados, encapsúlalos en métodos del repositorio:
        ...
        def get_con_muchos_externals(min_relaciones=3):
            instances = Category.objects.annotate(
                total_externals=Count('externals')
            ).filter(total_externals__gt=min_relaciones)
            return [Mapper.model_to_entity(inst, CategoryEntity) for inst in instances]
...
#### 6. **Uso de `select_related` y `prefetch_related`**
    La plantilla no los usa aún, pero son fáciles de integrar en `get_all()` o nuevos métodos:
        ...
        @staticmethod
        def get_all_with_relations():
            instance_list = Category.objects.select_related('external').prefetch_related('externals')
            if filters:
                instance_list = instance_list.filter(nombre__icontains=filters["nombre"])
            return [Mapper.model_to_entity(inst, CategoryEntity) for inst in instance_list]
...
#### 7. **Consultas RAW o expresiones ORM avanzadas**
    Usa `Q`, `F`, `Subquery`, o SQL crudo **dentro del repositorio** cuando el ORM no alcance:
        ...
        def search_advanced(query):
            instances = Category.objects.filter(
        ...
        def reactivar_registros():
            Category.objects.filter(estado='inactivo').update(estado=F('estado_anterior'))

        @staticmethod
        def busqueda_compleja_sql():
            from django.db import connection
            with connection.cursor() as cursor:
                cursor.execute("SELECT * FROM app_category WHERE estado = %s", ['activo'])
                rows = cursor.fetchall()
            return [Mapper.model_to_entity(row, CategoryEntity) for row in rows]
...
#### 8. **Documentación y claridad**
    Los métodos del repositorio deben reflejar intenciones del negocio, no solo operaciones técnicas:
        @staticmethod
        def get_all(filters=None) -> ListCategoryEntity:
            """
            Obtiene todos los category que coincidan con los filtros.
    ...
    Cada método debe tener pruebas. Ejemplo con `create()`:
        ...
        class UserRepositoryTests(TestCase):
            ...
            def test_create_con_external_y_externals(self):
                entity = CategoryEntity(nombre="Test")
                ...
                # Verifica relaciones
                instance = Category.objects.get(id=created.id)
