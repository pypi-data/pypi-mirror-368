...
class CategoryEntity:
    ...
    def validate(self) -> None:
        ...
        if not self.attributeName or len(self.attributeName) < 3:
            raise CategoryValueError("El attributeName debe tener al menos 3 caracteres")

        if self.attributeEmail and len(self.attributeEmail) > 500:
            raise CategoryValueError("El attributeEmail no puede superar los 500 caracteres")
    ...
    def update(self, data:dict, addMode:bool = False) -> None:
        ...
        :raises CategoryValueError: Si hay un error de estructura en los datos.
        """
        ...
        for key, value in data.items():
            if hasattr(self, key) or addMode:
                try:
                    ...
                    raise CategoryValueError(key, f"Error de estructura en los datos: {str(e)}") from e
    ...
    @staticmethod
    def from_dict(data: dict) -> "CategoryEntity":
        """
        ...
        try:
            entity = CategoryEntity(**data)
        except TypeError as e:
            raise CategoryValueError("Error al construir la entidad", str(e)) from e
