...
This server provides tools to generate GCP diagrams using the Python diagrams package.
...
mcp = FastMCP(
    ...
    log_level='ERROR',
    instructions="""Use this server to generate professional GCP diagrams using the Python diagrams package.
IMPORTANT:
- Always start with get_diagram_examples to understand the syntax
- If context7 tool is available in the environment, use it to get documentation for the diagrams package
...
- GCP AI Platform diagrams: Example of using GCP AI services
...
async def mcp_list_diagram_icons(
    provider_filter: Optional[str] = Field(
        default=None, description='Filter icons by provider name (e.g., "gcp", "k8s", "onprem", "saas")'
    ),
    service_filter: Optional[str] = Field(
        default=None,
        description='Filter icons by service name (e.g., "compute", "database", "network", "analytics", "ml", "storage")',
    ),
):
