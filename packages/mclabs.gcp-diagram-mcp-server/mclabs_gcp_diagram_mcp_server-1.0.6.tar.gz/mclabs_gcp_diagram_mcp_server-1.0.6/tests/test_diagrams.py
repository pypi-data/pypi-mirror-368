...
import os
import pytest
from mclabs.gcp_diagram_mcp_server.diagrams_tools import (
    generate_diagram,
    get_diagram_examples,
    list_diagram_icons,
)
from mclabs.gcp_diagram_mcp_server.models import DiagramType
...
class TestGetDiagramExamples:
    ...
    def test_get_gcp_examples(self):
        ...
        assert 'gcp_grouped_workers' in response.examples
        assert 'gcp_clustered_web_services' in response.examples
        assert 'gcp_event_processing' in response.examples
        assert 'gcp_ai_image_processing' in response.examples
...
class TestListDiagramIcons:
    ...
    def test_list_icons_with_provider_filter(self):
        ...
        response = list_diagram_icons(provider_filter='gcp')
        assert response.providers is not None
        assert len(response.providers) == 1
        assert 'gcp' in response.providers
        assert response.filtered is True
        assert response.filter_info == {'provider': 'gcp'}
        # Check that we have services for GCP
        assert 'compute' in response.providers['gcp']
        assert 'database' in response.providers['gcp']
        assert 'network' in response.providers['gcp']
        # Check that we have icons for GCP compute
        assert 'ComputeEngine' in response.providers['gcp']['compute']
        assert 'Functions' in response.providers['gcp']['compute']

    def test_list_icons_with_provider_and_service_filter(self):
        """Test listing diagram icons with provider and service filter."""
        response = list_diagram_icons(provider_filter='gcp', service_filter='compute')
        assert response.providers is not None
        assert len(response.providers) == 1
        assert 'gcp' in response.providers
        assert len(response.providers['gcp']) == 1
        assert 'compute' in response.providers['gcp']
        assert response.filtered is True
        assert response.filter_info == {'provider': 'gcp', 'service': 'compute'}
        # Check that we have icons for GCP compute
        assert 'ComputeEngine' in response.providers['gcp']['compute']
        assert 'Functions' in response.providers['gcp']['compute']
    ...
    def test_list_icons_with_invalid_service(self):
        ...
        response = list_diagram_icons(provider_filter='gcp', service_filter='invalid_service')
        assert response.providers is not None
        assert 'gcp' in response.providers
        assert response.providers['gcp'] == {}
        assert response.filtered is True
        assert response.filter_info is not None
        assert response.filter_info.get('provider') == 'gcp'
...
class TestGenerateDiagram:
    ...
    @pytest.mark.asyncio
    async def test_generate_diagram_success(self, gcp_diagram_code, temp_workspace_dir):
        """Test successful diagram generation."""
        result = await generate_diagram(
            code=gcp_diagram_code,
            filename='test_gcp_diagram',
        ...
        expected_path = os.path.join(
            temp_workspace_dir, 'generated-diagrams', 'test_gcp_diagram.png'
        )
        assert result.path == expected_path

    @pytest.mark.asyncio
    async def test_generate_diagram_with_absolute_path(self, gcp_diagram_code, temp_workspace_dir):
    ...
    async def test_generate_diagram_with_random_filename(
        self, gcp_diagram_code, temp_workspace_dir
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_diagram_with_timeout(self, gcp_diagram_code, temp_workspace_dir):
