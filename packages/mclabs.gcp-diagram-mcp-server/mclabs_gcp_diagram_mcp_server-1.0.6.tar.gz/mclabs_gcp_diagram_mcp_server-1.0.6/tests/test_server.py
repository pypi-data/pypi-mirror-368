...
from mclabs.gcp_diagram_mcp_server.server import (
...
class TestMcpGenerateDiagram:
    ...
    @pytest.mark.asyncio
    @patch('gcplabs.gcp_diagram_mcp_server.server.generate_diagram')
    async def test_generate_diagram(self, mock_generate_diagram):
        ...
        result = await mcp_generate_diagram(
            code='with Diagram("Test", show=False):\n    LoadBalancing("lb") >> ComputeEngine("web")',
            filename='test',
            timeout=60,
...
class TestMcpGetDiagramExamples:
    ...
    @pytest.mark.asyncio
    @patch('gcplabs.gcp_diagram_mcp_server.server.get_diagram_examples')
    async def test_get_diagram_examples(self, mock_get_diagram_examples):
        ...
        mock_get_diagram_examples.return_value = MagicMock(
            model_dump=MagicMock(
                return_value={
                    'examples': {
                        'gcp': 'with Diagram("GCP", show=False):\n    LoadBalancing("lb") >> ComputeEngine("web")',
    ...
    async def test_get_diagram_examples_with_specific_type(self, mock_get_diagram_examples):
        ...
        # Call the function
        result = await mcp_get_diagram_examples(diagram_type=DiagramType.GCP)
        ...
        # Check that get_diagram_examples was called with the correct arguments
        mock_get_diagram_examples.assert_called_once_with(DiagramType.GCP)
...
class TestMcpListDiagramIcons:
    ...
    @pytest.mark.asyncio
    @patch('gcplabs.gcp_diagram_mcp_server.server.list_diagram_icons')
    ...
    async def test_list_diagram_icons_with_provider_filter(self, mock_list_diagram_icons):
        ...
        mock_list_diagram_icons.return_value = MagicMock(
            model_dump=MagicMock(
                return_value={
                    'providers': {
                        'gcp': {
                            'compute': ['ComputeEngine', 'Functions'],
                            'database': ['SQL', 'Firestore'],
                        }
                    },
                    'filtered': True,
                    'filter_info': {'provider': 'gcp'},
                }
            )
        )

        # Call the function
        result = await mcp_list_diagram_icons(provider_filter='gcp')
        ...
        mock_list_diagram_icons.assert_called_once()
        # We don't check the exact arguments because they are Field objects
        # But we can check that the first argument contains 'gcp'
        ...
        mock_list_diagram_icons.return_value = MagicMock(
            model_dump=MagicMock(
                return_value={
                    ...
                    'filtered': True,
                    'filter_info': {'provider': 'gcp', 'service': 'compute'},
                }
            )
        )

        # Call the function
        result = await mcp_list_diagram_icons(provider_filter='gcp', service_filter='compute')
