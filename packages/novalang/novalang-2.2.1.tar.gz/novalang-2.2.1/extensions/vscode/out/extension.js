...
function activate(context) {
    ...
    const serverModule = path.join(context.extensionPath, '..', 'language-server', 'novalang_lsp.py');
    // If the extension is launched in debug mode then the debug server options are used
    // Otherwise the run options are used
    const serverOptions = {
        run: { command: 'python', args: [serverModule], transport: node_1.TransportKind.stdio },
        debug: { command: 'python', args: [serverModule], transport: node_1.TransportKind.stdio }
    };
    ...
    client = new node_1.LanguageClient('novalangLanguageServer', 'NovaLang Language Server', serverOptions, clientOptions);
...
function registerCommands(context) {
    ...
    const runFileCommand = vscode.commands.registerCommand('novalang.runFile', async () => {
        ...
        const mainScript = config.get('mainScript', 'main.py');
...
app.start()
`;
                await vscode.workspace.fs.writeFile(vscode.Uri.joinPath(projectUri, 'app.nova'), Buffer.from(appContent));
                // Create README
...
`;
                await vscode.workspace.fs.writeFile(vscode.Uri.joinPath(projectUri, 'README.md'), Buffer.from(readmeContent));
    ...
    context.subscriptions.push(runFileCommand, createComponentCommand, createEntityCommand, createServiceCommand, createControllerCommand, createProjectCommand);
}
