...
class Par2Data(Base):
    ...
    async def exchange(self, c: Client) -> result.Simple[cdt.CommonDataType] | result.Error:
        ...
        res = await ReadObjAttr(obj, self.par.i).exchange(c)
...
class WriteParValue(Base):
    ...
    async def exchange(self, c: Client) -> result.Result:
        ...
        if (a_data := obj.get_attr(self.par_value.par.i)) is None:
            if (res := await Par2Data(self.par_value.par).exchange(c)).is_ok():
                a_data = res.value
            else:
                return res
        ...
        elif not (res := await ReadObjAttr(obj, self.par_value.par.i).exchange(c)).is_ok():
            return res
        else:
            data = a_data
