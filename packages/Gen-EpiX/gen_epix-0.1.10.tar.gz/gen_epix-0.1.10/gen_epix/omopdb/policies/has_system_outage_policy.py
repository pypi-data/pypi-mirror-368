...
from gen_epix.omopdb.domain.policy.system import BaseHasSystemOutagePolicy
...
class HasSystemOutagePolicy(BaseHasSystemOutagePolicy):
    def is_allowed(self, cmd: Command) -> bool:
        ...
        if not cmd.user:
            return True
        assert cmd.user.id
        return self._is_permitted(cmd.user)  # type: ignore
    ...
    def _is_permitted(self, tgt_user: model.User) -> bool:
        return (
            ...
            in self.system_service.app.user_manager.retrieve_user_permissions(
                tgt_user  # type: ignore
            )
        )

    @cached(cache=TTLCache(maxsize=10, ttl=10))
    def _is_allowed(self) -> bool:
        outages: list[model.Outage] = self.system_service.crud(  # type: ignore
