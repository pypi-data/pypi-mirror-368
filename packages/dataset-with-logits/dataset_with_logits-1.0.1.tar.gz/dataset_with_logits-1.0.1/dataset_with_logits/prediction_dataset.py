...
if __name__ == '__main__':
    ...
    def test_dataset_with_real_data():
        ...
        """
        logger = logging.getLogger(__name__)
        logger.info("--- Running test for ImageNetWithLogits with real data ---")
        ...
        if not os.path.isdir(IMAGENET_ROOT):
            logger.warning(f"Warning: ImageNet root directory not found at '{IMAGENET_ROOT}'. Skipping test.")
            return
        if not os.path.isfile(PREDICTIONS_PATH):
            logger.warning(f"Warning: Predictions CSV not found at '{PREDICTIONS_PATH}'. Skipping test.")
            return

        # 1. Read some sample predictions to use as our ground truth for the test
        logger.info(f"Reading sample predictions from {PREDICTIONS_PATH}...")
        predictions_df = pd.read_csv(PREDICTIONS_PATH)
        if len(predictions_df) < 5:
            logger.warning("Warning: Not enough predictions in file to run a comprehensive test. Skipping.")
            return
        ...
        logger.info(f"Selected {len(test_samples)} samples to test.")
        ...
        for _, test_entry in test_samples.iterrows():
            ...
            if test_index is None:
                logger.error(f"Error: Could not find image with ID '{test_id}' in the dataset at '{IMAGENET_ROOT}'.")
                continue  # Move to the next test sample

            # Test the __getitem__ method on the specific image
            logger.info(f"Fetching item at index {test_index}...")
            sample, target, loaded_logits = dataset[test_index]

            # Verify the results
            logger.info(f"  Sample shape: {sample.shape}")
            logger.info(f"  Loaded logits shape: {loaded_logits.shape}")
            ...
            logger.info(f"  Assertions passed for ID: {test_id}")
