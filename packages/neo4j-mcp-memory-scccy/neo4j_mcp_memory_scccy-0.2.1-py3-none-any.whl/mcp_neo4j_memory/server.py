...
from .neo4j_memory import Neo4jMemory, Entity, Relation, ObservationAddition, ObservationDeletion, KnowledgeGraph, KnowledgeGraphSummary
...
def create_mcp_server(memory: Neo4jMemory) -> FastMCP:
    ...
    mcp: FastMCP = FastMCP("neo4j-mcp-memory-scccy", dependencies=["neo4j", "pydantic"])

    @mcp.tool(annotations=ToolAnnotations(title="Understand Knowledge System",
                                          readOnlyHint=True,
    ...
    @mcp.tool(annotations=ToolAnnotations(title="Create multiple new entities in the knowledge graph",
                                          readOnlyHint=False,
                                          destructiveHint=False,
                                          idempotentHint=True,
                                          openWorldHint=True))
    async def create_entities(entities: list[Entity] = Field(..., description='''
    Rules for creating entities:
    
    - name: unique token in camelCase format
    - label: array of tokens in camelCase format  
    - observations: JSON object, concise and clear
    
    Additional rules by entity type:
    - Rule-based: include 'must' and 'forbidden' constraints
    - Document-based: include relative path; name is filename
    - Code-based: name is filename; node_type is parent node token; observations must include a brief description of the main functionality
    ''')) -> list[Entity]:
    ...
    @mcp.tool(annotations=ToolAnnotations(title="Create Relations",
                                          ...
                                          idempotentHint=True,
                                          openWorldHint=True))
    async def create_relations(relations: list[Relation] = Field(..., description='''
    List of relations to create, focusing on active relationships that capture actions, dependencies, and dynamic connections between entities.
    
    Rules:
    - relationType: unique camelCase token representing the relationship
    ...
    @mcp.tool(annotations=ToolAnnotations(title="Add Observations",
                                          ...
                                          idempotentHint=True,
                                          openWorldHint=True))
    async def add_observations(observations: list[ObservationAddition] = Field(..., description='''
    Add new observations as key-value pairs to existing entities.
    ...
    @mcp.tool(annotations=ToolAnnotations(title="Delete Entities",
                                          ...
                                          idempotentHint=True,
                                          openWorldHint=True))
    async def delete_entities(entityNames: list[str] = Field(..., description='''
    ...
    @mcp.tool(annotations=ToolAnnotations(title="Delete Observations",
                                          ...
                                          idempotentHint=True,
                                          openWorldHint=True))
    async def delete_observations(deletions: list[ObservationDeletion] = Field(..., description='''
    Delete specific key-value pairs from the observations of entities.
    ''')) -> str:
    ...
    async def delete_relations(relations: list[Relation] = Field(..., description='''
    ...
    @mcp.tool(annotations=ToolAnnotations(title="Search Memories",
                                          ...
                                          idempotentHint=True,
                                          openWorldHint=True))
    async def search_memories(query: str = Field(..., description='''
    Search for memories based on a query containing search token.
    ...
    @mcp.tool(annotations=ToolAnnotations(title="Find Memories by Name",
                                          ...
                                          idempotentHint=True,
                                          openWorldHint=True))
    async def find_memories_by_name(names: list[str] = Field(..., description='''
    Find specific memories by token.
    ''')) -> KnowledgeGraphSummary:
