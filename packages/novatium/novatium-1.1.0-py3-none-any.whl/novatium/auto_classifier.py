...
def _require_tf():
    if not HAS_TF:
        raise ImportError(
            'TensorFlow is required. Install via optional extras:\n'
            '  pip install "novatium[tensorflow]"'
        )
...
def _fit_oof_single(X_tr, y_tr, seed, kind="hgb", n_splits=5, params=None):
    ...
    # fit a full model for inference
...
# ===================== TF heads (logit integral-delta) =====================
class BasePlusIntegralDeltaLogit(Model):
    """
    Input: x, f0_logit (mixture logit). Predicts an integrated delta and a softplus gate g(x),
    outputs p = sigmoid(f0_logit + g*integ).
    """
...
# ===================== Binary engine =====================
class _NovaAutoClassifierBinary(BaseEstimator, ClassifierMixin):
    """
    Binary lambda-gated (HGB + XGB) classifier with integral-delta correction in logit space.
    Expects y in {0,1} (the public wrapper handles arbitrary labels & multiclass).
    """
    ...
    def fit(self, X, y):
        ...
        X = _dense32(X); y = _dense32(y).reshape(-1, 1)
        uniq = np.unique(y)
        if not set(uniq.tolist()).issubset({0.0, 1.0}):
            raise ValueError("Binary engine expects labels in {0,1}.")

        # OOF probs and full bases (persist for inference)
        ...
        self._base_hgb_clf_ = oof_hgb["base_full"]
        self._has_xgb_ = bool(HAS_XGB and self.use_xgb)
        self._base_xgb_clf_ = oof_xgb["base_full"] if self._has_xgb_ else None
        ...
        def train_step(x_gate_in, x_raw, pxgb, phgb, ytrue):
            with tf.GradientTape(persistent=True) as tape:
                ...
                # responsibility: who was better (per-sample)
    ...
    def predict_proba(self, X):
        _require_tf()
        check_is_fitted(self, ["_delta_head_", "_lam_gate_", "_base_hgb_clf_"])
        X = check_array(X); X = _dense32(X)

        # Persisted base models (no refit)
        ph = self._base_hgb_clf_.predict_proba(X)[:, 1].astype(np.float32).reshape(-1, 1)
        if self._has_xgb_ and self._base_xgb_clf_ is not None:
            px = self._base_xgb_clf_.predict_proba(X)[:, 1].astype(np.float32).reshape(-1, 1)
        else:
            px = ph
...
# ===================== Public wrapper (auto multiclass) =====================
class NovaAutoClassifier(BaseEstimator, ClassifierMixin):
    """
    Auto-detects binary vs multiclass.
    - Binary: uses _NovaAutoClassifierBinary (lambda-gated HGB+XGB with logit integral-delta).
    - Multiclass: wraps One-vs-Rest with the same binary engine per class.
    """

    def __init__(self, **kwargs):
        # kwargs flow into the binary engine (use_xgb, lr, K, etc.)
        self.kwargs = kwargs
        self._is_multiclass_ = None
        self._le_ = None
        self._clf_ = None   # _NovaAutoClassifierBinary or OneVsRestClassifier

    def fit(self, X, y):
        X = check_array(X)
        y = np.asarray(y)
        self._le_ = LabelEncoder().fit(y)
        y_enc = self._le_.transform(y)  # 0..K-1 for any label types
        n_classes = len(self._le_.classes_)
        if n_classes == 1:
            raise ValueError("NovaAutoClassifier: y has only one class.")
        self._is_multiclass_ = (n_classes > 2)

        if n_classes == 2:
            # Binary path: map to {0,1} float and train binary engine
            y01 = (y_enc == 1).astype(np.float32)
            self._clf_ = _NovaAutoClassifierBinary(**self.kwargs).fit(X, y01)
        else:
            # Multiclass path: OvR with the same binary engine
            base = _NovaAutoClassifierBinary(**self.kwargs)
            self._clf_ = OneVsRestClassifier(base).fit(X, y_enc)
        return self

    def predict_proba(self, X):
        check_is_fitted(self, ["_clf_", "_is_multiclass_", "_le_"])
        X = check_array(X)
        if not self._is_multiclass_:
            # Binary → two columns
            return self._clf_.predict_proba(X)
        else:
            # OvR → shape [n_samples, K]
            return self._clf_.predict_proba(X)
    ...
    def predict(self, X):
        ...
        if not self._is_multiclass_:
            ...
            y01 = (proba >= 0.5).astype(int)
            return self._le_.inverse_transform(y01)
        else:
            proba = self.predict_proba(X)
            y_idx = np.argmax(proba, axis=1)
            return self._le_.inverse_transform(y_idx)
