...
class NovaAutoRegressor(BaseEstimator, RegressorMixin):
    ...
    def predict(self, X):
        ...
        check_array(X)
        X = _dense32(X)
        # Build base outputs for inference using fitted full models:
        # We didn’t keep full models to save memory; use OOF test recipe:
        # reuse the small sampled “test” recipe we used in fit to compute shapes; for real use,
        # just refit quick bases here (cheap compared to TF head).
        seed = self.seed
        # make quick bases on full X vs a small shadow set X[:min(64,...)] to reuse OOF function
        _, oof_hgb = fit_bases_oof(X, np.zeros((len(X),1), np.float32), X[:min(64, len(X))], seed=seed, n_splits=2)
        if HAS_XGB and self.use_xgb:
            oof_xgb, _ = fit_bases_oof(X, np.zeros((len(X),1), np.float32), X[:min(64, len(X))], seed=seed, n_splits=2)
            fxgb = oof_xgb["f0_train_oof"]
        else:
            fxgb = oof_hgb["f0_train_oof"]
        fhgb = oof_hgb["f0_train_oof"]

        X_gate = concat_gate_inputs(X, fxgb, fhgb)
        mu, _, _, _ = _forward(self._delta_model_, self._lam_gate_, X_gate, X, fxgb, fhgb)
        return mu.reshape(-1)
