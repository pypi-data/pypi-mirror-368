...
def _require_tf():
    if not HAS_TF:
        raise ImportError('TensorFlow is required for NovaAutoRegressor. Install with: pip install "novatium[tensorflow]"')
...
class NovaAutoRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        # OOF bases on train; also get fully-fitted models
        ...
        # Train/val split for TF head
        X_tr, X_va, Y_tr, Y_va, FX_tr, FX_va, FH_tr, FH_va = train_test_split(
            X, y, fxgb_tr, fhgb_tr, test_size=0.15, random_state=self.seed
        )
        ...
        reg = 1e-4
        lam_gate = tf.keras.Sequential([
            layers.Dense(24, activation="gelu", kernel_regularizer=regularizers.l2(reg)),
            layers.Dense(24, activation="gelu", kernel_regularizer=regularizers.l2(reg)),
            layers.Dense(1, kernel_regularizer=regularizers.l2(reg)),
    ...
    def predict(self, X):
        ...
        # ✅ Use persisted base models learned during fit
        ...
        f0  = lam * fxgb + (1.0 - lam) * fhgb
        mu, _, _, _ = self._delta_model_(tf.constant(X, tf.float32), tf.constant(f0, tf.float32), training=False)
