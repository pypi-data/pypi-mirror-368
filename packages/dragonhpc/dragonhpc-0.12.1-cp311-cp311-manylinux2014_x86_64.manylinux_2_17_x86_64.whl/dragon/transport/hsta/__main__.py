...
def start_hsta_job(node_index, num_threads_per_node, ls_in_wh):
    ...
    if enable_valgrind and node_index == 0:
        args = [
            ...
            "--leak-check=yes",
            str(hsta_binary),
            f"--num-threads={num_threads_per_node}",
        ]
        ...
        handle = CriticalPopen(
            args,
            env=create_hsta_env(num_threads_per_node),
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
            name=f"dragon-hsta:node={node_index},hostname={socket.gethostname()}",
            notify_channel=ls_in_wh,
...
def hsta(node_index: str, ch_in_sdesc: bytes):
    ...
    if num_nodes > 1:
        ...
        handle, pid = start_hsta_job(int(node_index), num_threads_per_node, ls_in_wh)

        # Start logging thread
        log_thread = threading.Thread(target=logger_thread, args=(handle,))
        log_thread.start()
...
def main(args=None):
    ...
    parser.add_argument("--dragon-logging", type=lambda x: bool(strtobool(x)), help=argparse.SUPPRESS)
    parser.add_argument(
        "--no-dragon-logging", dest="dragon_logging", action="store_false", help="Disable Dragon logging"
    )
    parser.add_argument("--log-sdesc", type=B64.from_str, help="Serialized descriptor for Dragon logs")
    ...
    parser.add_argument("node_index", help="Node index in LAChannelsInfo message of this agent")
    parser.add_argument("--ch-in-sdesc", type=B64.from_str, help="Base64 encoded serialized input channel descriptor")

    parser.set_defaults(log_level="NOTSET", dragon_logging=True, log_sdesc=os.environ.get(DRAGON_LOGGER_SDESC))

    args = parser.parse_args()

    if args.dragon_logging:
        setup_BE_logging(service=DragonLoggingServices.TA, logger_sdesc=args.log_sdesc)
        LOGGER = logging.getLogger(str(dls.TA)).getChild("dragon.transport.hsta.__main__")
    else:
        logging.basicConfig(
    ...
    # TODO: Remove as part of AICI-1422 work
    hsta_log_level = os.environ.get("_DRAGON_HSTA_SET_LOGGING_LEVEL", None)
    if hsta_log_level:
        LOGGER.setLevel(str(hsta_log_level).upper())
