...
from sympy import Abs, Mod, atan2, pi
...
def _reflect(entity: GeometryEntity, line: Line) -> GeometryEntity:
    """..."""
    result = entity.reflect(line)
    if isinstance(result, Circle):
        result = Circle(result.center, Abs(result.radius))
    elif isinstance(result, Ellipse):
        result = Ellipse(result.center, Abs(result.hradius), Abs(result.vradius))
    return result
...
class Arc(GeometrySet):
    ...
    def bounds(self) -> tuple[Scalar, Scalar, Scalar, Scalar]:
        ...
        angles = [start, end]
        incr = pi / 2 if self.length > 0 else -pi / 2
        angle = Mod(start + incr - start, incr)
        while angle < end if self.length > 0 else angle > end:
            angles.append(angle)
            angle += incr
        a = Symbol("a", real=True)
        point = self.ellipse.arbitrary_point(a)
        points = [point.subs(a, angle) for angle in angles]
    ...
    def encloses(self, o: GeometryEntity) -> bool:
        """Return False, as no arc can enclose another gemoetric entity."""
        return False
    ...
    def reflect(self, line: Line) -> "Arc":
        ...
        return Arc(_reflect(self.ellipse, line), start, -self.length)
...
class Transformer:
    ...
    def scale(self, x: Scalar = 1, y: Scalar = 1, pt: Point | None = None) -> None:
        ...
        • pt: the point relative to which scaling is performed
        """
        self._transforms.append(lambda e: e.scale(x, y, pt=pt))
    ...
    def reflect(self, line: Line) -> None:
        ...
        """
        self._transforms.append(lambda e: _reflect(e, line))
