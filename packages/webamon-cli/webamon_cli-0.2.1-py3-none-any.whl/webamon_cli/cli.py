...
def main(ctx, api_key: Optional[str], config_file: Optional[str], verbose: bool, args):
    ...
    if ctx.invoked_subcommand is None and args:
        ...
        if first_arg and first_arg not in known_commands:
            ...
            # Only set results if explicitly provided as second argument
            # If not provided, let API use its default fields
            results = args[1] if len(args) > 1 else None
...
def search(ctx, search_term: str, results: Optional[str], size: int, from_offset: int,
    ...
    RESULTS: Comma-separated list of fields to search within
             (Optional - API uses default fields when not specified)
    ...
    # For non-Lucene searches, only set results if not provided
    # If results is None, let the API use its default fields
    # This avoids sending redundant data in the request
