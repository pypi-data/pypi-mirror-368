...
def test_console_scripts_available():
    ...
    try:
        result = subprocess.run(
            ["pandoc-diagram-fixer"], input="", capture_output=True, text=True
        )
...
def test_diagram_fixer_with_test_file():
    ...
    try:
        result = subprocess.run(
            ["diagram-fixer", str(test_file)], capture_output=True, text=True
        )
        assert result.returncode == 0
...
"""
    ...
    try:
        ...
        result = subprocess.run(
            ["pandoc-diagram-fixer"], input=test_content, capture_output=True, text=True
        )
        assert result.returncode == 0
        assert "┌─────┐" in result.stdout
        ...
        try:
            pandoc_result = subprocess.run(
                ["pandoc", "--filter", "pandoc-diagram-fixer", "-t", "html"],
                input=test_content,
                capture_output=True,
                text=True,
            )
