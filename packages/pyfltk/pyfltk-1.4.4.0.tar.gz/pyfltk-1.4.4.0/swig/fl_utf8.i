...
//  PyObject $1 = PyUnicode_FromObject($input);
// }
...
FL_EXPORT int fl_utf8encode(unsigned ucs, char* OUTPUT);
...
FL_EXPORT unsigned fl_utf8toUtf16(const char* src, unsigned srclen, unsigned short* OUTPUT, unsigned dstlen);
...
//FL_EXPORT unsigned fl_utf8fromwc(char *OUTPUT, unsigned OUTPUT, const wchar_t *INPUT, unsigned INPUT);

/* F2: Convert a UTF8 string into ASCII, eliding untranslatable glyphs */
FL_EXPORT unsigned fl_utf8toa (const char *src, unsigned srclen, char *OUTPUT, unsigned OUTPUT);
...
FL_EXPORT int fl_utf_tolower(const unsigned char *str, int len, char *OUTPUT);
