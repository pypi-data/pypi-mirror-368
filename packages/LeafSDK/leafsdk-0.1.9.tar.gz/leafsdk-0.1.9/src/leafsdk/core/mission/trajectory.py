...
class WaypointTrajectory:
    ...
    def _generate_trajectory(self):
        ...
        logger.info(f"Generated polynomial coefficients: {poly_coeffs}")
        logger.info(f"Time scale for trajectory: {time_scale}")
        logger.info(f"Segment length (3-element): {seg_len}")
        logger.info(f"Rotation axis (3-element): {rotation_axis}")
        return ts, pos, vel, acc, yaw, yaw_rate, poly_coeffs, time_scale, seg_len, rotation_axis, rotation_angle
