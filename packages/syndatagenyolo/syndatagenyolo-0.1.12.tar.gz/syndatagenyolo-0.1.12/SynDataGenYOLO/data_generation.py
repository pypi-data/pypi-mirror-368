...
@dataclass
class ScalingConfig:
    by_background_size: bool = True
    factors: tuple[float, float] = (
        0.25,
        0.85,
    )  # Min and Max percentage size of the short side of the background image
    fixed_sizes: bool = True  # Whether to use fixed image sizes or not


@dataclass
class BlendingConfig:
    methods: List[BlendingMode] = field(
        default_factory=lambda: [
            BlendingMode.ALPHA_BLENDING,
            BlendingMode.GAUSSIAN_BLUR,
            BlendingMode.POISSON_BLENDING_NORMAL,
            BlendingMode.POISSON_BLENDING_MIXED,
            BlendingMode.PYRAMID_BLEND,
        ]
    )
    pyramid_blending_levels: int = 6
    gaussian_kernel_size: int = 15
    gaussian_sigma: int = 30


@dataclass
class ColorHarmonizationConfig:
    enabled: bool = False
    alpha: float = 0.5
    random_alpha: bool = False  # Randomize the color harmonization blending factor (overrides --color_harmon_alpha)
...
class SyntheticImageGenerator:
    def __init__(
        ...
        self.fixed_image_sizes = scaling_config.fixed_sizes
        self.scale_foreground_by_background_size = scaling_config.by_background_size
        self.scaling_factors = scaling_config.factors
        self.zero_padding = 8
        ...
        self.blending_methods = blending_config.methods
        self.pyramid_blending_levels = blending_config.pyramid_blending_levels
        self.gaussian_options = (
            blending_config.gaussian_kernel_size,
    ...
    def _validate_augmentation_path(self):
        ...
        if (
            self.augmentation_path is False
            or self.augmentation_path == "False"
            or self.augmentation_path == "false"
        ):
    ...
    def _generate_image(self, image_number: int):
        ...
        for i in range(num_foreground_images):
            ...
            foregrounds.append({"category": category, "image_path": foreground_path})
        ...
        (
            ...
            fg_images_sized,
            mask_images_sized,
        ) = self._gen_compose_images(foregrounds, background_image_path)
    ...
    def _gen_compose_images(self, foregrounds, background_image_path):
            ...
            if bg_width >= self.image_width and bg_height >= self.image_height:
                ...
                composite = background_image.crop(
                    (
                        crop_x_pos,
                        crop_y_pos,
                        crop_x_pos + self.image_width,
                        crop_y_pos + self.image_height,
                    )
                )
            else:
                composite = background_image.resize(
        ...
        for fg in foregrounds:
            ...
            if not self.scale_foreground_by_background_size:
                ...
                new_size = (
                    int(fg_image.size[0] * scale),
                    int(fg_image.size[1] * scale),
                )
            else:
            ...
            assert max_x_position >= 0 and max_y_position >= 0, f"""foreground {
                fg["image_path"]
            } is too big({fg_image.size[0]}x{
                fg_image.size[1]
            }) for the requested output size({self.image_width}x{
                self.image_height
            ...
            if self.avoid_collisions:
                ...
                while colliding_point is not None:
                    ...
                    new_centroid_pos = self._get_new_centroid_pos(
                        colliding_point, (curr_centroid_x, curr_centroid_y), step_size
                    )

                    if self._visited_point_before(new_centroid_pos, visited_centroids):
                        # print("Tried to re-visit point {}".format(new_centroid_pos))
                        logger.debug(f"Tried to re-visit point {new_centroid_pos}")
            ...
            fg_image_sized = Image.new("RGBA", composite.size, color=(0, 0, 0, 0))
            ...
            alpha_threshold = 200
            mask_arr = np.array(
                np.greater(np.array(alpha_mask_sized), alpha_threshold), dtype=np.uint8
            )
        ...
        for blending_mode in self.blending_methods:
            ...
            for (
                ...
                if blending_mode == BlendingMode.ALPHA_BLENDING:
                    img = Image.composite(fg_image_sized, img, mask_image_sized)
                    ...
                    blurred_mask = cv2.GaussianBlur(
                        mask_arr,
                        (self.gaussian_options[0], self.gaussian_options[0]),
                        sigmaX=self.gaussian_options[1],
                        sigmaY=self.gaussian_options[1],
                    )
                    ...
                    blending_mode == BlendingMode.POISSON_BLENDING_NORMAL
                    or blending_mode == BlendingMode.POISSON_BLENDING_MIXED
                ):
                    center = (
            ...
            blending_mode_images.append(
                {"image": img, "blending_mode": blending_mode.value}
            )
    ...
    def _color_harmonization(self, target, source, alpha):
        ...
        source = cv2.cvtColor(source, cv2.COLOR_RGB2LAB).astype(np.float32) / 255.0
        target = cv2.cvtColor(target, cv2.COLOR_RGB2LAB).astype(np.float32) / 255.0
    ...
    def _outside_img(img, fg_rect):
        ...
        return (
            curr_centroid_x < 0
            or curr_centroid_x > img.size[0]
            or curr_centroid_y < 0
            or curr_centroid_y > img.size[1]
        )
    ...
    def generate_images(self):
        ...
        if self.parallelize:
            Parallel(n_jobs=4)(
                delayed(self._generate_image)(i)
