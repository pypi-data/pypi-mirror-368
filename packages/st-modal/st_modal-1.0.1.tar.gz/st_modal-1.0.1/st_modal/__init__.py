...
class Modal:
    ...
    def container(self):
        st.markdown(
            ...
            div[data-modal-container='true'][key='{self.key}'] button[data-testid="baseButton-tertiary"] {{
            ...
            div[data-modal-container='true'][key='{self.key}'] button[data-testid="baseButton-tertiary"]:hover {{
                background-color: rgba(0, 0, 0, 0.1) !important;
                transform: scale(1.1) !important;
            }}
            
            /* Ensure title stays within modal bounds and reduce spacing */
            div[data-modal-container='true'][key='{self.key}'] h1,
            div[data-modal-container='true'][key='{self.key}'] h2,
            div[data-modal-container='true'][key='{self.key}'] h3,
            div[data-modal-container='true'][key='{self.key}'] h4,
            div[data-modal-container='true'][key='{self.key}'] h5,
            div[data-modal-container='true'][key='{self.key}'] h6 {{
            ...
            /* Reduce spacing before divider */
            div[data-modal-container='true'][key='{self.key}'] hr {{
        ...
        with st.container():
            ...
            # Create header row with title and close button
            if self.title and self.show_close_button:
                title_col, close_col = _container.columns([0.9, 0.1])
                with title_col:
                    st.markdown(self.title, unsafe_allow_html=True)
                with close_col:
                    close_ = st.button('✕', key=f'{self.key}-close', type='tertiary', help='Close modal')
                    if close_:
                        self.close()
            elif self.title:
                # Use columns to maintain consistent layout without close button
                title_col, empty_col = _container.columns([0.95, 0.05])
                with title_col:
                    st.markdown(self.title, unsafe_allow_html=True)
            elif self.show_close_button:
                _, close_col = _container.columns([0.9, 0.1])
                with close_col:
                    close_ = st.button('✕', key=f'{self.key}-close', type='tertiary', help='Close modal')
                    if close_:
                        self.close()
