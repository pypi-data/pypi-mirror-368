...
class DovetailCommand(MotifCommand):
    ...
    def execute(self, motif=None):
        ...
        ### Create a new motif
            ...
            if top_seq:
                st_state.motif_buffer = (f"motif = pf.Dovetail(sequence = "
                                         f"'{top_seq}', sign = str({sign}), "
                                         f"up_cross = {up_cross}, "
                                         f"down_cross = {down_cross})")
                motif = pf.Dovetail(sequence=top_seq, 
                                    sign=sign, 
                                    up_cross=up_cross, 
                                    down_cross=down_cross)
            else:
                st_state.motif_buffer = (f"motif = pf.Dovetail(length = {seq_length}, "
                                         f"wobble_interval = {wobble_interval}, "
                                         f"wobble_tolerance = {wobble_tolerance}, "
                                         f"up_cross = {up_cross}, "
                                         f"down_cross = {down_cross}, "
                                         f"wobble_insert = 'middle')")
                motif = pf.Dovetail(length=seq_length, 
                                    sign=sign,
            ...
            if specific_seq:   
                ...
                with col2:
                    sign = st.selectbox('Sign:', 
                                        [-1, +1], 
                                        key=f'txt_sign_dt{key}')
                with col3:
                    up_cross = st.toggle('Up cross', 
                                         key=f'up_cross_dt{key}', 
                                         value=up_cross)
                with col4:
                    down_cross = st.toggle('Down cross', 
                                            key=f'down_cross_dt{key}', 
                                            value=down_cross)
            else:
                subcols = st.columns([1, 1, 1, 1, 1], vertical_alignment='bottom')
                with subcols[0]:
                    seq_length = st.number_input('Length:', 
                                                 key=f'dt_length{key}', 
                                                 min_value=-100, 
                                                 value=len_default)
                with subcols[1]:
                ...
                with subcols[2]:
                    down_cross = st.toggle('Down cross', 
                                           ...
                                           value=down_cross)
                with subcols[3]:
                    wobble_interval = st.number_input('Wobble interval:', 
                                                       key=f'dt_wobble_interval{key}',
                                                       min_value=0, 
                                                       value=wobble_interval, 
                                                       help="Add a wobble every "
                                                            "n° nucleotides")
                with subcols[4]:
                    wobble_tolerance = st.number_input('Wobble tolerance:', 
                                                       key=f'dt_wobble_tolerance{key}',
                                                       min_value=0, 
                                                       value=wobble_tolerance)


        return (sign, top_seq, seq_length, 
                wobble_interval, wobble_tolerance, 
                up_cross, down_cross)
