...
def git_pull(engine, target):
    ...
    with engine.connect() as conn:
        insert_task(conn=conn, task_type=0, worker_id=f"^{target}$", definition=git_update_task, input={})
...
def restart_worker(engine, target):
    ...
    with engine.connect() as conn:
        insert_task(conn=conn, task_type=0, worker_id=f"^{target}$", definition=task, input={})
...
def terminate_worker(engine, target):
    ...
    with engine.connect() as conn:
        insert_task(conn=conn, task_type=0, worker_id=f"^{target}$", definition=eq_abort_task, input={})
