...
class Convolution2D(tf.keras.layers.Layer):
    # Modify parameter ordering to match torch's ordering & parameter names
    #def __init__(self, filters, kernel_size, strides = (1,1), padding = 'valid', use_bias = True, **kwargs):
    def __init__(self, out_channels, kernel_size, stride = 1, padding = 0, dilation = 1, bias = True):
        super(Convolution2D, self).__init__()
        """
        padding tuple of 4 ints => left, right, top & bottom
        """

        self.filters = out_channels#filters
        self.kernel_size = kernel_size

        if isinstance(strides, int):
            self.strides = (stride, stride)#(strides, strides)
        elif isinstance(strides, tuple):
            self.strides = stride#strides
        else:
            raise TypeError('Invalid Input type. Only int or tuple (int, int) allowed')
        #self.strides = strides
        self.use_bias = bias#use_bias
