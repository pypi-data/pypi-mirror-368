...
class ExecutionEngine:
    ...
    async def _execute_hybrid_keyword(self, session: ExecutionSession, keyword_name: str, args: List[str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if keyword_info.library == "SeleniumLibrary" and args:
                # Convert first argument (usually a selector) for SeleniumLibrary keywords
                if any(kw in keyword_name.lower() for kw in ['click', 'fill', 'get text', 'wait', 'select']):
                    converted_args[0] = self._convert_locator_for_library(args[0], "SeleniumLibrary")
                    if converted_args[0] != args[0]:
                        logger.info(f"Converted locator for SeleniumLibrary: '{args[0]}' -> '{converted_args[0]}'")
