...
`radionets` follows the [PEP8 style guide][pep8] for Python. This is enforced via the [Ruff][ruff]
...
:::{code-block} python
:caption: Example of a docstring in `radionets` (taken from {func}`~radionets.evaluation.contour.area_of_contour`).
...
`radionets` uses [`pytest`][pytest] as testing framework (see {ref}`testing`). All tests
are located in the `tests` directory in the https://github.com/radionets-project/radionets
...
Coverage is also automatically reported using [Codecov][codecov] with our CI: ![codecovbadge](https://codecov.io/github/radionets-project/radionets/badge.svg)
...
[codecov]: https://app.codecov.io/github/radionets-project/radionets
[codecovbadge]:  https://codecov.io/github/radionets-project/radionets/badge.svg
