...
def test_cli_missing_file_exit_code():
    ...
    assert result.returncode == 2, f"CLI should return exit code 2 for missing files, got {result.returncode}"
    
    # Assert STDERR contains "File not found"
    assert "Error: File not found:" in result.stderr, f"STDERR should contain 'File not found', got: {result.stderr}"
        ...
        for line in lines:
            if line.strip():  # Skip empty lines
                try:
                    ...
                    assert "title" in data, "Each JSON object should contain 'title' field"
                except json.JSONDecodeError as e:
