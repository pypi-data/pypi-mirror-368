...
pip_installed_version=$(pip list | grep klvm_tools_rs | awk '{print $2}')
python_import_version=$(python -c 'import klvm_tools_rs; print(klvm_tools_rs.get_version())')
...
if [ "$expected_version" == "$pip_installed_version" ] && [ "$expected_version" == "$python_import_version" ]; then
    ...
    echo "klvm_tools_rs VERSIONS does not match expected version"
