...
from impact.sdk.prompts.registry import PromptRegistry
from impact.sdk.prompts.client import PromptRegistryClient
from impact.sdk.tracing.content_allow_list import ContentAllowList
from impact.sdk.version import __version__

MAX_RETRIES: int = int(os.getenv("IMPACT_PROMPT_MANAGER_MAX_RETRIES") or 3)
POLLING_INTERVAL: int = int(os.getenv("IMPACT_PROMPT_MANAGER_POLLING_INTERVAL") or 5)
...
class Fetcher:
    ...
    def post(self, api: str, body: Dict[str, str]):
        post_url(f"{self._base_url}/v1/impact/{api}", self._api_key, body)
...
def fetch_url(url: str, api_key: str):
    response = requests.get(
        ...
        headers={
            ...
            "X-Impact-SDK-Version": __version__,
        },
    )

    if response.status_code != 200:
        if response.status_code == 401 or response.status_code == 403:
            logging.error("Authorization error: Invalid Impact API key.")
...
def refresh_data(
    ...
    response = fetch_url(f"{base_url}/v1/impact/prompts", api_key)
    prompt_registry.load(response)

    response = fetch_url(f"{base_url}/v1/impact/pii/tracing-allow-list", api_key)
