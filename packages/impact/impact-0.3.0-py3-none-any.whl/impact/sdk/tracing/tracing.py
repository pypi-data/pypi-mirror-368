...
from impact.sdk.semconv_impact import SpanAttributes
from impact.sdk import Telemetry
from impact.sdk.images.image_uploader import ImageUploader
from impact.sdk.instruments import Instruments
from impact.sdk.tracing.content_allow_list import ContentAllowList
from impact.sdk.utils import is_notebook
from impact.sdk.utils.package_check import is_package_installed
from typing import Callable, Dict, Optional, Set


TRACER_NAME = "impact.tracer"
EXCLUDED_URLS = """
    iam.cloud.ibm.com,
...
class TracerWrapper(object):
    ...
    def _span_processor_on_start(self, span, parent_context):
        ...
        if workflow_name is not None:
            span.set_attribute(SpanAttributes.IMPACT_WORKFLOW_NAME, workflow_name)

        entity_path = get_value("entity_path")
        if entity_path is not None:
            span.set_attribute(SpanAttributes.IMPACT_ENTITY_PATH, entity_path)
        ...
        if is_llm_span(span):
            ...
            if managed_prompt is not None:
                span.set_attribute(
                    SpanAttributes.IMPACT_PROMPT_MANAGED, managed_prompt
                )

            prompt_key = get_value("prompt_key")
            if prompt_key is not None:
                span.set_attribute(SpanAttributes.IMPACT_PROMPT_KEY, prompt_key)

            prompt_version = get_value("prompt_version")
            if prompt_version is not None:
                span.set_attribute(
                    SpanAttributes.IMPACT_PROMPT_VERSION, prompt_version
                )

            prompt_version_name = get_value("prompt_version_name")
            if prompt_version_name is not None:
                span.set_attribute(
                    SpanAttributes.IMPACT_PROMPT_VERSION_NAME, prompt_version_name
                )

            prompt_version_hash = get_value("prompt_version_hash")
            if prompt_version_hash is not None:
                span.set_attribute(
                    SpanAttributes.IMPACT_PROMPT_VERSION_HASH, prompt_version_hash
                )

            prompt_template = get_value("prompt_template")
            if prompt_template is not None:
                span.set_attribute(
                    SpanAttributes.IMPACT_PROMPT_TEMPLATE, prompt_template
                )
            ...
            if prompt_template_variables is not None:
                for key, value in prompt_template_variables.items():
                    span.set_attribute(
                        f"{SpanAttributes.IMPACT_PROMPT_TEMPLATE_VARIABLES}.{key}",
                        value,
                    )
    ...
    def verify_initialized(cls) -> bool:
        ...
        if (os.getenv("IMPACT_SUPPRESS_WARNINGS") or "false").lower() == "true":
            return False

        print(
            Fore.RED
            + "Warning: Impact not initialized, make sure you call Impact.init()"
        )
        print(Fore.RESET)
        return False
...
def _set_association_properties_attributes(span, properties: dict) -> None:
    for key, value in properties.items():
        span.set_attribute(
            f"{SpanAttributes.IMPACT_ASSOCIATION_PROPERTIES}.{key}", value
        )
    ...
    for instrument in instruments:
        if instrument == Instruments.ALEPHALPHA:
            ...
            print(
                "Usage:\n"
                "from impact.sdk.instruments import Instruments\n"
                "Impact.init(app_name='...', instruments=set([Instruments.OPENAI]))"
            )
            print(Fore.RESET)
...
def init_azure_search_instrumentor():
    try:
        if is_package_installed("azure-search-documents"):
            Telemetry().capture("instrumentation:azure_search:init")
...
def metrics_common_attributes():
    ...
    workflow_name = get_value("workflow_name")
    if workflow_name is not None:
        common_attributes[SpanAttributes.IMPACT_WORKFLOW_NAME] = workflow_name

    entity_name = get_value("entity_name")
    if entity_name is not None:
        common_attributes[SpanAttributes.IMPACT_ENTITY_NAME] = entity_name

    association_properties = get_value("association_properties")
    if association_properties is not None:
        for key, value in association_properties.items():
            common_attributes[
                f"{SpanAttributes.IMPACT_ASSOCIATION_PROPERTIES}.{key}"
            ] = value
