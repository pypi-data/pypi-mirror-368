...
from impact.sdk.semconv_impact import SpanAttributes, ImpactSpanKindValues

from impact.sdk.telemetry import Telemetry
from impact.sdk.tracing import get_tracer, set_workflow_name
from impact.sdk.tracing.tracing import (
    TracerWrapper,
    set_entity_path,
    get_chained_entity_path,
)
from impact.sdk.utils import camel_to_snake
from impact.sdk.utils.json_encoder import JSONEncoder
...
def _should_send_prompts():
    return (
        os.getenv("IMPACT_TRACE_CONTENT") or "true"
...
def _setup_span(entity_name, tlp_span_kind, version):
    ...
    if tlp_span_kind in [
        ImpactSpanKindValues.WORKFLOW,
        ImpactSpanKindValues.AGENT,
    ]:
    ...
    with get_tracer() as tracer:
        ...
        if tlp_span_kind in [
            ImpactSpanKindValues.TASK,
            ImpactSpanKindValues.TOOL,
        ]:
            entity_path = get_chained_entity_path(entity_name)
            set_entity_path(entity_path)

        span.set_attribute(
            SpanAttributes.IMPACT_SPAN_KIND, tlp_span_kind.value
        )
        span.set_attribute(SpanAttributes.IMPACT_ENTITY_NAME, entity_name)
        if version:
            span.set_attribute(SpanAttributes.IMPACT_ENTITY_VERSION, version)
    ...
    try:
        if _should_send_prompts():
            ...
            if _is_json_size_valid(json_input):
                span.set_attribute(
                    SpanAttributes.IMPACT_ENTITY_INPUT,
                    json_input,
                )
    ...
    try:
        if _should_send_prompts():
            ...
            if _is_json_size_valid(json_output):
                span.set_attribute(
                    SpanAttributes.IMPACT_ENTITY_OUTPUT,
                    json_output,
                )
    except TypeError as e:
        Telemetry().log_exception(e)
