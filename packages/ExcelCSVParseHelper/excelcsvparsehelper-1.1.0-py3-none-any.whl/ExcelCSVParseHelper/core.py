...
def tag_scanner_csv_helper_v5(
    ...
    *,
    ...
    """
    Return the Pandas-style row index (0-based, header excluded) of the Nth row
    where `keyword` matches. Returns -1 if not found.

    Matching:
      - exact=False: substring match in selected cells (default).
      - exact=True: whole-cell equality in selected cells.
      - case_sensitive=False by default; set True for case-sensitive matching.

    Columns:
      - By default, searches all columns.
      - Restrict with `columns=[0, 2, ...]` (0-based).
      - Or with `columns_by_name=["Tag", ...]` to map from header (raises ValueError if missing).

    Delimiter:
      - If `delimiter` provided, use it.
      - Else if `sniff=True`, try csv.Sniffer with `sniff_candidates`.
      - Else fallback to comma.

    Files:
      - Supports plain text and gzip-compressed files (.gz/.gzip).

    occurrence:
      - 1 = first match (default), 2 = second, etc. Must be >= 1.
    """
    if occurrence < 1:
    ...
    with _open_text(file_path, encoding=encoding) as f:
        ...
        # Read header; align return index with pandas default (exclude header)
        header = next(reader, None)
        if header is None:
            return -1

        # Map columns_by_name → columns once (backward-compatible: only if provided)
        if columns is None and columns_by_name:
            columns = _map_column_names(header, columns_by_name)

        seen = 0
        for idx, row in enumerate(reader):  # idx is pandas-style row index
            cells = row if case_sensitive else [c.lower() for c in row]
            try:
                probe = cells if columns is None else [cells[i] for i in columns]
            except IndexError:
                # Column restriction asks for out-of-range columns
                return -1
