...
from spicelib import QschEditor
from spicelib.simulators.qspice_simulator import Qspice
from spicelib.sim.tookit.montecarlo import Montecarlo

sallenkey = QschEditor("sallenkey.qsch")  # Reads the qsch file into memory
runner = SimRunner(simulator=Qspice, output_folder='./temp_mc',
...
# Finally the netlist is saved to a file. This file contains all the instructions to run the simulation in Qspice
mc.save_netlist('sallenkey_mc.net')
# -- End of Example 1 --
...
logs.dataset['fcut'] = [x[0] for x in logs.dataset['fcut']]  # Discards the second element
