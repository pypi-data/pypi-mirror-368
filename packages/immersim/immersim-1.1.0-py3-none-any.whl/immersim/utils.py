...
def filepath(filename: str, method: ISType, directory: str=warehouse_directory, cell_id=None, funcs=False) -> str:
    ...
    match method:  # Get correct extension
        case ISType.PICKLE:  # Quick-Save or Checkpoint
            if cell_id is not None:  # Used for checkpoints
                if funcs:
                    save_filename = f'.checkpoints/checkpoint_{cell_id}_funcs{pickle_extension}'
                else:
                    save_filename = f'.checkpoints/checkpoint_{cell_id}{pickle_extension}'
        ...
        case ISType.SHELVE:  # Shelf
            save_filename = f'storage{shelf_extension}'  # Normal shelf storage

        case _:
            raise ValueError(f'Invalid method: {method}')

    # Ensure room in the warehouse
    ensure_directory(directory=directory, filename=filename, checkpoints=cell_id is not None)
