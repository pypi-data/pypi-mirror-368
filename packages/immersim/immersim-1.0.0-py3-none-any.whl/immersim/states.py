...
from .utils import notebook_name, filepath, ISType, saved_vars_blacklist, saved_module_name
...
def quick_save(index=None):
    ...
    saved_module.__dict__.update(filtered_namespace)
...
def quick_load(index=None):
    ...
    loaded_module = dill.load_module(filepath(nb_name, ISType.PICKLE, index=index))
