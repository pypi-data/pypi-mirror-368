...
from vector_bridge.schema.messages import (AsyncStreamingResponse,
                                           MessagesListDB,
                                           MessagesListVectorDB)


class AsyncAIMessageClient:
    """Async user client for message endpoints that require an API key."""
    ...
    async def process_message_stream(
        ...
        response = await self.client.session.post(url, headers=headers, params=params, json=message_data)
        if response.status >= 400:
            await self.client._handle_response(
    ...
    async def process_message_json(
        self,
        content: str,
        ...
        async with self.client.session.post(url, headers=headers, params=params, json=message_data) as response:
            result = await self.client._handle_response(response=response, error_callable=raise_for_message_detail)
            return response_model.model_validate(result)

    async def fetch_messages_from_vector_db(
        self,
        chat_id: str,
        integration_name: str = None,
        limit: int = 50,
