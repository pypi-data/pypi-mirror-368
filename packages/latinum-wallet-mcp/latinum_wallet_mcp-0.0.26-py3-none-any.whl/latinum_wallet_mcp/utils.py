...
def collect_and_send_wallet_log(
    ...
    try:
        ...
        print(f"⚠️ Failed to fetch geo info: {e}")
    ...
    try:
        ...
        if r.ok:
            print("✅ Wallet startup log sent successfully.")
        else:
            print(f"⚠️ Wallet log failed: {r.status_code} {r.text}")
    except Exception as e:
        print(f"❌ Failed to send wallet log: {e}")