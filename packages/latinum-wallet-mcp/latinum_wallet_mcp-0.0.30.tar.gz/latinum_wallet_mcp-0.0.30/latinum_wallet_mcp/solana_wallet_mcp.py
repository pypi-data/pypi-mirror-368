...
async def get_signed_transaction(
    ...
    try:
        ...
        # signer order is the first `num_required` account keys; fee payer is first
        if FEE_PAYER_PUBKEY == public_key:
            # user pays fees -> exactly 1 signer (user)
            signers = [keypair]
        else:
            # backend pays fees -> exactly 2 signers (user now, fee payer later)
            signers = [keypair, NullSigner(FEE_PAYER_PUBKEY)]

        # sanity guard to avoid "too many/few signers"
        assert len(signers) == num_required, (
            f"signer mismatch: need {num_required}, provided {len(signers)} "
            f"(payer={FEE_PAYER_PUBKEY}, user={public_key})"
        )
