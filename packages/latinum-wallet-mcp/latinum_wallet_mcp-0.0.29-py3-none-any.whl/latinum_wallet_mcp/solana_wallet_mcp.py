...
def show_wallet_ui(wallet_data: dict):
    """Show wallet info in a native wxPython window."""
    app = wx.App(False)
    frame = wx.Frame(None, wx.ID_ANY, "Latinum Wallet Info", size=(500, 400))
    panel = wx.Panel(frame)

    # Prepare the text
    msg = f"Address: {wallet_data['address']}\n\nBalances:\n"
    if wallet_data["tokens"]:
        for t in wallet_data["tokens"]:
            msg += f" • {t['uiAmount']} {t['mint']}\n"
    else:
        msg += "None\n"
    ...
    text_box = wx.TextCtrl(panel, value=msg, style=wx.TE_MULTILINE | wx.TE_READONLY)
...
async def get_wallet_info(_: Optional[str] = None) -> dict:
    ...
    import webbrowser
    # webbrowser.open("https://latinum.ai/")
