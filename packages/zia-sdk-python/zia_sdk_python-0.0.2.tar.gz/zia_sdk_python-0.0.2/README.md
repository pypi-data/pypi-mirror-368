...
await client.image_recognition.list_tasks()
await client.image_recognition.create_task(task)
...
# Get specific result
result = await client.image_recognition.get_result(
    task_uuid="task-uuid",
...
For data analysis, you can convert results to pandas DataFrames:

```python
from zia import ir_results_to_dataframe, ir_results_to_summary_dataframe
...
# Analyze the data
print(f"Average detection score: {df_detailed['detection_score'].mean():.3f}")
print(f"Top products: {df_detailed['product_name'].value_counts().head()}")
```
