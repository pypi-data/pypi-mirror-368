...
FEATUREALLOFGEOMETRY1_ONE_OF_SCHEMAS = ["Feature", "FeatureCollection", "GeometryCollection", "LineString", "MultiLineString", "MultiPoint", "MultiPolygon", "Point", "Polygon"]

class FeatureAllOfGeometry1(BaseModel):
    """
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = FeatureAllOfGeometry1.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in FeatureAllOfGeometry1 with oneOf schemas: Feature, FeatureCollection, GeometryCollection, LineString, MultiLineString, MultiPoint, MultiPolygon, Point, Polygon. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in FeatureAllOfGeometry1 with oneOf schemas: Feature, FeatureCollection, GeometryCollection, LineString, MultiLineString, MultiPoint, MultiPolygon, Point, Polygon. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into FeatureAllOfGeometry1 with oneOf schemas: Feature, FeatureCollection, GeometryCollection, LineString, MultiLineString, MultiPoint, MultiPolygon, Point, Polygon. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into FeatureAllOfGeometry1 with oneOf schemas: Feature, FeatureCollection, GeometryCollection, LineString, MultiLineString, MultiPoint, MultiPolygon, Point, Polygon. Details: " + ", ".join(error_messages))
        else:
            return instance
...
FeatureAllOfGeometry1.model_rebuild(raise_errors=False)
