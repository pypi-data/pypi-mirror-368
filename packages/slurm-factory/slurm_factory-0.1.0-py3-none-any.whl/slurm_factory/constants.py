...
CONTAINER_BUILD_OUTPUT_DIR = "/srv/build-output"
...
SPACK_BUILD_CACHE_SCRIPT = Template(
    textwrap.dedent(
        ...
        # Initialize the buildcache mirror if it doesn't have proper layout
        echo 'Initializing buildcache mirror layout...'
        spack buildcache update-index local-buildcache

        # Cache everything that was just built - iterate through all installed packages
        echo 'Checking buildcache for existing packages and pushing new ones...'
        CACHE_UPDATED=false
        for pkg in $$(spack find --format '{{name}}@{{version}}'); do
            echo "Checking if $$pkg exists in buildcache..."
            if ! spack buildcache list local-buildcache | grep -q "^$$pkg"; then
                echo "Pushing $$pkg to buildcache..."
                spack buildcache push local-buildcache "$$pkg"
                CACHE_UPDATED=true
            else
                echo "$$pkg already exists in buildcache, skipping"
            fi
        done

        if [ "$$CACHE_UPDATED" = "true" ]; then
            echo 'Updating buildcache index after new packages...'
            spack buildcache update-index local-buildcache
        else
            echo 'No new packages to cache'
        fi
...
PACKAGE_CREATION_SCRIPT = Template(
    textwrap.dedent(
        ...
        cd ${project_dir}
        MODULE_ROOT=$$(spack location --module-dir)
        SLURM_SPEC="slurm@${slurm_spec_version}"

        echo "Looking for modules in: $$MODULE_ROOT"
        echo "Found module directory at: $$MODULE_ROOT"
        mkdir -p ${slurm_dir}/modules

        # Find and copy Slurm-specific modules
        find "$$MODULE_ROOT" -name "*.lua" -path "*slurm*" -exec cp {} ${slurm_dir}/modules/ \\;

        # Also copy any modules for the current environment
        if [ -d "$$MODULE_ROOT/lmod" ]; then
            cp -r "$$MODULE_ROOT/lmod"/* ${slurm_dir}/modules/
        fi

        # Generate modules explicitly
        echo "Generating modules explicitly..."
        spack module lmod refresh --delete-tree -y
        spack module lmod refresh -y

        # Try to find the generated modules again
        find "$$MODULE_ROOT" -name "*.lua" -exec cp {} ${slurm_dir}/modules/ \\;
...
def get_mkdir_commands(base_dir: str, subdirs: list[str]) -> str:
    """Generate mkdir commands for cache subdirectories."""
    return "\n".join(f"mkdir -p {base_dir}/{subdir}" for subdir in subdirs)
