...
def _get_data_file(filename: str) -> Path:
    """Get path to a data file, handling both development and installed package scenarios."""
    # First try development mode (files in current directory)
    dev_path = Path("data") / filename
    if dev_path.exists():
        return dev_path.resolve()

    # For installed packages, look for data files in standard locations
    try:
        import site
        import sys
    ...
    # Fall back to development mode path
    return dev_path.resolve()
...
def _wait_for_cloud_init_with_output(instance: lxd.LXDInstance):
    ...
    # Use cloud-init status --wait which blocks until completion
    try:
        # This command will block until cloud-init is done
        logger.debug("Running cloud-init status --wait command")
        ...
        if result.returncode == 0:
            logger.debug("Cloud-init completed successfully")
...
def _get_base_instance_name(project_name: str) -> str:
    """Get the base instance name for the project with timestamp."""
    # Use current date as timestamp for base instance naming
    timestamp_str = datetime.now().strftime("%Y%m%d")
    return f"{BASE_INSTANCE_PREFIX}-{timestamp_str}"


def _base_instance_exists(lxc: lxd.LXC, project_name: str) -> bool:
    """Check if the base instance exists and is not expired."""
    base_instance_name = _get_base_instance_name(project_name)
...
def _setup_base_instance_buildcache(lxd_instance: lxd.LXDInstance, version: str, gpu_support: bool = False):
    ...
    spack_config_yaml = generate_yaml_string(slurm_version=version, gpu_support=gpu_support)
    ...
    _stream_exec_output(
        ...
        f"Setting up build cache with dynamic Spack configuration (version={version}, gpu={gpu_support})",
    )


def _create_base_instance(lxc: lxd.LXC, project_name: str, version: str = "25.05") -> str:
    """Create a base instance with cloud-init completed and build cache set up."""
    console = Console()
    base_instance_name = _get_base_instance_name(project_name)

    logger.debug(
        f"Creating base instance '{base_instance_name}' from fresh Ubuntu {LXD_IMAGE} with build cache"
    )
    ...
    try:
        ...
        _setup_base_instance_buildcache(
            lxd_instance, version, gpu_support=False
        )  # Base instance uses CPU-only
...
def _launch_from_base_instance(
    lxc: lxd.LXC, instance_name: str, project_name: str, version: str = "25.05"
) -> str:
    """Launch an instance from the base instance if available, otherwise create it."""
    console = Console()
    base_instance_name = _get_base_instance_name(project_name)

    if _base_instance_exists(lxc, project_name):
        ...
        _create_base_instance(lxc, project_name, version)
...
def build(
    ...
    gpu_support: bool = typer.Option(
        False, "--gpu", help="Enable GPU support (CUDA/ROCm) - creates larger packages"
    ),
):
    """
    ...
    used_instance = _launch_from_base_instance(lxc, instance_name, project_name, version)
    ...
    logger.debug(
        f"Generating dynamic Spack configuration for Slurm {slurm_version} with GPU support: {gpu_support}"
    )

    # Generate the dynamic configuration YAML string
    spack_yaml_content = generate_yaml_string(slurm_version, gpu_support)

    console.print(f"[bold cyan]Generated dynamic Spack configuration for Slurm {slurm_version}[/bold cyan]")
    ...
    console.print(
        f"[bold green]✓ Redistributable packages created in ~/.slurm-factory/build/{version}/[/bold green]"
    )
    ...
    logger.info(f"Build artifacts successfully created at: ~/.slurm-factory/build/{version}/")
    console.print(
        f"[bold green]🎉 Build artifacts available in: ~/.slurm-factory/build/{version}/[/bold green]"
    )
