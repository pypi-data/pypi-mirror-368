...
complete -c git-cliff -l config-url -d 'Sets the URL for the configuration file' -r
...
complete -c git-cliff -s s -l strip -d 'Strips the given parts from the changelog' -r -f -a "header\t''
footer\t''
all\t''"
complete -c git-cliff -l sort -d 'Sets sorting of the commits inside sections' -r -f -a "oldest\t''
newest\t''"
...
complete -c git-cliff -l gitea-repo -d 'Sets the Gitea repository' -r
...
complete -c git-cliff -l use-branch-tags -d 'Include only the tags that belong to the current branch'
...
complete -c git-cliff -l use-native-tls -d 'Load TLS certificates from the native certificate store'
