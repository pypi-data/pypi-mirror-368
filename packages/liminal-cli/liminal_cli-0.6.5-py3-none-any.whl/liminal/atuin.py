...
def uninstall_atuin():
	shutil.rmtree(Path.home() / '.atuin', ignore_errors=True)
	shutil.rmtree(Path.home() / '.local/share/atuin', ignore_errors=True)
...
def test_atuin_recording_locally(shell: Shell) -> dict:
	...
	# some times it takes 1 second for the history to update (perhaps for larger dbs?) (or just be created first time)
	max_attempts = 6
	for _i in range(max_attempts):
		try:
			expected_details = local_history_row(full_command)
