...
class ModifierGroupV2(BaseModel):
    """
    ...
    guid: Optional[StrictStr] = Field(default=None, description="A unique identifier for this modifier group.")
    reference_id: Optional[StrictInt] = Field(default=None, description="An integer identifier that is used to refer to this modifier group.", alias="referenceId")
    pricing_strategy: Optional[StrictStr] = Field(default=None, description="The pricing strategy used for this modifier group.", alias="pricingStrategy")
    modifier_option_references: Optional[List[StrictInt]] = Field(default=None, description="An array of referenceIds for ModifierOption objects.", alias="modifierOptionReferences")
    __properties: ClassVar[List[str]] = ["name", "guid", "referenceId", "pricingStrategy", "modifierOptionReferences"]
    ...
    def pricing_strategy_validate_enum(cls, value):
        ...
        if value not in set(['NONE', 'SEQUENCE_PRICE', 'SIZE_PRICE', 'SIZE_SEQUENCE_PRICE']):
            raise ValueError("must be one of enum values ('NONE', 'SEQUENCE_PRICE', 'SIZE_PRICE', 'SIZE_SEQUENCE_PRICE')")
        return value
