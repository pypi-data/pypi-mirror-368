...
class BPSAPIInterface:
    ...
    def extract_hs_codes(self, keywords: List[str]) -> List[dict[str, str]]:
        ...
        # 尝试多种方式定位PDF文件，以确保在不同环境下都能找到
        pdf_paths_to_try = [
            # 1. 相对于当前工作目录的路径（开发环境）
            "./resources/hs-code/HSCode Master BPS.pdf",
            # 2. 相对于模块文件的路径（打包环境）
            os.path.join(os.path.dirname(__file__), "resources", "hs-code", "HSCode Master BPS.pdf"),
            # 3. 使用pkg_resources获取资源路径（推荐用于打包环境）
            None,  # 特殊值，稍后处理
        ]
        ...
        # 首先尝试前两个路径
        for pdf_path in pdf_paths_to_try[:-1]:
            try:
        ...
        # 如果前两个路径都失败了，尝试使用pkg_resources（适用于打包环境）
        if doc is None:
            try:
                import pkg_resources
                pdf_path = pkg_resources.resource_filename(
                    'bps_searcher', 'resources/hs-code/HSCode Master BPS.pdf'
                )
                doc = fitz.open(pdf_path)
            except Exception as e:
                last_error = e
        ...
        if doc is None:
            raise Exception(
                f"无法找到或打开PDF文件 'HSCode Master BPS.pdf'. "
                f"已尝试多个路径位置。最后一个错误: {str(last_error)}"
            )
