...
class TestBPSAPI:
    ...
    def test_all_models(self):
        ...
        models = [
            ...
            ("vervar", self.bps_client.search_vervar),
    ...
    def test_make_foreign_trade_request(self):
        """测试外贸数据API请求方法"""
        # 使用mock模拟API响应
        with patch('requests.get') as mock_get:
            ...
            result = self.bps_client._make_foreign_trade_request(params)
    ...
    def test_search_foreign_trade_data(self):
        """测试外贸数据查询方法"""
        # 使用mock模拟API响应
        with patch('requests.get') as mock_get:
            mock_response = mock_get.return_value
            mock_response.text = '{"status": "OK", "data": [{"year": "2024", "value": 1000}]}'
            mock_response.raise_for_status.return_value = None
            
            result = self.bps_client.search_foreign_trade_data(
                sumber=1,
                periode=2,
                kodehs="26040000",
                jenishs=2,
    ...
    def test_search_foreign_trade_data_cache(self):
        """测试外贸数据查询缓存机制"""
        # 使用mock模拟API响应
        with patch('requests.get') as mock_get:
            mock_response = mock_get.return_value
            ...
            # 第一次调用
            result1 = self.bps_client.search_foreign_trade_data(
                sumber=1,
                periode=2,
                kodehs="26040000",
                jenishs=2,
            ...
            # 第二次调用相同的参数
            result2 = self.bps_client.search_foreign_trade_data(
                sumber=1,
                periode=2,
                kodehs="26040000",
                jenishs=2,
            ...
            # 验证API只被调用一次（第二次应该从缓存获取）
            assert mock_get.call_count == 1
            assert result1.status == result2.status
            assert result1.data == result2.data

    def test_search_foreign_trade_data_error_handling(self):
        """测试外贸数据查询错误处理"""
        # 使用mock模拟API错误
        with patch('requests.get') as mock_get:
            ...
            assert result.data_availability == "unavailable"
            assert result.data == []
