...
class DatabaseConnection(Connection):
    ...
    def get_by_id(self, id_, cache=False):
        ...
        if r.status_code != 200:
            ...
            try:
                resp = r.json()
                # Hack: With .single() an object SHOULD be returned. This workaround lets us query older PostgRest versions 
...
class PostgrestTabDataConnection():
    ...
    def upsert(self, data, merge_columns=None):
        r = self.api.post(self.table)\
            .jwt_auth(self._jwt_token, { "role": self._db_role })\
            .upsert(merge_on=merge_columns)\
            .json(data)\
            .request()
