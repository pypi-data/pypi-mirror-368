...
class ApiRequest(object):
    ...
    def upsert(self, duplicates="merge", merge_on=None):
        """Enable bulk upsert.

        For single item upsert use PUT + eq. parameter
        http://postgrest.org/en/v6.0/api.html#upsert

        :param duplicates (str): "ignore" or "merge"
        :param merge_on (str): Column(s) that has a UNIQUE constraint that will be used to merge on.
        :returns: The API request object.
        :rtype: ApiRequest
        """
    ...
    def request(self, cache=False):
        ...
        """
        def the_request(attempt=1, max_attempts=2, retry_codes=[503]):
            r = requests.request(self.method,
                                    ...
                                    json=self._json)
            # Hack: Fly.dev returns 503 on a sleeping instance. Retry once.
            # 502 seems to the a temporary error, so we should retry as well
            if r.status_code in [502, 503] and "fly.dev" in r.request.url:
                if attempt < max_attempts:
                    import time
                    time.sleep(3)
                    return the_request(attempt=attempt+1)
                else:
                    return r
            return r
