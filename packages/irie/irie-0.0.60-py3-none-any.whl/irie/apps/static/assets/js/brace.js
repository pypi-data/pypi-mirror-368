...
document.addEventListener("DOMContentLoaded", function(event) {
  ...
  */
    ...
    if (document.getElementById('input-slider-range')) {
        var rng =  document.getElementById("input-slider-range"),
            low =  document.getElementById("input-slider-range-value-low"),
            hgh =  document.getElementById("input-slider-range-value-high"),
            lst = [document, hgh];

        noUiSlider.create(c, {
            start: [parseInt(low.getAttribute('data-range-value-low')), 
                    parseInt(hgh.getAttribute('data-range-value-high'))],
            connect: !0,
            tooltips: true,
            range: {
                min: parseInt(rng.getAttribute('data-range-value-min')),
                max: parseInt(rng.getAttribute('data-range-value-max'))
            }
        }), rng.noUiSlider.on("update", function (a, b) {
            lst[b].textContent = a[b]
        });
    }
