...
import cx_Oracle
from .global_service_pkg import config
...
def enable_debug_logs(debug_active_by):
    ...
    def expire_debug_logs(start_by, start_time):
        expiry_time = (datetime.strptime(start_time, '%Y-%m-%d %H:%M:%S') + timedelta(minutes=5)).strftime('%Y-%m-%d %H:%M:%S')
        logging.info(f"[DEBUG LOG FLAG] TTL thread started for debug logs, will expire at {expiry_time}")
        # Wait for 2 minutes or until interrupted
        interrupted = _ttl_interrupt_event.wait(timeout=300)
        if interrupted:
