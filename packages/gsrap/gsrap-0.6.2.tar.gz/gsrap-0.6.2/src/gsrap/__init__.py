...
def main():
    ...
    runsims_parser.add_argument("--fva", action='store_true', help="Perform FVA during growth simulations (use with --media).")
    runsims_parser.add_argument("--synth", action='store_true', help="Test metabolites' synthesis on media (use with --media).")
    runsims_parser.add_argument("-s", "--cnps", metavar='', type=str, default='-', help="Starting C, N, P and S source metabolites (comma-separated IDs); if not provided, anternative substrate analysis will be skipped (use with --media).")
    runsims_parser.add_argument("-b", "--biolog", metavar='', type=str, default='-', help="Strain ID associated to binary Biolog(R) PM1, PM2A, PM3B and PM4A plates; if not provided, Biolog(R) simulations will be skipped (use with --cnps and --media).")
    runsims_parser.add_argument("--omission", action='store_true', help="Perform single omission experiments to study auxotrophies (use with --media).")
    runsims_parser.add_argument("--essential", action='store_true', help="Predict essential genes (single-gene knock-out simulations) (use with --media).")
    runsims_parser.add_argument("--factors", action='store_true', help="Predict putative growth factors (use with --media).")
