...
class Rectangle(_SimpleShape):
    ...
    # @property
    # def elastic(self):
    #     Iy = self.b * self.d**3 / 12
    #     Iz = self.d * self.b**3 / 12
    #     A  = self.b * self.d
        # def cmm(self):
        #     """
        #     Compute the second moment of area about the centroidal axes.
        #     """
        #     b = self.b
        #     d = self.d
        #     I2 = b*d**3/12
...
class Angle(_SimpleShape):
    ...
    b: float
    d: float
    _parameters = ["b", "d", "t", "k", "mesh_scale"]
    def __init__(self, b, d, t, k=None,
        ...
        if k-t > 0:
            ...
            exterior = np.array([
                ...
                # ),
                *(
                    [k,k] - (k-t)*np.array([np.cos(angle), np.sin(angle)])
                    for angle in np.linspace(np.pi/2, 0, nk)
                ),
                [   t,  d],
                # *(
        ...
        if angle is not None:
            polygon.rotate(exterior, 
                           angle-polygon.principal_angle(exterior))
