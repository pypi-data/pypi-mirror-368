...
def gemm(architecture_dict: OrderedDict, workload_dict: OrderedDict=None):
    ...
    mac_array_dict = OrderedDict({'count': mac_array_total, 'factor': {'cycle_count': 0.5, 'dynamic_energy': 0.75}})
    sram_rd_dict = OrderedDict({'count': sram_rd_num, 'factor': {'cycle_count': 0.25, 'dynamic_energy': 0.75}})
    sram_wr_dict = OrderedDict({'count': sram_wr_num, 'factor': {'cycle_count': 0.75, 'dynamic_energy': 0.5}})
...
def mac_array(architecture_dict: OrderedDict, workload_dict: OrderedDict=None):
    ...
    mac_array_size = get_prod(architecture_dict['multiplier']['instance'])
    multiplication_dict = OrderedDict({'count': mac_array_size, 'factor': {'dynamic_energy': 0.5, 'cycle_count' : 0.5}})
    adder_dict = OrderedDict({'count': mac_array_size, 'factor': {'dynamic_energy': 0.5, 'cycle_count' : 0.5}})
    
    performance_dict['subevent'] = OrderedDict({'multiplication' : multiplication_dict, 
                                                'adder' : adder_dict})

    return performance_dict


def multiplication(architecture_dict: OrderedDict, workload_dict: OrderedDict=None):
    # this is an array of mac units
    performance_dict = OrderedDict()
    performance_dict['cycle_count'] = OrderedDict({'value': 1., 'unit': 'cycles'})
    performance_dict['runtime'] = OrderedDict({'value': 1 / 1000 / architecture_dict['multiplier']['query']['frequency'] , 'unit': 'ms'})
    ...
    multiplier_dict = OrderedDict({'count': 1, 'factor': {'dynamic_energy': 1, 'cycle_count': 0.5}})
...
def sram_rd(architecture_dict: OrderedDict, workload_dict: OrderedDict=None):
    ...
    sram_dict = OrderedDict({'operation': OrderedDict({'dynamic_energy' : 'read'}), 'factor': {'dynamic_energy': 0.5}})
...
def sram_wr(architecture_dict: OrderedDict, workload_dict: OrderedDict=None):
    ...
    performance_dict['cycle_count'] = OrderedDict({'value': 1., 'unit': 'cycle'})
    performance_dict['runtime'] = OrderedDict({'value': 1 / 1000 / architecture_dict['sram']['query']['frequency'] , 'unit': 'ms'})
    sram_dict = OrderedDict({'operation': OrderedDict({'dynamic_energy' : 'write'}), 'factor': {'dynamic_energy': 0.5}})
    performance_dict['subevent'] = OrderedDict({'sram' : sram_dict})
    return performance_dict
