...
from archx.metric import create_metric_dict, save_metric_dict, aggregate_event_metric, create_event_metrics, query_module_metric, aggregate_tag_metric, aggregate_event_count
...
def test_count():
    ...
    workload = 'gemm32'
    event = 'mac_array'
    logger.info(f'\n\nTest <{index}>: Aggregate count for event <{event}> in workload <{workload}>.')
    result = aggregate_event_count(event_graph=event_graph, workload=workload, event=event)
    logger.success(f'result <{result}>.')
