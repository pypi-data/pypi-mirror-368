...
def scatter_plot(
  ...
  #  for Proj in proj:
       ...
       if Sigma is not None:
           ...
           # Preliminares para límites y paleta según variable
           OMPm = [value for value in OMP if isinstance(value, float)]
           vmin = np.nanmin(OMPm)
           vmax = np.nanmax(OMPm)
           Ninterv = 10
       
           # Ajustes por tipo de variable
...
#
           ...
           elif vartyp in ['omp', 'oma', 'bcorr', 'stdomp']:
               vmin = np.min(OMP)
               vmax = np.max(OMP)
               if len(filesin) > 1:
                   OMP =(abs(avg2)-abs(avg1))*100/-abs(avg1)
           ...
           # Sincronización: límites, colormap y norm para poligonos y colorbar
           if len(filesin) > 1:
              N = 21  # Debe ser impar!
             # vmax = max(np.abs(OMP))
             # vmin = -vmax
              boundaries = np.linspace(vmin, vmax, N)
              y = (boundaries[:-1] + boundaries[1:]) / 2
              neg_colors = [cm.RdYlBu_r(x) for x in np.linspace(0.08, 0.40, N//2)]
              blanco = (1, 1, 1, 1) 
              blanco = (1.0, 1.0, 0.6, 1.0) 
              pos_colors = [cm.RdYlBu_r(x) for x in np.linspace(0.60, 0.92, N//2)]
       #       colores = neg_colors + [blanco] + pos_colors 
              over_color = '#e600c7'   # fucsia, igual que set_over
              under_color = '#0235ad'  # azul, igual que set_under
              colores =  neg_colors + [blanco] + pos_colors  

            #a  colores = neg_colors + pos_colors
              cmap = colors.ListedColormap(colores)
              cmap.set_under('#0235ad')
              cmap.set_over('#e600c7')
              #cmap.set_under(colores[0])     # mismo que el extremo negativo
              #cmap.set_over(colores[-1])  
              norm = colors.BoundaryNorm(boundaries, N)
              extend_color = 'both'
              ticks = boundaries
            #  if 0 not in ticks:
            #      ticks = sorted(np.append(ticks, 0))
           else:
...
###########
           ...
           for i in range(len(nombre)):
               ...
               if mask:
                   ...
                   if POINTS == 'ON':
                       ...
                       if boxsizex >=10:
                          plt.text(point[0], point[1], round_first_digit_3(OMP[i]), color="k", fontsize=5, zorder=5, ha='center', va='center', weight='bold')
                       ...
                       if valor < vmin:
                           col = '#0235ad'         # mismo que cmap.set_under
                       elif valor > vmax:
                           col = '#e600c7'         # mismo que cmap.set_over
                       else:
                           ind = np.digitize(valor, boundaries) - 1
                           # Asegúrate de que ind esté dentro de rango (casi siempre lo estará aquí)
                           col = Colors[inds[i]]       # tu array de N-1 colores (NO incluye over/under)
                       if len(filesin) == 1:
           ...
           # Colorbar: usa el mismo cmap y norm que los poligonos
          ...
          # if vartyp == 'obs':
         #      vartyp = f'AVG(OBS) {units}'
          #     units = f'AVG(obsvalue) {units}'
          #     labels = [str(int(round(b))) for b in ticks]
          ...
          # else:
          #     print ('sum(OBS)')
          #     cb2.ax.set_ylabel(f'sum(nobs)/day', fontsize=18, rotation=90, labelpad=20)
          ...
          #     cb2.ax.yaxis.set_major_formatter(FuncFormatter(scientific))
           ...
           # Box info inf. derecha
           props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
           if vartyp in [f'dens', 'nobs']:
               textstr = f'Nobs=%.2i (in %.2f days)' % (ONMAP, NDAYS)
           else:
               textstr = '$\\bar{\\mu}=%.3f$ $\\bar{\\sigma}=%.3f$ \nNobs=%.2i (in %.2f days)' % (Mu, Sigma, ONMAP, NDAYS)
           ...
           if vcoord == 'join':
               plt.savefig(f'{pathwork}/{family}/{fonction}_{proj}_{layers}_id_stn_{id_stn}_{region}_vcoord{vcoord}_varno{varno}.png', dpi=600, format='png')
           else:
               plt.savefig(f'{pathwork}/{family}/{fonction}_{proj}_{layers}_id_stn_{id_stn}_{region}_vcoord{int(vcoord)}_varno{varno}.png', dpi=600, format='png')
           plt.close(fig) 
