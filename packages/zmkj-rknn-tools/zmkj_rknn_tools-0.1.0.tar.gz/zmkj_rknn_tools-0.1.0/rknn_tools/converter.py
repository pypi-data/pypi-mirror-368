...
def pt_to_onnx(model_path, output_path=None, img_size=640, batch_size=1, opset=12, simplify=True):
    ...
    try:
        ...
        if success:
            ...
            # 判断文件是否存在
            if not os.path.exists(torchscript_file):
                logging.error(f"导出的TorchScript模型文件不存在: {torchscript_file},使用.rknn")
                torchscript_file = os.path.join(model_dir, name_without_ext + '.rknn')
                if not os.path.exists(torchscript_file):
                    logging.error(f"导出的TorchScript模型文件不存在: {torchscript_file},使用.无法继续")
                    return False, None
