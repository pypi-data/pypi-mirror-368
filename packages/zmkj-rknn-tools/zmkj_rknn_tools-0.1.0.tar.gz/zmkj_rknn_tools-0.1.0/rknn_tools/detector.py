...
class YOLOv8Detector:
    ...
    def detect(self, image_path=None, image=None, conf_thresh=None, nms_thresh=None):
        """
        检测图像中的目标
        
        Args:
            image_path: 图像路径，如果为None则使用image参数
            image: 图像数据，如果为None则使用image_path参数
        ...
        if get_config('skip_frames') > 0 and self.frame_count % (get_config('skip_frames') + 1) != 0:
            if self.last_frame_hash in self.result_cache:
                ...
                logging.debug(f"使用缓存结果，跳过第{self.frame_count}帧处理")
        ...
        try:
            ...
            if orig_height > 720 and get_config('downscale_factor') > 1:
                ...
                logging.debug(f"图像下采样处理: {self.downscale_factor}倍")
            else:
