...
def call_llm_stream(ctx: CallContext, input_text: str) -> Generator[str, None, None]:
    ...
    # For streaming, use mock response to avoid event loop complexity
    mock_response = f"{input_text}는 프로그래밍 언어입니다. 이것은 스트리밍 응답의 예시입니다."
    response_parts = []
...
def stream_example():
    ...
    result_parts = []
    for part in stream:
        print(f"📡 Streamed: {part}", end="", flush=True)
...
async def async_stream_example():
    ...
    async for part in stream:
        print(f"📡 Async streamed: {part}", end="", flush=True)
        result_parts.append(part)
