  ...
  for (int op = 0; op < params.num_operators; op++) {
    ...
    if (op_type == yirage::type::TB_INPUT_OP) {
      ...
      header << "\t" << stensor_ptr_name(input_smem_offset)
             ...
             << ",\n";
      header << "\t\tshape = (" << dtensor_matrix_shape.x << ", "
             << dtensor_matrix_shape.y << "),\n";
      header << "\t\tblock_shape = (" << stensor_matrix_shape.x << ", "
             << stensor_matrix_shape.y << "),\n";
      string tb_offset_row =
          block_offset_calculation(input_matrix_row_offset_block_stride.x,
                                   input_matrix_row_offset_block_stride.y,
                                   input_matrix_row_offset_block_stride.z);
      string tb_offset_col =
          block_offset_calculation(input_matrix_column_offset_block_stride.x,
                                   input_matrix_column_offset_block_stride.y,
                                   input_matrix_column_offset_block_stride.z);
      header << "\t\toffsets = (" << tb_offset_row << ", " << tb_offset_col
             << "),\n";
      // Assume row major layout for now
      header << "\t\tstrides = (" << stensor_matrix_shape.y << ", 1),\n";
      header << "\t\torder = (1, 0))\n";
      if ((input_matrix_row_offset_forloop_stride != 0) ||
          (input_matrix_column_offset_forloop_stride != 0)) {
        main << "\t\t" << stensor_name(input_smem_offset) << " = tl.load("
      ...
      header << "\t\tblock_shape = (" << stensor_matrix_shape.x << ", "
             << stensor_matrix_shape.y << "),\n";
      string tb_offset_row =
          block_offset_calculation(output_matrix_row_offset_block_stride.x,
                                   output_matrix_row_offset_block_stride.y,
                                   output_matrix_row_offset_block_stride.z);
      string tb_offset_col =
          block_offset_calculation(output_matrix_column_offset_block_stride.x,
      ...
      if (op_type >= yirage::type::TB_REDUCTION_0_TO_DIMX_OP &&
        ...
        if (reduction_dim == stensor.num_dims - 1) {
          ...
          main << "\t\t" << stensor_name(output_smem_offset) << " = tl.reshape("
               ...
               << reduction_dimx << ", " << stensor.dim[stensor.num_dims - 1]
               << "))\n";
          main << "\t\t" << stensor_name(output_smem_offset) << " = tl.sum("
        ...
        if (reduction_dim == stensor.num_dims - 1) {
          main << "\t\t" << stensor_name(output_smem_offset) << " = tl.reshape("
               << stensor_name(input_smem_offset) << ", ("
               << stensor.dim[stensor.num_dims - 2] << ", "
               << stensor.dim[stensor.num_dims - 1] << ", " << 1 << "))\n";
          main << "\t\t" << stensor_name(output_smem_offset) << " = tl.sum("
               << stensor_name(output_smem_offset) << ", axis=1)\n";
        } else if (reduction_dim == stensor.num_dims - 2) {
          main << "\t\t" << stensor_name(output_smem_offset) << " = tl.reshape("
               << stensor_name(input_smem_offset) << ", ("
               << stensor.dim[stensor.num_dims - 2] << ", " << 1 << ", "
...
#ifdef DEADCODE
        // The tensor in triton is 2D, so we need to adjust the reduction
        // dimension
        main << "\t\t" << stensor_name(output_smem_offset) << " = tl.sum("
             << stensor_name(input_smem_offset)
             << ", axis=" << reduction_dim - stensor.num_dims + 2
             << ", keep_dims=True)\n";
#endif
      } else {
        assert(false);
