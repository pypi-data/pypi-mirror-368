...
public:
  ...
  CUTLASS_DEVICE
  RedunctionExecutor(yirage::type::TBOperatorType type,
                     char *smem_buffer,
                     STensor const &input,
                     STensor const &output,
                     int thread_id,
                     int num_threads) {
    int reduction_dim = yirage::utils::get_reduction_dim(type);
...
public:
  CUTLASS_DEVICE
  SimpleRedunctionExecutor( // yirage::type::TBOperatorType type,
