...
class ASGCN_Unit(nn.Module):
    ...
    def forward(self, inputs):
        ...
        aspect_double_idx = torch.cat(
            [
                left_len.unsqueeze(1),
                torch.where(
                    (left_len + aspect_len - 1) < self.config.max_seq_len,
...
class ASGCN(nn.Module):
    ...
    def __init__(self, bert, config):
        ...
        self.dense = (
            nn.Linear(self.config.hidden_dim * 6, self.config.output_dim)
            if self.config.lsa
            else nn.Linear(self.config.hidden_dim * 2, self.config.output_dim)
        )
