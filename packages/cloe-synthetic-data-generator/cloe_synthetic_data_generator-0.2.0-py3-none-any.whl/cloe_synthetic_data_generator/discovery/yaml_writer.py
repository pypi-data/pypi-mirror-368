...
def write_config_to_yaml(config: DataGenConfig, output_path: Path) -> None:
    ...
    config_dict = {
        ...
        "target": {
            ...
            "write_mode": config.target.write_mode,
        },
    }
    ...
    for column in config.columns:
        column_dict = {
            "name": column.name,
            "data_type": column.data_type.value,  # Convert enum to string
            "nullable": column.nullable,
            "faker_function": column.faker_function,
            "faker_options": column.faker_options,
        }
...
def write_configs_to_directory(
    ...
    Args:
        ...
        filename_template: Template for filenames (can use {table_name}, {catalog}, {schema})
