...
pub fn enrich_trade_direction(events: &mut [SwapEvent]) {
    ...
    for event in events {
        ...
        // Prefer explicit flag if present; otherwise, fall back to amount0In > 0
        let v2_token0_sold = event
            .raw_data
            .get("v2_token0_sold")
            .and_then(|v| v.as_bool())
            .unwrap_or_else(|| {
                // Fallback: derive from amount0In > 0
                event
                    .raw_data
                    .get("amount0In")
                    .and_then(|v| v.as_str())
            ...
            (&event.token0_address, &event.token1_address)
        {
            if v2_token0_sold {
                ...
                // Direction label for analytics
                event.add_enriched_field(
                    "trade_direction".to_string(),
                    serde_json::Value::String("token0_to_token1".to_string()),
                );
                event.trade_direction = Some("token0_to_token1".to_string());
            } else {
                ...
                event.add_enriched_field(
                    "trade_direction".to_string(),
                    serde_json::Value::String("token1_to_token0".to_string()),
                );
                event.trade_direction = Some("token1_to_token0".to_string());
            }
        }
    }
}
