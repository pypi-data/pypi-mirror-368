...
class Deepl:
    ...
    async def TranslateAsync(self, text, target_lang, source_lang = None, target_model = None):
        ...
        try:
            trans = await asyncio.create_task(self.get_translations(text, target_lang, source_lang, target_model))
        except Exception as e:
    ...
    def Translate(self, text, target_lang, source_lang = None, target_model = None):
        if self.connection == False:
            return {"status":1,"msg":"Not connected to an session (Do you forget to call \"*.Session()\" ?)"}
        elif self.connection == None:
            return {"status":1,"msg":"Invalid Session (Session might break in case of errors)"}
        ...
        if (source_lang != None and source_lang not in self.sources_langs):
            return {"status":1,"msg":f"Invalid source language <{source_lang}>"}
        try:
            trans = asyncio.run(self.get_translations(text, target_lang, source_lang, target_model))
        except Exception as e:
            return {"status":1,"msg":e}
        if (trans ==  ""):
            return {"status":1,"msg":""}
