import click
from .modbus.client import create_client, ModbusClient, ModbusReadRequest, RequestResponse, RequestResult
...
def modbus() -> None:
    """
    Commande pour effectuer une requete Modbus
    """
    pass

@comtools.group()
def demo() -> None:
    """demo CLI - (utile pour test)
    """
...
def serial(port, baudrate, slave_id, parity, command_type, address, datatype) -> None:
    ...
    if command_type == 'read_coil':
        ...
        client = create_client("serial", port=port, baudrate=baudrate, parity=parity)
