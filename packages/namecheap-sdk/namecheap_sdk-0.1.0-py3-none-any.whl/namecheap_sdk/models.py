...
from namecheap_sdk.logging import logger
...
class XMLModel(BaseModel):
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        logger.debug(f"Attempted to access non-existent field '{name}' on {self.__class__.__name__}")
        logger.debug(f"Available fields: {list(self.model_fields.keys())}")
        if extra_data:
            logger.debug(f"Extra data contains: {list(extra_data.keys())}")

        raise AttributeError(f"'{self.__class__.__name__}' has no attribute '{name}'. " f"Available fields: {', '.join(self.model_fields.keys())}")
...
class DomainCheck(XMLModel):
    ...
    # Fee fields
    icann_fee: Decimal | None = Field(default=None, alias="@IcannFee", description="ICANN fee if applicable")
    ...
    your_price: Decimal | None = Field(default=None, alias="@YourPrice", description="Your account's discounted price")
    retail_price: Decimal | None = Field(default=None, alias="@RetailPrice", description="Suggested retail price")
...
class DNSRecord(XMLModel):
    ...
    type: Literal["A", "AAAA", "CNAME", "MX", "NS", "TXT", "URL", "URL301", "FRAME"] = Field(alias="@Type")
