...
from namecheap_sdk.models import Contact, Domain, DomainCheck
        ...
        if include_pricing and results:
            ...
            if needs_pricing:
                try:
                    ...
                    logger.warning(f"Could not fetch pricing (domains will show as available " f"but without prices): {type(e).__name__}: {e}")
    ...
    def _get_pricing(self, domains: builtins.list[str]) -> dict[str, dict[str, Decimal | None]]:
        ...
        for tld, domain_list in tld_groups.items():
            try:
                ...
                logger.debug(f"Response keys: " f"{list(result.keys()) if isinstance(result, dict) else 'Not a dict'}")
                ...
                if isinstance(result, dict):
                    ...
                    for category in categories:
                        ...
                        category_name_normalized = category.get("@Name_normalized", category_name.lower())
                        logger.debug(f"Checking category: {category_name} " f"(normalized: {category_name_normalized})")
                                ...
                                if product_name.lower() == tld.lower():
                                    ...
                                    for price in price_info:
                                        ...
                                        if duration == "1":
                                            ...
                                            logger.debug(f"Found prices for {tld}: " f"regular={regular_price}, " f"your={your_price}, " f"retail={retail_price}")

                                            # Apply to all domains with this TLD
                                            for domain in domain_list:
                                                pricing[domain] = {
                                                    "regular_price": Decimal(regular_price) if regular_price else None,
                                                    "your_price": Decimal(your_price) if your_price else None,
