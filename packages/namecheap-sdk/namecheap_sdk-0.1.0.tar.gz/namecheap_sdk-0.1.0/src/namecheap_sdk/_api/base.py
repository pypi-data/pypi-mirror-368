...
from namecheap_sdk.errors import ErrorCode, NamecheapError
from namecheap_sdk.logging import ErrorDisplay, logger

if TYPE_CHECKING:
    from namecheap_sdk.client import Namecheap
...
def normalize_xml_response(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in data.items():
        ...
        if "@YourAdditionalCost" in data and "@YourAdditonalCost" in data:
            logger.warning("🎉 Namecheap might be fixing their typo! Both @YourAdditionalCost and " "@YourAdditonalCost exist in response. Monitor this!")

        # Recursively normalize nested structures
        if isinstance(value, dict):
            normalized[key] = normalize_xml_response(value)
        elif isinstance(value, list):
            normalized[key] = [normalize_xml_response(item) if isinstance(item, dict) else item for item in value]
...
class BaseAPI:
    ...
    def _request(
        ...
        *,
        ...
        url = "https://api.sandbox.namecheap.com/xml.response" if self.config.sandbox else "https://api.namecheap.com/xml.response"
