...
class AgentAI:
        ...
        if not self._initialization_complete:
            try:
                ...
                if self.agent.ai_config.audio_model != self.agent.ai_config.model:
                    await self.agent.ai_config.get_model_limits(self.agent.ai_config.audio_model)
                
                if self.agent.ai_config.vision_model != self.agent.ai_config.model:
                    await self.agent.ai_config.get_model_limits(self.agent.ai_config.vision_model)
    ...
    async def ai(
        ...
        **kwargs,
        ...
        if not hasattr(final_config, "enable_rate_limit_retry") or final_config.enable_rate_limit_retry is None:
        ...
        try:
            ...
            raise ImportError("litellm is required for token-aware prompt trimming. Please install it with `pip install litellm`")

        # Determine token limit from config or LiteLLM model info
        model_max_tokens = None
        try:
            model_max_tokens = final_config.max_input_tokens or get_max_tokens(final_config.model)
        except Exception as e:
            log_debug(f"Could not auto-detect max tokens for model {final_config.model}: {e}")
        ...
        if final_config.max_tokens and final_config.max_tokens < model_max_tokens:
            safe_token_limit = max(1, model_max_tokens - final_config.max_tokens)
        else:
            safe_token_limit = model_max_tokens

        # Only trim if necessary based on tokens
        trimmed_messages = trim_messages(messages, final_config.model, max_tokens=safe_token_limit)
        if len(trimmed_messages) != len(messages) or any(m1 != m2 for m1, m2 in zip(messages, trimmed_messages)):
            log_debug(f"Prompt exceeded safe token limit ({safe_token_limit} tokens), applied token-aware trimming via LiteLLM")
        ...
        log_debug("Making LiteLLM call", payload={k: str(v)[:100] + "..." if len(str(v)) > 100 else v for k, v in litellm_params.items()})
        ...
        async def _execute_with_fallbacks():
            ...
            if not fallback_models and getattr(final_config, "final_fallback_model", None):
            ...
            if fallback_models:
                ...
                for m in all_models:
                    try:
                        if "/" not in m:
                            log_debug(f"Skipping model {m} - no provider specified in model name")
                            raise ValueError(f"Invalid model spec: '{m}'. Must include provider prefix, e.g. 'openai/gpt-4'.")
                        litellm_params["model"] = m
                        return await litellm.acompletion(**litellm_params)
                    except Exception as e:
                        log_debug(f"Model {m} failed with {e}, trying next fallback if available...")
                        last_exception = e
                        continue
                ...
                if "/" not in final_config.model:
                    raise ValueError(f"Invalid model spec: '{final_config.model}'. Must include provider prefix, e.g. 'openai/gpt-4'.")
                return await _make_litellm_call()

        if final_config.enable_rate_limit_retry:
            rate_limiter = self._get_rate_limiter()
            try:
                response = await rate_limiter.execute_with_retry(_execute_with_fallbacks)
            except Exception as e:
            ...
            try:
                ...
                log_debug(f"LiteLLM HTTP call failed: {e.response.status_code} - {e.response.text}")
                raise
