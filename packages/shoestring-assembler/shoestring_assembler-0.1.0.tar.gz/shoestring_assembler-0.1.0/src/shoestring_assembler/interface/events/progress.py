...
class Progress:
    ...
    @classmethod
    async def new_tracker(cls, key, label, total, initial_value=0):
        inst = cls.get_instance()
...
class ProgressBar:
    def __init__(self, label, total, current, callback):
        self.total = total
    ...
    @property
    def details(self):
        return {"label": self.label, "value": self.current, "total": self.total}
