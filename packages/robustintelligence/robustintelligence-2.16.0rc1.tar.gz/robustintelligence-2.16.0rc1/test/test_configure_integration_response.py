# coding: utf-8

"""
    Robust Intelligence REST API

    API methods for Robust Intelligence. Users must authenticate using the `rime-api-key` header.

    The version of the OpenAPI document: 1.0
    Contact: dev@robustintelligence.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501

import unittest

from ri.apiclient.models.configure_integration_response import ConfigureIntegrationResponse

class TestConfigureIntegrationResponse(unittest.TestCase):
    """ConfigureIntegrationResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ConfigureIntegrationResponse:
        """Test ConfigureIntegrationResponse
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ConfigureIntegrationResponse`
        """
        model = ConfigureIntegrationResponse()
        if include_optional:
            return ConfigureIntegrationResponse(
                integration_info = ri.apiclient.models.integration_info.IntegrationInfo(
                    configured = True, 
                    integration = ri.apiclient.models.integration.Integration(
                        creation_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        id = ri.apiclient.models.id.ID(
                            uuid = '', ), 
                        level = 'INTEGRATION_LEVEL_UNSPECIFIED', 
                        name = '', 
                        schema = ri.apiclient.models.integration_schema.IntegrationSchema(
                            variables = [
                                ri.apiclient.models.for_further_information_on_supported_integration_variables,_visit
https://docs/robustintelligence/com/en/latest/administration/workspace_configuration/integrations/configuring_integrations/html.For further information on supported integration variables, visit
https://docs.robustintelligence.com/en/latest/administration/workspace_configuration/integrations/configuring_integrations.html(
                                    name = '', 
                                    sensitivity = 'VARIABLE_SENSITIVITY_UNSPECIFIED', 
                                    value = '', )
                                ], ), 
                        type = 'INTEGRATION_TYPE_UNSPECIFIED', 
                        workspace_id = ri.apiclient.models.id.ID(
                            uuid = '', ), ), )
            )
        else:
            return ConfigureIntegrationResponse(
        )
        """

    def testConfigureIntegrationResponse(self):
        """Test ConfigureIntegrationResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()