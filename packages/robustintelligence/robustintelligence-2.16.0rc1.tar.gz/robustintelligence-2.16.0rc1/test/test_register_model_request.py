# coding: utf-8

"""
    Robust Intelligence REST API

    API methods for Robust Intelligence. Users must authenticate using the `rime-api-key` header.

    The version of the OpenAPI document: 1.0
    Contact: dev@robustintelligence.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501

import unittest

from ri.apiclient.models.register_model_request import RegisterModelRequest

class TestRegisterModelRequest(unittest.TestCase):
    """RegisterModelRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> RegisterModelRequest:
        """Test RegisterModelRequest
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `RegisterModelRequest`
        """
        model = RegisterModelRequest()
        if include_optional:
            return RegisterModelRequest(
                agent_id = ri.apiclient.models.id.ID(
                    uuid = '', ),
                external_id = '',
                integration_id = ri.apiclient.models.id.ID(
                    uuid = '', ),
                metadata = ri.apiclient.models.metadata.Metadata(
                    extra_info = '', 
                    tags = [
                        ''
                        ], ),
                model_endpoint_integration_id = ri.apiclient.models.id.ID(
                    uuid = '', ),
                model_info = ri.apiclient.models.model_info.ModelInfo(
                    hugging_face = ri.apiclient.models.hugging_face_model_info.HuggingFaceModelInfo(
                        kwargs = '', 
                        model_uri = '', ), 
                    model_path = ri.apiclient.models.model_path_info.ModelPathInfo(
                        path = '', ), ),
                name = '',
                project_id = ri.apiclient.models.uniquely_specifies_a_project/.Uniquely specifies a Project.(),
                skip_validation = True
            )
        else:
            return RegisterModelRequest(
                name = '',
        )
        """

    def testRegisterModelRequest(self):
        """Test RegisterModelRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()