...
def DFG_ballerina(root_node, index_to_code, states):
    ...
    if (
        ...
        if root_node.type == "param":
            ...
            # Parameters have no initial value, just declare them
            ...
            # Handle variable declarations: local_var_decl_stmt
            # Find typed_binding_pattern and expression
            typed_binding_pattern = None
            expression = None
            
            for child in root_node.children:
                if child.type == "typed_binding_pattern":
                    typed_binding_pattern = child
                elif child.type == "expression":
                    expression = child
            
            # Extract variable name from typed_binding_pattern -> binding_pattern -> identifier
            name_nodes = []
            if typed_binding_pattern:
                for child in typed_binding_pattern.children:
                    if child.type == "binding_pattern":
            ...
            if expression is None:
                ...
                # Has initial value - process expression first
                temp, states = DFG_ballerina(expression, index_to_code, states)
                DFG += temp
                
                # Create data flow from expression to variable
                for name_node in name_nodes:
                    name_indexs = tree_to_variable_index(name_node, index_to_code)
                    value_indexs = tree_to_variable_index(expression, index_to_code)
                    for index1 in name_indexs:
                        idx1, code1 = index_to_code[index1]
                        for index2 in value_indexs:
                            idx2, code2 = index_to_code[index2]
        ...
        # Handle assignments: assign_stmt
        DFG = []
        
        # Find lvexpr and expression
        lvexpr = None
        expression = None
        
        for child in root_node.children:
            if child.type == "lvexpr":
                lvexpr = child
            elif child.type == "expression":
                expression = child
        
        # Process right side (expression) first
        if expression:
            temp, states = DFG_ballerina(expression, index_to_code, states)
            DFG += temp
            
        # Create data flow from expression to lvexpr
        if lvexpr and expression:
            left_indexs = tree_to_variable_index(lvexpr, index_to_code)
            right_indexs = tree_to_variable_index(expression, index_to_code)
            for index1 in left_indexs:
                idx1, code1 = index_to_code[index1]
                for index2 in right_indexs:
                    idx2, code2 = index_to_code[index2]
                    DFG.append((code1, idx1, "computedFrom", [code2], [idx2]))
                states[code1] = [idx1]
        return sorted(DFG, key=lambda x: x[1]), states
    elif root_node.type in function_statement:
        # Handle function definitions - process ALL children including signature (parameters)
        DFG = []
        for child in root_node.children:
        ...
        for child in root_node.children:
            if child.type == "stmt_block":  # Correct node type for Ballerina
                temp, new_states = DFG_ballerina(child, index_to_code, states)
                DFG += temp
                others_states.append(new_states)
            else:
                temp, current_states = DFG_ballerina(child, index_to_code, current_states)
                DFG += temp
        ...
        for key in current_states:
            new_states[key] = current_states[key]
        for other_states in others_states:
            for key in other_states:
                if key not in new_states:
                    new_states[key] = other_states[key]
                else:
                    new_states[key] = sorted(list(set(new_states[key] + other_states[key])))
        return sorted(DFG, key=lambda x: x[1]), new_states
    elif root_node.type in while_statement:
        # Handle while loops
        DFG = []
        ...
        # Handle return statements - process the return expression to capture variable references
        # Note: In Ballerina, identifiers are deeply nested in expression layers, so we need
        # to recursively process ALL children to find the actual variable references
        DFG = []
        for child in root_node.children:
            temp, states = DFG_ballerina(child, index_to_code, states)
            DFG += temp
