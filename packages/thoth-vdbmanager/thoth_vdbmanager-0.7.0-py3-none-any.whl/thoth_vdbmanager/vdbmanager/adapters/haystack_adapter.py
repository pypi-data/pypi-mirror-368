...
class HaystackVectorStoreAdapter(VectorStoreInterface):
    ...
    def _create_external_embedding_manager(self) -> ExternalEmbeddingManager:
        ...
        embedding_provider = os.environ.get('EMBEDDING_PROVIDER') or self.embedding_provider
        embedding_model = os.environ.get('EMBEDDING_MODEL') or self.embedding_model
        embedding_api_key = os.environ.get('EMBEDDING_API_KEY')
        
        if not embedding_provider or not embedding_api_key:
            raise ValueError(
                "External embedding provider configuration missing. "
                "Please set EMBEDDING_PROVIDER and EMBEDDING_API_KEY environment variables"
            )
        
        logger.info(f"Creating external embedding manager: {embedding_provider}")
        return ExternalEmbeddingManager.from_env()
