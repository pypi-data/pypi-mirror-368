...
class QdrantAdapter(HaystackVectorStoreAdapter):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Filter out embedding-related parameters that QdrantDocumentStore doesn't accept
        embedding_provider = kwargs.pop('embedding_provider', None)
        
        # Create Qdrant document store with only supported parameters
        document_store = QdrantDocumentStore(
            ...
            **{k: v for k, v in kwargs.items() if k not in ["hnsw_config", "embedding_provider", "embedding_model"]}
        )
