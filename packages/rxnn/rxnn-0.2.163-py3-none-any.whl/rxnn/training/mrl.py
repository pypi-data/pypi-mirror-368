...
class MrlReplayBuffer:
    ...
    def initialize(self, collected: list[MrlTrajectoryEpisode]):
        self.trajectories = [{ **self._process_episode(ep), 'age': 0 } for ep in collected].sort(
            key=lambda ep: ep['score'], reverse=True
        )[:self.size].sort(key=lambda _: random.random())
    ...
    def update(self, collected: list[MrlTrajectoryEpisode]):
        ...
        if removed < self.update_size:
            self.trajectories = filtered.sort(key=lambda ep: ep['score'])[self.update_size - removed:]
        else:
            self.trajectories = filtered

        processed_collected = [{ **self._process_episode(ep), 'age': 0 } for ep in collected].sort(
            key=lambda ep: ep['score'], reverse=True
        )[:self.update_size]

        self.trajectories = (self.trajectories + processed_collected).sort(key=lambda _: random.random())
