...
int     gen_ml_kem_test_vec(void);
int     check_ml_kem_use_test_vec(void);
...
int gen_ml_kem_test_vec(void) {
    ...
    unsigned char keypair_coins[2*ML_KEM_SYMBYTES];
    unsigned char kem_enc_coins[ML_KEM_SYMBYTES];

    char filename[32] = {0};
    sprintf(filename, "./ml_kem_%d_test_vectors.txt", ML_KEM_MODE);
    ...
    for(int i = 0; i < NTESTS; ++i) {
        ...
        randombytes(keypair_coins, 2*ML_KEM_SYMBYTES);
        randombytes(kem_enc_coins, ML_KEM_SYMBYTES);
        ...
        fprintBstr(fp, "keypair coins = ", keypair_coins, 2*ML_KEM_SYMBYTES);
        fprintBstr(fp, "kem_enc coins = ", kem_enc_coins, ML_KEM_SYMBYTES);
...
int check_ml_kem_use_test_vec(void) {
    ...
    char filename[32] = {0};
    sprintf(filename, "./ml_kem_%d_test_vectors.txt", ML_KEM_MODE);
    printf("Check ml_kem_%d using test vector file: %s\n", ML_KEM_MODE, filename);
    ...
    do {
        ...
        if ( !ReadHex(fp, keypair_coins, 2*ML_KEM_SYMBYTES, "keypair coins = ") ){
            printf("\n\t[-] Read keypair coins error! Make sure TEST VEC file is correct.\n");
            continue;
        }
        if ( !ReadHex(fp, kem_enc_coins, ML_KEM_SYMBYTES, "kem_enc coins = ") ){
        ...
        if ( memcmp(sk, sk_ground_truth, CRYPTO_SECRETKEYBYTES) ) {
            ...
            for(int i = 0; i < CRYPTO_SECRETKEYBYTES; i++) {
                if (sk[i] != sk_ground_truth[i]) {
                    printf("at idx: %d error, sk: 0x%x, ground: 0x%x\n", i,sk[i],sk_ground_truth[i]);
                }
            }
            failed = 1;
            continue;
