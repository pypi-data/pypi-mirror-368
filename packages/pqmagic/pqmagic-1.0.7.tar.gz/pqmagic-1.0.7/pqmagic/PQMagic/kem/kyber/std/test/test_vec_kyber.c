...
int     gen_kyber_test_vec(void);
int     check_kyber_use_test_vec(void);
...
int gen_kyber_test_vec(void) {
    ...
    unsigned char keypair_coins[2*KYBER_SYMBYTES];
    unsigned char kem_enc_coins[KYBER_SYMBYTES];

    char filename[32] = {0};
    sprintf(filename, "./kyber_%d_test_vectors.txt", KYBER_MODE);
    ...
    for(int i = 0; i < NTESTS; ++i) {
        ...
        randombytes(keypair_coins, 2*KYBER_SYMBYTES);
        randombytes(kem_enc_coins, KYBER_SYMBYTES);
        ...
        fprintBstr(fp, "keypair coins = ", keypair_coins, 2*KYBER_SYMBYTES);
        fprintBstr(fp, "kem_enc coins = ", kem_enc_coins, KYBER_SYMBYTES);
...
int check_kyber_use_test_vec(void) {
    ...
    char filename[32] = {0};
    sprintf(filename, "./kyber_%d_test_vectors.txt", KYBER_MODE);
    printf("Check kyber_%d using test vector file: %s\n", KYBER_MODE, filename);
    ...
    do {
        ...
        if ( !ReadHex(fp, keypair_coins, 2*KYBER_SYMBYTES, "keypair coins = ") ){
            printf("\n\t[-] Read keypair coins error! Make sure TEST VEC file is correct.\n");
            continue;
        }
        if ( !ReadHex(fp, kem_enc_coins, KYBER_SYMBYTES, "kem_enc coins = ") ){
