...
void core_alg_cycle_bench(void)
{
  ...
  print_results("SLH-DSA (SPHINCS+) keypair:", t, NTESTS);

  for(i = 0; i < NTESTS; ++i) {
    t[i] = cpucycles();
    crypto_sign_signature(sm, &smlen, sm, SPX_MLEN, sk);
  }
  print_results("SLH-DSA (SPHINCS+) sign:", t, NTESTS);

  for(i = 0; i < NTESTS; ++i) {
    t[i] = cpucycles();
    crypto_sign_verify(sm, CRYPTO_BYTES, sm, SPX_MLEN, pk);
  }
  print_results("SLH-DSA (SPHINCS+) verify:", t, NTESTS);
...
void core_alg_benchmark(double bench_time) {
  ...
  printf("SLH-DSA (SPHINCS+) keypair %lf times in %lf s.\n", n, (double)clk2 / CLOCKS_PER_SEC);

  printf("SLH-DSA (SPHINCS+) keypair speed: %10.3lf times/s\n", (n) / (((double)clk2) / CLOCKS_PER_SEC));
  ...
  printf("SLH-DSA (SPHINCS+) sign %lf times in %lf s.\n", n, (double)clk2 / CLOCKS_PER_SEC);

  printf("SLH-DSA (SPHINCS+) sign speed: %10.3lf times/s\n", (n) / (((double)clk2) / CLOCKS_PER_SEC));
  ...
  printf("SLH-DSA (SPHINCS+) verify %lf times in %lf s.\n", n, (double)clk2 / CLOCKS_PER_SEC);

  printf("SLH-DSA (SPHINCS+) verify speed: %10.3lf times/s\n", (n) / (((double)clk2) / CLOCKS_PER_SEC));
