...
int     gen_aigis_sig_test_vec(void);
int     check_aigis_sig_use_test_vec(void);
...
int gen_aigis_sig_test_vec(void) {
    ...
    char filename[32] = {0};
    sprintf(filename, "./aigis_sig_%d_test_vectors.txt", AIGIS_SIG_MODE);
    ...
    for(int i = 0; i < NTESTS; ++i) {
        ...
        // randombytes(sign_coins, RNDBYTES);
        ...
        crypto_sign_signature_internal(sig, &siglen, mext, MLEN + 5 + 2, sk);
        ...
        // fprintBstr(fp, "sign coins = ", sign_coins, RNDBYTES);
...
int check_aigis_sig_use_test_vec(void) {
    ...
    unsigned char keypair_coins[SEEDBYTES];
    // unsigned char sign_coins[RNDBYTES];

    char filename[32] = {0};
    sprintf(filename, "./aigis_sig_%d_test_vectors.txt", AIGIS_SIG_MODE);
    printf("Check aigis_sig_%d using test vector file: %s\n", AIGIS_SIG_MODE, filename);
    ...
    do {
        ...
        // if ( !ReadHex(fp, sign_coins, RNDBYTES, "sign coins = ") ){
        ...
        // Generate sig.
        if ( (ret_val = crypto_sign_signature_internal(sig, &siglen, mext, mlen + ctx_len + 2, sk)) != 0) {
...
int main() {
    // gen_aigis_sig_test_vec();
    check_aigis_sig_use_test_vec();
}