...
def fused_layer_norm_forward_kernel_wrapper(
    x,
    w,
    b,
    eps,
...
def fused_layer_norm_backward_kernel_wrapper(
    grad_out,
    x,
    w,
    mean,
    ...
    grad_w = torch.empty((B, num_tiles, D), dtype=torch.float32, device=w.device)
    grad_b = torch.empty((B, num_tiles, D), dtype=torch.float32, device=w.device)
    ...
    if elementwise_affine:
        grad_w = grad_w.view(-1, D).sum(dim=0).to(w.dtype)
        grad_b = grad_b.view(-1, D).sum(dim=0).to(w.dtype)
    else:
...
class LayerNorm(torch.autograd.Function):
    ...
    def forward(ctx, x, w, b, eps=1e-5, elementwise_affine=True, layout=Layout.BND_BND):
        ...
        out, mean, rstd = fused_layer_norm_forward_kernel_wrapper(
            x,
            w,
            b,
            eps,
    ...
    def backward(ctx, grad_out):
        ...
        grad_x, grad_w, grad_b = fused_layer_norm_backward_kernel_wrapper(
            grad_out,
            x,
            w,
            mean,
