...
from pyappleinternal.bonjour import DEFAULT_BONJOUR_TIMEOUT, browse_remoted
from pyappleinternal.common import get_home_folder
from pyappleinternal.exceptions import InvalidServiceError, NoDeviceConnectedError, pyappleinternalException, \
    StartServiceError
from pyappleinternal.lockdown import LockdownClient, create_using_remote
from pyappleinternal.lockdown_service_provider import LockdownServiceProvider
from pyappleinternal.pair_records import get_local_pairing_record, get_remote_pairing_record_filename
from pyappleinternal.remote.remotexpc import RemoteXPCConnection
from pyappleinternal.service_connection import ServiceConnection
...
class RemoteServiceDiscoveryService(LockdownServiceProvider):
    ...
    def start_lockdown_service(self, name: str, include_escrow_bag: bool = False) -> ServiceConnection:
        ...
        checkin = {'Label': 'pyappleinternal', 'ProtocolVersion': '2', 'Request': 'RSDCheckin'}
        ...
        if response['Request'] != 'RSDCheckin':
            raise pyappleinternalException(f'Invalid response for RSDCheckIn: {response}. Expected "RSDCheckIn"')
        response = service.recv_plist()
        if response['Request'] != 'StartService':
            raise pyappleinternalException(f'Invalid response for RSDCheckIn: {response}. Expected "ServiceService"')
        return service
    ...
    def start_lockdown_developer_service(self, name, include_escrow_bag: bool = False) -> ServiceConnection:
        try:
            ...
            logging.getLogger(self.__module__).error(
                ...
                'You can do so using: pyappleinternal mounter mount'
            )
            raise
