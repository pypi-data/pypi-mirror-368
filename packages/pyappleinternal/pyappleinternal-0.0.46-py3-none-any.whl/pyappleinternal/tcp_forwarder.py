...
from pyappleinternal.exceptions import ConnectionFailedError
...
class TcpForwarderBase:
    ...
    def __init__(self, listening_event: threading.Event = None):
        """
    ...
    def start(self, address='127.0.0.1',taskid=None,udid=None,port=None,callback=None):
    ...
    def _handle_data(self, from_sock, closed_sockets):
        ...
        try:
            ...
            if not data:
                if callable(self.callback_):
                    self.callback_({"disconnect":"Connection closed by the peer."})
    ...
    def _handle_server_connection(self):
        ...
        try:
            ...
            if callable(self.callback_):
                self.callback_({"connect_error":f'failed to connect to port: {22}'})
            self.logger.error(f'failed to connect to port: {22}')
...
class UsbmuxTcpForwarder(TcpForwarderBase):
    ...
    def __init__(self, serial: str,   listening_event: threading.Event = None,
        ...
        """
        super().__init__( listening_event)
        self.serial = serial
        self.dst_port = 22
...
class LockdownTcpForwarder(TcpForwarderBase):
    ...
    def __init__(self, service_provider: LockdownServiceProvider,  service_name: str,
