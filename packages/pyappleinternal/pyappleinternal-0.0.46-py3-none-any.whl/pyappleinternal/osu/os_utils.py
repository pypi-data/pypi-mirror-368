...
from pyappleinternal.exceptions import FeatureNotSupportedError, OSNotSupportedError
    ...
    def create(cls) -> 'OsUtils':
        if cls._instance is None:
            ...
            if cls._os_name == 'win32':
                from pyappleinternal.osu.win_util import Win32
                cls._instance = Win32()
            elif cls._os_name == 'darwin':
                from pyappleinternal.osu.posix_util import Darwin
                cls._instance = Darwin()
            elif cls._os_name == 'linux':
                from pyappleinternal.osu.posix_util import Linux, Wsl
                cls._instance = Wsl() if is_wsl() else Linux()
            elif cls._os_name == 'cygwin':
                from pyappleinternal.osu.posix_util import Cygwin
