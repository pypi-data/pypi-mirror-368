...
from pyappleinternal.exceptions import ConnectionFailedError, DeprecationError, pyappleinternalException
    ...
    * Reboot, shutdown or put the device in sleep mode.
    ...
    def mobilegestalt(self, keys: list[str] = None) -> dict:
        ...
        if (response['Status'] != 'Success') or (response['Diagnostics']['MobileGestalt']['Status'] != 'Success'):
            raise pyappleinternalException('failed to query MobileGestalt')
    ...
    def action(self, action: str) -> Optional[dict]:
        ...
        if response['Status'] != 'Success':
            raise pyappleinternalException(f'failed to perform action: {action}')
