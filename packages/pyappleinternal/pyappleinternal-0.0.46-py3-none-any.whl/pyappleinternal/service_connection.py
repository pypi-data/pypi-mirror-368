...
from pyappleinternal.exceptions import ConnectionTerminatedError, DeviceNotFoundError, NoDeviceConnectedError, \
    pyappleinternalException
from pyappleinternal.osu.os_utils import get_os_utils
from pyappleinternal.usbmux import MuxDevice, select_device
...
def parse_plist(payload: bytes) -> dict:
    ...
    :raises pyappleinternalException: If the payload is invalid.
    ...
    try:
        ...
        try:
            ...
            raise pyappleinternalException(f'parse_plist invalid data: {payload[:100].hex()}')
    except plistlib.InvalidFileException:
        raise pyappleinternalException(f'parse_plist invalid data: {payload[:100].hex()}')
