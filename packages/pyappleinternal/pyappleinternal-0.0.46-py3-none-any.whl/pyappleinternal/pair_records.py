...
from pyappleinternal.exceptions import MuxException, NotPairedError
...
def get_local_pairing_record(identifier: str, pairing_records_cache_folder: Path) -> Optional[dict]:
    ...
    :rtype: Optional[dict]
    """
    logger.debug('Looking for pyappleinternal pairing record')
    path = pairing_records_cache_folder / f'{identifier}.{PAIRING_RECORD_EXT}'
    if not path.exists():
        logger.debug(f'No pyappleinternal pairing record found for device {identifier}')
        return None
