...
FILE_DOCSTRING_REGEX = re.compile(r"^(\s*)\/\*(.*?)\*\/", re.DOTALL)
DEF_DOCSTRING_REGEX = re.compile(r".*\/\*(.*?)\*\/(\s*)$", re.DOTALL)
...
class OuterScope(ScopeContents):
    ...
    def _set_file_docstring(self):
        match = FILE_DOCSTRING_REGEX.search(self._scad_code)
        if not match:
    ...
    def _set_definition_docstrings(self):
        ...
        for i, definition in enumerate(self.definitions):
            ...
            end_of_last = [definition.tree.end_line, definition.tree.end_column]
            if not code_before:
                ...
                continue
            match = DEF_DOCSTRING_REGEX.search(code_before)
            if match:
                if i == 0 and match.group(1) == self._docs:
