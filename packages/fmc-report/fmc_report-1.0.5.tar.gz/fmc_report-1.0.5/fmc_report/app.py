    ...
    if request.method == 'GET':
        if check_and_set_credentials():
            ...
            if fmc:
                ...
                app.logger.info(f'The domain UUID is {uuid}')
                access_policies = fmc.policy.accesspolicy.get(uuid=uuid)
                app.logger.info(f'The access policies are: {access_policies}')
...
def fmc_download():
    ...
    if request.method == 'POST':
        ...
        return send_file(csv_file_path, as_attachment=True, download_name="export.csv")
...
def get_uuid(fmc: FMC) -> Optional[str]:
    try:
        ...
        app.logger.error(f"get_uuid: {error}")
