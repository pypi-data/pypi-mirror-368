...
def optimize_wf_heat_pump(dir_name, power_act, config_0, bounds_0, **kwargs):
    """
    ...
    **kwargs :
    ...
    if opt_global == "dif_evol":
        result = differential_evolution(_opti_hp_func,
                                        bnds,
                                        args=(power_act, config_0, dir_name, paths),
                                        #workers=3,
                                        maxiter=max_iter,
                                        )
...
def _opti_hp_func(x, power_act, conf, dir_name, paths, verbose=False, **kwargs):
    """
    ...
    **kwargs : TYPE
    ...
    try:
        cop, ou, wa, fi, ax = cb.hp_comp.heat_pump(dir_name, power_act,
                                                   config=conf_act)
        ...
        return 5.9
    if any(ns.value > 0 for ns in wa.values()):
        return sum(item.value for item in wa.values())
    return -cop * factor
...
def _opti_orc_func(x, cop, q_dot_h, conf, dir_name, paths, verbose=False, **kwargs):
    ...
    **kwargs : TYPE
    ...
    try:
        eta_th, ou, wa, fi, ax = cb.orc_comp.orc(dir_name, cop, q_dot_h,
                                                   config=conf_act)
    ...
    if eta_th < 0:
        return - eta_th * 10.00
    elif any(ns.value > 0 for ns in wa.values()):
        return sum(item.value for item in wa.values())
...
def optimize_cb(dir_names, power, configs, bounds, **kwargs):
    """
    ...
    **kwargs :
    ...
    bnds_tot=[]
    for key in ["hp", "orc"]:
        x[key], bnds[key], paths[key] = extract_optim_data_with_paths(configs[key], bounds[key])
        x0+=x[key]
        bnds_tot+=bnds[key]
    ...
    args_act =(power, configs, dir_names, paths)
...
def _opti_cb_func(x, power, configs, dir_names, paths, verbose=False, **kwargs):
    """
    Function for heat pump optimization, p_high and 4-component mixture
    ...
    **kwargs : TYPE
    ...
    try:
        rte, result = cb.cb_comp.cb_calc(dir_names, power,
                                                   config=conf_act)
            ...
            if len(path) >= 2 and path[-2] == "fractions":
                ...
                for key in path[:-2]:  # Go through path up to 'fractions'
                    if key == "working_fluid" and "working_fluid" in config_dict:
                        if "species" in config_dict["working_fluid"]:
                            species_list = parse_species_string(config_dict["working_fluid"]["species"])
                            break
                ...
                if species_list:
                    ...
                    if fraction_idx < len(species_list):
                        all_names.append(f"{species_list[fraction_idx]}_fraction")
                    else:
