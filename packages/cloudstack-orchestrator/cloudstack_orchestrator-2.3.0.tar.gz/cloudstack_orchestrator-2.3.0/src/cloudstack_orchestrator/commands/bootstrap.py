"""Bootstrap Kubernetes clusters for CloudStack Orchestrator."""
...
console = Console()
app = typer.Typer(help="Bootstrap Kubernetes clusters for CSO")
...
"""
            ...
            config_path = Path.home() / ".cso" / "kind-config.yaml"
    ...
    async def destroy(self, runtime: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        ...
        if runtime == "kind":
            cluster_name = kwargs.get("cluster_name", "cso-local")
            result = subprocess.run(
...
class GCPKubernetesBootstrapper(KubernetesBootstrapper):
    ...
    def __init__(self, console: Console):
        ...
        self.terraform_dir = Path.home() / ".cso" / "terraform" / "gcp"
...
resource "google_container_cluster" "cso_cluster" {
...
resource "google_container_node_pool" "cso_nodes" {
  name       = "${var.cluster_name}-pool"
  cluster    = google_container_cluster.cso_cluster.id
  location   = local.zone
  node_count = 3
  ...
  node_config {
    ...
    service_account = google_service_account.cso_nodes.email
    oauth_scopes = [
    ...
    tags = ["cso-cluster", var.cluster_name]
  }
}

resource "google_service_account" "cso_nodes" {
  account_id   = "${var.cluster_name}-nodes"
  display_name = "CSO GKE Nodes Service Account"
  project      = var.project_id
}
...
resource "google_project_iam_member" "node_log_writer" {
  ...
  member  = "serviceAccount:${google_service_account.cso_nodes.email}"
}
...
output "cluster_name" {
  value = google_container_cluster.cso_cluster.name
}

output "cluster_endpoint" {
  value     = google_container_cluster.cso_cluster.endpoint
  sensitive = true
}

output "cluster_ca_certificate" {
  value     = google_container_cluster.cso_cluster.master_auth[0].cluster_ca_certificate
  sensitive = true
}
...
def create_cluster(
    provider: BootstrapProvider = typer.Argument(
        ..., help="Provider to use for bootstrapping (local or gcp)"
    ),
    project_id: Optional[str] = typer.Option(
        None, "--project-id", "-p", help="GCP Project ID (required for GCP)"
    ),
    region: str = typer.Option(
        "us-central1", "--region", "-r", help="GCP region for GKE cluster"
    ),
    cluster_name: str = typer.Option(
        "cso-cluster", "--cluster-name", "-c", help="Cluster name"
    ),
    ...
    if provider == BootstrapProvider.LOCAL:
        ...
        console.print("1. Run 'cso setup' to install CloudStack Orchestrator")
        ...
        console.print("\n[cyan]Next steps:[/cyan]")
        console.print(
            "1. Run 'cso setup --provider gcp --domain your-domain.com' to install CSO"
        )
...
def destroy_cluster(
    provider: BootstrapProvider = typer.Argument(
        ..., help="Provider of the cluster to destroy (local or gcp)"
    ),
    cluster_name: str = typer.Option(
        "cso-cluster", "--cluster-name", "-c", help="Cluster name (for Kind clusters)"
    ),
    ...
    if not force:
        ...
        if provider == BootstrapProvider.GCP:
            console.print(
...
def cluster_status(
    provider: Optional[BootstrapProvider] = typer.Argument(
        None, help="Provider to check (local or gcp). If not specified, checks both."
    ),
):
    ...
    providers_to_check = (
        [provider] if provider else [BootstrapProvider.LOCAL, BootstrapProvider.GCP]
    )
