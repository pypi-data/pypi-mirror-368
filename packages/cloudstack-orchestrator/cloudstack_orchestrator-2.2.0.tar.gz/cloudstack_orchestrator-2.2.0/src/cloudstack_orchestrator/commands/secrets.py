"""CSO Secrets Management with Native Python SDKs - No CLI dependencies required"""
...
console = Console()
secrets = typer.Typer(help="Manage secrets with AWS and GCP (SDK-based, no CLI required)")
    ...
    def setup_gcp(self) -> str:
        ...
        if not os.getenv("GOOGLE_APPLICATION_CREDENTIALS"):
            if self.credentials and Path(self.credentials).exists():
                os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = str(Path(self.credentials).absolute())
            else:
                default_creds = Path.home() / ".gcp" / "cso-secrets-key.json"
                if default_creds.exists():
                    os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = str(default_creds)
        
        return self.project or os.getenv("GOOGLE_CLOUD_PROJECT") or "cloudstack-468414"
...
def secrets_callback(
    project: Optional[str] = Option(None, "--project", envvar="GOOGLE_CLOUD_PROJECT", help="GCP project ID"),
    region: Optional[str] = Option(None, "--region", envvar="AWS_REGION", help="AWS region"),
    credentials: Optional[str] = Option(None, "--credentials", envvar="GOOGLE_APPLICATION_CREDENTIALS", help="Path to credentials file")
):
...
class AWSSecretsManager:
    ...
    def __init__(self, region: str = None, profile: str = None):
        ...
        try:
            ...
            console.print(f"[green]✓ Connected to AWS Account: {self.account_id}[/green]")
        except NoCredentialsError:
            console.print("[red]AWS credentials not found. Please configure:[/red]")
            console.print("  Option 1: Set environment variables AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY")
    ...
    def create_secret(self, name: str, data: Dict[str, Any], description: str = None) -> bool:
        ...
        try:
            ...
            response = self.client.create_secret(
                Name=full_name,
                Description=description or f"CSO managed secret: {name}",
                SecretString=secret_string
            )
            console.print(f"[green]✓ Created secret '{full_name}' in AWS Secrets Manager[/green]")
            return True
        except ClientError as e:
            if e.response['Error']['Code'] == 'ResourceExistsException':
                ...
                try:
                    ...
                    console.print(f"[green]✓ Updated secret '{full_name}' in AWS Secrets Manager[/green]")
                    return True
...
class GCPSecretsManager:
    ...
    def __init__(self, project_id: str = None, credentials_path: str = None):
        ...
        if not self.project_id:
            ...
            try:
                ...
                console.print("[yellow]No GCP project specified. Set GOOGLE_CLOUD_PROJECT or use --project[/yellow]")
        ...
        try:
            ...
            console.print(f"[green]✓ Connected to GCP Project: {self.project_id}[/green]")
        except Exception as e:
            console.print(f"[red]GCP Authentication Error: {e}[/red]")
            console.print("[yellow]Please configure GCP credentials:[/yellow]")
            console.print("  Option 1: Set GOOGLE_APPLICATION_CREDENTIALS to service account key file")
            console.print("  Option 2: Run 'gcloud auth application-default login' if gcloud is installed")
    ...
    def create_secret(self, name: str, data: Dict[str, Any], description: str = None) -> bool:
        ...
        try:
            ...
            secret = self.client.create_secret(
                request={
                    ...
                    "secret": {
                        ...
                        "labels": {"managed-by": "cso", "description": (description or "")[:63]}
                    }
                }
            )
            ...
            console.print(f"[green]✓ Created secret '{secret_id}' in GCP Secret Manager[/green]")
            return True
            ...
            try:
                ...
                console.print(f"[green]✓ Updated secret '{secret_id}' in GCP Secret Manager[/green]")
                return True
            except Exception as e:
...
@secrets.command()
def init(
    provider: Provider = Option(Provider.AWS, "--provider", "-p", help="Cloud provider"),
    project: Optional[str] = Option(None, "--project", help="GCP project ID"),
    region: Optional[str] = Option(None, "--region", help="AWS region"),
    credentials: Optional[str] = Option(None, "--credentials", help="Path to credentials file")
):
    """Initialize and test connection to secrets provider"""
    console.print(f"[bold]Initializing {provider.value.upper()} Secrets Manager...[/bold]")
    
    try:
        if provider == Provider.AWS or provider == Provider.BOTH:
            aws_mgr = AWSSecretsManager(region=region)
            console.print("[green]✓ AWS Secrets Manager ready[/green]")
        
        if provider == Provider.GCP or provider == Provider.BOTH:
            gcp_mgr = GCPSecretsManager(project_id=project, credentials_path=credentials)
            console.print("[green]✓ GCP Secret Manager ready[/green]")
        
        console.print("\n[bold green]Secrets management initialized successfully![/bold green]")
        console.print("\nYou can now use:")
        console.print("  cso secrets create <name> --provider <aws|gcp>")
        console.print("  cso secrets list --provider <aws|gcp>")
        console.print("  cso secrets get <name> --provider <aws|gcp>")
...
def create(
    ...
    data: Optional[str] = Option(None, "--data", "-d", help="JSON data or @filename or key=value,key2=value2"),
    interactive: bool = Option(False, "--interactive", "-i", help="Interactive mode"),
    description: Optional[str] = Option(None, "--description", help="Secret description")
):
    ...
    if interactive or (not data):
        console.print("[cyan]Enter secret key-value pairs (empty key to finish):[/cyan]")
        while True:
            key = Prompt.ask("Key", default="")
            if not key:
                break
            hide_input = "password" in key.lower() or "secret" in key.lower() or "key" in key.lower()
        ...
        if data.startswith('@'):
            ...
            try:
                ...
                if '=' in data:
                    ...
                    console.print("[red]Invalid data format. Use JSON, @filename, or key=value,key2=value2[/red]")
                    raise typer.Exit(1)
        ...
        for key in list(secret_data.keys()):
            if 'password' in key.lower() and not key.endswith('_hash'):
                if Confirm.ask(f"Hash password for '{key}'?", default=True):
                    hashed = bcrypt.hashpw(secret_data[key].encode('utf-8'), bcrypt.gensalt())
...
def test_connection(
    ...
    try:
        ...
        if provider == Provider.AWS:
            console.print(f"[green]✓ Connected to AWS Secrets Manager[/green]")
            console.print(f"  Account: {mgr.account_id}")
            console.print(f"  Region: {mgr.region}")
        elif provider == Provider.GCP:
            console.print(f"[green]✓ Connected to GCP Secret Manager[/green]")
...
def list_secrets(
    provider: Provider = Option(Provider.BOTH, "--provider", "-p", help="Cloud provider"),
    prefix: Optional[str] = Option(None, "--prefix", help="Filter by prefix")
):
    """List secrets from provider(s)"""
    
    table = Table(title="CSO Secrets")
...
def get_secret(
    ...
    output: Optional[str] = Option(None, "--output", "-o", help="Output format (json, yaml)")
):
...
@secrets.command()
def setup(
    provider: Provider = Option(..., "--provider", "-p", help="Cloud provider to setup"),
    project: Optional[str] = Option(None, "--project", help="GCP project ID"),
    key_file: Optional[str] = Option(None, "--key-file", help="Path to service account key file"),
    auto_open: bool = Option(True, "--auto-open/--no-open", help="Auto-open browser for setup")
):
    """Quick setup for cloud provider authentication"""
    
    if provider == Provider.GCP:
        project_id = project or "cloudstack-468414"
        
        # Quick auth check
        try:
            mgr = GCPSecretsManager(project_id=project_id)
            console.print(f"[green]✓ Already authenticated for project: {project_id}[/green]")
            return
        except:
            pass
        
        sa_email = f"cso-secrets-manager@{project_id}.iam.gserviceaccount.com"
        
        # If key file provided, just set it up
        if key_file:
            if Path(key_file).exists():
                os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = str(Path(key_file).absolute())
                os.environ["GOOGLE_CLOUD_PROJECT"] = project_id
                
                # Test connection
                try:
                    mgr = GCPSecretsManager(project_id=project_id)
                    console.print(f"[green]✓ Setup complete! Project: {project_id}[/green]")
                    
                    # Save to user's .gcp directory
                    import platform
                    if platform.system() == "Windows":
                        dest_path = Path(f"C:\\Users\\{os.environ.get('USERNAME')}\\.gcp\\cso-secrets-key.json")
                    else:
                        dest_path = Path.home() / ".gcp" / "cso-secrets-key.json"
                    ...
                    if platform.system() == "Windows":
                        import subprocess
                        subprocess.run(f'setx GOOGLE_APPLICATION_CREDENTIALS "{dest_path}"', shell=True, capture_output=True)
                        subprocess.run(f'setx GOOGLE_CLOUD_PROJECT "{project_id}"', shell=True, capture_output=True)
                        console.print("[dim]Environment variables set (restart terminal to use)[/dim]")
        ...
        if auto_open:
            ...
            webbrowser.open(url)
            console.print(f"[green]✓ Opened GCP Console[/green]")
        
        console.print(f"\n1. Create service account: [cyan]cso-secrets-manager[/cyan]")
        console.print(f"2. Grant role: [cyan]Secret Manager Admin[/cyan]")
        console.print(f"3. Download JSON key")
        console.print(f"\nThen run: [yellow]cso secrets setup --provider gcp --key-file <path-to-key.json>[/yellow]")
    
    elif provider == Provider.AWS:
        # Quick auth check
        try:
            mgr = AWSSecretsManager()
            console.print(f"[green]✓ Already authenticated! Account: {mgr.account_id}[/green]")
            return
        except:
            pass
        ...
        console.print("\nThen run: [yellow]cso secrets init --provider aws[/yellow]")

@secrets.command()
def configure(
    provider: Provider = Option(..., "--provider", "-p", help="Cloud provider to configure")
):
    """Interactive configuration wizard for cloud providers"""
    console.print(f"[bold cyan]Configuring {provider.value.upper()} Secrets Management[/bold cyan]\n")
    ...
    console.print(f"cso secrets init --provider {provider.value}")
