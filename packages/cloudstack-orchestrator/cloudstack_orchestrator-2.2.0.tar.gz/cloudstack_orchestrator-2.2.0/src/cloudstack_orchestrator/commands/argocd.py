...
def update_argocd_password(password: str) -> bool:
    ...
    try:
        ...
        subprocess.run([
            ...
            "-n", ARGOCD_NAMESPACE, "--type", "merge",
            "-p", json.dumps({"data": {"password": encoded_password}})
        ], capture_output=True, text=True)
    ...
    try:
        with httpx.Client(timeout=10.0) as client:
            ...
            if response.status_code == 200:
                ...
                if token:
                    console.print("[green]✓ Successfully authenticated with ArgoCD[/green]")
                    return True
            else:
        ...
        console.print("[yellow]Cannot connect to ArgoCD. Ensure port-forward is active.[/yellow]")
        console.print("[dim]Run: kubectl port-forward svc/argocd-server -n argocd 30080:80[/dim]")
        return False
    except Exception as e:
...
def reset_password(
    ...
    password_stdin: bool = typer.Option(
        False,
        "--password-stdin",
        help="Read password from stdin (more secure)"
    ),
    verify: bool = typer.Option(
        True,
        "--verify/--no-verify",
        help="Verify login after reset"
    )
):
    ...
    if result.returncode != 0:
        console.print(f"[red]ArgoCD namespace '{ARGOCD_NAMESPACE}' not found. Is ArgoCD installed?[/red]")
        raise typer.Exit(1)
    ...
    if update_argocd_password(password):
        ...
        if verify:
            ...
            if verify_argocd_login(password):
                ...
                console.print("[yellow]Could not verify login. Password may still be updated.[/yellow]")
                console.print("[dim]Try accessing ArgoCD UI at http://localhost:30080[/dim]")
    else:
    ...
    # Get password if not provided
        ...
        try:
            result = subprocess.run([
                ...
                "-n", ARGOCD_NAMESPACE, "-o", "jsonpath={.data.password}"
            ], capture_output=True, text=True)
            
            if result.returncode == 0 and result.stdout:
                password = base64.b64decode(result.stdout).decode('utf-8')
                console.print("[dim]Using password from argocd-initial-admin-secret[/dim]")
            else:
...
def info():
    ...
    if result.returncode == 0:
        ...
        if ready == desired:
            ...
            console.print(f"[yellow]Server Status: Not Ready ({ready}/{desired})[/yellow]")
    
    # Get ArgoCD version from image
    result = subprocess.run([
        "kubectl", "get", "deployment", "argocd-server",
        "-n", ARGOCD_NAMESPACE, "-o", "jsonpath={.spec.template.spec.containers[0].image}"
    ], capture_output=True, text=True)
    ...
    console.print(f"  Port-forward: kubectl port-forward svc/argocd-server -n {ARGOCD_NAMESPACE} 30080:80")
