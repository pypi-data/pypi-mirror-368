"""Keycloak Identity Provider configuration for CloudStack Orchestrator."""
...
class KeycloakIdentityProviderManager:
    ...
    def _check_keycloak(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["kubectl", "get", "pod", "-n", "platform", "-l", "app.kubernetes.io/name=keycloak", "-o", "json"],
                capture_output=True,
                text=True
            )
            ...
            try:
                response = requests.get(f"{self.keycloak_url}/", timeout=2)
            except:
                self.console.print("[yellow]Setting up port-forward to Keycloak...[/yellow]")
                # Start port-forward in background
                subprocess.Popen(
                    ["kubectl", "port-forward", "-n", "platform", "svc/cloudstack-platform-keycloak", "30081:80"],
    ...
    def _get_admin_password(self) -> Optional[str]:
        ...
        try:
            result = subprocess.run(
                ["kubectl", "get", "secret", "keycloak-admin-creds", "-n", "platform", 
            ...
            if result.returncode == 0 and result.stdout:
                ...
                self.console.print("[red]❌ Could not retrieve Keycloak admin password[/red]")
                return None
    ...
    def _prompt_for_config(self, provider_type: IdentityProviderType) -> Optional[IdentityProviderConfig]:
        ...
        if provider_type == IdentityProviderType.GITHUB:
            panel = Panel(
                ...
                "   [yellow]Application name:[/yellow] CloudStack Platform (Local)\n"
        ...
        client_secret = Prompt.ask(f"Enter {provider_type.value.title()} Client Secret", password=True)
        ...
        if provider_type == IdentityProviderType.GITHUB:
            org = Prompt.ask("GitHub organization to restrict access (optional)", default="")
            if org:
                config.github_org = org
                
        elif provider_type == IdentityProviderType.GOOGLE:
            domain = Prompt.ask("Google Workspace domain to restrict (optional)", default="")
            if domain:
                config.google_hosted_domain = domain
                
        return config
        
    def _get_config_from_secret(self, provider_type: IdentityProviderType) -> Optional[IdentityProviderConfig]:
        """Get provider configuration from Kubernetes secret."""
        secret_name = f"{provider_type.value}-oauth-creds"
        
        try:
            result = subprocess.run(
                ["kubectl", "get", "secret", secret_name, "-n", "platform", "-o", "json"],
                capture_output=True,
                text=True
            )
            
            if result.returncode != 0:
                self.console.print(f"[yellow]No existing configuration found for {provider_type.value}[/yellow]")
                return None
            ...
            client_secret = base64.b64decode(data.get("client-secret", "")).decode('utf-8')
    ...
    def _store_config_secret(self, config: IdentityProviderConfig) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                self.console.print(f"[red]❌ Failed to create secret manifest[/red]")
                return False
            ...
            if apply_result.returncode == 0:
                self.console.print(f"[green]✅ Configuration stored in Kubernetes secret[/green]")
                return True
            else:
                self.console.print(f"[red]❌ Failed to apply secret: {apply_result.stderr}[/red]")
                return False
    ...
    def _configure_in_keycloak(self, config: IdentityProviderConfig, admin_password: str) -> bool:
        ...
        if success:
            ...
            self.console.print(f"1. Go to: {self.keycloak_url}/realms/{self.realm}/account")
            self.console.print(f"2. Click 'Sign in with {config.provider_type.value.title()}'")
            return True
        else:
            return False
    ...
    def _get_admin_token(self, admin_password: str) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.console.print(f"[red]❌ Failed to get admin token: {response.status_code}[/red]")
                return None
    ...
    def _check_provider_exists(self, provider_type: IdentityProviderType, token: str) -> bool:
    ...
    def _create_provider(self, config: IdentityProviderConfig, token: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 201:
                self.console.print(f"[green]✅ Identity provider created[/green]")
                return True
            else:
                self.console.print(f"[red]❌ Failed to create provider: {response.status_code}[/red]")
                if response.text:
    ...
    def _update_provider(self, config: IdentityProviderConfig, token: str) -> bool:
        ...
        try:
            ...
            if response.status_code in [200, 204]:
                self.console.print(f"[green]✅ Identity provider updated[/green]")
                return True
            else:
                self.console.print(f"[red]❌ Failed to update provider: {response.status_code}[/red]")
                return False
    ...
    def _build_provider_data(self, config: IdentityProviderConfig) -> Dict:
        ...
        if config.provider_type == IdentityProviderType.GITHUB:
            ...
            base_data["config"]["authorizationUrl"] = f"https://login.microsoftonline.com/{config.microsoft_tenant_id or 'common'}/oauth2/v2.0/authorize"
            base_data["config"]["tokenUrl"] = f"https://login.microsoftonline.com/{config.microsoft_tenant_id or 'common'}/oauth2/v2.0/token"
            base_data["config"]["userInfoUrl"] = "https://graph.microsoft.com/oidc/userinfo"
