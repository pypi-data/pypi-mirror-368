"""GitHub OAuth Device Flow implementation for CloudStack Orchestrator."""
...
class GitHubDeviceFlow:
    ...
    # Default CSO OAuth App Client ID
    DEFAULT_CLIENT_ID = "Ov23liefXYUwEpx4AMWz"
    
    def __init__(self, console: Optional[Console] = None, client_id: Optional[str] = None):
            ...
            while True:
                try:
                    ...
                    if error == "authorization_pending":
                        ...
                        progress.update(task, description="[red]❌ Authentication expired[/red]")
                        self.console.print("[red]The device code has expired. Please try again.[/red]")
                        return None
                        ...
                        # Unknown error
                        progress.update(task, description=f"[red]❌ Error: {error}[/red]")
                        return None
                        
                except requests.exceptions.RequestException as e:
                    progress.update(task, description=f"[red]❌ Network error[/red]")
    ...
    def validate_token(self, token: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.console.print(f"[red]Token validation failed: {response.status_code}[/red]")
                return False
