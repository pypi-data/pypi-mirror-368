...
class CodeRefType(IntEnum):
    ...
    UNKNOWN = ida_xref.fl_U  # Unknown - for compatibility with old versions
    CALL_FAR = ida_xref.fl_CF  # Call Far - creates a function at referenced location
    CALL_NEAR = ida_xref.fl_CN  # Call Near - creates a function at referenced location
    JUMP_FAR = ida_xref.fl_JF  # Jump Far
    JUMP_NEAR = ida_xref.fl_JN  # Jump Near
    USER_SPECIFIED = ida_xref.fl_USobsolete  # User specified (obsolete)
    ORDINARY_FLOW = ida_xref.fl_F  # Ordinary flow to next instruction


class DataRefType(IntEnum):
    """Data reference types."""

    UNKNOWN = ida_xref.dr_U  # Unknown - for compatibility with old versions
    OFFSET = ida_xref.dr_O  # Offset reference or OFFSET flag set
    WRITE = ida_xref.dr_W  # Write access
    READ = ida_xref.dr_R  # Read access
    TEXT = ida_xref.dr_T  # Text (for forced operands only)
    INFORMATIONAL = ida_xref.dr_I  # Informational reference
    SYMBOLIC = ida_xref.dr_S  # Reference to enum member (symbolic constant)
