...
class GetAdCountListRequest(APIBaseModel):
    ...
    def validate_media(cls, v: list[str]) -> list[str]:
        if v:
            ...
            if invalid_media:
                raise ValueError(
                    f"不支持的媒体渠道: {invalid_media}，支持的渠道: {Config.SUPPORTED_MEDIA}"
                )
        return v
    ...
    def validate_group_key(cls, v: str) -> str:
        if v and v not in Config.SUPPORTED_GROUP_KEYS:
            raise ValueError(
                f"不支持的分组维度: {v}，支持的维度: {list(Config.SUPPORTED_GROUP_KEYS.keys())}"
            )
        return v
    ...
    def validate_ad_status(cls, v: list[str]) -> list[str]:
        if v:
            ...
            if invalid_status:
                raise ValueError(
                    f"不支持的广告状态: {invalid_status}，支持的状态: {Config.SUPPORTED_AD_STATUS}"
                )
        return v
    ...
    def validate_toushou(cls, v: list[str]) -> list[str]:
        if v:
            ...
            if invalid_toushou:
                raise ValueError(
                    f"不支持的投手: {invalid_toushou}，支持的投手: {Config.SUPPORTED_TOUSHOU}"
                )
        return v
...
class GetMaterialCountListRequest(APIBaseModel):
    ...
    def validate_producer(cls, v: list[str]) -> list[str]:
        if v:
            ...
            if invalid_producers:
                raise ValueError(
                    f"不支持的制作人: {invalid_producers}，支持的制作人: {Config.SUPPORTED_PRODUCERS}"
                )
        return v
    ...
    def validate_creative_user(cls, v: list[str]) -> list[str]:
        if v:
            ...
            if invalid_users:
                raise ValueError(
                    f"不支持的创意人: {invalid_users}，支持的创意人: {Config.SUPPORTED_CREATIVE_USERS}"
                )
        return v
    ...
    def validate_originality_type(cls, v: list[str]) -> list[str]:
        if v:
            ...
            if invalid_types:
                raise ValueError(
                    f"不支持的素材类型: {invalid_types}，支持的类型: {Config.SUPPORTED_MATERIAL_TYPES}"
                )
        return v
...
class AdDataRecord(APIBaseModel):
    ...
    广告计划名称: Optional[str] = None
    创意名称: Optional[str] = None
    项目名称: Optional[str] = None
    广告状态: Optional[str] = None

    # 指标字段 - 使用Union[str, int, float]因为API可能返回不同类型
    消耗: Optional[Union[str, int, float]] = None
    新增注册: Optional[Union[str, int]] = None
    新增创角: Optional[Union[str, int]] = None
    创角率: Optional[Union[str, float]] = None
    点击率: Optional[Union[str, float]] = None
    激活率: Optional[Union[str, float]] = None
    点击成本: Optional[Union[str, float]] = None
    活跃用户: Optional[Union[str, int]] = None
    曝光次数: Optional[Union[str, int]] = None
    千次展现均价: Optional[Union[str, float]] = None
    点击数: Optional[Union[str, int]] = None
    当日充值: Optional[Union[str, float]] = None
    当日付费次数: Optional[Union[str, int]] = None
    当日充值人数: Optional[Union[str, int]] = None
    新增付费人数: Optional[Union[str, int]] = None
    首充付费人数: Optional[Union[str, int]] = None
    首充付费次数: Optional[Union[str, int]] = None
    老用户付费人数: Optional[Union[str, int]] = None
    新增付费金额: Optional[Union[str, float]] = None
    首充付费金额: Optional[Union[str, float]] = None
    老用户付费金额: Optional[Union[str, float]] = None
    新增付费率: Optional[Union[str, float]] = None
    活跃付费率: Optional[Union[str, float]] = None
    活跃arppu: Optional[Union[str, float]] = None
    新增arppu: Optional[Union[str, float]] = None
    小游戏注册首日广告变现金额: Optional[Union[str, float]] = None
    小游戏注册首日广告变现ROI: Optional[Union[str, float]] = None
    当月注册用户充值金额: Optional[Union[str, float]] = None
    注册成本: Optional[Union[str, float]] = None
    创角成本: Optional[Union[str, float]] = None
    首日ROI: Optional[Union[str, float]] = None
    累计ROI: Optional[Union[str, float]] = None
    分成后首日ROI: Optional[Union[str, float]] = None
    分成后累计ROI: Optional[Union[str, float]] = None
    付费成本: Optional[Union[str, float]] = None
    新增付费成本: Optional[Union[str, float]] = None
...
class MaterialDataRecord(APIBaseModel):
    ...
    # 基础维度字段
    日期: Optional[str] = None
    素材id: Optional[str] = None
    素材名称: Optional[str] = None
    素材类型: Optional[str] = None
    素材封面uri: Optional[str] = None
    制作人: Optional[str] = None
    创意人: Optional[str] = None
    素材创造时间: Optional[str] = None

    # 素材特有指标
    三秒播放率: Optional[Union[str, float]] = Field(None, alias="3秒播放率")
    完播率: Optional[Union[str, float]] = None
    是否低效素材: Optional[Union[str, int]] = None
    是否AD低质素材: Optional[Union[str, int]] = None
    是否AD优质素材: Optional[Union[str, int]] = None
    低质原因: Optional[str] = None
    ...
    小游戏注册首日广告变现金额: Optional[Union[str, float]] = None
    小游戏注册首日广告变现ROI: Optional[Union[str, float]] = None
    消耗: Optional[Union[str, float]] = None
    新增付费成本: Optional[Union[str, float]] = None
    付费成本: Optional[Union[str, float]] = None
    注册成本: Optional[Union[str, float]] = None
    创角成本: Optional[Union[str, float]] = None
...
class APIResponseData(APIBaseModel):
    ...
    # 总计数据（从数据列表中提取的总计项）
    summary: Optional[dict[str, Any]] = Field(None, description="总计数据行")

    # 分组别名（素材API特有）
    group_key_alias: Optional[str] = Field(
        None, alias="groupKeyAlias", description="分组别名"
    )
