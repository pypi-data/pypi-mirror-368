...
class IndicatorManager(Manager):
    ...
    def recommend_related_indicators(self, current_indicators: list[str]) -> list[str]:
        """基于当前指标推荐相关指标"""
        ...
        for group_id, group_data in self.groups.items():
            group_indicators = set(group_data.get("indicators", []))

            # 如果当前指标中有任何一个属于这个分组，推荐该分组的其他指标
            if any(indicator in group_indicators for indicator in current_indicators):
                related_indicators.update(group_indicators)

        # 移除已经选择的指标
        related_indicators -= set(current_indicators)

        return list(related_indicators)
