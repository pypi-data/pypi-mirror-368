...
from kavo._models import CacheEntry, Request, Response, StampedeInfo
from kavo._pack import pack, unpack

if TYPE_CHECKING:
    from kavo._types_ import Database, Transaction

### Response


def get_response(
    txn: "Transaction", response_id: uuid.UUID, response_db: "Database"
) -> Response | None:
...
def get_response_batch(
    txn: "Transaction", ids: list[uuid.UUID], response_db: "Database"
) -> dict[uuid.UUID, Response]:
...
def get_entry(
    txn: "Transaction", entry_key: str, entries_db: "Database"
) -> CacheEntry | None:
...
def put_request(
    request_id: uuid.UUID, txn: "Transaction", data: Request, db: "Database"
) -> None:
...
def get_stampede(
    txn: "Transaction", key: str, db: "Database"
) -> Union[StampedeInfo, None]:
...
def put_stampede(
    txn: "Transaction", key: str, stampede_info: StampedeInfo, db: "Database"
) -> None:
