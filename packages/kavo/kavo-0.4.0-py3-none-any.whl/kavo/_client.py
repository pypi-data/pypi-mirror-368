...
from ._pack import unpack

from kavo._config import Config, get_default_config
...
if TYPE_CHECKING:
    from ._types_ import Database, Transaction, Environment
...
class KavoClient:
    ...
    def __init__(
        self, config: Config | None = None, env: "Environment" | None = None
    ) -> None:
        ...
        self.requests_chunk_db = self.env.open_db(
            self.config["requests_chunk_db"].encode()
        )
        self.responses_chunk_db = self.env.open_db(
            self.config["responses_chunk_db"].encode()
        )
        self.stampede_db = self.env.open_db(self.config["stampede_db"].encode())
        self.staleness_tracker_db = self.env.open_db(
            self.config["staleness_tracker_db"].encode()
        )

    def get_cache_entry(
        self, key: str, options: GetCacheEntryOptions
    ) -> CacheEntry | None:
        """
        ...
        with self.env.begin() as txn:
            ...
            responses = list(
                get_response_batch(
                    txn, cache_entry["responses"], self.response_db
                ).items()
            )
            ...
            if not options.get("allow_incomplete"):
                responses = [
                    ...
                    if self._is_stream_complete(
                        self.responses_chunk_db, txn, response_id.bytes
                    )
                ]
    ...
    def update_response_time_to_stale(self, response_id: uuid.UUID) -> None:
        ...
        with self.env.begin(write=True) as txn:
            ...
            old_staleness_tracker: Optional[bytes] = response.get(
                get_label("staleness_tracker"), None
            )

            if old_staleness_tracker is not None and response.get(
                get_label("no_refresh_on_access")
            ):
            ...
            if old_staleness_tracker is not None:
                delete_staleness_tracker(
                    txn, key=old_staleness_tracker, db=self.staleness_tracker_db
                )
    ...
    def put_response_stream(
        self, request_id: uuid.UUID, stream: Iterable[bytes]
    ) -> None:
        """
        ...
        with self.env.begin(write=True, db=self.responses_chunk_db) as txn:
            return self._stream_data_to_cache(
                stream, request_id.bytes, txn, self.responses_chunk_db
            )

    def get_response_stream(self, request_id: uuid.UUID) -> Iterable[bytes]:
        with self.env.begin() as txn:
            for chunk in self._stream_data_from_cache(
                request_id.bytes, txn, self.responses_chunk_db
            ):
                yield chunk

    def get_request_stream(self, request_id: uuid.UUID) -> Iterable[bytes]:
        """
        Retrieve a request stream from the cache.
        This method fetches a request stream by its unique identifier and yields the data in chunks.
        Args:
            request_id (uuid.UUID): The unique identifier of the request to retrieve.
        Yields:
            Iterable[bytes]: The data chunks of the request stream.
        """
        with self.env.begin() as txn:
            for chunk in self._stream_data_from_cache(
                request_id.bytes, txn, self.requests_chunk_db
            ):
                yield chunk
    ...
    def store_request(
        ...
        with env.begin(write=True) as txn:
            ...
            self._stream_data_to_cache(
                stream, request_id.bytes, txn, self.requests_chunk_db
            )
    ...
    def _soft_remove_stale_responses(self, txn: "Transaction") -> None:
        ...
        for key, value in cursor:
            ...
            response = get_response(
                txn, unpacked_value["response_id"], response_db=self.response_db
            )
    ...
    def _release_stampede_lock(self, key: str, stampede_info: StampedeInfo) -> None:
        ...
        with self.env.begin(write=True, db=self.stampede_db) as txn:
            ...
            if not deleted:
                raise RuntimeError(
                    f"Failed to release stampede lock for key: {key}. "
                    "Lock may not exist or was already released."
                )
    ...
    def _acquire_stamapede_lock(
        ...
        with self.env.begin(write=True, db=self.stampede_db) as txn:
            ...
            if stampede_info is None:
                ...
                stampede_info = StampedeInfo(
                    acquiredAt=get_unix_timestamp_seconds(), lock_id=uuid.uuid4()
                )
                put_stampede(
                    txn,
                    key,
    ...
    def _is_lock_stale(
        ...
        return (
            ...
            < get_unix_timestamp_seconds("float")
            - self.config["max_stampede_wait_time"]
        )

    def _hard_remove_soft_deleted_responses(
        self, txn: "Transaction", cache_entry: CacheEntry
    ) -> None:
        ...
        for response_id in cache_entry["responses"]:
            ...
            if deleted_timestamp is not None and (
                deleted_timestamp + self.config["hard_delete_after"]
                < get_unix_timestamp_seconds()
            ):
                ...
                self._hard_remove_stream(
                    txn, response_id.bytes, self.responses_chunk_db
                )

        new_responses = [
            response_id
            for response_id in cache_entry["responses"]
            if response_id not in deleted_response_ids
        ]
    ...
    def _stream_data_to_cache(
        ...
        for chunk in stream:
            put_chunk(
                txn, b":".join([request_id, f"chunk_{i}".encode("utf-8")]), chunk, db
            )
            i += 1
    ...
    def _stream_data_from_cache(
        ...
        while True:
            chunk = get_chunk(
                txn, b":".join([request_id, f"chunk_{i}".encode("utf-8")]), db=db
            )
            if chunk is None:
                break
            if chunk == b"":
    ...
    def _hard_remove_stream(
        self, txn: Transaction, request_id: bytes, db: "Database"
    ) -> None:
        """
    ...
    def _is_stream_complete(
        self, chunks_db: "Database", txn: Transaction, request_id: bytes
    ) -> bool:
        """
        Check if the stream is complete.
        """
        return (
            get_chunk(txn, b":".join([request_id, b"complete"]), db=chunks_db)
            is not None
        )
...
class AsyncKavoClient:
    ...
    async def put_response(
        ...
        return await to_thread.run_sync(
            self._sync_client.put_response, key, response, request_id, response_options
        )

    async def put_response_stream(
        self, request_id: uuid.UUID, stream: AsyncIterable[bytes]
    ) -> None:
        """
        Store a response stream in the cache.
        """
    ...
    async def get_request_stream(self, request_id: uuid.UUID) -> AsyncIterable[bytes]:
        """
        Retrieve a request stream from the cache.
        This method fetches a request stream by its unique identifier and yields the data in chunks.
        Args:
            request_id (uuid.UUID): The unique identifier of the request to retrieve.
        Yields:
            AsyncIterable[bytes]: The data chunks of the request stream.
        """
        txn = await to_thread.run_sync(
            self._sync_client.env.begin,
        )
        try:
            async for chunk in self._stream_data_from_cache(
                request_id.bytes,
                txn,  # type: ignore
                self._sync_client.requests_chunk_db,
            ):
                yield chunk
        finally:
            txn.__exit__(None, None, None)
    ...
    async def update_response_time_to_stale(self, response_id: uuid.UUID) -> None:
        return await to_thread.run_sync(
            self._sync_client.update_response_time_to_stale, response_id
        )
    ...
    async def async_stampede_lock(
        ...
        """
        acquired, stampede_info = await to_thread.run_sync(
            self._sync_client._acquire_stamapede_lock, key
        )

        if not acquired:
            # If we are waiter, then we should wait for the lock to be released
            while await to_thread.run_sync(
                self._sync_client._should_wait, key, stampede_info
            ):
        ...
        try:
            ...
            if acquired:
                await to_thread.run_sync(
                    self._sync_client._release_stampede_lock, key, stampede_info
                )
