...
class TestNebulaClient:
    ...
    def test_is_r2r_api_key_detection(self):
        client = NebulaClient(api_key="key_abc.def")
        assert client._is_r2r_api_key() is True
        client2 = NebulaClient(api_key="not-a-jwt-or-r2r")
        assert client2._is_r2r_api_key() is False
        client3 = NebulaClient(api_key="key_only_without_dot")
        assert client3._is_r2r_api_key() is False
        client4 = NebulaClient(api_key="key_ab.c.d")
        assert client4._is_r2r_api_key() is False

    def test_build_auth_headers_for_r2r_key(self):
