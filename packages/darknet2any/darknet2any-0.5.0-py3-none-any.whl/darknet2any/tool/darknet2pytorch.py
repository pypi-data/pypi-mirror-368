...
import numpy as np
from darknet2any.tool.region_loss import RegionLoss
from darknet2any.tool.yolo_layer import YoloLayer
from darknet2any.tool.config import *
...
class Darknet(nn.Module):
  ...
  def get_example(self):
    return torch.rand(1, 3, self.height, self.width)
  ...
  def process_tensors(self) -> torch.Tensor:
    all_parameters = []
    for module in self.models:
      for param in module.parameters():
        all_parameters.append(param.view(-1)) # Flatten each parameter tensor
    return torch.cat(all_parameters)

  def save_weights(self, outfile):
    self.cpu()
    #example = self.get_example()
    traced_model = torch.jit.script(self)
    traced_model.save(outfile)
