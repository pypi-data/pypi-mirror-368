    ...
    def create_folder(
        ...
        *,
        ...
        try:
            ...
            # already exists
            if exists_ok and self.folder_exists(path):
                # all good, the folder we wanted to create exists
                return
            # we can end up due to
            # https://hub.datalad.org/git-annex/git-annex-remote-internxt/issues/14.
            # invalidate listing cache entry for the parent
            # essentially undo what what inserted above, but in a previous call
            if 'folder does not exist or is not yours' in str(e):
                # message parsing is ugly, but appears to be the only way to catch
                # this "invalid cache" issue. Let's fix it...
                # remove the cache entry for the path reported absent
                del self._list_cache[path.parent]
                # remove it also from its own parent
                if path.parent != PurePosixPath():
                    self._list_cache[path.parent.parent].folders = [
                        node
                        for node in self._list_cache[path.parent.parent].folders
                        if node.uuid != parent_id
                    ]
                    return self.create_folder(
                        path=path, parents=parents, exists_ok=exists_ok
                    )
            raise
