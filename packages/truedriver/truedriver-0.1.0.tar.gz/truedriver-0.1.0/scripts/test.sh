...
if [ "${#command[@]}" -eq 0 ]; then
  command=( "pytest --cov=truedriver --cov-report=xml" )
fi

chrome_executable=$(uv run python -c "from truedriver.core.config import find_executable;print(find_executable())")
