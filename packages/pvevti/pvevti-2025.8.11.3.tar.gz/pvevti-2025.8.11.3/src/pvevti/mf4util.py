    ...
    def dataFrame(self, channels:list=[], resample:float=1.0):
        ...
        for channel in channels:
            try:
               ...
               df = DataFrame({chname:samples.samples}, index=to_timedelta(samples.timestamps, unit='s')).fillna(0)
               signals.append(df)
               print(channel['Name'])
               print(df)
        ...
        if signals:
            ...
            result = roundDF(concat(signals, axis=1).resample(rule=str(resample)+'s').mean().ffill(), 5)
...
def roundDF(df:DataFrame, n_figs):
    power = 10 ** floor(log10(abs(df).clip(1e-200)))
