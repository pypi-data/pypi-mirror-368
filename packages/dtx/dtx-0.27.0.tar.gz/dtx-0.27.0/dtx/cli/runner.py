...
from dtx.core.engine.scanner import AdvOptions 
...
class RedTeamRunnerInput(BaseModel):
    ...
    threads: int = Field(default=1, ge=1, le=1, description="Thread count is fixed to 1 for now.")
...
class RedTeamTestRunner:
    ...
    def run(self, input: RedTeamRunnerInput) -> EvalReport:
        ...
        """
        if input.threads != 1:
            raise ValueError("Multithreading is not supported yet. 'threads' must be 1.")
        ...
        engine_config = EngineConfig(
            ...
            adv_options=AdvOptions(
                attempts=input.samples_per_prompt,
...
class QuickRedteamRunnerInput(BaseModel):
    ...
    samples_per_prompt: int = Field(default=1, ge=1, description="Number of times to send each prompt for sampling.")
    threads: int = Field(default=1, ge=1, le=1, description="Number of threads to use. Currently restricted to 1.")
...
class QuickRedteamRunner:
    ...
    def _create_scope(self):
        ...
        """
        config = ScopeInput(description="Generated during run", plugins=self.input.plugins)
