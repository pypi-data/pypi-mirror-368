...
async def getconfig(action, account: str | None = None):
    if action == "account":
        await account_maker()
    if action == "config":
        if account is not None:
            await config_maker_barons(account)
        else:
            print("Error: account name must be provided for config action.")


async def account_maker():
    ...
    account_name = await questionary.text("Enter account name:").ask_async()
    if not account_name:
    ...
    username = await questionary.text("Enter nickname:").ask_async()
    password = await questionary.password("Enter password:").ask_async()
    server = await questionary.text("Enter server:").ask_async()
...
async def config_maker_barons(filename: str):
    config_dir = os.path.join('.', 'configs')
    os.makedirs(config_dir, exist_ok=True)
    ...
    save_name = await questionary.text(
    ...
    async def input_int_list(prompt):
        while True:
            raw = await questionary.text(prompt).ask_async()
            try:
    ...
    max_flank = await input_int("Enter number of units on a flank (0 if none):")
    config.set(save_name, "max_flank", max_flank)

    max_front = await input_int("Enter number of units on the front (0 if none):")
    config.set(save_name, "max_front", max_front)

    unit_choice = (await questionary.select(
    ...
    flank_choice = (await questionary.select(
    ...
    if flank_tool_options[flank_choice][0] != "--   None":
        flank_amt = await input_int("Enter amount of those tools per flank:")
        config.set(save_name, "flank_tool_ammount", flank_amt)
    else:
        config.set(save_name, "flank_tool_ammount", "0")

    front_choice1 = (await questionary.select(
    ...
    if front_tool_options_1[front_choice1][0] != "--   None":
        front_amt1 = await input_int("Enter amount of those tools per front:")
        config.set(save_name, "front_tool_ammount1", front_amt1)
    else:
        config.set(save_name, "front_tool_ammount1", "0")

    front_choice2 = (await questionary.select(
    ...
    if front_tool_options_2[front_choice2][0] != "--   None":
        front_amt2 = await input_int("Enter amount of those tools per front:")
