...
def account_maker():
    ...
    account_name = questionary.text("Enter account name:").unsafe_ask()
    if not account_name:
    ...
    username = questionary.text("Enter nickname:").unsafe_ask()
    password = questionary.password("Enter password:").unsafe_ask()
    server = questionary.text("Enter server:").unsafe_ask()
...
def config_maker_barons(filename: str):
    ...
    def input_int_list(prompt):
        while True:
            raw = questionary.text(prompt).unsafe_ask()
            try:
