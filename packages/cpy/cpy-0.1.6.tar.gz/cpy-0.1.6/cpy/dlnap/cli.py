...
class DLNAFS(Operations):
    ...
    def rename(self, a, b): raise FuseOSError(errno.EROFS)
    ...
    def truncate(self, path, length, fh=None): raise FuseOSError(errno.EROFS)
...
def main():
    ...
    FUSE(DLNAFS(tree, filemap), mountpoint, nothreads=True, foreground=True, ro=True)
