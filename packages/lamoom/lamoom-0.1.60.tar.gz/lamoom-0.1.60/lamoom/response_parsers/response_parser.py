...
def get_thinking_from_response(response: str, start_from: int = 0) -> TaggedContent:
    """Extract thinking content from response using <think> tags."""
    content, start_ind, end_ind = _get_format_from_response(
        response,
        [Tag("<think>", "</think>", 0)],
        start_from=start_from,
    ...
    if content:
        return TaggedContent(
            content=content,
            parsed_content=content,  # For thinking, the parsed content is the same as content
            start_ind=start_ind,
            end_ind=end_ind,
        )
    return None
