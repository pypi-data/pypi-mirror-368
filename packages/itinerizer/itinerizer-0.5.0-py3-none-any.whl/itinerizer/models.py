...
class FlightSegment(BaseSegment):
    ...
    @model_validator(mode='after')
    def sync_times(self):
        self.start_datetime = self.departure_datetime
        self.end_datetime = self.arrival_datetime
...
class HotelSegment(BaseSegment):
    ...
    @model_validator(mode='after')
    def sync_dates(self):
        # Convert check-in/out dates to datetime for start/end
        check_in_hour = int(self.check_in_time.split(':')[0])
        check_out_hour = int(self.check_out_time.split(':')[0])
