...
class TestConfig:
    ...
    @pytest.mark.parametrize("file_format", ["py"])
    def test_init(self, file_format):
        ...
        cfg_file = osp.join(
            self.data_path, f"config/{file_format}_config/simple_config.{file_format}"
        )
        ...
        with pytest.raises(KeyError, match="filename is reserved for config " "file"):
            Config.fromfile(cfg_file)

    def test_fromfile(self):
        # test whether import `custom_imports` from cfg_file.
        cfg_file = osp.join(self.data_path, "config", "py_config/test_custom_import.py")
        ...
        with pytest.raises(ImportError, match="Failed to import custom modules from"):
    ...
    def test_merge_from_dict(self):
        cfg_file = osp.join(self.data_path, "config/py_config/simple_config.py")
        cfg = Config.fromfile(cfg_file)
        ...
        cfg_file = osp.join(self.data_path, "config/py_config/test_merge_from_dict.py")
        cfg = Config.fromfile(cfg_file)
    ...
    def test_auto_argparser(self):
        ...
        sys.argv = sys.argv[:2]
        sys.argv[1] = osp.join(
            self.data_path, "config/py_config/test_merge_from_multiple_bases.py"
        )
    ...
    def test_pre_substitute_base_vars(self, tmp_path):
        cfg_path = osp.join(
            self.data_path, "config", "py_config/test_pre_substitute_base_vars.py"
        )
    ...
    def _predefined_vars(self):
        ...
        cfg_file = osp.join(self.data_path, "config/py_config/test_predefined_var.py")
        path = osp.join(self.data_path, "config/py_config")

        path = Path(path).as_posix()
        cfg_dict_dst = dict(
            item1="test_predefined_var.py", item2=path, item3="abc_test_predefined_var"
        )
        ...
        assert Config._file2dict(cfg_file, False)[0]["item1"] == cfg_dict_ori["item1"]
        assert Config._file2dict(cfg_file, False)[0]["item2"] == cfg_dict_ori["item2"]
        assert Config._file2dict(cfg_file, False)[0]["item3"] == cfg_dict_ori["item3"]

    def _environment_vars(self):
        # test parse predefined_var in config
        cfg_file = osp.join(self.data_path, "config/py_config/test_environment_var.py")
    ...
    def _merge_from_base(self):
        cfg_file = osp.join(
            self.data_path, "config/py_config/test_merge_from_base_single.py"
        )
        ...
        with pytest.raises(TypeError):
            Config.fromfile(
                osp.join(
                    self.data_path, "config/py_config/test_merge_from_base_error.py"
                )
            )
    ...
    def _merge_from_multiple_bases(self):
        ...
        assert cfg_dict["item7"] == dict(a=[0, 1, 2], b=dict(c=[3.1, 4.2, 5.3]))
        # Redefine key
        with pytest.raises(KeyError):
            Config.fromfile(
                osp.join(
                    self.data_path, "config/py_config/test_merge_from_multiple_error.py"
                )
            )

    def _base_variables(self):
        for file in ["py_config/test_base_variables.py"]:
        ...
        # test nested base
        for file in ["py_config/test_base_variables_nested.py"]:
            cfg_file = osp.join(self.data_path, "config", file)
            cfg_dict = Config._file2dict(cfg_file)[0]
        ...
        assert cfg_dict["item25"] == dict(
            ...
            b=[[3.1, 4.2, 5.3]],
            c=[[{"e": "test_base_variables.py"}], [{"a": 0}, {"b": 1}]],
        ...
        # test nested base
        cfg_file = osp.join(self.data_path, "config/py_config/test_py_nested_path.py")
        cfg = Config.fromfile(cfg_file)
        assert isinstance(cfg, Config)
        assert cfg.filename == cfg_file
        # cfg.field
        ...
        cfg_file = osp.join(
            self.data_path, "config/py_config/test_py_function_global_var.py"
        )
        ...
        # config.
        cfg_file = osp.join(self.data_path, "config/py_config/test_py_modify_key.py")
        cfg = Config._file2dict(cfg_file)[0]
    ...
    def _merge_recursive_bases(self):
        cfg_file = osp.join(
            self.data_path, "config/py_config/test_merge_recursive_bases.py"
        )
        cfg_dict = Config._file2dict(cfg_file)[0]
    ...
    def _merge_delete(self):
        cfg_file = osp.join(self.data_path, "config/py_config/test_merge_delete.py")
        cfg_dict = Config._file2dict(cfg_file)[0]
        # cfg.field
    ...
    def _merge_intermediate_variable(self):
        ...
        cfg_file = osp.join(
            self.data_path, "config/py_config/test_merge_intermediate_variable_child.py"
        )
        cfg_dict = Config._file2dict(cfg_file)[0]
        # cfg.field
        assert cfg_dict["item1"] == [1, 2]
    ...
    def _code_in_config(self):
        cfg_file = osp.join(self.data_path, "config/py_config/test_code_in_config.py")
        cfg = Config.fromfile(cfg_file)
        # cfg.field
    ...
    def _deprecation(self):
        deprecated_cfg_files = [
            ...
            osp.join(self.data_path, "config", "py_config/test_deprecated_base.py"),
        ]
    ...
    def test_deepcopy(self):
        cfg_file = osp.join(
            self.data_path, "config", "py_config/test_dump_pickle_support.py"
        )
        cfg = Config.fromfile(cfg_file)
...
class TestConfigDict(TestCase):
    ...
    def test_keep_custom_dict(self):
        ...
        self.assertIsInstance(cfg_dict["a"], CustomDict)
        self.assertIsInstance(list(cfg_dict.values())[0], CustomDict)
        self.assertIsInstance(list(cfg_dict.items())[0][1], CustomDict)
    ...
    def test_build_lazy(self):
        ...
        raw = dict(
            ...
            b=dict(
                c=2,
                e=[dict(f=dict(h=LazyObject("mmengine")), g=LazyObject("mmengine"))],
            ),
        )
        cfg_dict = ConfigDict(raw)
        ...
        self.assertIs(cfg_dict.get("b").get("e")[0].get("f").get("h"), mmengine)
    ...
    def _check(self, cfg_dict):
        mmengine = import_module("mmengine")
        self._recursive_check_lazy(cfg_dict, lambda x: not isinstance(x, LazyObject))
        self._recursive_check_lazy(
            cfg_dict._to_lazy_dict(), lambda x: x is not mmengine
        )
        self._recursive_check_lazy(
            cfg_dict._to_lazy_dict(),
            lambda x: not isinstance(x, ConfigDict) if isinstance(x, dict) else True,
        )
        self._recursive_check_lazy(
            cfg_dict,
            lambda x: isinstance(x, ConfigDict) if isinstance(x, dict) else True,
        )

    def _recursive_check_lazy(self, cfg, expr):
        if isinstance(cfg, dict):
            {key: self._recursive_check_lazy(value, expr) for key, value in cfg.items()}
