...
def validateVisionArgs(model, user, paths):
...
def validateGenerationArgs(model, user, path):
    validateResponseArgs(model, user)
...
# crops 3–6% from top and 2–5% from bottom (defaults within range)
def saveImageFromUrl(image, path: str, cropT=5, cropB=5):
    # --- fetch ---
    res = requests.get(image, timeout=30)
    res.raise_for_status()

    # --- open & normalize orientation ---
    with Image.open(io.BytesIO(res.content)) as im_raw:
        im = ImageOps.exif_transpose(im_raw)  # honor camera rotation
        w, h = im.size

        # --- clamp crop percent ranges ---
        # top: 3–6%, bottom: 2–5%
        top_pct = max(3, min(float(cropT), 10))
        bot_pct = max(2, min(float(cropB), 10))

        # --- compute crop box ---
        top_px = int(math.floor(h * (top_pct / 100.0)))
        bot_px = int(math.floor(h * (bot_pct / 100.0)))
        left, right = 0, w
        top, bottom = top_px, max(top_px + 1, h - bot_px)  # ensure at least 1px tall

        # guard against pathological small images / large percents
        if bottom <= top:
            # fall back to no-op crop if over-cropped
            top, bottom = 0, h

        im_cropped = im.crop((left, top, right, bottom))

        # --- pick format from original or extension ---
        ext_format_map = {
            ".jpg": "JPEG", ".jpeg": "JPEG",
            ".png": "PNG",  ".webp": "WEBP",
        ...
        fmt = ext_format_map.get(p.suffix.lower(), fmt or "PNG")
        ...
        im_cropped.save(str(p), format=fmt, **save_kwargs)
