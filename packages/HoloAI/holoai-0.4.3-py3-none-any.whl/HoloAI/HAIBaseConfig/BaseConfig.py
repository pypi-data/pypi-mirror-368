...
class BaseConfig:
    ...
    def getGeneration(self, **kwargs):
        ...
        if ('skills' in kwargs) or ('actions' in kwargs):
            ...
            params.pop('skills', None)
        params.pop('actions', None)
        validateGenerationArgs(params["model"], params["user"], params["path"])
    ...
    def getKeys(self, key):
        ...
        extras = {
            "response":   ["skills", "actions", "tools", "choice", "show", "effort", "budget", "files"],
    ...
    def saveImageFromUrl(self, image, path: str):
        """
        Save an image from a URL to a specified path.
        ...
        """
        try:
            return saveImageFromUrl(image, path)
        except Exception as e:
            logger.error(f"Failed to save image from URL: {image} to path: {path}", exc_info=True)
            return ""
