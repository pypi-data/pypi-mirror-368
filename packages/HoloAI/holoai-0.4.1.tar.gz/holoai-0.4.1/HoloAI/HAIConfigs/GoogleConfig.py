...
class GoogleConfig(BaseConfig):
    ...
    def Response(self, **kwargs) -> str:
        ...
        # Use reasoning flag only if model supports it
    ...
    def Vision(self, **kwargs):
        ...
        # Vision typically doesn't use reasoning configs—pass reasoning=False
    ...
    def processSkills(self, instructions, user, tokens) -> str:
        ...
        params = {
            "system": instructions,
            "tokens": tokens,
            # Not needed for skills: model/tools/effort/budget
        }
        # Always pass reasoning=False here
        generateConfig = self._configArgs(params, reasoning=False)
    ...
    def _configArgs(self, params, reasoning=False):
        ...
        if params.get("creativity"):
            configArgs["temperature"] = params["creativity"]

        if params.get("tools"):
            configArgs["tools"] = params["tools"]
            # Map the normalized choice param for Google GenAI SDK
            configArgs["function_calling_config"] = types.FunctionCallingConfig(
    ...
    def _mapChoice(self, choice):
        # Always work in lower-case for safety
        mapping = {
            "auto": "AUTO",        # Let model decide
            "required": "ANY",     # Force a tool call (Google uses "ANY" for this)
