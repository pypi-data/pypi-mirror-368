...
class HoloAI:
    ...
    def _routeCompletion(self, **kwargs):
        ...
        text = str(raw[-1]["content"] if isinstance(raw, list) and isinstance(raw[-1], dict) and "content" in raw[-1] else raw)
        ...
        def visionMode():
            return self.Vision(
                model   = models['vision'], # will default to response model if not set
                system  = system,
                input   = cleanedInput,
                files   = allImages,             # images only
                collect = collect,
                **kwargs,  # pass all other kwargs directly
            )
        ...
        mode = "hybrid" if (allImages and allFiles) else ("vision" if allImages else "holo")
        return modeMap[mode]()
