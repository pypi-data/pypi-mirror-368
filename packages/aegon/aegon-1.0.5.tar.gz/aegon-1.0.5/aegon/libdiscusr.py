...
def disc_USR(listmol, threshold):
    ...
    elementos_unicos = set(listmol[0].get_chemical_symbols())
    mono=True if len(elementos_unicos)==1 else False
    if mono:
        n_features=8
...
#------------------------------------------------------------------------------------------
def comparator_usr_parallel(poscarlist, threshold, nproc, base_name, tol_energy=0.1):
    ...
    for ifolder in folderlist:
        ...
        for imol in molx:
            ...
            if ( delw < tol_energy ) or ( deup < tol_energy ):
...
def make_similarity_matrix_compare(moleculein, moleculeref):
    elementos_unicos = set(moleculein[0].get_chemical_symbols())
    mono=True if len(elementos_unicos)==1 else False
    if mono:
        n_features=8
...
#------------------------------------------------------------------------------------------
def molin_sim_molref(moleculein, moleculeref, tol=0.95):
    #elementos_unicos = set(moleculein[0].get_chemical_symbols())
    #mono=True if len(elementos_unicos)==1 else False
    start = time.time()
    ni=len(moleculein)
