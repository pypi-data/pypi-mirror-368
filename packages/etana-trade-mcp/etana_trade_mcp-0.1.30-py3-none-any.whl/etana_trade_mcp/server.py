...
@mcp.tool()
async def get_order_history(days_back: str = Field(default="30", description="number of days from current time to start order history list. Example order list for the past week, days_back would be 7, for the past month, days_back would be 30, for the quarter, days_back would be 90, for the past year, days_back would be 365.")) -> str:
    ...
    dt = datetime.now() - timedelta(days=past_days)
    ...
    if 'status_code' in order_history:
        print(order_history['status_code'])

    order_history_new = []
    for i in range(len(order_history)):
        ts_string = order_history[i]['timestamp']
    ...
    #For Openwebui formatting
    table = "Date | Source | Security | Status | Type | Side | Quantity | Average Price" + '\n' + "| :--- | :--- | :--- | :---  | :--- | :---  | :---" + '\n'

    for index, row in df_sorted.iterrows():
        table = table + str(f"{row['datetime']} | {row['source']} | {row['security']} | {row['status']} | {row['type']} | {row['side']} | {row['quantity']:,.2f} | {row['average_price']:,.2f} ") + '\n'
