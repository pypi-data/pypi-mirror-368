...
def create_feedback_text(feedback_data: dict) -> str:
    ...
    # 基本反馈内容：兼容 interactive_feedback / feedback / text
    text_value = None
    for key in ("interactive_feedback", "feedback", "text"):
        if key in feedback_data:
            try:
                text_value = str(feedback_data.get(key) or "")
            except Exception:
                text_value = ""
            break

    if text_value is not None:
        if text_value.strip():  # 有实际内容
            text_parts.append(f"=== 用户反馈 ===\n{text_value}")
        else:  # 用户提交了但内容为空
            text_parts.append("=== 用户反馈 ===\n(用户未输入文字反馈)")

    # 命令执行日志：兼容 command_logs / logs
    command_logs = str(feedback_data.get("command_logs", "") or "")
    if not command_logs.strip() and "logs" in feedback_data:
        try:
            command_logs = str(feedback_data.get("logs") or "")
        except Exception:
            command_logs = ""
    if command_logs.strip():
            ...
            if img.get("data"):
                try:
                    ...
                    if img_base64:
                        # 只显示前50个字符的预览
                        preview = (
                            img_base64[:50] + "..." if len(img_base64) > 50 else img_base64
                        )
                        img_info += f"\n     Base64 预览: {preview}"
...
def process_images(images_data: list[dict]) -> list[dict]:
    """
    处理图片数据，转换为符合 MCP 协议的 ImageContent 字典

    返回的每个图片对象为可直接 JSON 序列化的字典，形如：
    {
        "type": "image",
        "data": "<base64>",
        "mimeType": "image/png"
    ...
    Returns:
        List[dict]: ImageContent 字典列表
    """
    mcp_images: list[dict] = []
    ...
    for i, img in enumerate(images_data, 1):
        try:
            ...
            if isinstance(img["data"], bytes):
                ...
                debug_log(f"图片 {i} 使用原始 bytes 数据，大小: {len(image_bytes)} bytes")
            elif isinstance(img["data"], str):
                image_bytes = base64.b64decode(img["data"])  # base64 -> bytes
            ...
            # 转为 base64 字符串（MCP 要求可序列化）
            image_b64 = base64.b64encode(image_bytes).decode("utf-8")

            # 构造符合 MCP 协议的 ImageContent 字典
            image_content = {
                "type": "image",
                "data": image_b64,
                "mimeType": mime_type,
            ...
            debug_log(f"图片 {i} ({file_name}) 处理成功，MIME: {mime_type}")
...
async def interactive_feedback(
    ...
    Returns:
        list: List containing TextContent and ImageContent(dict) items representing user feedback
    """
    # 环境检测
    ...
    try:
        ...
        # 调试：记录收到的反馈数据结构
        debug_log(f"收到的反馈数据: {list(result.keys())}")
        debug_log(f"interactive_feedback: '{result.get('interactive_feedback', 'NOT_FOUND')}'")
        debug_log(f"command_logs: '{result.get('command_logs', 'NOT_FOUND')}'")
        debug_log(f"images count: {len(result.get('images', []))}")

        # 额外调试：检查是否有其他可能的键名
        for key in result.keys():
            if 'log' in key.lower():
                debug_log(f"发现日志相关键: {key} = '{result[key]}'")
            if 'feedback' in key.lower():
                debug_log(f"发现反馈相关键: {key} = '{result[key][:100]}...' (截断显示)")

        # 创建反馈项目列表
        feedback_items = []

        # 修复：始终添加文字反馈部分，即使只有图片
        # 检查是否有任何形式的反馈内容（兼容多种键名）
        has_text_feedback = any(k in result for k in ("interactive_feedback", "feedback", "text"))
        logs_val = (result.get("command_logs", "") or "").strip()
        if not logs_val and "logs" in result:
            logs_val = (result.get("logs", "") or "").strip()
        has_command_logs = bool(logs_val)
        has_images = bool(result.get("images"))

        debug_log(f"反馈内容检查 - 文字: {has_text_feedback}, 日志: {has_command_logs}, 图片: {has_images}")

        if has_text_feedback or has_command_logs or has_images:
