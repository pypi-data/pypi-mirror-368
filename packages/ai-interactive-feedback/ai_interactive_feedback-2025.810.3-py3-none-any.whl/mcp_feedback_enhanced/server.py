...
def create_feedback_text(feedback_data: dict) -> str:
    ...
    # 基本反馈内容 - 修复：检查是否存在键而不是值是否为真
    interactive_feedback = feedback_data.get("interactive_feedback", "")
    if "interactive_feedback" in feedback_data:
        if interactive_feedback.strip():  # 有实际内容
            text_parts.append(f"=== 用户反馈 ===\n{interactive_feedback}")
        else:  # 用户提交了但内容为空
            text_parts.append("=== 用户反馈 ===\n(用户未输入文字反馈)")

    # 命令执行日志
    command_logs = feedback_data.get("command_logs", "")
...
async def interactive_feedback(
    ...
    try:
        ...
        # 检查是否有任何形式的反馈内容
        has_text_feedback = "interactive_feedback" in result
        has_command_logs = result.get("command_logs", "").strip()
        has_images = result.get("images")

        debug_log(f"反馈内容检查 - 文字: {has_text_feedback}, 日志: {bool(has_command_logs)}, 图片: {bool(has_images)}")
