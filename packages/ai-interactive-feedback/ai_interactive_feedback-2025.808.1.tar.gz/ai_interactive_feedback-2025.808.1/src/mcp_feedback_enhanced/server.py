...
# Import FastMCP compatibility utilities to fix Image serialization issues
from .utils.fastmcp_compatibility import (
    ensure_image_compatibility,
    get_image_format,
...
def process_images(images_data: list[dict]) -> list[MCPImage]:
    ...
    for i, img in enumerate(images_data, 1):
        try:
            ...
            # 创建 MCPImage 对象并确保兼容性
            mcp_image = MCPImage(data=image_bytes, format=image_format)
            mcp_image = ensure_image_compatibility(mcp_image)
            mcp_images.append(mcp_image)

            # 使用兼容性工具进行安全的调试日志记录
            image_info = serialize_image_info(mcp_image)
            debug_log(f"图片 {i} ({file_name}) 处理成功，格式: {image_info['format']}, "
                     f"大小: {image_info['data_length']} bytes")
