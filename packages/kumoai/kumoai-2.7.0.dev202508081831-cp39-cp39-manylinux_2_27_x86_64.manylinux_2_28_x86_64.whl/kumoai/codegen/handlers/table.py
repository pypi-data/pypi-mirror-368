...
def get_handlers() -> Dict[Type, Handler]:
    ...
    def _table_parents(obj: object) -> list[object]:
    ...
    def _table_emit_lines(
        ...
        connector_id = id(obj.source_table.connector)
        connector_var = parent_map[connector_id]
    ...
    def _table_detect_edits(
        ...
        logger.debug(
            f"Detecting edits between tables {id(target)} and {id(baseline)}")

        try:
            result = detect_edits_recursive(target, baseline, "", name_manager)
            logger.debug(f"Found {len(result.edits)} edits with "
