...
from maleo.metadata.client.controllers import MaleoMetadataGenderControllers
...
from maleo.metadata.schemas.parameter.client.gender import ReadMultipleParameter
    ...
    async def read_genders(
        ...
        if result_str is not None:
            ...
            ReadMultipleResourceOperationSchema[
                ...
                summary="Successfully retrieved multiple genders",
        ...
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.read_genders(
    ...
    async def read_gender(
        ...
        if result_str is not None:
            ...
            ReadSingleResourceOperationSchema[GenderDataSchema, None](
                ...
                summary="Successfully retrieved single gender",
                request_context=request_context,
                authentication=authentication,
                action=operation_action,
                resource=self._resource,
        ...
        # Retrieve genders using chosen controller
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.read_gender(
                operation_id=operation_id,
                authentication=authentication,
                parameters=parameters,
                headers=headers,
            ...
            if response.is_success:
                ...
                ReadSingleResourceOperationSchema[GenderDataSchema, None](
                    ...
                    summary="Successfully retrieved single genders",
                    request_context=request_context,
                    authentication=authentication,
                    action=operation_action,
                    resource=self._resource,
