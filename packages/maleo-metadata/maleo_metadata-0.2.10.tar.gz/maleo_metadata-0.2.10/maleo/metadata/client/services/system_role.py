...
from maleo.metadata.client.controllers import MaleoMetadataSystemRoleControllers
...
from maleo.metadata.schemas.parameter.client.system_role import ReadMultipleParameter
...
class MaleoMetadataSystemRoleClientService(MaleoClientService):
    ...
    async def read_system_roles(
        ...
        SystemRoleDataSchema, StrictPagination, None
    ]:
        ...
        if result_str is not None:
            ...
            ReadMultipleResourceOperationSchema[
                ...
                summary="Successfully retrieved multiple system roles",
        ...
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.read_system_roles(
    ...
    async def read_system_role(
        ...
        if result_str is not None:
            ...
            ReadSingleResourceOperationSchema[SystemRoleDataSchema, None](
                ...
                summary="Successfully retrieved single system role",
                request_context=request_context,
                authentication=authentication,
                action=operation_action,
                resource=self._resource,
        ...
        # Retrieve system roles using chosen controller
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.read_system_role(
                operation_id=operation_id,
                authentication=authentication,
                parameters=parameters,
                authorization=authorization,
            ...
            if response.is_success:
                ...
                result = ReadSingleResourceOperationResult[SystemRoleDataSchema, None](
                    data=data,
                    pagination=validated_response.pagination,
                    metadata=None,
                    other=None,
                ...
                ReadSingleResourceOperationSchema[SystemRoleDataSchema, None](
                    ...
                    summary="Successfully retrieved single system roles",
                    request_context=request_context,
                    authentication=authentication,
                    action=operation_action,
                    resource=self._resource,
