...
from maleo.metadata.client.controllers import MaleoMetadataUserTypeControllers
...
from maleo.metadata.schemas.parameter.client.user_type import ReadMultipleParameter
    ...
    async def read_user_types(
        ...
        if result_str is not None:
            ...
            ReadMultipleResourceOperationSchema[
                ...
                summary="Successfully retrieved multiple user types",
        ...
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.read_user_types(
    ...
    async def read_user_type(
        ...
        if result_str is not None:
            ...
            ReadSingleResourceOperationSchema[UserTypeDataSchema, None](
                ...
                summary="Successfully retrieved single user type",
                request_context=request_context,
                authentication=authentication,
                action=operation_action,
                resource=self._resource,
        ...
        # Retrieve user types using chosen controller
        if controller_type is ClientControllerType.HTTP:
            response = await self._controllers.http.read_user_type(
                operation_id=operation_id,
                authentication=authentication,
                parameters=parameters,
                authorization=authorization,
            ...
            if response.is_success:
                ...
                ReadSingleResourceOperationSchema[UserTypeDataSchema, None](
                    ...
                    summary="Successfully retrieved single user types",
                    request_context=request_context,
                    authentication=authentication,
                    action=operation_action,
                    resource=self._resource,
