            ...
            def run(self, **kwargs) -> Dict[str, Any]:
                ...
                if operation == "create":
                    ...
                    try:
                        ...
                        # Use self.model_name instead of closure variable to avoid conflicts
                        ...
                        # CRITICAL FIX: For PostgreSQL, bypass AsyncSQLDatabaseNode parameter conversion
                        # issue by using direct asyncpg execution
                        if database_type.lower() == "postgresql":
                            try:
                                # Direct asyncpg execution to avoid parameter conversion
                                import asyncio
                                ...
                                try:
                                    row = loop.run_until_complete(execute_direct())
                                finally:
                                    loop.close()

                                if row:
                                    logger.warning(
                                        "DATAFLOW POSTGRESQL FIX: Direct asyncpg success!"
                                    )
                                    ...
                                    return {**kwargs, **serialized_row}

                                logger.warning(
                                    "DATAFLOW POSTGRESQL FIX: No data returned"
                                )
                                return {"id": None, **kwargs}

                            except Exception as direct_error:
                                logger.warning(
                                    f"DATAFLOW POSTGRESQL FIX: Direct execution failed: {direct_error}"
                                )
                                # Fall back to AsyncSQLDatabaseNode
                                pass

                        # Execute using AsyncSQLDatabaseNode (original approach for non-PostgreSQL or fallback)
                    ...
                    if conditions and "id" in conditions:
                        ...
                        # Fall back to direct parameters for backward compatibility
                        # Ignore the node ID parameter that appears as 'id'
                        id_param = kwargs.get("id")
                        if id_param is not None and id_param != operation:
                            try:
                                # Try to convert to int
                                record_id = int(id_param)
                            except (ValueError, TypeError):
                    ...
                    # Convert id to int if it's a valid number
                    id_param = kwargs.get("id")
                    if id_param is not None:
                        try:
                            # Try to convert to int
                    ...
                    if cache_integration and enable_cache:
                        ...
                        result = cache_integration.execute_with_cache(
