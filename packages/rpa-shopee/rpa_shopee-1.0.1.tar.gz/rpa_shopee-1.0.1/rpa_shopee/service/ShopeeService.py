...
class ShopeeService():
    ...
    def account_login(self, driver: unChrome, shop_data, options):
        '''
        @Desc    : 账号登录（非本土）
        @Author  : 祁国庆
        @Time    : 2024/05/31 15:42:22
        ...
        try:
            ...
            raise LoginException('登陆页面加载失败 account/signin')
        time.sleep(1)
    ...
    def native_account_login(self, driver: unChrome, shop_data, options):
        ...
        try:
            ...
            raise LoginException('登陆页面加载失败 seller/login')
        time.sleep(1)

        print("检查页面状态")
        if 'seller/login' not in driver.current_url:
            print('✅ 已不在登陆页 登陆成功')
            # 保存店铺缓存
            self.save_storage(driver, shop_global_id)
            return common.back(1, '登录成功')
