...
def get_unit_registry():
    ...
    deflator_df["value"] = deflator_df["value"].astype(str).map(Decimal)
    exchange_df["value"] = exchange_df["value"].astype(str).map(Decimal)
    ...
    for _, row in deflator_df.iterrows():
        ...
        if unit_name != BASE_CURRENCY:
            ...
            logger.info(f"Defining Monetary Unit A: {definition}")
    ...
    for _, row in usd_rates.iterrows():
        ...
        factor = Decimal("1.0") / rate_per_usd  # 1 USD = X EUR → 1 EUR = 1/X USD

        if eur_unit != usd_unit:  # Avoid self-reference
            definition = f"{usd_unit} = {factor} * {eur_unit}"
            ureg.define(definition)
            logger.info(f"Defining Monetary Unit B: {definition}")
    ...
    for _, row in other_rates.iterrows():
        ...
        if unit_name != usd_unit:
            factor = Decimal("1.0") / rate_per_usd
            try:
                definition = f"{unit_name} = {factor} * {usd_unit}"
                logger.info(f"Defining Monetary Unit C: {definition}")
                ureg.define(definition)
            except Exception as e:
                logger.info(f"Skipping {unit_name} due to error: {e}")
        ...
        for unit in list(ureg._units):  # Access defined unit names
            ...
            if match:
                ...
                try:
                    ...
                    logger.info(f"Defined alias: {alias_unit} = {unit}")
    ...
    with importlib.resources.path(
        ...
        for _, row in df2.iterrows():
            ...
            logger.info(f"Defining Physical Unit: {definition}")
            ureg.define(definition)
    ...
    with importlib.resources.path(
        ...
        for _, row in df3.iterrows():
            ...
            logger.info(f"Defining Magnitude: {definition}")
            ureg.define(definition)
