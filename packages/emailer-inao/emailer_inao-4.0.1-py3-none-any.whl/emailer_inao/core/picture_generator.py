...
class PictureGeneratorProject:
    ...
    def _validate_fusion_row(self, row: Dict[str, str], row_num: int) -> List[str]:
        ...
        # Validate positioning format (x:y)
        positioning = row.get('positioning', '').strip()
        if not re.match(r'^\d+:\d+$', positioning):
            errors.append(f"Row {row_num}: positioning must be in format 'x:y', got '{positioning}'")
...
class PictureGenerator:
    ...
    def validate_projects(self, recipients: List[Dict[str, str]]) -> ValidationResult:
        """Validate all picture generator projects against recipients."""
        errors = []
        warnings = []
    ...
    def _check_data_consistency(self, project: PictureGeneratorProject, 
        ...
        if not os.path.exists(project.data_folder):
            errors.append(f"Project '{project.project_name}': No generated data found. Run 'emailer-inao picture generate {project.project_name}' to generate pictures.")
            return ValidationResult(False, errors, warnings)
        ...
        for recipient in recipients:
            ...
            if primary_key:
                expected_files.add(f"{primary_key}.jpg")
        ...
        try:
            ...
            if missing_files or extra_files:
                errors.append(f"Project '{project.project_name}': Recipients and generated pictures are out of sync. Run 'emailer-inao picture generate {project.project_name}' to regenerate pictures for current recipients.")
    ...
    def generate_pictures(self, project_name: str, recipients: List[Dict[str, str]]) -> int:
