...
class BaseArrays:
    ...
    def mask(self, mask: ak.Array | np.ndarray, inplace: bool = True) -> ak.Array | BaseArrays:
        if inplace:
    ...
    def __len__(self) -> int:
        if type(self.array) is ak.Array:
            return len(self.array)
        elif type(self.array) is ak.Record:
            return 1
        else:
...
class Arrays:
    ...
    def mask_flat(self, mask: ak.Array | np.ndarray) -> tuple[ak.Array | None, ak.Array | None]:
        if self.flat_arrays is not None:
    ...
    def mask_jagged(self, mask: ak.Array | np.ndarray) -> ak.Array | None:
        if self.jagged_arrays is not None:
            jagged_masked = self.jagged_arrays.mask(mask, inplace=False)
        else:
            jagged_masked = None
