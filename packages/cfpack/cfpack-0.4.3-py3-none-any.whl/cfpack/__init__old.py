...
class legend_formatter:
    pos = (0.5, 0.5) # custom placement (normalised coordinates)
    loc = 'center left' # relative position
    textpad = 0.01
    length = 1
    fontsize = None
    ...
    if y is not None:
        ...
        if legend_formatter is not None:
            ...
            if builtins.type(legend_formatter) != builtins.type(legend_formatter):
                print('legend_formatter must be of type class cfpack.legend_formatter', error=True)
            # add the new legend
            leg = ax.legend(
...
def plot_map(image=None, xedges=None, yedges=None, dims=None, vmin=None, vmax=None, log=False,
    ...
    if image is not None:
        ...
        if colorbar:
            ...
            cb = plt.colorbar(pc, cax=cax, label=cmap_label, pad=0.01, aspect=25)
...
# === symlogspace ===
# Creates a symmetrical log-spaced array spanning both negative and positive values.
def symlogspace(start, stop, num=50, base=10.0):
    log_values = np.logspace(start, stop, num=num, base=base)
    ret = np.concatenate((-log_values[::-1], [0], log_values))
    return ret
