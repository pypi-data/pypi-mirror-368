    ...
    def batch_beam_search(
        ...
        for t in range(maxlen):
            ...
            for dec_layer in self.layer_stack:
                dec_output = dec_layer.forward(
                    dec_output, encoder_outputs, tgt_mask, src_mask, cache=caches[i]
                )
                caches[i] = dec_output
                i += 1
        ...
        for n in range(N):
            ...
            for i, score in enumerate(nbest_scores[n]):
                new_hyp = {"yseq": nbest_ys[n, i, 1 : nbest_ys_lengths[n, i]]}
...
class DecoderLayer(nn.Module):
    ...
    def forward(
        self, dec_input, enc_output, self_attn_mask, cross_attn_mask, cache=None
    ):
        x = dec_input
        residual = x
