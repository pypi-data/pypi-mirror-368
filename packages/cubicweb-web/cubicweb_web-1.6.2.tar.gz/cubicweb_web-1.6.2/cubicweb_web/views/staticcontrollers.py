...
class DataController(StaticFileController):
    ...
    def __init__(self, *args, **kwargs):
        ...
        route_prefix = urlparse(base_url).path.strip("/")

        if config["receives-base-url-path"]:
            formatted_route_prefix = f"{route_prefix}/" if route_prefix else ""
            if config["datadir-url"]:  # we don't want to add data/
                self.base_datapath = (
                    f"{formatted_route_prefix}{config.instance_md5_version()}/"
                )
            else:
                self.base_datapath = f"{formatted_route_prefix}{config.data_relpath()}"
