    ...
    def init(self, schema, check=True):
        ...
        for eschema in schema.entities():
            ...
            if eschema.schema_entity():
                self.setdefault(eschema.type, "schema")
            elif eschema in INTERNAL_TYPES or eschema in WORKFLOW_TYPES:
                self.setdefault(eschema.type, "system")
            elif eschema.is_subobject(strict=True):
                self.setdefault(eschema.type, "subobject")
            else:
                self.setdefault(eschema.type, "application")
    ...
    def relations_by_section(self, entity, formtype, section, permission, strict=False):
        ...
        for rschema, targetschemas, role in eschema.relation_definitions(True):
            ...
            rdef = eschema.relation_definition(
                rschema, role=role, target_type=targetschemas[0]
            )
