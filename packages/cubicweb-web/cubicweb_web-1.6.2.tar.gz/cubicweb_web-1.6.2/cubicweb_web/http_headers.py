...
def listGenerator(fun):
    ...
    def listGeneratorHelper(a_list):
        return generateList([fun(e) for e in a_list])
    ...
    for seq in headers:
        ...
        try:
            ...
            for k, v in dict(challenge).items():
                items.append("{}={}".format(k, quoteString(v)))

            _generated.append("{} {}".format(scheme, ", ".join(items)))
        except ValueError:
...
def parseSetCookie(headers):
    ...
    for header in headers:
        try:
            ...
            setCookies.append(makeCookieFromList(items, True))
        except ValueError:
