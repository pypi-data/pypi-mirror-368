...
class SummaryFormatter:
    ...
    def format(
        ...
        for trace_id, records in traces.items():
            for record in records:
                ...
                prompt_tokens = (
                    usage.get("prompt_tokens", 0) or record.get("prompt_tokens", 0) or 0
                )
                completion_tokens = (
        ...
        if total_tokens > 0:
            output.append(
                f"📊 Crashens Detector Summary – {timestamp} | Traces: {len(traces)} | Cost: {cost_str} | Tokens: {total_tokens:,}"
            )
        else:
            output.append(
                f"📊 Crashens Detector Summary – {timestamp} | Traces: {len(traces)} | Cost: {cost_str}"
            )
        output.append("")
    ...
    def _format_summary_only(
        ...
        if total_waste_cost > 0:
            savings_percentage = (
                (total_waste_cost / total_cost * 100) if total_cost > 0 else 0
            )
            output.append(f"💡 {savings_percentage:.0f}% potential savings")

        return "\n".join(output)

    def _add_top_traces_summary(
        self,
        output: List[str],
        traces: Dict[str, List[Dict[str, Any]]],
        summary_only: bool,
    ...
    def _calculate_record_cost(
        self, record: Dict[str, Any], model_pricing: Dict[str, Any]
    ) -> float:
    ...
    def _add_waste_analysis_summary(
        self,
        output: List[str],
        ...
        for detection in detections:
            ...
            waste_by_type[detector_type]["total_tokens"] += detection.get(
                "waste_tokens", 0
            )
        ...
        if total_waste_cost > 0:
            ...
            for detector_type, data in waste_by_type.items():
                if data["total_cost"] > 0:
                    display_name = display_names.get(
                        detector_type, detector_type.title()
                    )
                    cost_str = f"${data['total_cost']:.4f}"
                    if summary_only:
                        output.append(
                            f"| {display_name} | {data['count']} | {cost_str} |"
                        )
                    else:
                        output.append(
                            f"| {display_name} | {data['count']} | {cost_str} | {data['total_tokens']:,} |"
                        )
            ...
            if summary_only:
                output.append(
                    f"| **Total** | **{len(detections)}** | **{total_cost_str}** |"
                )
            else:
                output.append(
                    f"| **Total** | **{len(detections)}** | **{total_cost_str}** | **{total_waste_tokens:,}** |"
                )
        else:
            output.append("")
            output.append("✅ No waste patterns detected")
