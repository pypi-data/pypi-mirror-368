...
class MarkdownFormatter:
    ...
    def __init__(self):
        self.severity_colors = {"high": "🔴", "medium": "🟡", "low": "🟢"}
    ...
    def format(
        ...
        scrubbed_detections = [
            self.pii_scrubber.scrub_detection(detection) for detection in detections
        ]
        ...
        for detector_name, group_data in aggregated.items():
            ...
            output.append("|--------|-------|")
            output.append(
                f"| Total Waste Cost | ${group_data['total_waste_cost']:.4f} |"
            )
            output.append(
                f"| Total Waste Tokens | {group_data['total_waste_tokens']:,} |"
            )
            output.append("")
    ...
    def _aggregate_detections(
        ...
        for detection in detections:
            ...
            detector = detection.get(
                "type", "unknown"
            )  # Use 'type' field instead of 'detector'
            if detector == "unknown":
    ...
    def _format_aggregated_detection(
        self, group_data: Dict[str, Any], summary_only: bool = False
    ) -> str:
        ...
        if detector.lower() == "overkillmodeldetector":
            lines.append(
                f"**Suggested Fix**: Route short prompts to `{group_data['suggested_model']}`"
            )
    ...
    def _format_detection(
        self, detection: Dict[str, Any], index: int, summary_only: bool = False
    ) -> str:
        ...
        if detection["type"] == "retry_loop":
        ...
        elif detection["type"] in [
            "gpt4_short",
            "expensive_model_short",
            "expensive_model_overkill",
        ]:
            lines.append(
                f"- **Completion Length**: {detection.get('completion_length', 0)} tokens"
            )
            lines.append(f"- **Model Used**: {detection.get('model_used', 'unknown')}")
            lines.append(
                f"- **Suggested Model**: {detection.get('suggested_model', 'gpt-3.5-turbo')}"
            )
            ...
            lines.append(
                f"- **Primary Model**: {detection.get('primary_model', 'unknown')}"
            )
            lines.append(
                f"- **Fallback Model**: {detection.get('fallback_model', 'unknown')}"
            )
            lines.append(
                f"- **Time Between Calls**: {detection.get('time_between_calls', 'unknown')}"
            )
            if not summary_only:
                lines.append(
                    f"- **Primary Prompt**: `{detection.get('primary_prompt', '')[:50]}{'...' if len(detection.get('primary_prompt', '')) > 50 else ''}`"
                )
    ...
    def _calculate_total_ai_spend(
        ...
        for records in traces.values():
            for record in records:
                ...
                if model_pricing:
                    model = record.get(
                        "model", record.get("input", {}).get("model", "gpt-3.5-turbo")
                    )

                    # Get tokens from various possible locations
                    usage = record.get("usage", {})
                    input_tokens = (
                        record.get("prompt_tokens") or usage.get("prompt_tokens") or 0
                    )
                    output_tokens = (
    ...
    def _add_cost_breakdown_tables(
        self,
        output: List[str],
        traces: Dict[str, List[Dict[str, Any]]],
        summary_only: bool,
        ...
        if trace_costs:
            ...
            sorted_traces = sorted(
                trace_costs.items(), key=lambda x: x[1], reverse=True
            )[:3]
            for i, (trace_id, cost) in enumerate(sorted_traces, 1):
                ...
                if summary_only:
                    ...
                    model = first_record.get("input", {}).get(
