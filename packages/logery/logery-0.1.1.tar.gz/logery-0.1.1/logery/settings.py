...
# Configurações básicas - não validamos se existem ainda
...
default_logger_level = define_setting(
    getenv("DEFAULT_LOGGER_LEVEL", "INFO"), validator=validate_level
)


def validate() -> None:
    """
    Valida as configurações apenas se necessário.
    Agora é mais flexível e não exige que arquivos existam.
    """
    global \
        root_dir, \
        logs_dir, \
    ...
    # Validar apenas o diretório raiz (deve existir)
    if not root_dir.is_dir():
        raise NotADirectoryError(f"Root directory does not exist: {root_dir}")

    # Atualizar configurações a partir das variáveis de ambiente
    setup_logger_name = getenv("SETUP_LOGGER_NAME", "config_setup")
...
def change_settings(
    ...
    if new_logging_config_json:
        logging_config_json = new_logging_config_json
