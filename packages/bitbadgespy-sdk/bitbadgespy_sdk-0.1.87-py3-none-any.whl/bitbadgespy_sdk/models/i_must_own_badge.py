...
class IMustOwnBadge(BaseModel):
    ...
    amount_range: IUintRange = Field(description="The range of amounts the user must own (min to max).", alias="amountRange")
    ownership_times: List[IUintRange] = Field(description="The time ranges during which the user must own the tokens.", alias="ownershipTimes")
    badge_ids: List[IUintRange] = Field(description="The token IDs the user must own.", alias="badgeIds")
    ...
    must_satisfy_for_all_assets: StrictBool = Field(description="If true, the user must meet ownership requirements for all specified tokens; else, must meet requirements for any single token.", alias="mustSatisfyForAllAssets")
