...
class IMustOwnBadges(BaseModel):
    ...
    amount_range: IUintRange = Field(description="The min/max acceptable amount of tokens that must be owned (can be any values, including 0-0).", alias="amountRange")
    ownership_times: List[IUintRange] = Field(description="The range of the times that the tokens must be owned.", alias="ownershipTimes")
    badge_ids: List[IUintRange] = Field(description="The range of the token IDs that must be owned.", alias="badgeIds")
    ...
    must_satisfy_for_all_assets: StrictBool = Field(description="Whether or not the user must own all the specified tokens. If false, we will accept if they meet criteria for at least one token.", alias="mustSatisfyForAllAssets")
