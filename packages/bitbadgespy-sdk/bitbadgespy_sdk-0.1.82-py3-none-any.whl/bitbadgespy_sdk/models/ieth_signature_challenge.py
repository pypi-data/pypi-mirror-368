...
class IETHSignatureChallenge(BaseModel):
    """
    IETHSignatureChallenge
    """ # noqa: E501
    signer: StrictStr = Field(description="The Ethereum address that must sign the nonce for verification.")
    challenge_tracker_id: StrictStr = Field(description="The ID of this ETH signature challenge for tracking the number of uses per signature.", alias="challengeTrackerId")
    uri: StrictStr = Field(description="The URI associated with this ETH signature challenge, optionally providing metadata about the challenge.")
    custom_data: StrictStr = Field(description="Arbitrary custom data associated with this ETH signature challenge.", alias="customData")
    __properties: ClassVar[List[str]] = ["signer", "challengeTrackerId", "uri", "customData"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IETHSignatureChallenge from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IETHSignatureChallenge from a dict"""
        if obj is None:
            return None
