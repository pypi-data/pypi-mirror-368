...
class IStatusDoc(BaseModel):
    ...
    gas_price: Union[StrictFloat, StrictInt] = Field(description="The current gas price based on the average of recent transactions", alias="gasPrice")
    last_x_txs: Optional[List[ITransactionEntry]] = Field(default=None, description="The last X transactions with timestamps for dynamic reset functionality", alias="lastXTxs")
    __properties: ClassVar[List[str]] = ["_docId", "_id", "block", "nextCollectionId", "gasPrice", "lastXTxs"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in last_x_txs (list)
        _items = []
        if self.last_x_txs:
            for _item_last_x_txs in self.last_x_txs:
                if _item_last_x_txs:
                    _items.append(_item_last_x_txs.to_dict())
            _dict['lastXTxs'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "lastXTxs": [ITransactionEntry.from_dict(_item) for _item in obj["lastXTxs"]] if obj.get("lastXTxs") is not None else None
        })
        return _obj
