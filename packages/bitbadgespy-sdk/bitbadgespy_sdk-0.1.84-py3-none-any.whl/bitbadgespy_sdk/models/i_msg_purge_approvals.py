...
class IMsgPurgeApprovals(BaseModel):
    """
    IMsgPurgeApprovals
    """ # noqa: E501
    ...
    purge_expired: StrictBool = Field(description="Whether to purge expired approvals (approvals with no future valid transfer times).", alias="purgeExpired")
    ...
    purge_counterparty_approvals: StrictBool = Field(description="Whether to purge counterparty approvals (approvals where the creator is the only initiator).", alias="purgeCounterpartyApprovals")
    approvals_to_purge: List[IApprovalIdentifierDetails] = Field(description="Specific approvals to purge. If empty, purges all applicable approvals based on other flags.", alias="approvalsToPurge")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["creator", "collectionId", "purgeExpired", "approverAddress", "purgeCounterpartyApprovals", "approvalsToPurge"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IMsgPurgeApprovals from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in approvals_to_purge (list)
        _items = []
        if self.approvals_to_purge:
            for _item_approvals_to_purge in self.approvals_to_purge:
                if _item_approvals_to_purge:
                    _items.append(_item_approvals_to_purge.to_dict())
            _dict['approvalsToPurge'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IMsgPurgeApprovals from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "purgeExpired": obj.get("purgeExpired"),
            "approverAddress": obj.get("approverAddress"),
            "purgeCounterpartyApprovals": obj.get("purgeCounterpartyApprovals"),
            "approvalsToPurge": [IApprovalIdentifierDetails.from_dict(_item) for _item in obj["approvalsToPurge"]] if obj.get("approvalsToPurge") is not None else None
        })
