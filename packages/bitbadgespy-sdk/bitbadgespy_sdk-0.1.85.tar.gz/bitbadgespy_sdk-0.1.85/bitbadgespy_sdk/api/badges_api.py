...
class BadgesApi:
    ...
    def get_balance_by_address(
        ...
        Retrieves the balances of a specific address for a collection.  ```tsx const res = await BitBadgesApi.getBalanceByAddress(collectionId, address, { ...options }); console.log(res); ```   SDK Links: - **[Request Type](https://bitbadges.github.io/bitbadgesjs/interfaces/iGetBalanceByAddressPayload)** - **[Response Type](https://bitbadges.github.io/bitbadgesjs/interfaces/iGetBalanceByAddressSuccessResponse)** - **[SDK API Function](https://bitbadges.github.io/bitbadgesjs/classes/BitBadgesAPI.html#getbalancebyaddress)**  Scopes: - `readPrivateClaimData` - Required if fetching private claim data (user-level approvals)  Alternative Flow: Note that you can also set up a claim that checks badge ownership and check the success per user of that claim as well 
