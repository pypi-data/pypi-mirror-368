...
### Cart Operations
- `add_to_cart` - Add items to cart (⚠️ REQUIRES unit_price in cents!)
...
**For add_to_cart (MANDATORY workflow - will fail without these steps):**

⚠️ **CRITICAL**: `unit_price` (in cents) is REQUIRED. The tool will return a `missing_unit_price` error if not provided.
...
- `options`: Array of `{"option_id": "...", "name": "...", "quantity": 1}` from menu item details
  - Required for items with sizes/customizations (e.g., Medium fries, Large drink)
...
- ❌ Don't omit `unit_price` — backend price formatting/logging expects it
