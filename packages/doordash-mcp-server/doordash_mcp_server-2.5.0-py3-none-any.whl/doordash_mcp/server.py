...
async def add_to_cart(
    client_id: str,
    store_id: Union[int, str],  # Accept both int and string
    item_id: Union[int, str],  # Accept both int and string
    quantity: Union[int, str] = 1,  # Accept both int and string
    unit_price: Optional[Union[int, str]] = None,  # Accept both int and string
    item_name: Optional[str] = None,
    option_id_1: Optional[str] = None,  # First option ID (e.g., size)
    option_name_1: Optional[str] = None,  # First option name (e.g., "Medium")
    option_id_2: Optional[str] = None,  # Second option ID (e.g., topping)
    option_name_2: Optional[str] = None,  # Second option name
    option_id_3: Optional[str] = None,  # Third option ID
    option_name_3: Optional[str] = None,  # Third option name
    validate_options: bool = True,
    special_instructions: Optional[str] = None,
) -> Dict[str, Any]:
    """Add to Cart

⚠️ CRITICAL: unit_price (in cents) is REQUIRED. Will fail with 'missing_unit_price' error if not provided.

**📋 MANDATORY Workflow (will fail if skipped):**
1. Search restaurants → get store_id
2. Get restaurant details → find item_id and item_name  
3. Get menu item details → obtain unit_price (cents) and option_ids
4. Add to cart with EXACT values from above steps

**🔑 Required Parameters:**
- store_id: From search results (e.g., 653770)
- item_id: From restaurant menu (e.g., 6597052550)
- unit_price: In CENTS (e.g., $3.63 = 363), from menu item details
- item_name: Item display name from menu (e.g., "Apple Pie")

**📦 Optional but Often Needed:**
- option_id_1, option_name_1: First option (e.g., size: "29646324430", "Medium")
- option_id_2, option_name_2: Second option if needed
- option_id_3, option_name_3: Third option if needed
  - Required for items with sizes/customizations
  - Get exact option_ids from get_menu_item_details tool
- quantity: Number of items (default: 1)
- special_instructions: Custom notes

**❌ Common Failures:**
- Missing unit_price → "missing_unit_price" error
- Wrong price format → Use cents not dollars
- Missing options for sized items → Get from menu item details first

**✅ Working Example - McDonald's French Fries:**
- store_id: 653636, item_id: 6570953960
- item_name: "French Fries", unit_price: 589 (cents for Medium)
- option_id_1: "29646324430", option_name_1: "Medium"
    
Args:
    client_id: Session ID from initialize_doordash() (required)
    store_id: Restaurant/store ID (required)
    item_id: Menu item ID (required)
    unit_price: Price in CENTS (required)
    item_name: Item name from menu (required)
    option_id_1: First option ID from menu item details
    option_name_1: First option name (e.g., "Medium", "Large")
    option_id_2: Second option ID if needed
    option_name_2: Second option name if needed
    option_id_3: Third option ID if needed
    option_name_3: Third option name if needed
    quantity: Number of items to add (default: 1)
    special_instructions: Optional special instructions

Returns:
    Dict containing API response or missing_unit_price error
    """
    try:
        org_id = os.getenv("DOORDASH_ORG_ID")
        if not org_id:
        ...
        if unit_price is not None and isinstance(unit_price, str):
            unit_price = int(unit_price)
        
        # Build options array from individual parameters
        options = []
        if option_id_1 and option_name_1:
            options.append({
                "option_id": str(option_id_1),
                "name": option_name_1,
                "quantity": 1
            })
        if option_id_2 and option_name_2:
            options.append({
                "option_id": str(option_id_2),
                "name": option_name_2,
                "quantity": 1
            })
        if option_id_3 and option_name_3:
            options.append({
                "option_id": str(option_id_3),
                "name": option_name_3,
                "quantity": 1
            })
        
        # If no options were provided, set to None for backwards compatibility
        if not options:
            options = None
        
        # Validate required pricing param
        if unit_price is None:
            return {
                "success": False,
                "error": "missing_unit_price",
                "message": "add_to_cart requires unit_price (in cents). Call get_menu_item_details first and pass unit_price and required option ids.",
            }

        client = DoorDashClient(org_id=org_id, client_id=client_id)
        
        # Build kwargs for add_to_cart
        kwargs = {
        ...
        if validate_options is not None:
            kwargs["validate_options"] = validate_options
...
# Menu Item Details
@mcp.tool()
async def get_menu_item_details(client_id: str, store_id: Union[int, str], item_id: Union[int, str]) -> Dict[str, Any]:
...
Returns:
    ...
    try:
        ...
        # Convert string parameters to integers if needed (for consistency)
        if isinstance(store_id, str):
            store_id = int(store_id)
        if isinstance(item_id, str):
            item_id = int(item_id)
        
        # Use the Python client which calls the reliable endpoint
        client = DoorDashClient(org_id=org_id, client_id=client_id)
        result = client.get_menu_item(store_id, item_id)
        
        # The Python client returns the raw response from the API
        # Check if it's successful by looking for expected keys
        if "success" in result and not result["success"]:
            return result
        elif "error" in result:
            return {"success": False, "error": result.get("error"), "details": result}
        else:
            # Successful response - ensure it has success flag
