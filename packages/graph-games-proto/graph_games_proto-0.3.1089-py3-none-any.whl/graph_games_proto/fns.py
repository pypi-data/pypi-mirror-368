...
def is_move_pieces_to_path_action_legal(game, action):
    print("****************************** is_move_pieces_to_path_action_legal 1")
    player_idx = action.legal_action.player_idx
    proposed_cards_uuids = action.move_pieces_to_path.card_uuids
    path_idx = action.legal_action.config.move_pieces_to_path.path_idx
    ...
    fulfillment = get_path_fulfillment_from_resources(
        game,
        fulfillment, 
        proposed_cards, 
        remaining_segments,
    )
    print("****************************** is_move_pieces_to_path_action_legal 2")
    if fulfillment is None:
        return False
    
    print("****************************** is_move_pieces_to_path_action_legal 3: ", len(fulfillment))
    print("****************************** is_move_pieces_to_path_action_legal 4: ", len(proposed_cards_uuids))
    return len(fulfillment) == len(proposed_cards_uuids)
...
def get_legal_actions_for_path(game, player_idx, path_idx):
    ...
    legal_actions = []
    sample = get_sample_actionclaimpath(game, player_idx, path_idx)
    if sample:
        print(f"Path {path_idx} is claimable by player {player_idx} with fulfillment: {sample}")
...
def get_sample_actionclaimpath(game, player_idx, path_idx):
    fulfillment = get_sample_path_fulfillment(game, player_idx, path_idx)
    if not fulfillment:
        return None
    
    player_pieces = game.players[player_idx].pieces
    pile_template_0_pieces = [piece for piece in player_pieces if piece.pile_template_idx == 0]

    # If the player does not have enough pieces to fulfill the path, return None
    if len(pile_template_0_pieces) < len(fulfillment):
        return None
...
def get_path_fulfillment_from_resources(game, fulfillment, remaining_cards, remaining_segments):
