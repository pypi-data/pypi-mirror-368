...
class State(PClass):
    ...
    def __fromdict__(d):
        return State(
            player_scores=[PlayerScore.__fromdict__(x) for x in d["player_scores"]],
...
class PublicPlayerScore(PClass):
    items = field(type=list)  # List[ScoreItem]
    total = field(type=int)
    def __todict__(self):
        return {
...
class PlayerScore2(PClass):
    public_items = field(type=list)  # List[ScoreItem]
    private_items = field(type=list)  # List[ScoreItem]
    def __todict__(self):
        return {
    ...
    def __fromdict__(d):
        return PlayerScore2(
            public_items=[ScoreItem.__fromdict__(x) for x in d["public_items"]],
            private_items=[ScoreItem.__fromdict__(x) for x in d["private_items"]],
        )
...
def get_edges(board_config):
    ...
    for link in board_config.links:
        ...
        edges.append(BiEdge(uuid=link.uuid, node_1_uuid=link.c1, node_2_uuid=link.c2, node_1_idx=node_1_idx, node_2_idx=node_2_idx, paths=paths))
...
def getinitialstate(game_config):
    ...
    state = State(
        ...
        goals = [
            Goal(uuid="642f52d9-f11e-4d27-a275-63df2cea90e0", node_uuids=["e21f901e-c8db-44cf-b622-2d6586bcbbb7", "d82d8eb1-84e0-4717-93f6-1f99563081e7"]),
            Goal(uuid="f3d7d163-e86f-43aa-b7ca-046045bd4614", node_uuids=["1400b9bb-5070-447a-aee8-e896c609372f", "57e774ed-d37a-4546-b786-3f1bd606323c"]),
...
def default_handle_scoring(game):
    print("******************************12344 default_handle_scoring")
    return game.set(
        player_scores=[
            PlayerScore2(public_items=[ScoreItem2(amount=6, description="Player {} score".format(i))], private_items=[]) for i in range(len(game.players))
        ]
    )
...
def getnextstate2(s, a, log=False):
    ...
    s = run_state_action_hooks(s, a, HANDLE_SCORING_HOOKS, log)
    return s
...
def getpublicplayerscore(s, player_score):
    print("*****************321 player_score.public_items: ", player_score.public_items)
    return PublicPlayerScore(
