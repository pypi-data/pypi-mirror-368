...
class PlayerGraph(PClass):
    player_idx = field(type=int)
    player_nodes = field(type=list)  # List[PlayerNode]
    def __todict__(self):
        return {
            "player_idx": self.player_idx,
            "player_nodes": [node.__todict__() for node in self.player_nodes],
        }
    @staticmethod
    def __fromdict__(d):
        return PlayerGraph(
            player_idx=d["player_idx"],
            player_nodes=[PlayerNode.__fromdict__(node) for node in d["player_nodes"]],
        )
...
def get_nodes(board_config):
    if board_config and board_config.points:
        return [
            Node(idx=idx, uuid=node.uuid, name=f"{node.num}", pieces=[]) 
...
def get_edges(board_config):
    ...
    for link in board_config.links:
        ...
        edges.append(BiEdge(uuid=link.uuid, node_1_idx=node_1_idx, node_2_idx=node_2_idx, paths=paths))
    ...
    for edge in game.edges:
        for segment in edge.segments:
            for piece in segment.pieces:
                if piece.player_idx == player_idx:
                    for node_idx in segment.node_idxs:
                        if node_idx not in node2neighbors:
                            node2neighbors[node_idx] = set()
                        for neighbor_node_idx in segment.node_idxs:
                            if neighbor_node_idx != node_idx:
                                node2neighbors[node_idx].add(neighbor_node_idx)
    return PlayerGraph(
        player_idx=player_idx, 
        player_nodes=[
            PlayerNode(
                player_idx=player_idx, 
                node_idx=node_idx,
                neighbors=list(node2neighbors.get(node_idx, set())),
            ) for node_idx in range(len(nodes))
        ]
    )
...
def get_goals(s, player_idx):
    ...
    return [
        goaluuid2goal[card.goal_uuid] for card in player.cards
        if card.goal_uuid in goaluuid2goal
    ]
