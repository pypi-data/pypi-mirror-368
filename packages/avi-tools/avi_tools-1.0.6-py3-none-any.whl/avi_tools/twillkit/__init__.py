...
def exception_collector(func: Callable):
    @wraps(func)
    def wrapper(*args, **kwargs):

        is_method = args and hasattr(args[0], '__dict__')
        if is_method:
            self_obj = args[0]
            if not hasattr(self_obj, '_exceptions'):
                self_obj._exceptions = []
            try:
                return func(*args, **kwargs)
            except Exception as e:
                self_obj._exceptions.append({
            ...
            if not hasattr(wrapper, '_exceptions'):
                wrapper._exceptions = []
            try:
                return func(*args, **kwargs)
            except Exception as e:
                wrapper._exceptions.append({
                    'function': func.__name__,
                    'args': args,
                    'kwargs': kwargs,
                    'exception': e
    ...
    def show_exceptions(obj=None):
        if obj is not None:
            exceptions = getattr(obj, '_exceptions', [])
        else:
            exceptions = getattr(wrapper, '_exceptions', [])
        if not exceptions:
            print("No exceptions recorded.")
            return
        print(f"There have been {len(exceptions)} errors:")
        for i, e in enumerate(exceptions, start=1):
            print(f"\n--- Error {i} ---")
            print(f"Function: {e['function']}")
            print(f"Args: {e['args']}")
            print(f"Kwargs: {e['kwargs']}")
            print(f"Exception: {e['exception']}")

    def count_errors(obj=None):
        if obj is not None:
            return len(getattr(obj, '_exceptions', []))
        return len(getattr(wrapper, '_exceptions', []))

    wrapper.show_exceptions = show_exceptions
    wrapper.count_errors = count_errors
    return wrapper
