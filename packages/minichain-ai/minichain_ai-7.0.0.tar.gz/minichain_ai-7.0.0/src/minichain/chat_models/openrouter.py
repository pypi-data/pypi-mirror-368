...
# # Use the 'requests' library for direct, flexible API calls
# import requests
...
# class OpenRouterChatModel(BaseChatModel):
#     """
#     A chat model that correctly interacts with the OpenRouter API,
#     supporting standard and extended parameters like 'reasoning'.
#     """
...
#             "stream": stream,
#             **self.api_kwargs, # This is where 'reasoning' will be
#         }
#         if self.max_tokens:
#             payload["max_tokens"] = self.max_tokens
#         return payload
...
#         }
#         if isinstance(self.config, OpenRouterConfig):
...
#     def generate(self, input_data: Union[str, List[BaseMessage]]) -> ChatResult:
#         """Generates a response using a direct HTTP request."""
#         messages = self._prepare_messages(input_data)
#         payload = self._build_payload(messages, stream=False)
#         headers = self._build_headers()
...
#         # Parse the response into our standard ChatResult object
#         usage_data = data.get("usage", {})
#         token_usage = TokenUsage(
#             completion_tokens=usage_data.get("completion_tokens"),
#             prompt_tokens=usage_data.get("prompt_tokens"),
...
#     def stream(self, input_data: Union[str, List[BaseMessage]]) -> Iterator[str]:
#         """Streams a response using a direct HTTP request."""
#         messages = self._prepare_messages(input_data)
#         payload = self._build_payload(messages, stream=True)
#         headers = self._build_headers()
...
#                             yield delta["content"]
#                     except json.JSONDecodeError:
#                         continue # Ignore empty lines or other non-JSON data
# # # src/minichain/chat_models/openrouter.py
# # import os
