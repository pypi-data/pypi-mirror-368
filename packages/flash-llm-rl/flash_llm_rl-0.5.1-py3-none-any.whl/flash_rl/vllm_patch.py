            ...
            def hacked_init_(
                ...
                **kwargs
                ...
                if config is not None:
                    ...
                    logger.info(f"Loading flash_rl config from: {config}")
                ...
                if hasattr(self, 'flash_rl_profile') and \
                    (not hasattr(model, 'beforeflashrl_load_weights')) and \
                    ...
                    def hacked_load_weights(
                        ...
                        for name, (shape, stride, dtype, nbytes) in model.hacked_original_weights_rebuild_keys.items():
                            if name in existing_params:
                                # existing_nbytes = existing_params[name].untyped_storage().nbytes()
                                # print(
                                #     f"flash_rl load_weights review {name} ({existing_nbytes}): "
                                #     f"{existing_params[name].dtype} x {existing_params[name].shape}"
                                #     f" --> {dtype} x {shape}"
                                # )
                        ...
                        if len(self.flash_rl_module_attribute_to_preserve) > 0:
                            for _, module in model.named_modules():
                                for attr in self.flash_rl_module_attribute_to_preserve:
                                    if torch.is_tensor(getattr(module, attr, None)):
                                        # print(f"flash_rl recovering {attr} in module {module}")
        ...
        if not hasattr(Sampler, 'beforeflashrl_forward'):
            ...
            def hacked_logprob_forward(
                ...
                if sampling_metadata.max_num_logprobs is not None:
                    ...
                    logprobs_tensors = self.gather_logprobs(processed_logprobs, 0, token_ids=sampled)
                else:
            ...
            def hacked_sample(
                ...
                random_sampled, processed_logits = self.topk_topp_sampler(
                    logits,
            ...
            Sampler.beforeflashrl_sample = original_sample
            Sampler.sample = hacked_sample

            from .vllm_sampler import HackedTopKTopPSampler
            
            def hacked_init_(self):
                super(self.__class__, self).__init__()
                self.topk_topp_sampler = HackedTopKTopPSampler()
            original_init = Sampler.__init__
            Sampler.beforeflashrl_init = original_init
            Sampler.__init__ = hacked_init_
