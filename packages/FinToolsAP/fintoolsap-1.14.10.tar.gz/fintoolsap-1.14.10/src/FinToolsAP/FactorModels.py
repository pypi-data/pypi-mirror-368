...
def tangency_portfolio(returns: pandas.DataFrame, risk_free_rate: float = 0.0):
    """
    Calculate the tangency portfolio weights (maximum Sharpe ratio portfolio).
    
    The tangency portfolio is the portfolio of risky assets that maximizes the 
    Sharpe ratio when combined with a risk-free asset. It represents the optimal
    risky portfolio in the Capital Asset Pricing Model (CAPM) framework.
    
    Parameters
    ----------
    returns : pandas.DataFrame
        DataFrame of asset returns where each column represents an asset and 
        each row represents a time period. Shape should be (T, N) where T is 
        the number of time periods and N is the number of assets.
    risk_free_rate : float, default 0.0
        The risk-free rate of return. This rate is subtracted from asset returns
        to compute excess returns for the optimization.
    
    Returns
    -------
    numpy.ndarray
        Column vector of portfolio weights with shape (N, 1) where N is the number
        of assets. Weights sum to 1 and represent the optimal allocation to maximize
        the Sharpe ratio.
    ...
    .. math::
        w_T = \\frac{\\Sigma^{-1} \\mu}{\\mathbf{1}^T \\Sigma^{-1} \\mu}
        
    where :math:`\\mu` is the vector of expected excess returns, :math:`\\Sigma` 
    is the covariance matrix of returns, and :math:`\\mathbf{1}` is a vector of ones.
    ...
    >>> weights = tangency_portfolio(returns, risk_free_rate=0.02)
    >>> print(f"Portfolio weights: {weights.flatten()}")
...
def minimum_variance_portfolio(returns: pandas.DataFrame):
    ...
    The minimum variance portfolio is the portfolio that achieves the lowest possible
    variance (risk) among all possible portfolio combinations. It represents the
    ...
    numpy.ndarray
        Column vector of portfolio weights with shape (N, 1) where N is the number
        of assets. Weights sum to 1 and represent the minimum variance allocation.
        
    Notes
    -----
    The minimum variance portfolio weights are computed as:
    
    .. math::
        w_{MVP} = \\frac{\\Sigma^{-1} \\mathbf{1}}{\\mathbf{1}^T \\Sigma^{-1} \\mathbf{1}}
        
    where :math:`\\Sigma` is the covariance matrix of returns and :math:`\\mathbf{1}` 
    is a vector of ones.
    ...
    >>> print(f"Min variance weights: {weights.flatten()}")
