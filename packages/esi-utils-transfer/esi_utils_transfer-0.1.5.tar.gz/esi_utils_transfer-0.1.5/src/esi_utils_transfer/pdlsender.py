...
class PDLSender(Sender):
    ...
    _pdlcmd_pieces = [
        ...
        "--privateKey=[PRIVATEKEY]  --signatureVersion=[SIGNATUREVERSION] "
        "--configFile=[CONFIGFILE] [FILE] [DIRECTORY]",
    ]
    ...
    def _replace_required_properties(self, cmd):
        for propkey in self._required_properties:
            ...
            if propkey == "configfile":
                if self._sender_is_aws(self._properties[propkey]):
                    cmd = cmd.replace("[SIGNATUREVERSION]", "v2")
                else:
                    cmd = cmd.replace("[SIGNATUREVERSION]", "v1")
    ...
    def _replace_cmdline_args(self, cmd):
        if self.cmdline_args is None or not len(self.cmdline_args):
            cmd = cmd.replace("[CMDLINE_ARGS]", " ")
            return cmd
        arg_nuggets = []
    ...
    def _sender_is_aws(self, configfile: str) -> bool:
        """Determine if the pdl config uses an AWS product sender"""
        fpath = pathlib.Path(configfile)
        if not fpath.exists():
            raise FileNotFoundError(f"PDL config file ({configfile}) does not exist!")
        config = configparser.ConfigParser()
        # read the config and add a default header
        with open(fpath) as f:
            config_str = "[DEFAULT]\n" + f.read()
        config.read_string(config_str)
        senders = config["DEFAULT"]["senders"].split(",")

        # check senders for an aws product sender type
        for sender in senders:
            sender_type = config[sender]["type"]
            if "AwsProductSender" in sender_type:
                return True
        return False

    def send(self, dry_run: bool = False) -> Union[Tuple[int, str], str]:
        ...
        if dry_run:
            ...
            if not retcode:
                ...
                fmt = (
                    f'Could not send product "{ptype}" due to error "{stdout + stderr}"'
                )
                raise IOError(fmt)
