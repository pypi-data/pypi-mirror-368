...
class EmailSender(Sender):
    ...
    def send(self):
        ...
        try:
            ...
            for address, bcc in address_tuples:
                ...
                _send_email(all_addresses, msgtxt, smtp_servers, protocol)
    ...
    def cancel(self, cancel_content=None):
        ...
        for address, bcc in address_tuples:
            msg = _get_encoded_message(
                address, subject, cancel_content, sender, [], bcc=bcc
            )

            _send_email(address, msg, smtp_servers, protocol)
...
def _send_email(address, msgtxt, smtp_servers, protocol):
    ...
    logging.info(f'Message sent to "{address}" via SMTP server "{servername}"')
...
def _get_encoded_message(address, subject, text, sender, attachments, bcc=None):
    ...
    if bcc is None:
        ...
        msg["To"] = ", ".join([address] + bcc)
    msg.preamble = "You will not see this in a MIME-aware mail reader.\n"
    msg.set_content(text)
    # Open the files in binary mode, guess the type.
