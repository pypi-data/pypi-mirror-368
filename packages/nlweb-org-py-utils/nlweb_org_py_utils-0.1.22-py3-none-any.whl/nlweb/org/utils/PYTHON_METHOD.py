...
class PYTHON_METHOD(PRINTABLE):
    ...
    def _InitCodeType(self, method:CodeType, cla):
        ...
        all_parts = self._moduleName.split('.')
        if len(all_parts) > 1:
            self._packageName = '.'.join(all_parts[:-1])
        else:
            self._packageName = all_parts[0]
    ...
    def _InitCallable(self, method:callable):
        ...
        # remove the last part after split('.')
        all_parts = self.__module__.split('.')
        if len(all_parts) > 1:
            self._packageName = '.'.join(all_parts[:-1])
        else:
            self._packageName = ''
    ...
    def GetClass(self):
        ...
        if not cls:
            ...
            try:
                ...
                #"the 'package' argument is required to perform a relative import for {name!r}"
                ...
                try:
                    ...
                    raise Exception(
                        f'PYTHON_METHOD1({self.GetType()}): Cannot import class [{self.GetModuleName()}] from package [{self.GetPackageName()}].'
                        f'\n Error [{e}]'
                    ) from None
                
                except ImportError as e:
                    raise Exception(
                        f'PYTHON_METHOD2({self.GetType()}): Cannot import class [{self.GetClassName()}] from package [{self.GetPackageName()}].'
                    ) from e
