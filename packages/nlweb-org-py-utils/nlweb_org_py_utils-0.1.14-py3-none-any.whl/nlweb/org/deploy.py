...
# --- Config ---
PACKAGE_NAME = "nlweb-org-py-utils"   # project.name in pyproject.toml
IMPORT_CHECK  = "import nlweb.org.hello"
HELLO_PATHS = [
    pathlib.Path("src/nlweb/org/hello.py"),
...
def update_hello(new_version: str):
    ...
    if not hello_path:
        print("[warn] hello.py not found; skipping hello() string sync")
        return
    txt = read(hello_path)
...
def upload_testpypi():
    sh("twine", "upload", "--verbose", "--repository", "testpypi", "dist/*")
...
def pip_install_from_testpypi():
    sh(sys.executable, "-m", "pip", "install", "--force-reinstall",
       "-i", "https://test.pypi.org/simple/", PACKAGE_NAME)
...
def main():
    ...
    print("\nUpload complete!")
    print(f"Target version on TestPyPI: {new}")
    print("Next: install & test from TestPyPI in 10 seconds...\n")
