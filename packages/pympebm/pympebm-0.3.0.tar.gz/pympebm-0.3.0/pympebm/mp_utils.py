...
class PlackettLuce:
    ...
    def compute_certainty(self) -> float:
        ...
        for freq in count.values():
            p = freq / total 
            entropy += -p * np.log2(p)
        # Use number of unique rankings as maximum entropy
        n_effective = len(count)  # Number of unique rankings (N_effective)
        if n_effective <= 1:  # Avoid division by zero or log(1) = 0
            max_entropy = 1e-10  # Small value to handle edge case
            certainty = 1.0  # Maximum certainty if only one ranking
        else:
            max_entropy = np.log2(n_effective)
