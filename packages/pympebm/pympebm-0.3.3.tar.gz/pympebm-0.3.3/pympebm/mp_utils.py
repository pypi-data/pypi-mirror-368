...
def compute_conflict2(ordering_array: np.ndarray) -> float:
    ...
    for i in range(K-1):
        for j in range(i+1, K):
            ...
            # build rank maps
            rank1 = {ordering1[idx]: idx for idx in range(len(ordering1))}
            rank2 = {ordering2[idx]: idx for idx in range(len(ordering2))}
            
            # the index of each item in common items according to ordering1 and ordering2
            r1 = np.array([rank1[item] for item in common_items])
            r2 = np.array([rank2[item] for item in common_items])
            ...
            total += discordant/(concordant + discordant)
...
def rankings_to_matrix(rankings):
    ...
    benchmark_list = rankings[0]
    # indices that would sort benchmark_list
    argsort_indices = np.argsort(benchmark_list)

    # Step 2: Convert each ranking to integer ranks
    int_rankings = np.zeros((n_raters, n_items))
    for idx, ranking in enumerate(rankings):
        result = argsort_indices[np.searchsorted(benchmark_list[argsort_indices], ranking)]
        int_rankings[idx] = result
...
def get_unique_rows(partial_rankings):
    # Convert ragged object array to fixed-length padded array for uniqueness
    max_len = max(len(r) for r in partial_rankings)
    padded = np.full((len(partial_rankings), max_len), -1, dtype=int)
    for i, row in enumerate(partial_rankings):
        padded[i, :len(row)] = row
    _, idx = np.unique(padded, axis=0, return_index=True)
    return np.array([partial_rankings[i] for i in sorted(idx)], dtype=object)
...
def get_combined_order(
    ...
    unique_partial_rankings = np.array([])
    # to make sure no two partial orderings are the same
    while len(unique_partial_rankings) != n_partial_rankings:
...
class PlackettLuce:
    def __init__(self, ordering_array: np.ndarray, rng: np.random.Generator, sample_count:int=200):
        """
        ordering_array: list/array of partial rankings (integers, arbitrary IDs)
        rng: np.random.Generator
        """
        self.rng = rng 
        ...
        self.unique_elements = np.array(
            sorted(set(item for ordering in ordering_array for item in ordering)),
            dtype=np.int64
        )
        ...
        ordering_list = List()
        for ordering in ordering_array:
            ordering_list.append(np.array([self.elem2idx[x] for x in ordering], dtype=np.int64))
