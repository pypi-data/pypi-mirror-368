...
def create_triangle_templates():
    ...
    if not os.path.exists(triangle_black_path):
        ...
        # Draw black triangle: top-left to bottom-left to bottom-right
        triangle_points = [(0, 0), (0, height), (width, height)]
        draw.polygon(triangle_points, fill=(0, 0, 0, 180))  # Semi-transparent black
    ...
    if not os.path.exists(triangle_white_path):
        ...
        # Draw white triangle: top-left to bottom-left to bottom-right
        triangle_points = [(0, 0), (0, height), (width, height)]
        draw.polygon(triangle_points, fill=(255, 255, 255, 180))  # Semi-transparent white
...
class FinalThumbnailGenerator:
    ...
    def generate_final_thumbnail(self, 
                               ...
                               youtube_ready: bool = False) -> str:  # 是否优化为YouTube API兼容格式
        """生成最终版缩略图"""
        ...
        if youtube_ready:
            ...
            youtube_output_path = output_path.replace('.jpg', '_youtube_ready.jpg')
            youtube_optimized_path = optimize_for_youtube_api(output_path, youtube_output_path)
