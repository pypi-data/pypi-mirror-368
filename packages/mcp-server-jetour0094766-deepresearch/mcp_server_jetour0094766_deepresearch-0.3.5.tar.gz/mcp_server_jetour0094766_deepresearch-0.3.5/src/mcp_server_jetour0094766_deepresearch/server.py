...
def extract_json_from_response(content: str) -> dict:
    ...
    try:
        ...
        if "```json" in content:
            ...
            raise ValueError("JSON format data not found")
...
async def planning_node(state: ResearchState) -> Dict[str, Any]:
    """规划节点 - 生成搜索计划"""
    query = state["query"]
    logger.info(f"Start planning search: {query}")
    ...
    try:
        ...
        try:
            ...
            logger.warning("Using default search plan...")
...
async def search_node(state: ResearchState) -> Dict[str, Any]:
    ...
    logger.info(f"Start searching with {len(search_plan.searches)} items")
    
    async def perform_single_search(search_item: WebSearchItem, index: int) -> str:
        """执行单个搜索"""
        logger.info(f"  Search {index+1}/{len(search_plan.searches)}: {search_item.query}")
...
请提供2-3段文字的摘要，少于300字，捕捉核心要点。只返回摘要内容。"""
            ...
            logger.info(f"  Search {index+1} completed")
            return f"【{search_item.query}】\n{result_text}"
            
        except Exception as e:
            logger.error(f"  Search {index+1} failed: {e}")
            return f"【{search_item.query}】\n搜索失败: {str(e)}"
    ...
    for i, result in enumerate(search_results):
        if isinstance(result, str):
            ...
            logger.warning(f"  Search {i+1} exception: {result}")
    
    logger.info(f"Search completed with {len(valid_results)} valid results")
...
async def writing_node(state: ResearchState) -> Dict[str, Any]:
    ...
    logger.info(f"Start writing report based on {len(search_results)} search results")
...
请基于以上信息撰写详细的研究报告。"""
        ...
        try:
            ...
            logger.warning("Using original content as report...")
...
def run_server(api_key: str):
    ...
    try:
        create_llm_instance(api_key)
        logger.info("Default LLM instance created")
    except Exception as e:
        logger.warning(f"Default LLM instance creation failed: {e}")
...
async def run_test(api_key: str):
    ...
    print(f"Summary: {result.get('short_summary', '')}", file=sys.stderr)
    
    if result.get('success'):
        print("\n=== Full Report ===", file=sys.stderr)
        print(result.get('markdown_report', '')[:500] + "...", file=sys.stderr)
        
        print("\n=== Follow-up Research Suggestions ===", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.warning("Running event loop detected, using sync mode")
        main()
    except RuntimeError:
        # 没有事件循环，正常启动
        main()