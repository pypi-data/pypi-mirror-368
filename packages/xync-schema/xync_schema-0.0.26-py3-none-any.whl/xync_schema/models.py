...
class User(TgUser, TsTrait):
    ...
    receives: BackwardFKRelation["Transfer"]
    ...
    async def xync_balance(self) -> dict[int, Decimal]:
        xfs = await Fiat.filter(cred__pmcur__pm__norm="xyncpay", cred__person__user__id=self.id).prefetch_related(
            "cred__pmcur"
        )
        return {xf.cred.pmcur.cur_id: xf.amount for xf in xfs}
    ...
    async def send(self, amount: int, cur_id: int, receiver_id: int) -> "Transfer":
        ...
        async with in_transaction():
            ...
            sender_sig = sender_prv.sign(transfer.pack())
            transfer.proof = validator.proof_trans(transfer, sender_sig)
            await transfer.fetch_related("sender", "validator")
            if not transfer.check():
                raise ValueError("Transfer check failed")
            transfer.status = TransferStatus.valid
            await transfer.save(update_fields=["proof", "status"])
    ...
    def proof_trans(self, trans: "Transfer", sender_signature: bytes) -> bytes:
        assert trans.status == TransferStatus.transfer, "It is method for transfers only, not requests"
...
class Transfer(Model):
    ...
    validator_id: int
    status: TransferStatus = IntEnumField(TransferStatus)
    ...
    def is_expired(self) -> bool:
        ...
        assert self.request_deadline and not self.proof, "It is method for ttl requests only"
        return datetime.now(timezone.utc) > self.request_deadline
...
class Payment(Model):
    amount: Decimal = Dcm11Type
    order: ForeignKeyRelation[Order] = ForeignKeyField("models.Order", "taken_orders")
    order_id: int
    file: OneToOneNullableRelation["File"] = OneToOneField("models.File", related_name="payment", null=True)
    file_id: int
    accepted: bool | None = BooleanField(null=True)
...
class File(Model):
    ...
    payment: BackwardOneToOneRelation[Payment]
