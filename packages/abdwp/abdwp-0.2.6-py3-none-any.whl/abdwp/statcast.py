...
def download_seasons(start_year, end_year=None, force_download=False):
...
def load_season(year, force_download=False, regular_season=False):
    parquet_path = f"statcast/sc-{year}.parquet"
    if force_download or not os.path.exists(parquet_path):
        print(f"Downloading data for {year}.")
        download_seasons(year, year, force_download=force_download)
