...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (embedBtn) {
    embedBtn.addEventListener("click", () => {
      ...
      if (!modal || !checkbox || !linkInput || !embedTextarea) {
        console.warn('Embed functionality not available - missing required elements');
        return;
      }
...
export async function render_slides(slides) {
    ...
    if (!slides) {
        console.error('No slides data provided to render_slides');
        return;
    }
...
function add_component(id, data, outer_element) {
    ...
    if (data.type === 'Plotly') {
        ...
        element.dataset.plotlyData = data.figure; // Store data for lazy loading
        outer_element.appendChild(element);
    ...
    function loadPlotlyChart(element) {
      const figure = JSON.parse(element.dataset.plotlyData);
      const config = {
        responsive: true,
        scrollZoom: true,
        staticPlot: false
      ...
      element.observer = resizeObserver;
   ...
   // }
    ...
    function fullScreen() {
        ...
        function adjustFontSize() {
            ...
            const switchBtn = document.getElementById('switch-view-btn');
            if (switchBtn) {
              switchBtn.className = 'button-base';
            }
