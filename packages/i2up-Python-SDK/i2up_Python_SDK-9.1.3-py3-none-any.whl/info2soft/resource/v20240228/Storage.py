...
class Storage (object):
     ...
     * @return list
    '''
    def modifyStorageQuota(self, body, uuid, quota_uuid):
        if uuid is None:
            exit()
        url = '{0}/storage/{1}/quota/{2}'.format(config.get_default('default_api_host'), uuid, quota_uuid)
    ...
    '''
     * 磁带池 - 操作 - slot操作 重构catalog rebuild_catalog
     * 
     * @param dict $body  参数详见 API 手册
     * @return list
    '''
