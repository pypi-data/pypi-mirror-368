...
class CommonTemplate (object):
    ...
    def createCommonTemplate(self, body):
        ...
        url = '{0}/vers/v3/common_template/'.format(config.get_default('default_api_host'))
    ...
    def describeCommonTemplate(self, body, uuid):
        if uuid is None:
            exit()
        url = '{0}/vers/v3/common_template/{1}'.format(config.get_default('default_api_host'), uuid)
