...
from abstra_internals.jwt_auth import (USER_AUTH_COOKIE_KEY,
                                       USER_AUTH_HEADER_KEY, UserClaims)
from abstra_internals.repositories.project.project import (
    AccessSettings, ProjectRepository, Sidebar, StyleSettingsWithSidebar)
...
def default_auth_decoder(auth_header: str) -> Optional[UserClaims]:
    if auth_header.lower().startswith("bearer "):
        ...
        return UserClaims.from_jwt(auth_header)
...
class Guard:
    ...
    def should_allow(self,*, path: Optional[str] = None, id: Optional[str] = None, auth: Optional[str]) -> NavigationGuard:
        if path is not None:
            access_settings = ProjectRepository.load().get_access_control_by_stage_path(
                path
            )
        elif id is not None:
            access_settings = ProjectRepository.load().get_access_control_by_stage_id(id)
        else:
    ...
    def by(self, selector: Selector):
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                auth_header = flask.request.headers.get(USER_AUTH_HEADER_KEY)
                auth_cookie = flask.request.cookies.get(USER_AUTH_COOKIE_KEY)
