...
from abstra_internals.interface.sdk.tables.utils import (escape_ref,
                                                         quoted_identifier,
                                                         serialize)
...
def _dump():
    ...
    r = tables_api_http_client.dump()
    if not r.ok:
        raise Exception(f"Error dumping tables: {r.text}")
    return r.json()
...
def _make_count_query(
    ...
    *,
    ...
    return (
        f"SELECT COUNT(*) FROM {table} {where_exp}".strip(),
        values_list,
    )
...
def count(table: str, *, where: typing.Optional[dict] = None):
    query, params = _make_count_query(
        table=table,
        where=_make_row_dict(where),
    )
    return _run(query, params)[0]["count"]
