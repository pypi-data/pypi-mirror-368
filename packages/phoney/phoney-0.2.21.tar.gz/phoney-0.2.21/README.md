...
1) Download the latest delegated-*-extended-latest files from each RIR to `tools/data/`:
...
2) Run the builder to generate `ipv4_prefixes.<CC>.txt` and `ipv6_prefixes.<CC>.txt` files:
```powershell
python tools/build_prefixes.py
```
Alternatively, use the helper to fetch the datasets and build in one step:
```powershell
