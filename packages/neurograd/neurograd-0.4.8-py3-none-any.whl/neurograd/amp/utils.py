...
_FP16_SAFE_OPS: Set[str] = {
    'add', 'sub', 'mul', 'div', 'dot', 'matmul', 'conv2d', 'relu', 'gelu', 'tanh',
...
def maybe_cast_tensor(tensor, target_dtype=None, op_name: str = "unknown") -> 'Tensor':
    ...
    if target_dtype is None:
        op_name_lower = (op_name or "").lower()
        # For numerically sensitive ops, force execution in FP32
        if op_name_lower in _FP32_OPS:
            # Force FP32 for numerically sensitive ops regardless of input dtype
            target_dtype = xp.float32
        elif should_cast_to_fp16(op_name):
            target_dtype = autocast.get_autocast_dtype()
        else:
            # Keep in original precision for unknown ops
            return tensor
