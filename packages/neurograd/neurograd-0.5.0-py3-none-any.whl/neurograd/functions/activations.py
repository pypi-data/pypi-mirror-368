...
class Softmax(Function, Module):
    ...
    def backward(self, grad_output: xp.ndarray) -> xp.ndarray:
        # Softmax jacobian: s_i * (δ_ij - s_j) where δ_ij is Kronecker delta
        # Vectorized: softmax * (grad_output - (softmax * grad_output).sum(axis, keepdims=True))
        dot_product = xp.sum(self.softmax_x * grad_output, axis=self.axis, keepdims=True)
        return self.softmax_x * (grad_output - dot_product)
