...
class CategoricalCrossEntropy(Function, Module):
    ...
    def __init__(self, from_logits=False, epsilon=1e-7):
        ...
        self.from_logits = from_logits
        self.epsilon = epsilon
        self.y_true_data = None
        self.y_pred_softmax = None
        self.batch_size = None
    def forward(self, y_true: xp.ndarray, y_pred: xp.ndarray) -> xp.ndarray:
        # Store for backward pass
        self.batch_size = y_true.shape[0]   
        # Ensure loss computation is done in FP32 for numerical stability
        if y_true.dtype != xp.float32:
            y_true = y_true.astype(xp.float32)
        if y_pred.dtype != xp.float32:
            y_pred = y_pred.astype(xp.float32)
        self.y_true_data = y_true  
        if self.from_logits:
            # Apply softmax to logits
            y_pred_max = xp.max(y_pred, axis=-1, keepdims=True)
            exp_pred = xp.exp(y_pred - y_pred_max)
            exp_sum = xp.sum(exp_pred, axis=-1, keepdims=True)
            self.y_pred_softmax = exp_pred / exp_sum
            y_pred_final = self.y_pred_softmax
        else:
            # Clip predictions to prevent log(0)
            y_pred_final = xp.clip(y_pred, self.epsilon, 1.0 - self.epsilon)
            self.y_pred_softmax = y_pred_final
        # Compute cross entropy loss
        return -xp.mean(xp.sum(y_true * xp.log(y_pred_final), axis=-1))
    def backward(self, grad_output: xp.ndarray) -> Tuple[xp.ndarray, xp.ndarray]:
        if self.from_logits:
            # For softmax + cross entropy, gradient w.r.t. logits is: (softmax - true) / batch_size
            y_true_grad = None  # Usually y_true doesn't require gradients
            y_pred_grad = grad_output * (self.y_pred_softmax - self.y_true_data) / self.batch_size
        else:
            # Standard cross entropy gradients
            y_pred_clipped = xp.clip(self.y_pred_softmax, self.epsilon, 1.0)
            y_true_grad = -grad_output * xp.log(y_pred_clipped) / self.batch_size
            y_pred_grad = -grad_output * self.y_true_data / (y_pred_clipped * self.batch_size)
        return y_true_grad, y_pred_grad
