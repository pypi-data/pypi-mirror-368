...
class AvroAssetPlugin(AssetPluginAbstract):
    def source_type(self) -> SourceType:
        return SourceType.avro
    ...
    def click_options_keys(self) -> set[str]:
        return set(AvroConfig.__annotations__.keys())
    ...
    def extract_assets(
        ...
        typed_config = AvroConfig(**config)
        ...
        for file_path in files_list:
            try:
                ...
                raise click.ClickException(
                    f"{file_path}: Error parsing Avro file, or resolving local references: {exc}"
                ) from exc
            source_names.append(
        ...
        for source_name, schema_content_raw in zip(source_names, schema_contents_raw):
            schema_recap_types = AvroConverter().to_recap(schema_content_raw)
