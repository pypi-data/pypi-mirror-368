...
logging.getLogger('boto3').setLevel(logging.DEBUG)  # <-- Add this import
...
READ_CHUNK_SIZE = 100 * 1024 * 1024  # 8 MB
...
class S3StorageProviderBackend(StorageProvider):
    def multipart_upload(self, file_path, bucket, key, extra_args=None, chunk_size=READ_CHUNK_SIZE):
        ...
        # Use single-part upload for files < 5GB to avoid aws-chunked encoding
        ...
        try:
            with open(file_path, "rb") as f:
                ...
                while True:
                    ...
                    if not data:
                        break
                    logger.debug(f"Uploading part {part_number}, size {len(data)} bytes")
                    ...
                    logger.debug(f"Uploaded part {part_number}, ETag {part['ETag']}")
            ...
            logger.info("Multipart upload complete for %s", key)
        except Exception as e:
            logger.error("Multipart upload failed for %s: %s", key, str(e))
