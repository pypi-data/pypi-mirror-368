  ...
  for proc in psutil.process_iter():
    try:
      if process_name in proc.name():
        # Send SIGINT to all child processes first, then to the parent.
        for child in proc.children(recursive=True):
          child.send_signal(signal.SIGINT)
          print(f"Sent SIGINT to {child.name}")
        proc.send_signal(signal.SIGINT)
        print(f"Sent SIGINT to {proc.name}")
    except psutil.NoSuchProcess:
      print("Process not found!")


def stop_process_by_pid(process_pid: int) -> None:
  """Stops a process with a given name."""
  ...
  try:
    ...
    for child in proc.children(recursive=True):
      child.send_signal(signal.SIGINT)
      print(f"Sent SIGINT to child process: {child.name()} (PID: {child.pid})")
    proc.send_signal(signal.SIGINT)
    print(f"Sent SIGINT to process: {proc.name()} (PID: {proc.pid})")
  except psutil.NoSuchProcess:
...
class ProcessLauncher:
  ...
  class ProcessParams:
    ...
    def stop_process(self) -> None:
      """Stops the process."""
      if self.popen_process is not None:
        stop_process_by_pid(self.popen_process.pid)
      else:
