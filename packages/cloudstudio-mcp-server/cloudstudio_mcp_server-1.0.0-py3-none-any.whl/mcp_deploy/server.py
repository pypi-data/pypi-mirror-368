...
def create_workspace(title: str) -> dict:
    """在工作空间中执行命令（含服务启动与预览链接生成）
        ...
        3. **生成完整预览链接**：
           - 从 `create_workspace` 返回结果中获取 `lite_app_id (如 `23728372`) 和 `workspace_key`（如 `c148150324034f2580d775a9c0057b52`）和 `region`（如 `ap-shanghai`）。
           - 按模板 `https://{workspace_key}--{实际端口}.{region}.cloudstudio.club` 替换 `{实际端口}`（如 `3000`），生成可直接访问的链接。
