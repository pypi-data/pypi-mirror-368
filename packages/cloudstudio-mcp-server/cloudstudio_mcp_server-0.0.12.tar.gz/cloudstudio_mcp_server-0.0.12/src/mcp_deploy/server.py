...
def create_workspace(title: str) -> dict:
    ...
    功能说明:
    - 创建一个全新的Cloud Studio工作空间实例，`title` 参数将直接作为工作空间的名称
    - 使用环境变量中的API_TOKEN进行认证
    - 返回包含工作空间ID、访问地址等信息的字典，其中 `title` 字段应与输入的 `title` 参数一致
    ...
    注意事项:
    1. `title` 参数为必填项，不可为空或仅含空格，否则可能导致创建失败或名称为空
    2. 确保 `title` 参数的值正确传递，返回结果中的 `title` 字段应与输入完全一致，若不一致需检查参数传递是否有误
    3. 需要预先设置API_TOKEN环境变量，否则会因认证失败创建失败
    4. 每个API_TOKEN有创建频率限制，过于频繁调用会被限流
    5. 预览链接使用说明:
       - 格式为'https://{workspace_key}--{port}.{region}.cloudstudio.club'
       - 替换{port}为实际服务端口（如8000）即可访问对应服务

    典型响应示例(正确传递title时):
    {
        "workspace_key": "kmhhvqnlogr0il1pyvc48",
        "lite_app_id": "1234566",
        "title": "Demo Workspace",  # 与输入的title一致
        "webIDE": "https://kmhhvqnlogr0il1pyvc48--ide.ap-shanghai.cloudstudio.club",
        "preview": "https://kmhhvqnlogr0il1pyvc48--{port}.ap-shanghai.cloudstudio.club",
        "edit_url": "https://cloudstudio.net/a/1234566/edit"
    }
    """
