...
from .models import Connections, WorkspaceStatus, RuntimePool, Runtime, VGPUConfig, GPUConfig, RuntimeSpec, Storage, CBDStorage, WorkspaceResponse, WorkspaceResponseData, WorkspaceRequest, CommandInput, CommandOutput
...
def createLiteapp(
    ...
    workspace_response = WorkspaceResponse(
        ...
        data=WorkspaceResponseData(
            ...
            runtime=Runtime(**response_data['data']['workspace']['runtime']),
            runtimeSpec=RuntimeSpec(**response_data['data']['workspace']['runtimeSpec']),
            storage=Storage(
                cbd=CBDStorage(
                    id=response_data['data']['workspace']['storage']['diskId'],
                    quota=response_data['data']['workspace']['storage']['quota']
                ),
                type="cbd"
            ),
            status=WorkspaceStatus(**response_data['data']['workspace']['status'])
        )
    )
...
def uploadfile(api_token:str, workspace_key: str, region: str, files: list[File], directory: Optional[str] = None) -> str:
