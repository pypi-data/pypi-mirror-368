...
class ClassificationAnalyzer:
    ...
    def compare_models(self, test_train=True, test_size=0.25):
        ...
        Parameters:
        - test_train: Whether to use train-test split (default True)
        - test_size: Test set proportion when test_train=True
        
        Returns:
        - Dictionary containing comparison results and fitted models
        """
        # Ensure y is 1D
        y_encoded = self.y_encoded.ravel()
        
        # Split or use full data
        if test_train:
            ...
            X_train, y_train = self.X, y_encoded
            X_test, y_test = self.X, y_encoded
        ...
        # Train and evaluate with progress bar
        ...
        sort_column = 'F1' if is_binary else 'Balanced Accuracy' if 'Balanced Accuracy' in self.results.columns else 'Accuracy'
        self.results = self.results.sort_values(sort_column, ascending=False)
        
        # Store best model
        self.best_model = self.models[self.results.iloc[0]['Model']]
