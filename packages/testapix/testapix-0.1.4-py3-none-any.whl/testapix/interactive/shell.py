...
if TYPE_CHECKING:
    ...
    from testapix.core.client import APIClient, EnhancedResponse
else:
    try:
        from prompt_toolkit import PromptSession
        from prompt_toolkit.completion import WordCompleter
...
class InteractiveShell:
    ...
    async def _load_config(self, config_path: str) -> None:
        ...
        try:
            ...
            if base_url:
                self.client = APIClient(base_url=base_url, auth_provider=auth_provider)
                self.session_manager.current_session["base_url"] = base_url
    ...
    async def _recreate_client_with_auth(
        ...
        """Recreate APIClient with authentication"""
        if not self.client:
            print("⚠️  No API Client to update")
            return
        ...
        print("🔄 API Client updated with authentication")
