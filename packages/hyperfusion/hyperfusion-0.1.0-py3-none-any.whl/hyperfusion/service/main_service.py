...
class MainService:
    ...
    async def handle_execute_function(self, msg: ExecuteFunctionRequest):
        ...
        self.logger.info(f"Executing function: {exec_msg.name} with UUID: {exec_msg.uuid}")
        ...
        try:
            ...
            # Deserialize input
            input_batch = deserialize_record_batch(exec_msg.arrow_record_batch)
            input_table = pa.Table.from_batches([input_batch])

            self.logger.info(f"start {exec_msg.name}")
            
            # Execute the function
            output_table, error_table = await self.registry.execute(exec_msg.name, input_table)

            self.logger.info(f"done {exec_msg.name}")
            ...
            self.logger.info(f"Successfully executed function: {exec_msg.name}")
