...
def flux_spectra_xz_av(var, geom):
    ...
    if geom.cm.x_local:
        if geom.cm.y_local:
            temp = np.empty((geom.cm.pnt.nky0, geom.cm.pnt.nz0))
            temp[0, :] = np.real_if_close(np.real(np.sum(var[:, 0, :], axis=0)))
            temp[1:, :] = 2*np.real_if_close(np.real(np.sum(var[:, 1:, :], axis=0)))
...
def get_area(geom):
    ...
    if geom.cm.x_local:
        ...
        if geom.cm.y_local:
            ...
            area_fs = np.zeros((geom.cm.pnt.nx0))
            for z in range(0, geom.cm.pnt.nz0):
                for y in range(0, geom.cm.pnt.ny0):
                    area_fs = area_fs + np.sqrt(geom.gxx.T[:, y, z])*geom.jacobian.T[:, y, z]
            return area_fs*(2*np.pi) ** 2*abs(geom.Cy)/geom.cm.pnt.ny0/geom.cm.pnt.nz0
