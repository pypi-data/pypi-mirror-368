...
pool = JCPool( num_jobs=4, verbose=Context.all )
...
class _ParallelContextOperator( object ):
    ...
    def __init__(self, pool_verbose     : Context,      # context to print Pool progress to (in thread)
        ...
        with pool_verbose.write_t(f"Launching messaging queue '{cid}' using thread '{tid}'... ", end='') as tme:
            ...
            self._mp_context   = Context( f_verbose, channel=ParallelContextChannel( cid=self._cid, queue=self._queue, maintid=self._tid ) )
            self._thread.start()
...
class JCPool( object ):
    ...
    def __init__(self, num_jobs         : int = 1,
        ...
        assert num_jobs > 0, ("'num_jobs' must be positive", num_jobs)
        
        with self._verbose.write_t(f"Launching {num_jobs} processes with temporary path '{self.tmp_path}'... ", end='') as tme:
            self._pool = joblib_Parallel(n_jobs=num_jobs, backend="loky", return_as="generator_unordered", temp_folder=self.tmp_path, **parallel_kwargs)
    ...
    def parallel_to_dict(self, jobs : Mapping) -> Mapping:
        ...
        """
        assert isinstance(jobs, Mapping), ("'jobs' must be a Mapping")
    ...
    def parallel_to_list(self, jobs : Sequence ) -> Sequence:
        ...
        """
        assert not isinstance( jobs, Mapping ), ("'jobs' is a Mapping. Use parallel_to_dict() instead.")
