...
def draw(points, labels, width="50%", color='blue', size=1, shape='square', pick_radius=15, pick_limit=20, html_labels=False):
    # normalize X
    ...
    js_canvas = '''<div id="result:''' + rnd_id + '''" style="position:fixed;pointer-events:none;background:rgba(255, 255, 255, .5);display:none;"></div><canvas id="canvas:''' + rnd_id + '''" style="border:1px solid black; width:''' + str(width) + '''" width="128" height="128"></canvas>'''
...
canvas.onmousemove = function(e) {
    var rect = canvas.getBoundingClientRect();
    var x = (event.clientX - rect.left) / (width / ratio), y = (event.clientY - rect.top) / (height / ratio);
    var found = index.within(x, y, ''' + str(pick_radius) + ''' * ratio / width);
    var distances = {};
    for(i of found) {
    ...
    result.style.left = '' + (event.clientX + 15) + 'px';
