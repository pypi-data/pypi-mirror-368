...
class ResourceInfo(BaseSchema):
    ...
    def _check_count(self):  # python 3.10 incompatible:  -> Self:
        if self.acquired - self.used != self.available:
            raise ValueError(f"acquired - used != available ({self.acquired} - {self.used} != {self.available})")
...
def _matches_cuda_version_full(cuda_version_major: int | None, info: ValidationInfo):
    cuda_version_full = info.data.get("cuda_version_full")
    if cuda_version_major and cuda_version_full:
        assert cuda_version_major == _extract_major_cuda_version(
            cuda_version_full
...
class SlaveInfo2(SlaveInstanceBase):
    ...
    def has_storage(self, storage_path: str):
        ...
        return self.storage_aliases_available and any(
            storage_path.startswith(add_end_slash_ifneeded(sp)) for sp in self.storage_aliases_available
        )
    ...
    def allows_storage(  # noqa: C901, PLR0912
        ...
        project_urn: str,
        *,
        cluster_projects_allowed: list[str] = None,  # pyright: ignore [reportArgumentType]
    ):
    ...
    def sanitized_copy(self, logged_in=True) -> "SlaveInfo2":
        """Remove statistics for regular users."""

        sanitization_updates = {
            "statistics": None,
        }
        if not logged_in:
...
class ClusterInfo(BaseSchema):
    ...
    storage_paths_available: list[str] | None = None
    # A dict of storage aliases available on the cluster. (optional, where None means "unknown")
    storage_aliases_available: dict | None = None
