...
def validate_job_id(v: str):
    """Validate Job ID."""
    if not is_valid_uuid(v):
        raise ValueError("Not a valid Job ID.")
    return v
...
def before_validate_job_id(v: Any) -> str:
    ...
    """
    if v is None:
        raise ValueError("Job ID may not be None.")
...
def validate_reservation_id(v: str):
    """Validate Reservation ID."""
    if not is_valid_uuid(v):
        raise ValueError("Not a valid reservation ID.")
    return v
...
class JobPortMapping(BaseSchema):
    ...
    @classmethod
    def from_docker_dict(cls, d: dict, *, container_port: int | str | None = None) -> "JobPortMapping":
...
class JobEventTimes(BaseSchema):
    ...
    def sanitized_copy(self, _logged_in=True, _same_project=False) -> "JobEventTimes":
        """Get anonymized copy."""
        # all may be known
...
class JobStateResources(BaseSchema):
    ...
    def _validate_tmpfs_mem_gb(cls, v: Any) -> int:
        ...
        if not isinstance(v, int):
            raise TypeError("tmpfs_mem_gb must be an int")
        return v
    ...
    def sanitized_copy(self, logged_in=True, _same_project=False) -> "JobStateResources":
        """Anonymized copy."""
...
class JobRequestExtra(BaseSchema):
    ...
    def sanitized_copy(self, _logged_in=True, _same_project=False) -> "JobRequestExtra":
        """Anonymized copy."""
        # hide all
...
class JobRequestSchedulingBase(BaseSchema):
    ...
    @classmethod
    def _validate_max_duration(cls, max_duration: str | None):
        if max_duration:
            ...
            if max_dur_s < MINIMUM_MAX_DURATION_SECONDS:
                raise ValueError(f'max_duration must be at least 1 minute, not "{max_duration}"')
    ...
    @classmethod
    def _check_combination_with_interactive(cls, v: datetime.datetime | None, info: ValidationInfo):
        if v and info.data["interactive"]:
            raise ValueError(f"{info.field_name} cannot be combined with interactive")
        return v

    @field_validator("not_after")
    @classmethod
    def _check_order(cls, not_after: datetime.datetime | None, info: ValidationInfo):
        not_before = info.data.get("not_before")
        if not_after and not_before and not_before >= not_after:
            raise ValueError(f"not_before ({not_before}) must be before not_after ({not_after})")
        return not_after
...
class JobRequestScheduling(JobRequestSchedulingBase):
    ...
    def _validate_max_duration(cls, max_duration: str | None):
        if max_duration is None:
            raise ValueError("max_duration is mandatory")
        max_dur_s = duration_string_to_seconds(max_duration)
        assert max_dur_s  # already tested in the previous validator
        if max_dur_s < MINIMUM_MAX_DURATION_SECONDS:
            raise ValueError(f'max_duration must be at least 1 minute, not "{max_duration}"')
    ...
    def max_duration_s(self) -> int:
        ...
        if res is None:
            ...
            raise ValueError("max_duration is invalid")
        return res
    ...
    def min_duration_s(self) -> int:
        ...
        if res is None:
            ...
            raise ValueError("min_duration is invalid")
        return res
    ...
    def get_min_duration_delta(self, *, default_s: int | None) -> datetime.timedelta:
        ...
        if self.min_duration is None:
            ...
            if default_s is None:
                raise ValueError("None not allowed for default_s")
    ...
    def sanitized_copy(self, _logged_in=True, _same_project=False):  # python 3.10 incompatible:  -> Self:
        """Anonymized copy."""
...
class JobStateScheduling(BaseSchema):
    ...
    def sanitized_copy(self, _logged_in=True, _same_project=False) -> "JobStateScheduling":
        """Anonymized copy."""
        # Nothing to hide
        return self.model_copy()


def _allow_single_value(v: Any):
    if isinstance(v, str):
        return [v]
    return v
...
class JobRequestResources(BaseSchema):
    ...
    def sanitized_copy(self, _logged_in=True, _same_project=False) -> "JobRequestResources":
...
class JobStorage(BaseSchema):
    ...
    def check_either_container_path_or_host_path(cls, data: Any) -> Any:
        ...
        if (
            ...
            raise ValueError("either containerPath or hostPath must be specified")
        return data

    @model_validator(mode="after")
    def _validate_ssh(self):  # python 3.10 incompatible:  -> Self:
    ...
    @model_validator(mode="after")
    def _check_has_data(self):  # python 3.10 incompatible:  -> Self:
        if not self.container_path and not self.host_path:
            raise ValueError("need at least one of hostPath or containerPath")
    ...
    @classmethod
    def from_string(cls, s: str):  # python 3.10 incompatible:  -> Self:
...
def _convert_int_portmapping(v: Any):
    if isinstance(v, int):
        return {"containerPort": v}
    return v


def _convert_str_storage(v: Any):
    if isinstance(v, str):
        return {"hostPath": v}
    return v
...
class JobRequest(JobRequestBase):
    ...
    def sanitized_copy(self, logged_in=True, same_project=False) -> "JobRequest":
        ...
        return JobRequest(
            ...
            scheduling=self.scheduling.sanitized_copy(logged_in, same_project),  # pyright: ignore [reportArgumentType]  this is OK as model_copy does the right thing. Not sure how to express this in type so that pyright gets it.
...
PROJECT_URN_PATTERN = (
    r"urn:publicid:IDN\+(?P<authority>[^+:]+)(:(?P<subauthority>[^+]+))?\+project\+(?P<subject>[^+]+)$"  # noqa: E501
)
...
class JobOwner(JobOwnerBase):
    ...
    def userurn_auth(self) -> str:
        ...
        if not res:
            raise ValueError("Invalid User URN")
        return res
...
class JobBase(BaseSchema):
    ...
    @model_validator(mode="after")
    def _init_restart_info(self):  # python 3.10 incompatible:  -> Self:
    ...
    def replace_event_times_attrs(self, **kwargs):  # python 3.10 incompatible:  -> Self:
    ...
    def replace_request_resources_attrs(self, **kwargs):  # python 3.10 incompatible:  -> Self:
    ...
    def replace_request_scheduling_attrs(self, **kwargs):  # python 3.10 incompatible:  -> Self:
    ...
    def replace_request_extra_attrs(self, **kwargs):  # python 3.10 incompatible:  -> Self:
    ...
    def replace_state_scheduling(self, new_scheduling: JobStateScheduling):  # python 3.10 incompatible:  -> Self:
    ...
    def replace_state_scheduling_attrs(self, **kwargs):  # python 3.10 incompatible:  -> Self:
    ...
    def add_halt_info(self, halt_info: HaltInfo):  # python 3.10 incompatible:  -> Self:
        """Add halt info."""
        return self.replace_state_scheduling_attrs(halt_events=self.state.scheduling.halt_events + [halt_info])

    def replace_state_final_usage_statistics(
        self, final_usage_statistics: GPULabUsageStatistics
    ):  # python 3.10 incompatible:  -> Self:
    ...
    def replace_state_waste_review(self, waste_review: WasteReview | None):  # python 3.10 incompatible:  -> Self:
    ...
    def replace_state_resources(self, new_resources: JobStateResources):  # python 3.10 incompatible:  -> Self:
    ...
    def replace_state_resources_fields(self, **kwargs):  # python 3.10 incompatible:  -> Self:
...
class Job(JobBase):
    ...
    def replace_owner_attrs(self, **kwargs):  # python 3.10 incompatible:  -> Self:
        """Update owner in job."""
        return self.model_copy(
            update={"owner": self.owner.model_copy(update=kwargs) if self.owner else JobOwner(**kwargs)}
        )

    def sanitized_copy(self, logged_in=True, same_project=False):  # python 3.10 incompatible:  -> Self:
        """Copy with removed confidential data.
...
class JobV5(JobBase):
    ...
    def sanitized_copy(self, logged_in=True, same_project=False) -> "JobV5External":
        """Copy with removed confidential data.
