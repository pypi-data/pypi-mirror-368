...
class DockerImageName:
    ...
    def _username_and_password(self):  # python 3.10 incompatible:  -> Self:
        if not (self.user is None) == (self.password is None):
            raise ValueError("Both username and password are required, or neither can be present.")
        return self
    ...
    def from_str(cls, image: str) -> "DockerImageName":
        ...
        if not match:
            raise ValueError("Not a valid docker image name.")
