...
def _validate_allowed_states(allowed_states: list[JobStatus], info: ValidationInfo):
    if len(allowed_states):
        for state in ["pending", "finished", "running", "deleted"]:
            if info.data.get(f"{state}_state"):
                raise ValueError(f"Cannot combine {state} and allowed_states filters.")
...
def _serialize_allowed_states(allowed_states: list[JobStatus], _info) -> str:
    ...
    if isinstance(value, list):
        for item in value:
            if not isinstance(item, int):
                raise TypeError(f"{item} in list is not an int")
        return value
    ...
    if isinstance(value, list):
        for item in value:
            if not isinstance(item, str):
                raise TypeError(f"{item} in list is not a str")
        return value
