...
def generator():
    generator = SnowflakeGenerator(worker_id=1, data_center_id=1)
    ...
    for dtype in int_types:
        try:
            id_val = generator.get_id(dtype)
            print(f"{dtype:20} -> {id_val:20} (类型: {type(id_val).__name__})")
        except Exception as e:
            print(f"{dtype:20} 生成失败: {e}")
    ...
    string_formats = ['mixed', 'upper', 'lower', 'numeric']
    for dtype in string_types:
        for fmt in string_formats:
            try:
                id_val = generator.get_id(dtype, string_format=fmt)
                print(f"{dtype:15} 格式:{fmt:7} -> {id_val} (长度: {len(id_val)})")
            except Exception as e:
                print(f"{dtype:15} 格式:{fmt:7} 生成失败: {e}")
