...
small_types = ['int','integer','tinyint','smallint', 'mediumint']
...
class BaseModel(Base):
    ...
    def create(cls, **kwargs):
        ...
        if cls._should_generate_id(kwargs):
            ...
            if pk_col.name not in kwargs:
                if obj_type in small_types:
                    raise ValueError("创建记录失败: 'int','integer','tinyint','smallint', 'mediumint'类型的主键不支持自动生成主键值，数值类型的主键请使用BIGINT或自行传入主键")
        ...
        try:
            ...
            if not exists:
                raise RuntimeError(f"记录提交成功但未在数据库中找到，主键: {pk_name}={pk_value}")
    ...
    def bulk_create(cls, data_list):
        ...
        for data in data_list:
            ...
            if pk_col.name not in data.keys() and obj_type in small_types:
                unsupported = True
