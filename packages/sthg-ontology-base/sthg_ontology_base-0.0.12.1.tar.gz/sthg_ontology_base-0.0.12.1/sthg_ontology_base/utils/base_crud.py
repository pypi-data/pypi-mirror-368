...
auto_commit_flag = contextvars.ContextVar("auto_commit_flag", default="True")
...
class BaseModel(Base):
    ...
    @classmethod
    def _auto_rollback(cls):
        if auto_commit_flag.get() == "True":
            db = cls.get_session()
            db.rollback()

    @classmethod
    def _auto_commit(cls, **kwargs):
        if auto_commit_flag.get() == "True":
            obj = cls(**kwargs)
            db = cls.get_session()
            db.commit()
