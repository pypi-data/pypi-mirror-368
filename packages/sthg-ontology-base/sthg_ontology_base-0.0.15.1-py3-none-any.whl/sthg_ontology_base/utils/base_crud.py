...
class BaseModel(Base):
    ...
    def create(cls, **kwargs):
        ...
        try:
            ...
            # 手动查询确认记录已存在
            exists = db.query(cls).filter_by(**{pk_name: pk_value}).first()
            if not exists:
                raise RuntimeError(f"记录提交成功但未在数据库中找到，主键: {pk_name}={pk_value},或请检查主键值是否在主键类型支持范围内")
    ...
    def bulk_create(cls, data_list):
        ...
        pk_col = cls.get_primary_key()
        pk_name = pk_col.name
        unsupported = False  # 修正变量名拼写
        obj_type = str(pk_col.type.__visit_name__).lower()  # 移到循环外，只计算一次

        # 检查是否有不支持自动生成主键的情况
        for data in data_list:
            # 当主键不存在且类型为int时，标记为不支持
            if pk_name not in data.keys() and obj_type in small_types:
                unsupported = True
                break  # 发现一个即跳出循环，提高效率
        ...
        try:
            ...
            if len(existing_objects) != len(inserted_pks):
                # 找出缺失的主键
                existing_pks = {getattr(obj, pk_name) for obj in existing_objects}
                missing_pks = [pk for pk in inserted_pks if pk not in existing_pks]
                raise RuntimeError(f"部分记录插入失败，缺失主键: {missing_pks},或请检查主键值是否在主键类型支持范围内")
...
class JoinedQuerySet:
    ...
    def __init__(self, models, on_fields_list=None, on_list=None, join_types=None):
        ...
        for m in models:
            ...
            columns = [
                {
                    'name': col.name,
                    'full_name': f"{table_name}_{col.name}"
                }
    ...
    def _resolve_column(self, col):
        ...
        if isinstance(col, str):
            # 支持"表别名_字段名"格式的字符串解析
            if '_' in col:
                alias_part, field_part = col.split('_', 1)
        ...
        for i, (obj, alias) in enumerate(zip(row, self.model_aliases)):
            ...
            if obj is None:
                ...
                for col_info in columns_info:
                    result_key = f"{alias}_{col_info['name']}"
                    result[result_key] = None
            else:
                # 处理有数据的情况
                for col_info in columns_info:
                    col_name = col_info['name']
                    result_key = f"{alias}_{col_name}"
                    # 获取字段值
    ...
    def get_original_field_name(self, aliased_field: str) -> tuple:
        ...
        """
        if '_' not in aliased_field:
            raise ValueError(f"字段名 {aliased_field} 不是'别名_字段名'格式")

        alias_part, field_part = aliased_field.split('_', 1)
