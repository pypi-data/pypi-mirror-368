    ...
    def validate_url(
        ...
        try:
            ...
            if not parsed.scheme:
                raise ValidationError("URL deve ter esquema (http/https)", url)

            if parsed.scheme.lower() not in cls.ALLOWED_SCHEMES:
                raise ValidationError(f"Esquema não permitido: {parsed.scheme}", url)

            # Verificar netloc
            if not parsed.netloc:
                raise ValidationError("URL deve ter domínio válido", url)

            # Verificar extensões bloqueadas
            path = parsed.path.lower()
            for ext in cls.BLOCKED_EXTENSIONS:
                if path.endswith(ext):
                    raise ValidationError(f"Extensão não suportada: {ext}", url)
            ...
            for blocked in blocked_domains:
                if blocked in domain:
                    raise ValidationError(f"Domínio bloqueado: {blocked}", url)
            ...
            raise ValidationError(f"URL inválida: {str(e)}", url) from e
    ...
    def _check_private_ip(cls, netloc: str):
        ...
        try:
            ...
            if ip.is_private or ip.is_loopback:
                raise ValidationError(f"IP privado/loopback não permitido: {host}")
