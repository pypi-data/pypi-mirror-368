...
from .task_renderer import TaskTableRenderer
...
class MetricsCollector:
    ...
    def _get_gpu_multiplier(self, instance_type: Optional[str]) -> float:
...
class LiveDashboard:
    ...
    def _render_header(self) -> Panel:
        ...
        if not self.state.paused:
            ...
            live_indicator = Text("● PAUSED", style="yellow")

        # Status line
        status_parts = []
        if self.state.filter_status:
            status_parts.append(f"Filter: {self.state.filter_status}")
        status_parts.append(f"Sort: {self.state.sort_by}")
        status_parts.append(f"Updated: {self.state.last_update.strftime('%H:%M:%S')}")

        status_line = Text(" | ".join(status_parts), style=theme_manager.get_color("muted"))
    ...
    def _render_tasks(self) -> Panel:
        ...
        # Create table
        table = self.task_renderer._create_professional_table(
            None,
            layout,
        ...
        # Add columns
        self.task_renderer._add_responsive_columns(table, layout)

        # Add rows with selection highlight
        for idx, task in enumerate(self.state.tasks[:20]):  # Limit to 20 for performance
            row_data = self.task_renderer._build_row_data(task, layout)
        ...
        while not self._stop_event.is_set():
            if not self.state.paused:
                try:
                    ...
                    tasks = self.task_presenter._fetch_tasks(display_options)
    ...
    def run(self):
        ...
        try:
            ...
            self.console.print(f"[red]Error:[/red] Cannot configure terminal: {escape(str(e))}")
            return
