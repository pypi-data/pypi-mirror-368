...
Parses index sets like :1, :1-3, :1-3,5,7, and also accepts forms
without the leading colon (e.g., 1, 1-3, 1-3,5,7).
"""
...
class Selection:
    ...
    def parse(text: str) -> "Selection":
        ...
        Supports both leading-colon and bare-number forms. Examples:
    ...
    def to_task_ids(self, index_to_task_id: Dict[str, str]) -> Tuple[List[str], List[str]]:
        ...
        if not index_to_task_id:
            return task_ids, [
                "No recent task list. Run 'flow status' to refresh indices"
            ]

        # Determine max index for helpful error messages
        try:
            available_indices = [int(k) for k in index_to_task_id.keys() if str(k).isdigit()]
