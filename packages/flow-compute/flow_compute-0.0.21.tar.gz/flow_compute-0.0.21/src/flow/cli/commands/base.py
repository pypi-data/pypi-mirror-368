...
class BaseCommand(ABC):
    ...
    def show_next_actions(self, recommendations: list) -> None:
        ...
        # Transform bracketed commands into hyperlinks when supported
        processed: list[str] = []
        for rec in recommendations:
            try:
                # Simple pattern: turn 'flow xyz' substrings into hyperlinks
                if hyperlink_support.is_supported():
                    parts = []
                    for token in str(rec).split():
                        if token == "flow":
                            parts.append("[accent]flow[/accent]")
                        else:
                            parts.append(token)
                    rec = " ".join(parts)
            except Exception:
                pass
            processed.append(f"  • {rec}")

        body = "\n".join(processed)
        panel = Panel(body, title="[bold]Next steps[/bold]", border_style=theme_manager.get_color("muted"))
        console.print("\n")
        console.print(panel)
