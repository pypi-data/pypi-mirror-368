...
This module provides the central submission interface that:
1. Accepts input from various frontends (SLURM, Submitit, YAML, SDK)
2. Uses frontend adapters to parse into common TaskConfig
...
_submission_service: Optional[JobSubmissionService] = None


def get_submission_service(task_manager: Optional[TaskManager] = None) -> JobSubmissionService:
