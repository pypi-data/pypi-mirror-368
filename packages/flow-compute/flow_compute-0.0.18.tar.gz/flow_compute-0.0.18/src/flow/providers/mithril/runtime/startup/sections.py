...
class HeaderSection(ScriptSection):
    ...
    def generate(self, context: ScriptContext) -> str:
        from .origin import get_flow_origin_header
...
class PortForwardingSection(ScriptSection):
    ...
    def generate(self, context: ScriptContext) -> str:
        ...
        return textwrap.dedent(
            ...
            if command -v systemctl >/dev/null 2>&1; then
                ...
                echo "systemctl not found; attempting to start nginx directly"
    ...
    def _generate_foundrypf_service(self) -> str:
        ...
        return textwrap.dedent(
            """
            # Create systemd service for foundrypf if binary exists
            if command -v foundrypf >/dev/null 2>&1 || [ -x /usr/local/bin/foundrypf ]; then
                if command -v systemctl >/dev/null 2>&1; then
                    ...
                    echo "systemctl not found; skipping foundrypf service setup"
                fi
            else
                echo "foundrypf binary not found; skipping service creation"
            fi
        """
        ).strip()
        ...
        if context.has_gpu:
            gpu_setup = textwrap.dedent(
                ...
                if command -v apt-get >/dev/null 2>&1; then
                    ...
                    # Use keyrings instead of deprecated apt-key
                    mkdir -p /etc/apt/keyrings
                    curl -fsSL https://nvidia.github.io/nvidia-docker/gpgkey | gpg --dearmor -o /etc/apt/keyrings/nvidia-docker.gpg || true
                    curl -fsSL https://nvidia.github.io/nvidia-docker/$distribution/nvidia-docker.list | sed 's#deb \\[arch=.*\\] #deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/nvidia-docker.gpg] #' | tee /etc/apt/sources.list.d/nvidia-docker.list >/dev/null
                    apt-get update -qq && apt-get install -y -qq nvidia-container-toolkit || true
                elif command -v dnf >/dev/null 2>&1; then
                    dnf -y config-manager --add-repo https://nvidia.github.io/libnvidia-container/stable/rpm/nvidia-container-toolkit.repo || true
                    dnf -y install nvidia-container-toolkit || true
                elif command -v yum >/dev/null 2>&1; then
                    yum-config-manager --add-repo https://nvidia.github.io/libnvidia-container/stable/rpm/nvidia-container-toolkit.repo || true
                    yum -y install nvidia-container-toolkit || true
                elif command -v zypper >/dev/null 2>&1; then
                    zypper -n addrepo https://nvidia.github.io/libnvidia-container/stable/rpm nvidia-container || true
                    zypper -n install nvidia-container-toolkit || true
                else
                    echo "WARNING: Unsupported package manager for NVIDIA Container Toolkit installation"
                fi
                if command -v systemctl >/dev/null 2>&1; then
...
class GPUdHealthSection(ScriptSection):
    ...
    def should_include(self, context: ScriptContext) -> bool:
        # All Flow instances are GPU instances, so always include if health monitoring is enabled
