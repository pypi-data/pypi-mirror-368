...
from .exceptions import ScriptTooLargeError
from .models import PreparedScript
from .strategies import CompressionStrategy, InlineStrategy, ITransferStrategy, SplitStrategy
...
class ScriptSizeConfig:
    ...
    def from_dict(cls, config: Dict[str, Any]) -> "ScriptSizeConfig":
        ...
        return cls(
            max_script_size=config.get("max_script_size", 10_000),
...
class ScriptSizeHandler:
    ...
    def __init__(
        ...
        config: Optional[ScriptSizeConfig] = None,
        strategies: Optional[List[ITransferStrategy]] = None,
    ):
    ...
    def prepare_script(self, script: str) -> PreparedScript:
        ...
        for strategy in self.strategies:
            ...
            try:
                ...
                error_msg = f"Error in {strategy_name} strategy: {str(e)}"
        ...
        suggestions = self._get_failure_suggestions(script_size, strategies_tried)
    ...
    def _get_failure_suggestions(self, script_size: int, strategies_tried: List[str]) -> List[str]:
        ...
        if "split" not in strategies_tried and not self.storage_backend:
            suggestions.append("Configure a cloud storage backend to handle large scripts:")
            suggestions.append("  - For AWS: Set FLOW_STORAGE_BACKEND=s3")
            suggestions.append("  - For Google Cloud: Set FLOW_STORAGE_BACKEND=gcs")
            suggestions.append("  - For Azure: Set FLOW_STORAGE_BACKEND=azure")
        ...
        primary_suggestions = [
            "Set upload_code=False if your code is already in the Docker image",
            "Use a Docker image with dependencies pre-installed: flow.run('cmd', image='myimage:latest')",
            "Add .flowignore file to exclude unnecessary files (like .git, node_modules, __pycache__)",
        ]
    ...
    def validate_script_size(self, script: str) -> tuple[bool, Optional[str]]:
        ...
        suggestions = self._get_failure_suggestions(script_size, available_strategies)
