...
"""
        ...
        try:
            ...
            if "Name already used" in error_msg or "already exists" in error_msg.lower():
                if no_unique:
                    # User explicitly doesn't want unique names, let the error through
                    raise
...
Examples:
  ...
  flow dev "python train.py"  # Run script"""
            ...
            if upload:
                ...
                try:
                    if env_name == "default":
                        ...
                        # Upload to /root first (provider default)
                        provider = flow_client.provider
                        provider.upload_code_to_task(
                            task_id=vm.task_id,
                            source_dir=upload_path_resolved,
                        ...
                        copy_cmd = f"rsync -av /root/ {env_target_dir}/ --exclude='/envs/'"
                        with AnimatedEllipsisProgress(
                            console,
    ...
    def _wait_for_vm_ready(self, flow_client: Flow, vm: Task) -> Task:
        ...
        with AnimatedEllipsisProgress(console, "Initializing SSH connection", transient=True):
            pass
