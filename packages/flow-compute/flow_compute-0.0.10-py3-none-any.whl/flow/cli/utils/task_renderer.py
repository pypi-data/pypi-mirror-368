...
class TaskTableRenderer:
    ...
    def _build_row_data(self, task: Task, layout: Dict[str, Any]) -> List[Union[str, Align]]:
        ...
        if "status" in columns:
            ...
            # Show "starting" status based on explicit instance status
            # If we have instance_status metadata, trust it completely
            ...
            if display_status == "starting":
                # Show starting status in blue
                if layout.get("use_compact_status", False):
                    status = self.task_fmt.format_compact_status("starting")
                else:
                    status = self.task_fmt.format_status_with_color("starting")
            else:
                if layout.get("use_compact_status", False):
