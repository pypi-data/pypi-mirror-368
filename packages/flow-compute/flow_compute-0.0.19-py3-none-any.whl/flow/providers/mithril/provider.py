...
from .ssh_utils import SSHTunnelManager
...
class MithrilProvider(IProvider):
    ...
    def submit_task(
        ...
        try:
            ...
            if adjusted_config.upload_code and not using_scp_upload:
                try:
                    ...
                    # Rebuild and re-prepare script
                    fb_script_obj = self.startup_builder.build(fallback_config)
                    fb_raw = fb_script_obj.content
                    fb_prepared = self.script_size_handler.prepare_script(fb_raw)

                    # Success: adopt fallback
                    startup_script = fb_prepared.content
                    adjusted_config = fallback_config
                    using_scp_upload = True

                    if fb_prepared.requires_network:
                        logger.info(
                            f"Startup script requires network access for download "
                            f"(using {fb_prepared.strategy} strategy)"
                        )
            ...
            if adjusted_config.upload_code and not using_scp_upload:
                if exceeds_limit:
                    ...
                    error_msg = (
                        f"Startup script could not be prepared within size limits. "
                        f"This often happens when upload_code=True includes too many files that don't compress well. "
                        f"Try upload_strategy='scp' or upload_code=False."
                    )
                ...
                if exceeds_limit:
                    ...
                    error_msg = (
                        f"Startup script could not be prepared within size limits. "
                        f"The script content could not be handled by available strategies."
                    )
        ...
        # Populate created_by using /v2/me if possible (best-effort)
        try:
            me_resp = self.http.request(method="GET", url="/v2/me")
    ...
    def get_task(self, task_id: str) -> Task:
        ...
        def _page(next_cursor: str | None = None):
            ...
            return self.http.request(method="GET", url="/v2/spot/bids", params=params)
    ...
    def _build_task_from_bid(
        ...
        # Detect Flow origin from startup script header when available
        try:
            launch_spec = bid_data.get("launch_specification", {}) or {}
            startup_script = str(launch_spec.get("startup_script", ""))
            if startup_script:
                # Only inspect the beginning for performance
                head = startup_script[:512]
                import re

                if re.search(r"(?m)^\s*#\s*FLOW_ORIGIN:\s*flow-cli\b", head):
                    provider_metadata["origin"] = "flow-cli"
                    m_ver = re.search(r"FLOW_ORIGIN:\s*flow-cli\s+v([^\s]+)", head)
                    if m_ver:
                        provider_metadata["origin_version"] = m_ver.group(1)
                    m_sess = re.search(r"session:([a-z0-9\-]{4,32})", head)
                    if m_sess:
                        provider_metadata["origin_session"] = m_sess.group(1)
        except Exception:
            # Never fail task construction due to detection logic
            pass
