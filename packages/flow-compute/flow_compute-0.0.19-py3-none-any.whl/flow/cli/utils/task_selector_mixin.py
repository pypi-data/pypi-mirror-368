...
class TaskFilter:
    ...
    def with_ssh(tasks: List[Task]) -> List[Task]:
        ...
        try:
            from flow.api.models import TaskStatus
            return [t for t in tasks if getattr(t, "ssh_host", None) and getattr(t, "status", None) == TaskStatus.RUNNING]
        except Exception:
            ...
            return [
                ...
                and getattr(getattr(t, "status", None), "value", str(getattr(t, "status", "")).lower())
                == "running"
            ]
