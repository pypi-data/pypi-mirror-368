    ...
    def execute_on_task(self, task: Task, client: Flow, **kwargs) -> None:
        ...
        if not task.ssh_host:
            ...
            if age_message:
                ...
                console.print("[dim]Press Ctrl+C to cancel this SSH attempt (task keeps running)[/dim]\n")
        ...
        if command:
            ...
            try:
                ...
                # Get provider for error handling, try to resolve dynamically
                try:
                    provider_name = getattr(getattr(client, "config", None), "provider", None) or "mithril"
                except Exception:
