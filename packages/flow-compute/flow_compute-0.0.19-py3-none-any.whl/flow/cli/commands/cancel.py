...
def _invalidate_task_prefetch_cache():
    try:
        from ..utils.prefetch import _CACHE  # type: ignore
...
class CancelCommand(BaseCommand, TaskOperationCommand):
                ...
                if ids is not None:
                    ...
                    for tid in ids:
                        try:
                            ...
                            console.print(f"[red]✗[/red] Failed to cancel {tid[:12]}…: {escape(str(e))}")
    ...
    def execute_on_task(self, task: Task, client: Flow, **kwargs) -> None:
        ...
        try:
            from ..utils.prefetch import _CACHE, _prefetch_tasks  # type: ignore
            if hasattr(_CACHE, "_data"):
                for k in ("tasks_running", "tasks_pending", "tasks_all"):
                    _CACHE._data.pop(k, None)  # noqa: SLF001
            import threading
    ...
    def _execute_cancel_all(self, yes: bool) -> None:
        ...
        try:
            ...
            with AnimatedEllipsisProgress(
                ...
                for i, task in enumerate(cancellable):
                    ...
                    try:
                        ...
                        from rich.markup import escape
                        console.print(f"[red]✗[/red] Failed to cancel {task_name}: {escape(str(e))}")
                        failed_count += 1
            ...
            if cancelled_count > 0:
                ...
                console.print(f"[{success_color}]✓[/{success_color}] Successfully cancelled {cancelled_count} task(s)")
            if failed_count > 0:
                ...
                if task.name:
                    if use_wildcard:
                        ...
                        try:
                            ...
                            from rich.markup import escape
                            console.print(f"[red]Invalid regex pattern: {escape(str(e))}[/red]")
                            return
