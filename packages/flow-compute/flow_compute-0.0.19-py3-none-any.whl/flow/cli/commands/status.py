...
class StatusCommand(BaseCommand):
    ...
    def get_command(self) -> click.Command:
        ...
        @click.option("--no-origin-group", is_flag=True, help="Disable Flow/Other grouping in main view")
        def status(
            ...
            if (not output_json) and (not task_identifier) and (not watch):
                try:
                    ...
                    with AnimatedEllipsisProgress(console, "Fetching tasks", start_immediately=True):
                        ...
                        # TaskFetcher now consults prefetch cache for instant response
                    ...
                    # If auth is not configured, show a single, clear message and exit
                    msg = str(e)
                    if isinstance(e, ValueError) and (
                        ("Authentication not configured" in msg) or ("MITHRIL_API_KEY" in msg)
                    ):
                        self.handle_auth_error()
                        return
                    # Fallback to classic view without dumping the full error (less jarring)
                    console.print(
                        "[yellow]New UI unavailable. Falling back to classic view...[/yellow]",
                        markup=True,
                    )
    ...
    def _parse_timespec(self, value: Optional[str]) -> Optional[datetime]:
        """Back-compat wrapper using shared parser."""
        from ..utils.time_spec import parse_timespec
