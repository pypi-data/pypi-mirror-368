...
Note: No runtime limit is applied by default. To auto-terminate, set max_run_time_hours in your TaskConfig (YAML or SDK)."""
    ...
    def _execute(
        ...
        try:
            ...
            flow_client = Flow()
            # Resolve provider name dynamically (fallback to mithril)
            try:
                provider_name = getattr(getattr(flow_client, "config", None), "provider", None) or "mithril"
            except Exception:
            ...
            # Invalidate stale slices and trigger a refresh after submission
            try:
                from ..utils.prefetch import _CACHE, _prefetch_tasks  # type: ignore

                # Remove cached task lists so next status reflects the new task
                if hasattr(_CACHE, "_data"):
                    for k in ("tasks_running", "tasks_pending", "tasks_all"):
                        _CACHE._data.pop(k, None)  # noqa: SLF001

                # Best-effort immediate refresh in background
                import threading
