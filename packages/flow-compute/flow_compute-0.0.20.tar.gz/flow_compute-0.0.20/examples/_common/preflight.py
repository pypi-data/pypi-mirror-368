...
def ensure_ready(provider: Provider) -> list[str]:
    ...
    if not os.environ.get("FLOW_API_KEY"):
        warnings.append("FLOW_API_KEY not found in env (run 'flow init' if not configured).")
    if not os.environ.get("FLOW_PROJECT"):
        warnings.append("FLOW_PROJECT not found in env (run 'flow init' or set project).")
