...
Defines the core contracts for provider implementations using structural typing
(`typing.Protocol`). The main categories are:
...
These interfaces decouple the core from concrete providers and enable third
parties to add implementations without changing the SDK.
"""
...
class IRemoteOperations(Protocol):
    ...
    def open_shell(self, task_id: str, command: str | None = None) -> None:
