"""GPU compute orchestration with a unified provider abstraction.

High-level interface for submitting GPU workloads across multiple providers.
Focuses on explicit behavior and clear validation with comprehensive
observability.
...
class Flow:
    ...
    def dev_context(self, auto_stop: bool = False) -> DevEnvironment:
        ...
        Returns:
            Context manager for the dev environment.
    ...
    def _find_gpus_by_memory(
        ...
        Optimized path: ask the provider to filter at the source instead of
        loading the full catalog, which reduces payload and serialization.
        If provider filtering fails, fall back to the local catalog scan.

        Args:
            min_memory_gb: Minimum GPU memory in gigabytes
            max_price: Optional maximum hourly price in USD
    ...
    def list_tasks(
        self, status: TaskStatus | None = None, limit: int = 10, force_refresh: bool = False
    ) -> list[Task]:
    ...
    def mount_volume(self, volume_id: str, task_id: str, mount_point: str | None = None) -> None:
        ...
        /volumes/{volume_name} immediately after successful mounting.
        ...
        Example:
            ...
            >>> # Volume now available at /volumes/shared-data in the task
            >>>
        ...
        Args:
            ...
            mounts: Storage source specification. Accepts:
                - String: Single source with automatic mount path:
                    ...
                    - "volume://my-vol" → mounts to /volumes
    ...
    def _resolve_data_mounts(self, mounts: str | dict[str, str]) -> list[MountSpec]:
        ...
        - volume:// sources → /volumes (common for persistent storage)
