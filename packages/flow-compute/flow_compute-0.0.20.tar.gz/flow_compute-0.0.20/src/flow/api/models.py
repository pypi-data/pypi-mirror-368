...
Defines the type system for GPU compute orchestration. Models use Pydantic for
validation, serialization, and IDE support.
...
class ReservationStatus(str, Enum):
    """Lifecycle states for capacity reservations.

    These represent the scheduling and runtime state of a reservation window
    independent of any specific workload. Providers may use different labels;
    these are normalized for Flow UX.
    """
...
class ReservationSpec(BaseModel):
    """Specification for creating a reservation.

    Provider-agnostic shape used by reservation-capable providers.
    """
...
class Reservation(BaseModel):
    """Reservation information returned by providers."""
...
class VolumeSpec(BaseModel):
    ...
    def validate_volume_spec(self) -> "VolumeSpec":
        ...
        if not self.mount_path:
            ...
            if self.name:
                object.__setattr__(self, "mount_path", f"/volumes/{self.name}")
            elif self.volume_id:
                # Derive a readable stable path from volume id suffix
                suffix = self.volume_id[-6:] if len(self.volume_id) >= 6 else self.volume_id
                object.__setattr__(self, "mount_path", f"/volumes/volume-{suffix}")
            else:
                # Fallback (should be rare for new unnamed volumes)
                object.__setattr__(self, "mount_path", "/volumes/volume")
        return self

    @model_validator(mode="before")
    def _alias_target_to_mount_path(cls, data: Any):  # type: ignore[no-redef]
        """Allow 'target' as alias for 'mount_path' in YAML/JSON.

        Supports a consistent 'target' field across mounts and volumes while
        keeping the canonical field as 'mount_path' internally.
        """
...
class TaskConfig(BaseModel):
    ...
    def validate_config(self) -> "TaskConfig":
        ...
        restricted_paths = {"/var/lib/docker", "/var/run/docker.sock", "/etc/docker"}
        ...
        if self.upload_code:
            internal_targets.append("/workspace")
        ...
        if self.upload_code and any(t == "/workspace" for t in user_targets):
            raise ValueError(
                "Cannot mount a user volume at /workspace while upload_code=True. "
                "Either set upload_code=False and use a workspace volume, or mount the volume elsewhere."
            )

        # Duplicate and nesting checks across combined targets
        all_targets = user_targets + internal_targets
        seen = set()
        for t in all_targets:
            if t in seen:
                raise ValueError(f"Duplicate mount target detected: {t}")
            seen.add(t)
            ...
            for vol in self.volumes:
                ...
                if v.interface == StorageInterface.BLOCK:
                    raise ValueError(
                        ...
                        "Use interface: file for multi-attach or reduce num_instances to 1."
                    )
