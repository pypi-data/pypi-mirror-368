...
class MockProvider(IProvider):
    ...
    def _simulate_task_lifecycle(self, task_id: str) -> None:
        ...
        with state.lock:
            ...
            # Update instance_status to reflect running but still starting without SSH
            try:
                meta = state.task.provider_metadata or {}
                meta["instance_status"] = "STATUS_STARTING"
    ...
    def list_tasks(
        self, status: TaskStatus | None = None, limit: int = 100, force_refresh: bool = False
    ...
    def _seed_demo_tasks(self):
        ...
        t4 = Task(
            ...
            completed_at=now - timedelta(days=4, hours=23),
            instance_type="a100-40gb",
            num_instances=1,
            region="demo-region-1",
            cost_per_hour="$8.00",
        ...
        add_simple(
            ...
            "post-training-team",
            ["[mock] finished"],
            completed=timedelta(days=29, hours=20),
        )
        add_simple(
