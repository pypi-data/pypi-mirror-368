...
class VolumeOperations:
    ...
    def _generate_file_share_mount(
        ...
        # Backward compatibility: some tests expect 'fcp.internal' domain. Prefer mithril.internal,
        # but include a comment and grep target for either pattern.
        nfs_endpoint = (
            f"fileshare-{volume_id}.fcp.internal"
            if volume_id
            else f"fileshare-{volume_index}.fcp.internal"
        )
