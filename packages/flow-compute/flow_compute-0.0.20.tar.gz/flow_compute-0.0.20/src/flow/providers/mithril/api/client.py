...
class MithrilApiClient:
    ...
    def create_reservation(self, payload: dict[str, Any]) -> Any:
        """POST /v2/reservations to create a reservation."""
        return self._http.request(method="POST", url="/v2/reservations", json=payload)

    def list_reservations(self, params: dict[str, Any] | None = None) -> Any:
        """GET /v2/reservations to list reservations (optionally filter by project/region)."""
        return self._http.request(method="GET", url="/v2/reservations", params=params or {})

    def get_reservation(self, reservation_id: str) -> Any:
        """GET /v2/reservations/{id} to fetch reservation details."""
        return self._http.request(method="GET", url=f"/v2/reservations/{reservation_id}")
...
# Backwards-compatible alias expected by some tests/fixtures
# Expose CamelCase variant to satisfy patch targets: flow.providers.mithril.api.client.MithrilAPIClient
