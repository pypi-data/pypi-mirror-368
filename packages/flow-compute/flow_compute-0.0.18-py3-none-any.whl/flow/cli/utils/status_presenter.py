...
class StatusPresenter:
    ...
    def present(self, options: StatusDisplayOptions, tasks: Optional[List[Task]] = None) -> None:
        ...
        if options.group_by_origin:
            ...
            if flow_tasks:
                title_flow = (
                    f"Flow tasks (showing up to {options.limit}, last 24 hours)"
                    if not options.show_all
                    else f"Flow tasks (showing up to {options.limit})"
                )
                panel_flow = self.table.render(
                    flow_tasks, me=me, title=title_flow, wide=options.wide, return_renderable=True
                )
            ...
            if other_tasks:
                ...
                if flow_tasks:
                    self.console.print("")
                title_other = (
                    f"Other tasks (showing up to {options.limit}, last 24 hours)"
                    if not options.show_all
                    else f"Other tasks (showing up to {options.limit})"
                )
                panel_other = self.table.render(
                    other_tasks, me=me, title=title_other, wide=options.wide, return_renderable=True
                )
