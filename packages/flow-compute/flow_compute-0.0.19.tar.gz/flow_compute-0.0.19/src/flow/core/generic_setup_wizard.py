...
class GenericSetupWizard:
    ...
    def _show_configuration_status(self, existing_config: Dict[str, Any]) -> bool:
        ...
        # Add a touch of spacing before contextual guidance
        try:
            self.console.print()
        except Exception:
            pass

        # Contextual guidance beneath the status table
        try:
            # Brief, high-signal guidance with emphasis on SSH keys
            self.console.print(
                ...
                f"{format_text('muted', 'Manage keys:')} [link]https://app.mithril.ai/account/ssh-keys[/link]"
            )
        except Exception:
            pass
        ...
        for field in required_fields + optional_fields:
            ...
            if value:
                ...
                if field.mask_display and value:
                    ...
                    if field.name == "default_ssh_key" and isinstance(value, str) and value.strip() == "_auto_":
    ...
    def _configure_field(self, field_name: str, context: Dict[str, Any]) -> bool:
        ...
        if field.field_type == FieldType.CHOICE:
            if field.dynamic_choices:
                ...
                if is_ssh_key_selection:
                    header_parts.append("Tip: Upload from this machine:\n  flow ssh-keys upload ~/.ssh/id_ed25519.pub")
                    header_parts.append("Manage keys: https://app.mithril.ai/account/ssh-keys")
            ...
            try:
                if isinstance(value, str) and value:
                    preview_masked = mask_sensitive_value(value)
                    colors = get_colors()
                    self.console.print(f"[{colors['muted']}]Received: {preview_masked}[/{colors['muted']}]")
            except Exception:
                pass
        ...
        if validation_result.is_valid:
            ...
            if validation_result.display_value:
                ...
                display_value = mask_sensitive_value(str(self.config[field_name]))
            else:
                display_value = str(self.config[field_name])
            
            success_color = theme_manager.get_color("success")
            self.console.print(f"[{success_color}]✓[/{success_color}] {display_name}: {display_value}")
    ...
    def _show_completion(self):
        ...
        if (
            ...
            billing_reminder = (
                ...
                f"[{link_color}]https://app.mithril.ai/settings/billing[/{link_color}]"
            )
