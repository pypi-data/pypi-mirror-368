...
class ApiKeySetup:
    ...
    def setup(self, existing_config: Dict[str, Any]) -> Optional[str]:
        ...
        self.console.print("1. Visit: [link]https://app.mithril.ai/account/api-keys[/link]")
        ...
        if Confirm.ask("\n[bold]Open browser?[/bold]", default=True):
            ...
            webbrowser.open("https://app.mithril.ai/account/api-keys")
            time.sleep(2)
        ...
        with Progress(
            ...
            try:
                ...
                if result.status == ValidationStatus.VALID:
                    ...
                    progress.update(task, description=f"[{success_color}]✓ API key verified![/{success_color}]")
...
1. Upload your public key at: https://app.mithril.ai/account/ssh-keys
...
[dim]SSH access is optional. You can configure this later if needed.[/dim]
        """
        )
        ...
        self.console.print("1. Upload your public key at: [link]https://app.mithril.ai/account/ssh-keys[/link]")
        self.console.print("   Or upload locally via CLI: [cyan]flow ssh-keys upload ~/.ssh/id_ed25519.pub[/cyan]")
        ...
        if Confirm.ask("\nOpen SSH key management page?", default=True):
            webbrowser.open("https://app.mithril.ai/account/ssh-keys")
            time.sleep(2)
