...
class InitCommand(BaseCommand):
    ...
    async def _init_async(
        ...
        if reset:
            if await self._reset_configuration():
                ...
                console.print(f"[{success_color}]✓[/{success_color}] Configuration reset successfully")
    ...
    async def _configure_with_options(
        ...
        if api_key:
            ...
            from ..utils.theme_manager import theme_manager as _tm2
            success_color = _tm2.get_color("success")
            console.print(f"[{success_color}]✓[/{success_color}] API key validated: {mask_api_key(api_key)}")
        if project:
        ...
        if dry_run:
            ...
            if output_path:
                try:
                    ...
                    from ..utils.theme_manager import theme_manager as _tm3
        ...
        from ..utils.theme_manager import theme_manager as _tm4
        ...
        try:
            ...
            try:
                ...
                if shell_name == "bash":
                    ...
                    completion_line = 'if test -n "$FISH_VERSION"; _FLOW_COMPLETE=fish_source flow | source; end'
                else:
                    return
            ...
            from ..utils.theme_manager import theme_manager as _tm5
            ...
            console.print(f"  Enable now in this shell: [cyan]{completion_line}[/cyan]")
            console.print(
                f"  Or restart your shell, or run: [cyan]source {config_file}[/cyan]"
            )
