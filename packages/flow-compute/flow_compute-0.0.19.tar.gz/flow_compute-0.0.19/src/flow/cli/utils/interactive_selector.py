...
class InteractiveSelector(Generic[T]):
    ...
    def select(self) -> T | None | list[T]:
        ...
        # If not attached to a real TTY, try to use /dev/tty before falling back
        if not (sys.stdin.isatty() and sys.stdout.isatty()) and not force_interactive:
            ...
            if dev_tty_available:
                ...
                if os.environ.get("FLOW_DEBUG"):
                    self.console.print("[dim]Using /dev/tty for interactive I/O[/dim]")
            else:
                if os.environ.get("FLOW_DEBUG"):
                    self.console.print("[dim]stdin/stdout not TTY and /dev/tty unavailable, using fallback[/dim]")
                return self._fallback_selection()
        ...
        if len(self.items) == 1 and not self.allow_multiple:
            ...
            self.console.print(f"[{success_color}]Auto-selecting:[/{success_color}] {item.title} ({item.id})")
            return item.value
    ...
    def _run_interactive(self) -> T | None | list[T]:
        ...
        def get_body_text():
            ...
            if viewport_end < len(self.filtered_items):
                muted_color = _map_rich_to_prompt_toolkit_color(theme_manager.get_color("muted"))
                html += _style(f"  ↓ {len(self.filtered_items) - viewport_end} more below...", fg=muted_color) + "\n"
    ...
    def _format_item_line(
        ...
        if is_ssh_key:
            ...
            if is_selected:
                ...
                line = f"{_style('▸', fg=arrow_color)} {selection_marker} {name_part}  {subtitle_part}"
                return self._apply_selection_style(line, True)
            else:
                muted_color = _map_rich_to_prompt_toolkit_color(theme_manager.get_color("muted"))
        ...
        if is_selected:
            ...
            # Then name
            line += f"<b>{name:<{column_widths['name']}}</b>  "
            # Use selection-aware muted color for readability on highlight
            muted_sel_color = _map_rich_to_prompt_toolkit_color(
                theme_manager.get_color("selected_muted_fg")
            )
            line += _style(f"{gpu_info:<{column_widths['gpu']}}", fg=muted_sel_color)
            line += _style(f"{time_info:<{column_widths['time']}}", fg=muted_sel_color)
            ...
            if status_display:
                ...
                line += " " * (column_widths["status"] + 2)
            # Then name
            line += _style(f"{name:<{column_widths['name']}}", fg=default_color) + "  "
            line += _style(f"{gpu_info:<{column_widths['gpu']}}", fg=muted_color)
            line += _style(f"{time_info:<{column_widths['time']}}", fg=muted_color)
