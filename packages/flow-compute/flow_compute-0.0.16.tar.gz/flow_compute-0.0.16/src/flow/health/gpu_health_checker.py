"""GPU health checking with proper separation of concerns and SOLID principles."""
...
from flow.api.health_models import HealthStatus, NodeHealthSnapshot
...
class GPUdStatusDiagnoser:
    ...
    def diagnose(self, api_url: str, task_age_hours: Optional[float]) -> GPUdDiagnosis:
        ...
        health_result = self._check_health_endpoint(api_url)
    ...
    def _check_health_endpoint(self, api_url: str) -> str:
        """Check GPUd health endpoint.

        Returns:
            'healthy', 'timeout', 'connection_refused', or 'error'
        """
        try:
            resp = requests.get(f"{api_url}/healthz", timeout=self.HEALTH_CHECK_TIMEOUT)
    ...
    def _is_legacy_task(self, task_age_hours: Optional[float]) -> bool:
    ...
    def _is_new_task(self, task_age_hours: Optional[float]) -> bool:
    ...
    def check_gpud_marker(self, ssh_command_prefix: list[str]) -> Optional[str]:
            ...
            self.warnings.append(
                {
                    ...
                    "suggestion": "For future tasks: use 'flow run'. For this task: see 'flow health --gpu --help' for manual setup",
                }
            )
...
class SSHConnectionHandler:
    ...
    @staticmethod
    def analyze_ssh_error(error: Exception, task_age_hours: Optional[float]) -> GPUdDiagnosis:
