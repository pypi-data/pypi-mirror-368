...
class TaskFetcher:
    ...
    def fetch_all_tasks(
        ...
        try:
            ...
            from flow.cli.utils.mode import is_demo_active  # type: ignore
    ...
    def fetch_for_display(
        ...
        if not show_all and not status_filter:
            ...
            # No active tasks found - batch all three calls concurrently and pick best slice
            try:
                with ThreadPoolExecutor(max_workers=3) as ex:
                    futures = [
                        ex.submit(self.flow_client.list_tasks, status=TaskStatus.RUNNING, limit=min(100, limit)),
                        ex.submit(self.flow_client.list_tasks, status=TaskStatus.PENDING, limit=min(100, limit)),
                        ex.submit(self.flow_client.list_tasks, status=None, limit=limit),
                    ]
                    results: list[Task] = []  # type: ignore[name-defined]
                    for f in as_completed(futures, timeout=2.0):
                        try:
                            ...
                            pass
                    # Prefer active tasks if any present
                    active = [
                        t
                        for t in results
                        if getattr(getattr(t, "status", None), "value", None) in {"running", "pending"}
                    ]
                    if active:
                        ...
                        for t in active:
                            ...
                            if tid and tid not in seen:
                                seen[tid] = t
                        active = sorted(seen.values(), key=lambda t: getattr(t, "created_at", 0), reverse=True)
                        return active[:limit]
                    # Else, return most recent general list
                    general = [
                        t
                        for t in results
                        if getattr(getattr(t, "status", None), "value", None) not in {"running", "pending"}
                    ]
                    if general:
