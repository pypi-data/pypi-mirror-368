...
storage volumes to tasks. If the task isn't running yet, the volume
will attach now and the mount will finalize when the task starts
and is SSH-ready.
...
class MountCommand(BaseCommand):
    ...
    @property
    def help(self) -> str:
        return "Attach storage volumes to tasks; if not running, mount completes when the task starts"
            ...
            try:
                ...
                if not task_id:
                    ...
                    tasks = [
                        ...
                        and t.status.lower() in eligible_task_statuses
                    ]
                    if not tasks:
                ...
                if task.status.lower() in ["pending", "starting", "initializing"]:
                    console.print(
                        f"[green]✓[/green] Volume attached to task. Mount will complete when the task is ready."
                    )
                    console.print(
                        f"\n[yellow]Note:[/yellow] Task is still starting. The volume will be available at "
                        f"[cyan]{mount_path}[/cyan] once the task starts."
                    )
                    console.print(
