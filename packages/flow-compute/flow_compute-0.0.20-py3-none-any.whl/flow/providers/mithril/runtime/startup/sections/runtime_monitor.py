        ...
        if cancel_script_body is None:
            cancel_script_body = textwrap.dedent(
                ...
                TASK_ID=$(curl -s -H "Authorization: Bearer $MITHRIL_API_KEY" \
                    ...
                    python3 -c "import sys,json;d=json.load(sys.stdin);b=d if isinstance(d,list) else d.get('data',[]);\
print(next((x.get('fid') for x in b for i in x.get('instances',[]) if i.get('instance_id')=='$INSTANCE_ID'), 'unknown'))")
