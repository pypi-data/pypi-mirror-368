    ...
    try:
        ...
        class _MockSetupAdapter(ProviderSetupAdapter):  # type: ignore
            ...
            def get_configuration_fields(self):
                return [
                    ...
                    ConfigField(
                        ...
                        help_text=(
                            "Choose how SSH should behave in demos. Auto‑generate produces a fresh key per instance."
                        ),
                        dynamic_choices=True,
                    ),
                ]
            ...
            def validate_field(self, field_name: str, value: str, context=None) -> ValidationResult:
                ...
                if field_name == "default_ssh_key":
                    # Accept auto, none, or a local public key path prefixed with local:
                    if value in {"_auto_", "none"}:
                        display = (
                            "Auto‑generate per instance" if value == "_auto_" else "No SSH access"
                        )
                        return ValidationResult(
                            is_valid=True, display_value=display, processed_value=value
                        )
                ...
                if field_name == "demo_api_key":
                    ...
                    # Encourage DEMO- prefix for tutorials; still accept any non-empty
                    display = "••••"
                    if not str(value).startswith("DEMO-"):
                        # Provide a soft warning-style message via display (wizard shows ✓ with masked value)
                        display = "••••"
            ...
            def save_configuration(self, config):
                ...
                payload = {
                    "provider": "mock",
                    "mithril": {},  # keep structure consistent for downstream consumers
                }
                ...
                writer = ConfigWriter()
                writer.write(payload, validation=None)  # type: ignore[arg-type]
                return True
