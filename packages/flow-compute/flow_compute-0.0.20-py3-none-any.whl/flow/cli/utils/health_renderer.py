...
class HealthRenderer:
    ...
    def render_live_table(self, tasks: list[Any], snapshots: list[NodeHealthSnapshot]) -> Table:
        ...
        # Columns: compact, fixed widths for consistency
        table.add_column("Node", style=theme_manager.get_color("task.name"), no_wrap=True, width=20)
        table.add_column("Monitor", justify="center", width=8, no_wrap=True)
        table.add_column("GPUs", justify="center", width=6, no_wrap=True)
        table.add_column("Temp", justify="center", width=7, no_wrap=True)
        table.add_column("Usage", justify="center", width=7, no_wrap=True)
        table.add_column("Memory", justify="center", width=7, no_wrap=True)
        table.add_column("Status", style=theme_manager.get_color("table.row.dim"))
    ...
    def _add_live_table_row(self, table: Table, snapshot: NodeHealthSnapshot) -> None:
        ...
        if snapshot.gpu_metrics:
            gpu_count = f"[accent]{len(snapshot.gpu_metrics)}[/accent]"
