...
def is_demo_active() -> bool:
    ...
    # Fast path: explicit env override (support FLOW_DEMO alias)
    if _truthy(os.environ.get("FLOW_DEMO_MODE")) or _truthy(os.environ.get("FLOW_DEMO")):
        return True
...
def apply_demo_mode(demo: bool | None) -> bool:
    ...
    # No explicit flag: honor existing env/config (support FLOW_DEMO alias)
    if _truthy(os.environ.get("FLOW_DEMO")) and not os.environ.get("FLOW_DEMO_MODE"):
