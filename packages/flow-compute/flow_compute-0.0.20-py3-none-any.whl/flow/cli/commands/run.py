...
from flow.cli.commands.utils import display_config, wait_for_task
...
Note: No runtime limit is applied by default. To auto-terminate, set max_run_time_hours in your TaskConfig (YAML or SDK)."""
    ...
    def get_command(self) -> click.Command:
        ...
        @click.option(
            "--demo/--no-demo",
            default=None,
            help="Override demo mode for this run (mock provider, no real provisioning)",
        )
            ...
            if config_file:
                ...
                if is_slurm:
                    ...
                    try:
                        ...
                        _data.setdefault("unique_name", False)
            ...
            if is_slurm and len(configs) > 1:
                ...
                for cfg in configs:
                    try:
                        ...
                        msg = str(e)
                        policy = (
                            on_name_conflict or ("suffix" if (force_new or not name) else "error")
                        ).lower()
                        is_conflict = (
                        ...
                        if is_conflict and policy == "suffix":
                            import uuid as _uuid

                            base = cfg.name or "flow-task"
                            retry_name = f"{base}-{_uuid.uuid4().hex[:6]}"
                            cfg = cfg.model_copy(update={"name": retry_name, "unique_name": False})
                            t = flow_client.run(cfg, mounts=mount_dict)
                            tasks.append(t)
                            if not output_json:
                                console.print(
                                    f"[dim]Name conflict resolved by using: {retry_name}[/dim]"
                                )
                        else:
                            raise
            else:
                ...
                if not output_json and timeline:
                    ...
                    try:
                        ...
                        if is_conflict and policy == "suffix":
                            ...
                            base = config.name or "flow-task"
                            retry_name = f"{base}-{_uuid.uuid4().hex[:6]}"
                            config = config.model_copy(
                                update={"name": retry_name, "unique_name": False}
                            )
                            task = flow_client.run(config, mounts=mount_dict)
                            console.print(
                                f"[dim]Name conflict resolved by using: {retry_name}[/dim]"
            ...
            if status == "running":
                ...
                if cli_managed_scp and timeline:
                    ...
                    try:
                        ...
                        hint.append(" to stop waiting. Upload later with ")
