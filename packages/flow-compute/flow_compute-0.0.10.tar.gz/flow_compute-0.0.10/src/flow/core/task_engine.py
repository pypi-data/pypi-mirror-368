...
This module consolidates task lifecycle management, execution orchestration,
and progress monitoring into a unified engine. It handles the complete task
lifecycle from resource allocation through cleanup.

Following the principle of cohesion, related task management functionality
is kept together while maintaining clear separation of concerns.
"""
...
from flow.api.models import Instance, Task, TaskConfig, TaskStatus, Volume
...
class TaskEngine:
    ...
    def run_task(self, provider: IProvider, config: TaskConfig, dry_run: bool = False) -> Task:
        ...
        with self._resource_lifecycle(tracker):
            ...
            task = provider.submit_task(
                instance_id=instance.instance_type,
    ...
    def wait_for_completion(
        self, task: Task, timeout: Optional[int] = None, callback: Optional[callable] = None
    ) -> Task:
...
def wait_for_task(task: Task, timeout: Optional[int] = None) -> Task:
