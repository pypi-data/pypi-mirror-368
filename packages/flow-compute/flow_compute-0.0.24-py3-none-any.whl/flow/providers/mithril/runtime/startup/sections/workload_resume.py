    ...
    def _generate_resume_script(self, context: ScriptContext) -> str:
        ...
        if getattr(self, "template_engine", None):
            try:
                ...
                _log.debug("WorkloadResumeSection: template render failed; using inline", exc_info=True)
...
USER_SCRIPT_EOF
    ...
    def _build_docker_run_command_for_resume(self, context: ScriptContext) -> str:
        ...
        parts.append(f'$([ -d {WORKSPACE_DIR} ] && echo "-v {WORKSPACE_DIR}:{WORKSPACE_DIR} -w {WORKSPACE_DIR}")')
