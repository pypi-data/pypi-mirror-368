...
class HorizontalLayoutArgs(TypedDict):
...
class HorizontalLayout(BaseLayout):
    ...
    def __init__(self, size: Tuple[int, int] = (0, 0), *, flexbox: Optional[Flexbox] = None, align: Literal['top', 'middle', 'bottom'] | float = 0, parent_relative_alignment: Literal['left', 'middle', 'right'] | float = 0, **kwargs: Unpack[HorizontalLayoutArgs]):
    ...
    def __enter__(self):
        ...
        if imgui.begin_child(f'horizontal_layout {child_id()} ', **{'size': self._size, **self._kwargs}):
    ...
    def new_component(self, component: BaseWidget | BaseLayout): 
        ...
        # If it is second+ widget, do same line
    ...
    def _align_component(self, align_type: str | float | None, comp_size: ImVec2):
        content_region_y = imgui.get_content_region_max().y

        match align_type or self._align: # if align_type is not passed, using global one
            case 'top':
                imgui.set_cursor_pos_y(0)
            case 'middle':
                imgui.set_cursor_pos_y(content_region_y / 2 - comp_size.y / 2)
            case 'bottom':
                imgui.set_cursor_pos_y(content_region_y - comp_size.y)
            case int() | float() as align:
                imgui.set_cursor_pos_y(np.clip((content_region_y / 100) * (align * 100) - (comp_size.y / 2), 0, content_region_y - comp_size.y)) 
            case _:
                raise Exception('Wrong argument passed to "align"')