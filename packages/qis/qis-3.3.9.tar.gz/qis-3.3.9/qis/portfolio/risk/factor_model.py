...
class LinearModel:
    ...
    def compute_factor_risk_contribution(self, weights: pd.DataFrame,
        ...
        for date, factor_covar in self.x_covars.items():
            ...
            portfolio_var_idio = last_weight.T @ np.diag(self.residual_vars.loc[date, last_weight.index]) @ last_weight
            ...
            # Normalize factor contributions by total variance (systematic + idiosyncratic)
            # Store systematic variance for this date
            portfolio_factor_vars[date] = portfolio_var_factors
            # Store idiosyncratic variance for this date
