...
from urarovite.checker.auth import get_sheets_service
    ...
    def validate(
        ...
        if not input_sid or not output_sid:
            return {
                ...
                "diff_count": 0,
                "diffs": [],
                "notes": "Missing input/output spreadsheet IDs.",
            }

        # Get our own service instance
        ...
        try:
            ...
            if len(diffs) > max_report:
                notes = f"Showing first {max_report} of {len(diffs)} diffs."
