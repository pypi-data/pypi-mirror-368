...
class AzureStorageAccount:
    ...
    def upload_data(self, data, upload_path, metadata=None, overwrite=False, storage_account_name=None, settings_path=None, **kwargs):
        ...
        upload_results = blob_client.upload_blob(data, overwrite=overwrite, **kwargs)
        if metadata is not None:
            logging.debug(f'Updating metadata for uploaded file {metadata}')
            blob_properties = blob_client.get_blob_properties()
            blob_metadata = blob_properties.metadata
            for metadata_key, metadata_value in metadata.items():
                blob_metadata[metadata_key] = metadata_value
    ...
    def upload_file(self, file_path, upload_path, metadata=None, overwrite=False, storage_account_name=None, settings_path=None, **kwargs):
        data_lake_client = self._get_storage_account_client(storage_account_name, settings_path)
        container_name = self._get_container_name(upload_path)
        ...
        with open(file_path, 'rb') as f:
            file_content = f.read()
            upload_results = blob_client.upload_blob(file_content, overwrite=overwrite, **kwargs)
            if metadata is not None:
                set_metadata_results = blob_client.set_blob_metadata(metadata=metadata)
                return {'upload_results': upload_results, 'set_metadata_results': set_metadata_results}
        return upload_results

    def download(self, file_path, download_path, metadata=None, storage_account_name=None, settings_path=None, **kwargs):
        ...
        if metadata is not None:
            logging.debug(f'Updating metadata for downloaded file {metadata}')
            blob_properties = blob_client.get_blob_properties()
            blob_metadata = blob_properties.metadata
            for metadata_key, metadata_value in metadata.items():
                blob_metadata[metadata_key] = metadata_value
    ...
    def download_files(self, file_paths, metadata=None, storage_account_name=None, settings_path=None, download_folder='/adls',
                       remove_spaces_from_download_path=False, **kwargs):
    ...
    def download_data(self, file_path, metadata=None, storage_account_name=None, settings_path=None, **kwargs):
        container_name = self._get_container_name(file_path)
        blob_path = self.get_blob_path(file_path)
        data_lake_client = self._get_storage_account_client(storage_account_name, settings_path)
        blob_client = data_lake_client.get_blob_client(container=container_name, blob=blob_path)
    ...
    def upload_files(self, files_to_upload, storage_account_name=None, metadata=None, settings_path=None, overwrite=False, **kwargs):
        """
        ...
        for file_to_upload in files_to_upload:
            ...
            with open(local_path, 'rb') as f:
                file_content = f.read()
                upload_result = blob_client.upload_blob(file_content, overwrite=overwrite, **kwargs)
    ...
    def list(self, directory='', storage_account_name=None, settings_path=None, **kwargs):
        ...
        blobs = container_client.list_blobs(name_starts_with=blob_path, **kwargs)
        return blobs

    def get_file_info(self, file_path, storage_account_name=None, settings_path=None, **kwargs):
        container_name = self._get_container_name(file_path)
        data_lake_client = self._get_storage_account_client(storage_account_name, settings_path)
        ...
        blob_info = blob_client.get_blob_properties(**kwargs)
        return blob_info

    def delete_file(self, file_path, storage_account_name=None, settings_path=None, **kwargs):
        container_name = self._get_container_name(file_path)
        data_lake_client = self._get_storage_account_client(storage_account_name, settings_path)
        file_path = self._update_path_from_container_name(container_name, file_path)
        blob_client = data_lake_client.get_blob_client(container=container_name, blob=file_path)
        delete_results = blob_client.delete_blob(**kwargs)
