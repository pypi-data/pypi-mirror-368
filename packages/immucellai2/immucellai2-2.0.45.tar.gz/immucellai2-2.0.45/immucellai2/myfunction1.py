...
def ThreadRunEachSamples( 
    ...
    *args): 
    ...
    chain = np.empty((max_iter, nwalkers, K))
    converged = False
    for i in range(max_iter): 
        ...
        if (i + 1) % eval_step == 0 and i + 1 >= eval_step:
            segment = chain[(i + 1 - eval_step): (i + 1)].reshape(-1, K)
            mcse_vec = mcse_python(segment) 
            theta_hat = segment.mean(axis=0) 
            ratio = mcse_vec / theta_hat
            if np.all(np.abs(ratio) <= tol_ratio):
                converged = True
                chain = chain[:(i + 1)]   # 截断
                break 
    flat_samples = chain.reshape(-1, K) 
