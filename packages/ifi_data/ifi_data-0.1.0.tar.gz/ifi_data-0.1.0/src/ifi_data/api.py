...
def list_tables(*, include_meta: bool = False) -> list[str]:
    """
...
def insert_data_df(
    df: pd.DataFrame, table_name: str, datetime_col: str | None = None
) -> None:
    """
...
def create_table_from_file(
    ...
    create_table_df(schema, table_name, datetime_col, chunk_days)
...
def insert_single_row(
    ...
    if not table_exists(table_name):
        schema = infer_schema(df)
        create_table_df(schema, table_name, col, chunk_days)
...
def get_data(table_name: str, row_limit: int | None = None) -> list[dict]:
    """
...
def retrieve_data(
    ...
    print(f"Columns: {list(get_table_schema(table_name).keys())}")
    if datetime_col:
        print(f"Time range: {min_time} — {max_time}")

    preview_df = pd.DataFrame(get_data(table_name, row_limit=3))
