...
class DBConfig:
    ...
    def postgrest_url(self) -> str:
        ...
        logger.debug(
            f"Building PostgREST URL with host={self.postgres_host}, port={self.postgrest_port}"
        )
        return f"http://{self.postgres_host}:{self.postgrest_port}"

    @property
    def grafana_base_url(self) -> str:
        """Returns Grafana URL."""
        return f"http://{self.postgres_host}:{self.grafana_port}"
    ...
    def from_env(cls) -> "DBConfig | None":
        ...
        try:
            ...
            postgres_host = config.get("POSTGRES_HOST", cls._env_var("POSTGRES_HOST"))
    ...
    def prompt(cls) -> "DBConfig":
        ...
        host = input("  Host [localhost]: ").strip() or _ENV_DEFAULTS["POSTGRES_HOST"]
