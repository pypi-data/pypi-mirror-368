...
try:
    from lsco_network_from_reports_compare import load_scenario, ProcessNetworkLSCOReports
    LSCO_AVAILABLE = True
...
# Configuration
DATASET_PATH = os.getenv("DATASET_PATH", "tests/experiments/on_mdlz_multi_scenario")
    ...
    try:
        ...
        if LSCO_AVAILABLE:
            ...
            for scenario_id in scenarios_to_compare:
                ...
                try:
                    scenario_skus[scenario_id] = load_scenario(dataset_path, scenario_id, enable_landed_cost=True)
...
class GetLandedCostRequest(BaseModel):
    ...
    period_start_date: str = Field(..., description="Period start date (YYYY-MM-DD)")
    scenario_id: Optional[str] = Field(None, description="Scenario identifier (defaults to 'Base')")
...
@app.post("/landed-cost/calculate", tags=["Landed Cost"], response_model=List[LandedCostRow])
async def calculate_landed_cost(request: GetLandedCostRequest):
    """Calculate landed cost for a product-location combination on a specific date."""
    try:
        scenario_id = request.scenario_id or "Base"
        
        # Create SKU key and find SKU
        sku_key = f"{request.product}@{request.location}"
        sku = supply.PySKU.fetch(sku_key, scenario_id)
        if not sku:
            raise HTTPException(
                status_code=404,
                detail=f"SKU '{sku_key}' not found in scenario '{scenario_id}'"
            )

        # Get all demands for this SKU
        demands = supply.get_all_demands(scenario_id)
        relevant_demands = [
            d for d in demands 
            if d.sku == sku_key and d.request_date == request.period_start_date
        ]

        if not relevant_demands:
            return []

        # Create landed cost calculator
        landed_cost = supply.LandedCost.new()
        
        # Calculate landed costs for each relevant demand
        for demand in relevant_demands:
            landed_cost.calculate_for_demand(demand)
        
        # Convert landed cost rows to response model
        landed_cost_rows = []
        for row in landed_cost.get_rows():
            landed_cost_rows.append(LandedCostRow(
                product_location=row.source_sku_name,
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to calculate landed cost: {str(e)}"
        )
