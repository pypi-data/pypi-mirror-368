...
def run(
    ...
    scenario_ids = []
    if "execution_summary_report" in inputs and inputs["execution_summary_report"] is not None:
        execution_summary_df = inputs["execution_summary_report"]
        scenario_ids = execution_summary_df.get_column("auto_scenario_id").unique().to_list()
        print(f"Found {len(scenario_ids)} scenarios in execution summary: {scenario_ids}")
    else:
        print("Warning: execution_summary_report not found or is None. Using 'Base' as the only scenario.")
        scenario_ids = ["Base"]
...
if __name__ == "__main__":
    ...
    processor, consumption_df, production_df, period_df, demand_df, on_hand_df, lane_details_df, loc_prod_df, prod_line_util_df, locations_df, procurement_df = load_input_data(dataset_path, scenario_id)
