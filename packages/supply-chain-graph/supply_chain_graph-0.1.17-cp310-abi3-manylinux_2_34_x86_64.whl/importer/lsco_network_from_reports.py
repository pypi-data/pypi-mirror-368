...
class ProcessNetworkLSCOReports:
    ...
    def create_operation_plans(self, operations: dict, prod_df: pl.DataFrame, period_df: pl.DataFrame, transport_df: pl.DataFrame, skus: dict, scenario_id: str) -> dict:
...
def summarize_landed_costs(landed_costs_df: pl.DataFrame) -> pl.DataFrame:
    """Create a summary of total costs across all records.
    
    Args:
        landed_costs_df: DataFrame containing detailed landed cost data
        
    Returns:
        pl.DataFrame: Two-column DataFrame with Cost type and total Value
    """
    ...
    costs = [
        ...
        ('Total Demand', 'target_quantity')
    ]
    
    # Calculate sums after filtering out infinite values
    values = [
        f"{landed_costs_df.filter(pl.col(col).is_finite())[col].sum():,.3f}" 
        for _, col in costs
    ]
    
    # Create summary DataFrame
    summary_df = pl.DataFrame({
        'Cost': [name for name, _ in costs],
        'Value': values
    })
...
def convert_landed_costs_to_df(landed_costs):
    ...
    Args:
        landed_costs: List of landed cost objects
...
def load_input_data(dataset_path, auto_scenario_id, enable_landed_cost=True):
    ...
    return processor, consumption_df, production_df, period_df, demand_df, on_hand_df, lane_details_df, loc_prod_df, prod_line_util_df, locations_df, procurement_df
...
def load_scenario(processor, consumption_df, production_df, period_df, demand_df, on_hand_df, lane_details_df, loc_prod_df, prod_line_util_df, locations_df, procurement_df, auto_scenario_id, enable_landed_cost=False):
    ...
    period_dict = processor.create_operation_plans(operations, production_df, period_df, lane_details_df, skus, scenario_id=auto_scenario_id)
    ...
    if processor.enable_landed_cost:
        ...
        landed_costs = calculate_landed_costs(auto_scenario_id)
        time_end = time.time()
