...
export const ModuleUI: React.FC<ModuleProps<PeptideInput, PeptideOutput>> = ({
  input,
  onExecute,
  onInputChange,
  executionStatus,
    ...
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      {result && executionStatus === 'completed' && (
        <>
          ...
          >
            <Descriptions bordered column={2}>
              ...
              <Descriptions.Item label="Length">
                {result.analysis.length} aa
              </Descriptions.Item>
              <Descriptions.Item label="Molecular Weight">
                {result.analysis.molecular_weight} Da
              </Descriptions.Item>
              <Descriptions.Item label="Hydrophobicity">
                <Progress 
                  percent={result.analysis.hydrophobicity} 
                  size="small"
                  strokeColor="#1890ff"
                />
              </Descriptions.Item>
              <Descriptions.Item label="Charge Ratio">
                <Progress 
                  percent={result.analysis.charge_ratio} 
                  size="small"
                  strokeColor="#52c41a"
                />
              </Descriptions.Item>
              <Descriptions.Item label="Hydrophobic Residues">
                {result.analysis.hydrophobic_residues}
              </Descriptions.Item>
              <Descriptions.Item label="Charged Residues">
                {result.analysis.charged_residues}
              </Descriptions.Item>
            </Descriptions>
            ...
            <div style={{ marginTop: '20px' }}>
              ...
              <Space wrap>
                {Object.entries(result.analysis.composition).map(([aa, count]) => (
          ...
          <Card title="HLA Binding Prediction">
            <Row gutter={16}>
              <Col span={6}>
                <Statistic 
                  title="HLA Allele" 
                  value={result.predictions.hla_allele}
              ...
              <Col span={6}>
                <Statistic 
                  title="Binding Score" 
                  value={result.predictions.score}
                  suffix="/ 100"
                  precision={1}
                />
              </Col>
              <Col span={6}>
                <Statistic 
                  ...
                  value={result.predictions.percentile_rank}
                  suffix="%"
                  precision={1}
                />
              </Col>
              <Col span={6}>
                <div style={{ marginTop: '4px' }}>
                  ...
                  <div style={{ marginTop: '8px' }}>
                    {renderBindingTag(result.predictions.binding_class)}
                  </div>
                </div>
              </Col>
            </Row>
          ...
          {/* Similar Peptides */}
          {result.similar_peptides && result.similar_peptides.length > 0 && (
            <Card title={`Similar Peptides (${result.similar_peptides.length})`}>
              <Table
                columns={columns}
                dataSource={result.similar_peptides}
                rowKey="sequence"
