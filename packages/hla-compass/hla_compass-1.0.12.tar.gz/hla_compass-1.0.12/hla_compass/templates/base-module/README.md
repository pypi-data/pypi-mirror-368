...
# Option A: Use the installation helper (recommended)
./install-deps.sh
...
#### Troubleshooting Python 3.13 on macOS (Apple Silicon)

If you encounter issues installing pandas/numpy on Python 3.13 with Apple Silicon:

1. **Use the installation helper script**:
   ```bash
   ./install-deps.sh
   ```

2. **Or manually install updated versions**:
   ```bash
   pip install numpy>=1.26.0 pandas>=2.2.3
   ```
...
4. **Lightweight alternative (Polars)**:
   ```bash
   pip install polars  # Faster alternative to pandas
   ```
