...
class SymbolicRegressionBoosting:
    def __init__(self, n_boosting_rounds=3, start_round=1, base_features=None, output_dir="boosting_results"):
        """
        Modified initialization to support starting from a specific round
        
        Args:
            n_boosting_rounds (int): Total number of boosting rounds
            start_round (int): Which round to start from (1, 2, or 3)
            base_features (list): Initial features
    ...
    def save_round_predictions(self, round_num, train_pred, test_pred, train_mae, test_mae):
        """Save predictions and metrics for a round"""
        round_dir = os.path.join(self.output_dir, f"round_{round_num}")
        predictions = {
    ...
    def create_pysr_model(self, round_num):
        ...
        model_params = {
            'binary_operators': ["+", "*", "-", "/"],
            'unary_operators': ["exp", "square", "abs", "log10", "ceil", "floor", "cube"],
