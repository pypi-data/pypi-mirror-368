...
class SymbolicRegressionBoosting:
    def __init__(self, n_boosting_rounds=3, start_round=1, base_features=None, output_dir="boosting_results"):
        """Modified initialization to remove first round feature constraints"""
        self.n_boosting_rounds = n_boosting_rounds
        self.start_round = start_round
        self.boosted_models = []
        self.used_features = set()
        
        # Store all features and initialize available features
        self.all_features = base_features.copy() if base_features else []
        self.available_features = self.all_features.copy()  # All features available from start
        ...
        print(f"Available features for current round: {len(self.available_features)}")
    ...
    def load_previous_rounds_data(self, last_round):
        ...
        for round_num in range(1, last_round + 1):
            ...
            self.available_features = list(set(self.all_features) - self.used_features)  # Fixed: using all_features instead of base_features
    ...
    def save_round_predictions(self, round_num, train_pred, valid_pred, test_pred, train_mae, valid_mae, test_mae):
        """Modified to include validation set predictions"""
        round_dir = os.path.join(self.output_dir, f"round_{round_num}")
        ...
        # Save validation data and predictions
        valid_df = pd.DataFrame({
            'ground_truth': self.y_valid,
            'prediction': valid_pred,
            'cumulative_prediction': self.predictions_valid + valid_pred
        })
        valid_df.to_csv(os.path.join(round_dir, f"valid_{round_num}.csv"), index=False)
    ...
    def update_feature_pools(self, new_used_features, round_number):
        """Updated to remove special handling of first round"""
    ...
    def fit(self, X_train, y_train, X_valid, y_valid, X_test, y_test):
        """Modified fit function to use all features from start"""
        self.y_train = y_train
        ...
        if os.path.exists(round_dir) and pkl_files and not os.path.exists(predictions_file):
            ...
            if self.regenerate_round_files(self.start_round, X_train, y_train, X_valid, y_valid, X_test, y_test):
        ...
        if self.start_round == 1:
            self.predictions_train = np.zeros_like(y_train, dtype=float)
            self.predictions_valid = np.zeros_like(y_valid, dtype=float)
            self.predictions_test = np.zeros_like(y_test, dtype=float)
        else:
            # Load cumulative predictions from previous rounds
            self.predictions_train = np.sum(self.previous_predictions_train, axis=0)
            self.predictions_valid = np.sum(self.previous_predictions_valid, axis=0)
            self.predictions_test = np.sum(self.previous_predictions_test, axis=0)
        
        # Initial targets
        current_y_train = y_train - self.predictions_train
        current_y_valid = y_valid - self.predictions_valid
        current_y_test = y_test - self.predictions_test
        
        tracked_samples = range(min(5, len(y_train)))
        previous_mae = None if self.start_round == 1 else self.boosted_models[-1]['valid_mae']
        ...
        for round in range(self.start_round - 1, self.n_boosting_rounds):
            ...
            print(f"Available features for round {round_num}: {len(self.available_features)}")
                
            if round_num > 1:
                print(f"\nPrevious round Validation MAE: {previous_mae:.4f}")
            ...
            try:
                ...
                # Use all available features for every round
                X_train_round = X_train[self.available_features]
                X_valid_round = X_valid[self.available_features]
                X_test_round = X_test[self.available_features]
                ...
                try:
                    round_pred_train = loaded_model.predict(X_train_round)
                    round_pred_valid = loaded_model.predict(X_valid_round)
                    round_pred_test = loaded_model.predict(X_test_round)
                    
                    # Save round predictions with all three sets
                    train_mae = mean_absolute_error(y_train, self.predictions_train + round_pred_train)
                    valid_mae = mean_absolute_error(y_valid, self.predictions_valid + round_pred_valid)
                    test_mae = mean_absolute_error(y_test, self.predictions_test + round_pred_test)
                    
                    # Check for MAE improvement using validation set
                    if not self.check_mae_improvement(valid_mae, previous_mae):
                        print(f"\nEarly stopping: Validation MAE improvement less than minimum threshold")
                        print(f"Current Valid MAE: {valid_mae:.4f}")
                        print(f"Previous Valid MAE: {previous_mae:.4f}")
                        break
                    
                    self.save_round_predictions(round_num, round_pred_train, round_pred_valid, 
                                             round_pred_test, train_mae, valid_mae, test_mae)
                    
                    # Update cumulative predictions
                    self.predictions_train += round_pred_train
                    self.predictions_valid += round_pred_valid
                    ...
                    # Store model information with validation metrics
                    model_info = {
                        'model': loaded_model,
                        'equation': best_equation,
                        'used_features': new_used_features,
                    ...
                    print(f"Predictions - Valid: [{round_pred_valid.min():.2f}, {round_pred_valid.max():.2f}]")
                    ...
                    # Update previous_mae for next round using validation MAE
                    previous_mae = valid_mae
...
def main():
    ...
    test_data_path = "/home/kunchapus/projects/PolyMetriX/src/polymetrix/sym_datasets/uncommon_mdpi_trained_set.csv"
    ...
    # Print statistics about all datasets
    print("\nTraining Data Statistics:")
    print(f"Tg range: [{y_train_final.min():.2f}, {y_train_final.max():.2f}]")
    print(f"Tg mean: {y_train_final.mean():.2f}")
    print(f"Tg std: {y_train_final.std():.2f}")
    
    print("\nValidation Data Statistics:")
    print(f"Tg range: [{y_valid.min():.2f}, {y_valid.max():.2f}]")
    print(f"Tg mean: {y_valid.mean():.2f}")
    print(f"Tg std: {y_valid.std():.2f}")
    ...
    n_rounds = 1  # Set number of rounds
    start_round = 1  # Set starting round
    ...
    if start_round > 1:
        ...
        for prev_round in range(1, start_round):
            ...
            if prev_pkl and not os.path.exists(prev_predictions):
                ...
                if not boosting_model.check_round_files(prev_round, X_train_final, y_train_final, X_valid, y_valid, X_test, y_test):
    ...
    boosting_model.fit(X_train_final, y_train_final, X_valid, y_valid, X_test, y_test)
    ...
    try:
        with open(output_file, "w") as f:
            ...
            for i, model_info in enumerate(boosting_model.boosted_models, 1):
                ...
                f.write(f"Validation MAE: {model_info['valid_mae']:.4f}\n")
