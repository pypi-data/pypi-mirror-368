...
# Synchronize parameters once before returning model state_dict.
@no_type_check
...
# Collect the local sgd related state-dicts.
@no_type_check
def _local_sgd_state_dict(
    ...
    if not model._handle.use_outer_optim or model.global_step < model.local_sgd_warmup_steps:
    ...
    for fsdp_m in fsdp_modules:
        if fsdp_m._handle.outer_optimizer is None:
            fsdp_m_outer_optim_sd = {}
        else:
            fsdp_m_outer_optim_sd = fsdp_m._handle.outer_optimizer.state_dict()
        ...
        if hasattr(fsdp_m._handle, "local_sgd_anomaly_detector"):
            local_sgd_anomaly_detector_sd_l.append(fsdp_m._handle.local_sgd_anomaly_detector.state_dict())
        else:
...
# Save the local sgd related state-dicts.
@no_type_check
...
# Load the local sgd related state-dicts.
@no_type_check
def _load_local_sgd_state_dict(
    ...
    for i, fsdp_m in enumerate(fsdp_modules):
        ...
        if "outer_optim" in local_sgd_sd.keys() and local_sgd_sd["outer_optim"][i] != {}:
            _lazy_init_outer_optimizer(fsdp_m._handle, cpu_init=fsdp_m.local_sgd_cpu_offload)
            assert (
            ...
            fsdp_m._handle.outer_optimizer.load_state_dict(local_sgd_sd["outer_optim"][i])

        if "local_sgd_anomaly_detector" in local_sgd_sd.keys() and hasattr(
            fsdp_m._handle, "local_sgd_anomaly_detector"
        ):
            fsdp_m._handle.local_sgd_anomaly_detector.load_state_dict(local_sgd_sd["local_sgd_anomaly_detector"][i])
