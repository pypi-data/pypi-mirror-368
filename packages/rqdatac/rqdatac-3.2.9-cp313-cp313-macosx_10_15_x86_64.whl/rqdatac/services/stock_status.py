...
hk_share_fields = ["total_a", "total", "not_hk_shares", "preferred_shares", "authorized_shares"]
...
ANNOUNCEMENT_FIELDS = ["media", "category", "title", "language", "file_type", "info_type", "announcement_link"]
...
def get_staff_count(order_book_ids, start_date=None, end_date=None, market="cn"):
    ...
    :param order_book_ids: 股票合约 order_book_id
    :param start_date: 开始日期，使用 info_date 作为筛选条件
    :param end_date: 结束日期，使用 info_date 作为筛选条件
    """
    order_book_ids = ensure_order_book_ids(order_book_ids, market=market)
    if start_date:
        start_date = ensure_date_int(start_date)
