...
from mindscript.types import TypeChecker, TypeError
...
class Interpreter:
    ...
    def type_unary(self, node: ast.Expr):
        ...
        if (isinstance(expr, ast.TypeTerminal) 
            ...
            return expr
        elif (isinstance(expr, ast.TypeUnary)):
            return expr
        elif (isinstance(expr, ast.TypeBinary)):
            expr = ast.TypeGrouping(expr=expr)
        operator = node.operator
