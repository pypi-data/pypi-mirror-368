...
class JSONSchema():
    ...
    def type_unary(self, node, env=None, visited=None):
        obj = node.expr.accept(self, env=env, visited=visited)
        print(f"Type Unary: {obj}")
        if "type" in obj and obj["type"] == "null":
            return obj
        if "anyOf" in obj:
            if len([item for item in obj["anyOf"] if item["type"] == "null"]) == 0:
    ...
    def type_binary(self, node, env=None, visited=None):
        raise NotImplementedError("Function types do not have JSON Schemas.")
