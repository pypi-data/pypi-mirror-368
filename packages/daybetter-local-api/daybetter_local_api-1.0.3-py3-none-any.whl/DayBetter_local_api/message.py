...
M = TypeVar("M", bound=DayBetterMessage)


class ScanMessage(DayBetterMessage):
    command = "scan"
...
class DevStatusMessage(DayBetterMessage):
    command = "devStatus"
...
class StatusMessage(DayBetterMessage):
    command = "status"
...
class OnOffMessage(DayBetterMessage):
    command = "turn"
...
class BrightnessMessage(DayBetterMessage):
    command = "brightness"
...
class ColorMessage(DayBetterMessage):
...
class PtRealMessage(DayBetterMessage):
    command = "ptReal"
...
class ScanResponse(DayBetterMessage):
    command = "scan"
...
class DevStatusResponse(DayBetterMessage):
    command = "devStatus"
...
class StatusResponse(DayBetterMessage):
    command = "status"
...
class MessageResponseFactory:
    def __init__(self) -> None:
        self._messages: set[type[DayBetterMessage]] = {
            ScanResponse,
            DevStatusResponse,
            StatusResponse,
        }

    def create_message(self, data: bytes | bytearray | str) -> DayBetterMessage | None:
        ...
        message: type[DayBetterMessage] = next(
