...
from .device import DayBetterDevice
from .device_registry import DeviceRegistry
from .light_capabilities import (
    DayBetter_LIGHT_CAPABILITIES,
    ON_OFF_CAPABILITIES,
    DayBetterLightFeatures,
)
from .message import (
    HexMessage,
    BrightnessMessage,
...
class DayBetterController:
    def __init__(
        ...
        discovered_callback: Callable[[DayBetterDevice, bool], bool] | None = None,
        evicted_callback: Callable[[DayBetterDevice], None] | None = None,
        logger: logging.Logger | None = None,
    ) -> None:
        """Build a controller that handle DayBetter devices that support local API on local network.
        ...
        Args:
            ...
            discovered_callback (Callable[DayBetterDevice, bool]): An optional function to call when a device is discovered (or rediscovered). Default None
            evicted_callback (Callable[DayBetterDevice]): An optional function to call when a device is evicted.
        """
    ...
    def remove_device(self, device: str | DayBetterDevice) -> None:
        if isinstance(device, DayBetterDevice):
    ...
    def set_device_discovered_callback(
        self, callback: Callable[[DayBetterDevice, bool], bool] | None
    ) -> Callable[[DayBetterDevice, bool], bool] | None:
    ...
    async def turn_on_off(self, device: DayBetterDevice, status: bool) -> None:
        self._send_message(OnOffMessage(status), device)

    async def set_segment_rgb_color(
        self, device: DayBetterDevice, segment: int, rgb: tuple[int, int, int]
    ) -> None:
        if not device.capabilities:
            self._logger.warning("Capabilities not available for device %s", device)
            return

        if device.capabilities.features & DayBetterLightFeatures.SEGMENT_CONTROL == 0:
    ...
    async def set_scene(self, device: DayBetterDevice, scene: str) -> None:
        if (
            ...
            or device.capabilities.features & DayBetterLightFeatures.SCENES == 0
        ):
    ...
    async def send_raw_command(self, device: DayBetterDevice, command: str) -> None:
        self._send_message(HexMessage([command]), device)

    def get_device_by_ip(self, ip: str) -> DayBetterDevice | None:
        return self._registry.get_device_by_ip(ip)

    def get_device_by_sku(self, sku: str) -> DayBetterDevice | None:
    ...
    async def _handle_scan_response(self, message: ScanResponse) -> None:
        ...
        if device := self.get_device_by_fingerprint(fingerprint):
            ...
            capabilities = DayBetter_LIGHT_CAPABILITIES.get(message.sku, None)
            if not capabilities:
    ...
    def _call_discovered_callback(self, device: DayBetterDevice, is_new: bool) -> bool:
        if not self._device_discovered_callback:
            return True
        return self._device_discovered_callback(device, is_new)

    def _send_message(self, message: DayBetterMessage, device: DayBetterDevice) -> None:
        self._transport.sendto(bytes(message), (device.ip, self._device_command_port))

    def _send_update_message(self, device: DayBetterDevice):
