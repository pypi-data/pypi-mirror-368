...
async def animate(stop_event: asyncio.Event, final_message: str, prefix: str = "", interval: float = 0.3, animation_type: str = "ellipsis",) -> None:
    """
    Async console animation.

    Args:
        stop_event (asyncio.Event): Event to stop the animation.
        final_message (str): Message to print after animation is stopped.
    ...
    assert isinstance(final_message, str), "final_message must be a string"
    ...
    max_frame_len = max(len(f) for f in frames)
    total_len = len(prefix) + max_frame_len

    while not stop_event.is_set():
        frame = frames[i]
        print(f"\r{prefix}{frame}{' ' * (max_frame_len - len(frame))}", end="", flush=True)
        i = (i + 1) % len(frames)
        try:
    ...
    print("\r" + " " * total_len + "\r" + final_message, end="", flush=True)
