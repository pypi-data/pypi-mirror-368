"""If you use the OpenAI Python SDK, you can use the lighthouse drop-in replacement to get full logging by changing only the import.

```diff
- import openai
+ from lighthouse.openai import openai
```

lighthouse automatically tracks:
...
See docs for more details: https://lighthouse.com/docs/integrations/openai
"""
...
from lighthouse._client.get_client import get_client
from lighthouse._client.span import lighthouseGeneration
from lighthouse._utils import _get_timestamp
from lighthouse.media import lighthouseMedia
...
class OpenAiArgsExtractor:
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.args["name"] = name
        self.args["lighthouse_public_key"] = lighthouse_public_key
        self.args["lighthouse_prompt"] = lighthouse_prompt
        self.args["trace_id"] = trace_id
        self.args["parent_observation_id"] = parent_observation_id

        self.kwargs = kwargs

    def get_lighthouse_args(self) -> Any:
...
def _extract_chat_prompt(kwargs: Any) -> Any:
    ...
    if prompt:
        # uf user provided functions, we need to send these together with messages to lighthouse
        prompt.update(
            {
                "messages": [
        ...
        # vanilla case, only send messages in openai format to lighthouse
                ...
                processed_content.append(
                    {
                        "type": "input_audio",
                        "input_audio": {
                            "data": lighthouseMedia(base64_data_uri=base64_data_uri),
                            "format": format,
                        },
                    }
                )
...
def _extract_chat_response(kwargs: Any) -> Any:
    ...
    if kwargs.get("audio") is not None:
        ...
        if "data" in audio and audio["data"] is not None:
            ...
            audio["data"] = lighthouseMedia(base64_data_uri=base64_data_uri)
...
def _get_lighthouse_data_from_kwargs(resource: OpenAiDefinition, kwargs: Any) -> Any:
    ...
    lighthouse_public_key = kwargs.get("lighthouse_public_key", None)
    if lighthouse_public_key is not None and not isinstance(lighthouse_public_key, str):
        raise TypeError("lighthouse_public_key must be a string")
    ...
    lighthouse_prompt = kwargs.get("lighthouse_prompt", None)
    ...
    return {
        ...
        "model": model or None,
        "prompt": lighthouse_prompt,
    }


def _create_lighthouse_update(
    completion: Any,
    generation: lighthouseGeneration,
...
def _get_lighthouse_data_from_default_response(
...
def _wrap(
    ...
    lighthouse_args = arg_extractor.get_lighthouse_args()

    lighthouse_data = _get_lighthouse_data_from_kwargs(open_ai_resource, lighthouse_args)
    lighthouse_client = get_client(public_key=lighthouse_args["lighthouse_public_key"])

    generation = lighthouse_client.start_generation(
        name=lighthouse_data["name"],
        input=lighthouse_data.get("input", None),
        metadata=lighthouse_data.get("metadata", None),
        model_parameters=lighthouse_data.get("model_parameters", None),
        trace_context={
            "trace_id": cast(str, lighthouse_data.get("trace_id", None)),
            "parent_span_id": cast(
                str, lighthouse_data.get("parent_observation_id", None)
            ),
        },
        model=lighthouse_data.get("model", None),
        prompt=lighthouse_data.get("prompt", None),
    )
    ...
    try:
        ...
        if _is_streaming_response(openai_response):
            return lighthouseResponseGeneratorSync(
            ...
            model, completion, usage = _get_lighthouse_data_from_default_response(
                open_ai_resource,
...
async def _wrap_async(
    ...
    try:
        ...
        if _is_streaming_response(openai_response):
            return lighthouseResponseGeneratorAsync(
                resource=open_ai_resource,
                response=openai_response,
                generation=generation,
            )
...
class lighthouseResponseGeneratorSync:
    def __init__(
        self,
        *,
        resource: Any,
...
class lighthouseResponseGeneratorAsync:
    def __init__(
        self,
        *,
        resource: Any,
