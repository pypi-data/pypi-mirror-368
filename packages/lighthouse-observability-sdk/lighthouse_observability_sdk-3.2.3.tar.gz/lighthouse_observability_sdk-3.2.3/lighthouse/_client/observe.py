...
from lighthouse._client.get_client import _set_current_public_key, get_client
from lighthouse._client.span import lighthouseGeneration, lighthouseSpan
from lighthouse.types import TraceContext
...
class lighthouseDecorator:
    """Implementation of the @observe decorator for seamless lighthouse tracing integration.

    This class provides the core functionality for the @observe decorator, which enables
    automatic tracing of functions and methods in your application with lighthouse.
    It handles both synchronous and asynchronous functions, maintains proper trace context,
    and intelligently routes to the correct lighthouse client instance.
    ...
    - Thread-safe client resolution when multiple lighthouse projects are used
    """

    _log = logging.getLogger("lighthouse")
    ...
    def observe(
        ...
        *,
        ...
        """Wrap a function to create and manage lighthouse tracing around its execution, supporting both synchronous and asynchronous functions.

        This decorator provides seamless integration of lighthouse observability into your codebase. It automatically creates
        ...
        Returns:
            Callable: A wrapped version of the original function that automatically creates and manages lighthouse spans.
        ...
        Notes:
            ...
            - Special keyword arguments can be passed to control tracing:
              - lighthouse_trace_id: Explicitly set the trace ID for this function call
              - lighthouse_parent_observation_id: Explicitly set the parent span ID
              - lighthouse_public_key: Use a specific lighthouse project (when multiple clients exist)
            - For async functions, the decorator returns an async function wrapper.
            - For sync functions, the decorator returns a synchronous wrapper.
        """
        function_io_capture_enabled = os.environ.get(
            lighthouse_OBSERVE_DECORATOR_IO_CAPTURE_ENABLED, "True"
    ...
    def _async_observe(
        ...
        *,
        ...
        async def async_wrapper(*args: Tuple[Any], **kwargs: Dict[str, Any]) -> Any:
            trace_id = cast(str, kwargs.pop("lighthouse_trace_id", None))
            parent_observation_id = cast(
                str, kwargs.pop("lighthouse_parent_observation_id", None)
            )
            ...
            public_key = cast(str, kwargs.pop("lighthouse_public_key", None))

            # Set public key in execution context for nested decorated functions
            with _set_current_public_key(public_key):
                lighthouse_client = get_client(public_key=public_key)
                context_manager: Optional[
                    ...
                    (
                        lighthouse_client.start_as_current_generation(
                            name=final_name,
                        ...
                        else lighthouse_client.start_as_current_span(
                            name=final_name,
                            trace_context=trace_context,
                            input=input,
                            end_on_exit=False,  # when returning a generator, closing on exit would be to early
                ...
                with context_manager as lighthouse_span_or_generation:
                    ...
                    try:
                        ...
                        if capture_output is True:
                            ...
                            lighthouse_span_or_generation.update(output=result)

                        return result
                    except Exception as e:
                        lighthouse_span_or_generation.update(
                        ...
                        if not is_return_type_generator:
                            lighthouse_span_or_generation.end()
    ...
    def _sync_observe(
        ...
        *,
        ...
        def sync_wrapper(*args: Any, **kwargs: Any) -> Any:
            trace_id = kwargs.pop("lighthouse_trace_id", None)
            parent_observation_id = kwargs.pop("lighthouse_parent_observation_id", None)
            trace_context: Optional[TraceContext] = (
                {
                    "trace_id": trace_id,
                    "parent_span_id": parent_observation_id,
            ...
            public_key = kwargs.pop("lighthouse_public_key", None)
    ...
    def _wrap_sync_generator_result(
        self,
        lighthouse_span_or_generation: Union[lighthouseSpan, lighthouseGeneration],
        generator: Generator,
        transform_to_string: Optional[Callable[[Iterable], str]] = None,
    ) -> Any:
        items = []
        ...
        try:
            ...
            lighthouse_span_or_generation.update(output=output)
            lighthouse_span_or_generation.end()
...
_decorator = lighthouseDecorator()
