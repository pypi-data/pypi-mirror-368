...
from lighthouse.langchain.utils import _extract_model_name
...
try:
    ...
    raise ModuleNotFoundError(
        "Please install langchain to use the lighthouse langchain integration: 'pip install langchain'"
    )
...
class LangchainCallbackHandler(LangchainBaseCallbackHandler):
    def __init__(self, *, public_key: Optional[str] = None) -> None:
        ...
        self.runs: Dict[UUID, Union[lighthouseSpan, lighthouseGeneration]] = {}
    ...
    def on_llm_new_token(
        ...
        **kwargs: Any,
        ...
        if (
            run_id in self.runs
            and isinstance(self.runs[run_id], lighthouseGeneration)
            and run_id not in self.updated_completion_start_time_memo
        ):
            current_generation = cast(lighthouseGeneration, self.runs[run_id])
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            self._register_lighthouse_prompt(
            ...
            if parent_run_id is None:
                ...
                self.runs[run_id] = cast(
                    lighthouseSpan, self.runs[parent_run_id]
                ).start_span(
                    name=span_name,
                    metadata=span_metadata,
                    input=inputs,
    ...
    def _register_lighthouse_prompt(
        self,
        *,
        run_id: UUID,
        ...
        """We need to register any passed lighthouse prompt to the parent_run_id so that we can link following generations with that prompt.
        ...
        lighthouse_prompt = metadata and metadata.get("lighthouse_prompt", None)

        if lighthouse_prompt:
            self.prompt_to_parent_run_map[parent_run_id] = lighthouse_prompt
    ...
    def _deregister_lighthouse_prompt(self, run_id: Optional[UUID]) -> None:
    ...
    def on_chain_end(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            self._deregister_lighthouse_prompt(run_id)
        except Exception as e:
            lighthouse_logger.exception(e)
    ...
    def on_tool_start(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            self.runs[run_id] = cast(lighthouseSpan, self.runs[parent_run_id]).start_span(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if registered_prompt:
                self._deregister_lighthouse_prompt(parent_run_id)
    ...
    def _log_model_parse_warning(self) -> None:
        if not hasattr(self, "_model_parse_warning_logged"):
            lighthouse_logger.warning(
                "lighthouse was not able to parse the LLM model. The LLM call will be recorded without model name. Please create an issue: https://github.com/lighthouse/lighthouse/issues/new/choose"
            )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if run_id not in self.runs:
                ...
                lighthouse_generation = cast(lighthouseGeneration, self.runs[run_id])
                lighthouse_generation.update(
    ...
    def __join_tags_and_metadata(
        ...
        return _strip_lighthouse_keys_from_dict(final_dict) if final_dict != {} else None
...
def _parse_usage_model(usage: typing.Union[pydantic.BaseModel, dict]) -> Any:
    ...
    if not all(
        ...
        for model_key, lighthouse_key in conversion_list:
...
def _strip_lighthouse_keys_from_dict(metadata: Optional[Dict[str, Any]]) -> Any:
    if metadata is None or not isinstance(metadata, dict):
        return metadata

    lighthouse_metadata_keys = ["lighthouse_prompt"]

    metadata_copy = metadata.copy()

    for key in lighthouse_metadata_keys:
