"""Tracer implementation for lighthouse OpenTelemetry integration.

This module provides the lighthouseTracer class, a thread-safe singleton that manages OpenTelemetry
tracing infrastructure for lighthouse. It handles tracer initialization, span processors,
API clients, and coordinates background tasks for efficient data processing and media handling.

Key features:
- Thread-safe OpenTelemetry tracer with lighthouse-specific span processors and sampling
...
from lighthouse._client.environment_variables import (
    lighthouse_MEDIA_UPLOAD_ENABLED,
    lighthouse_MEDIA_UPLOAD_THREAD_COUNT,
    lighthouse_RELEASE,
    lighthouse_TRACING_ENVIRONMENT,
)
from lighthouse._client.span_processor import lighthouseSpanProcessor
from lighthouse._task_manager.media_manager import MediaManager
from lighthouse._task_manager.media_upload_consumer import MediaUploadConsumer
from lighthouse._task_manager.score_ingestion_consumer import ScoreIngestionConsumer
from lighthouse._utils.environment import get_common_release_envs
from lighthouse._utils.prompt_cache import PromptCache
from lighthouse._utils.request import lighthouseClient
from lighthouse.api.client import AsyncFernlighthouse, Fernlighthouse
from lighthouse.logger import lighthouse_logger
from lighthouse.types import MaskFunction

from ..version import __version__ as lighthouse_version


class lighthouseResourceManager:
    ...
    2. Managing the span processor for exporting spans to the lighthouse API
    3. Creating and managing lighthouse API clients (both synchronous and asynchronous)
    ...
    _instances: Dict[str, "lighthouseResourceManager"] = {}
    ...
    ) -> "lighthouseResourceManager":
        # Use a stable key for singleton instances. Prefer api_key, else public_key.
        instance_key = api_key or (public_key or "__no_key__")
        if instance_key in cls._instances:
            return cls._instances[instance_key]

        with cls._lock:
            if instance_key not in cls._instances:
                instance = super(lighthouseResourceManager, cls).__new__(cls)

                # Initialize tracer (will be noop until init instance)
                instance._otel_tracer = otel_trace_api.get_tracer(
                    lighthouse_TRACER_NAME,
                    lighthouse_version,
                    attributes={"public_key": public_key, "api_key": bool(api_key)},
                )
    ...
    def _initialize_instance(
        ...
        *,
        ...
        if tracing_enabled:
            ...
            lighthouse_processor = lighthouseSpanProcessor(
            ...
            tracer_provider.add_span_processor(lighthouse_processor)
        ...
        # API Clients
        # Prepare base headers for HTTPX client
        base_headers: Dict[str, str] = dict(self._additional_headers)
        if self.api_key:
            base_headers["X-API-KEY"] = self.api_key
        else:
            # fall back to Supabase JWT if present for control-plane calls
        ...
        if httpx_client is not None:
            ...
            # Do not mutate external client headers here to avoid side effects
        else:
            # Create a new httpx client with base and additional headers
            self.httpx_client = httpx.Client(timeout=timeout, headers=base_headers)
        # Create a matching async client for async API usage
        self.async_httpx_client = httpx.AsyncClient(timeout=timeout, headers=base_headers)

        # Construct Fern clients. If api_key is present, do not use basic auth
        self.api = Fernlighthouse(
            base_url=host,
            username=None if self.api_key else self.public_key,
            password=None if self.api_key else self.secret_key,
            x_lighthouse_sdk_name="python",
            x_lighthouse_sdk_version=lighthouse_version,
            x_lighthouse_public_key=self.public_key if self.public_key else None,
            httpx_client=self.httpx_client,
            timeout=timeout,
        )
        self.async_api = AsyncFernlighthouse(
            base_url=host,
            username=None if self.api_key else self.public_key,
            password=None if self.api_key else self.secret_key,
            x_lighthouse_sdk_name="python",
        ...
        # Score ingestion client keeps legacy headers for batch ingestion path
        score_ingestion_client = lighthouseClient(
            public_key=self.public_key,
            secret_key=self.secret_key,
            base_url=host,
        ...
        self._media_upload_enabled = os.environ.get(
            lighthouse_MEDIA_UPLOAD_ENABLED, "True"
        ...
        media_upload_thread_count = media_upload_thread_count or max(
            int(os.getenv(lighthouse_MEDIA_UPLOAD_THREAD_COUNT, 1)), 1
        )
        ...
        lighthouse_logger.info(
            f"Startup: lighthouse tracer successfully initialized | "
            f"public_key={self.public_key} | api_key={'set' if bool(self.api_key) else 'unset'} | "
            f"host={host} | "
    ...
    def flush(self) -> None:
        ...
        lighthouse_logger.debug("Successfully flushed OTEL tracer provider")

        self._score_ingestion_queue.join()
        lighthouse_logger.debug("Successfully flushed score ingestion queue")

        self._media_upload_queue.join()
        lighthouse_logger.debug("Successfully flushed media upload queue")
...
def _init_tracer_provider(
    *,
    ...
    environment = environment or os.environ.get(lighthouse_TRACING_ENVIRONMENT)
    release = release or os.environ.get(lighthouse_RELEASE) or get_common_release_envs()

    resource_attributes = {
        lighthouseOtelSpanAttributes.ENVIRONMENT: environment,
