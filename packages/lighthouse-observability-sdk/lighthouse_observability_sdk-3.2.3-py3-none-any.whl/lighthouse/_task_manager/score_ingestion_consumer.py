...
from lighthouse._utils.parse_error import handle_exception
from lighthouse._utils.request import APIError, lighthouseClient
from lighthouse._utils.serializer import EventSerializer

MAX_EVENT_SIZE_BYTES = int(os.environ.get("lighthouse_MAX_EVENT_SIZE_BYTES", 1_000_000))
MAX_BATCH_SIZE_BYTES = int(os.environ.get("lighthouse_MAX_BATCH_SIZE_BYTES", 2_500_000))
...
class ScoreIngestionConsumer(threading.Thread):
    ...
    def _upload_batch(self, batch: List[Any]) -> None:
        self._log.debug(
            f"API: Uploading batch of {len(batch)} score events to lighthouse API"
        )

        metadata = ScoreIngestionMetadata(
            batch_size=len(batch),
            sdk_name="python",
            sdk_version=lighthouse_version,
        ...
        self._log.debug(
            f"API: Successfully sent {len(batch)} score events to lighthouse API in batch mode"
        )
