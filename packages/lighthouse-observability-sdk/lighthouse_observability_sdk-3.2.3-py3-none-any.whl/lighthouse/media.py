"""This module contains the lighthouseMedia class, which is used to wrap media objects for upload to lighthouse."""
...
from lighthouse.api import MediaContentType
from lighthouse.types import ParsedMediaReference

T = TypeVar("T")


class lighthouseMedia:
    """A class for wrapping media objects for upload to lighthouse.

    This class handles the preparation and formatting of media content for lighthouse,
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a lighthouseMedia object.
        ...
        if base64_data_uri is not None:
            ...
            self._log.error(
                "base64_data_uri, or content_bytes and content_type, or file_path must be provided to lighthouseMedia"
            )
    ...
    def _reference_string(self) -> Optional[str]:
        ...
        return f"@@@lighthouseMedia:type={self._content_type}|id={self._media_id}|source={self._source}@@@"
    ...
    def parse_reference_string(reference_string: str) -> ParsedMediaReference:
        ...
        Example reference string:
            "@@@lighthouseMedia:type=image/jpeg|id=some-uuid|source=base64_data_uri@@@"
        ...
        Raises:
            ...
            ValueError: If the reference string does not start with "@@@lighthouseMedia:type=".
        ...
        if not reference_string.startswith("@@@lighthouseMedia:type="):
            raise ValueError(
                "Reference string does not start with '@@@lighthouseMedia:type='"
            )

        if not reference_string.endswith("@@@"):
            raise ValueError("Reference string does not end with '@@@'")

        content = reference_string[len("@@@lighthouseMedia:") :].rstrip("@@@")
    ...
    def resolve_media_references(
        *,
        ...
        in the format "@@@lighthouseMedia:...@@@". When found, it (synchronously) fetches the actual media content using
        the provided lighthouse client and replaces the reference string with a base64 data URI.
        ...
        Args:
            ...
            lighthouse_client: lighthouse client instance used to fetch media content.
        ...
        Example:
            obj = {
                "image": "@@@lighthouseMedia:type=image/jpeg|id=123|source=bytes@@@",
                "nested": {
                    "pdf": "@@@lighthouseMedia:type=application/pdf|id=456|source=bytes@@@"
                }
            }

            result = await lighthouseMedia.resolve_media_references(obj, lighthouse_client)
        ...
        def traverse(obj: Any, depth: int) -> Any:
            ...
            if isinstance(obj, str):
                ...
                for reference_string in reference_string_matches:
                    try:
                        parsed_media_reference = lighthouseMedia.parse_reference_string(
                            reference_string
                        )
                        media_data = lighthouse_client.api.media.get(
