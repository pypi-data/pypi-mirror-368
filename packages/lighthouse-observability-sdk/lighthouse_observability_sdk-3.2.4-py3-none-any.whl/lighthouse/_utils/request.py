...
class lighthouseClient:
    ...
    def generate_headers(self) -> dict:
        headers: dict = {
            ...
            "x_lighthouse_sdk_version": self._version,
        }
        # Prefer API key if available via environment or pre-configured on session
        api_key = os.getenv("LIGHTHOUSE_API_KEY")
        if api_key or self._session.headers.get("X-API-KEY"):
            headers["X-API-KEY"] = api_key or self._session.headers.get("X-API-KEY")
        else:
            headers.update(
                {
                    "Authorization": "Basic "
                    ...
                    "x_lighthouse_public_key": self._public_key,
                }
            )
        return headers
