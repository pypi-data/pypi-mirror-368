...
class TestCoreNoteOperations:
    ...
    def test_get_task_notes_basic(self, core_with_project_and_task: Any) -> None:
        ...
        # Get all notes
        notes = core.get_task_notes(task_id=task.id)
    ...
    def test_get_task_notes_with_filters(self, core_with_project_and_task: Any) -> None:
        ...
        # Test type filtering
        user_notes = core.get_task_notes(task_id=task.id, note_type=NoteType.USER)
        assert len(user_notes) == 2

        system_notes = core.get_task_notes(task_id=task.id, note_type=NoteType.SYSTEM)
        assert len(system_notes) == 1

        # Test author filtering
        alice_notes = core.get_task_notes(task_id=task.id, author="alice")
        assert len(alice_notes) == 1
        assert alice_notes[0]["content"] == "User note 1"

        # Test limit
        limited_notes = core.get_task_notes(task_id=task.id, limit=2)
    ...
    def test_get_task_notes_with_tags(self, core_with_project_and_task: Any) -> None:
        ...
        # Test tag filtering
        important_notes = core.get_task_notes(task_id=task.id, tag="important")
        assert len(important_notes) == 1
        assert important_notes[0]["content"] == "Important note"

        urgent_notes = core.get_task_notes(task_id=task.id, tag="urgent")
        assert len(urgent_notes) == 1

        regular_notes = core.get_task_notes(task_id=task.id, tag="regular")
    ...
    def test_get_task_notes_date_filtering(
        ...
        recent_notes = core.get_task_notes(task_id=task.id, since=yesterday)
        assert len(recent_notes) == 1

        # Test since filtering (should exclude the note)
        future_notes = core.get_task_notes(task_id=task.id, since=tomorrow)
        assert len(future_notes) == 0

        # Test until filtering (should include the note)
        past_notes = core.get_task_notes(task_id=task.id, until=tomorrow)
    ...
    def test_search_task_notes_basic(self, core_with_project_and_task: Any) -> None:
        ...
        # Test content search
        bug_notes = core.search_task_notes(task_id=task.id, query="bug")
        assert len(bug_notes) == 2

        feature_notes = core.search_task_notes(task_id=task.id, query="feature")
        assert len(feature_notes) == 1

        # Test case insensitive search
        case_notes = core.search_task_notes(task_id=task.id, query="BUG")
    ...
    def test_search_task_notes_with_filters(
        ...
        alice_bug_notes = core.search_task_notes(
            task_id=task.id, query="bug", author="alice"
        )
        ...
        limited_notes = core.search_task_notes(task_id=task.id, query="bug", limit=2)
        assert len(limited_notes) == 2
    ...
    def test_search_task_notes_empty_results(
        ...
        results = core.search_task_notes(task_id=task.id, query="nonexistent")
    ...
    def test_search_task_notes_markdown_content(
        ...
        bold_notes = core.search_task_notes(task_id=task.id, query="important")
        assert len(bold_notes) == 1

        code_notes = core.search_task_notes(task_id=task.id, query="code")
    ...
    def test_multiple_notes_chronological_order(
        ...
        core.add_task_note(task_id=task.id, content="First note")
        time.sleep(0.01)  # Small delay to ensure different timestamps
        core.add_task_note(task_id=task.id, content="Second note")
        time.sleep(0.01)
        core.add_task_note(task_id=task.id, content="Third note")
    ...
    def test_note_persistence(self, core_with_project_and_task: Any) -> None:
        ...
        notes = new_core.get_task_notes(task_id=task.id)
    ...
    def test_note_operations_error_handling(
        ...
        # Test with invalid task ID (task IDs are universally unique)
        with pytest.raises(TaskNotFoundError):
            core.get_task_notes(task_id="invalid-task")

        with pytest.raises(TaskNotFoundError):
            core.search_task_notes(task_id="invalid-task", query="test")
