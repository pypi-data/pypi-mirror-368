...
class TopologyViz:
  ...
  def _generate_main_layout(self) -> str:
    ...
    try:
      ...
      info_lines.append(version_line)
      info_lines.append("")  # Add spacing after version
    except:
      pass
    ...
    # Create a gradient bar using ASCII characters with colors
    # Use ASCII characters for better compatibility
    bar_chars = []
    for i in range(bar_width):
      position = i / max(1, bar_width - 1)
      if position < 0.25:
        # Red zone
        bar_chars.append("█")
      elif position < 0.5:
    ...
    # Add the bar characters
    for i, char in enumerate(bar_chars):
      if bar_start_x + i < viz_width:
        visualization[bar_y][bar_start_x + i] = char
